...
{
  S16 r = origSGetMsg(region, pool, mBuf);
  if(g_msgBufTable.isEnabled && (ROK == r)) pmRecordBufAlloc(&g_msgBufTable, *mBuf, 0, fileName, line);
  return r;
}

PUBLIC S16 SPutMsgPwav
(
  Buffer *mBuf,
  U32     line,
  U8     *fileName
)
{
  ...
  S16 r = origSPutMsg(mBuf);
  if(g_msgBufTable.isEnabled && (ROK == r)) pmRecordBufFree(&g_msgBufTable, mBuf, 0, fileName, line);
  return r;
}
...
)
{
  ...
  S16 r = origSGetSBuf(region, pool, ptr, size);
  if(g_staticBufTable.isEnabled && (ROK == r)) pmRecordBufAlloc(&g_staticBufTable, *ptr, size, fileName, line);
  return r;
}
...
{
  S16 r = origSPutSBuf(region, pool, ptr, size);
  if(g_staticBufTable.isEnabled && (ROK == r)) pmRecordBufFree(&g_staticBufTable, ptr, size, fileName, line);
  return r;
}
