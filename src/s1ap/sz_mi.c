...
void sendS1apStartIndToPrm(void)
{
  U32 version = getSvnVersion(MODID_S1AP);
  logInfo ("S1AP svn version : %d", version);
  printf  ("S1AP svn version : %d\n", version);
  /*send start indication to PRM indicating successful startup*/
  if( sendS1apMsgToPrm(EVT_START_IND, sizeof(U32), &version ) == ROK)
  {
    logMsgOut("%s", "Sent EVT_START_IND from S1AP to PRM" );
  }
}

inline PUBLIC S16 sendS1apMsgToPrm (Event event, MsgLen len, U8 *data) { return sendEventToOam (ENTSZ /*szCb.init.ent*/, ENT_PRM, event, len, data); }
//inline PUBLIC void sendS1apHelloAckToPrm (void) { sendS1apMsgToPrm (EVT_HELLO_ACK, 0, NULL); }


void sendS1apHelloAckToPrm(
  Buffer   *mBuf
  )

{
  ...
  if( sendS1apMsgToPrm(EVT_HELLO_ACK, sizeof(MsgModPrmHelloAck), (U8*)&helloAck) == ROK)
  {
    logMsgOut("%s", "Sent EVT_HELLO_ACK from S1AP to PRM" );
  }
}

void procRcvdS1apExitMsg(void)
{
...
#ifdef PWAV
	       SZDBGP(DBGMASK_MI, (szCb.init.prntBuf,
		     "\nszGenCntrl: DBG-Mask value :%02x", szCb.init.dbgMask));
#endif
               break;
            }
#endif /* DEBUGP */
...
#ifdef PWAV
	       SZDBGP(DBGMASK_MI, (szCb.init.prntBuf,
		     "\nszGenCntrl: DBG-Mask value :%02x resetting with ~%02x", szCb.init.dbgMask, cntrl->u.dbg.dbgMask));
#endif
               szCb.init.dbgMask &= ~(cntrl->u.dbg.dbgMask);
               break;
            }
...
#endif
{
   ...
   ret = szLiTermAssoc(&peerCb, FALSE);

   if ((ret != ROK) && (ret != ROKPENDING))
   {
#if (ERRCLASS & ERRCLS_DEBUG)
            SZLOGERROR(ERRCLS_DEBUG, ESZ104, (ErrVal)peerCb->peerId,
                      "szDelPeer: Failed to terminate an association");
#endif /* (ERRCLASS & ERRCLS_DEBUG) */
   }
