...
inline PUBLIC S16 sendTuclMsgToPrm (Event event, MsgLen len, U8 *data) { return sendEventToOam (ENTHI, ENT_PRM, event, len, data); }
//inline PUBLIC void sendTuclHelloAckToPrm (void) { sendTuclMsgToPrm (EVT_HELLO_ACK, 0, NULL); }
void sendTuclStartIndToPrm(void)
{
  U32 version = getSvnVersion(MODID_TUCL);
  logInfo ("TUCL svn version : %d", version);
  printf  ("TUCL svn version : %d\n", version);

  /*send start indication to PRM indicating successful startup*/
  if( sendTuclMsgToPrm(EVT_START_IND, sizeof(U32), &version ) == ROK)
  {
    logMsgOut("%s", "Sent EVT_START_IND from TUCL to PRM" );
  }
}

void sendTuclHelloAckToPrm(
  Buffer  *mBuf
  )
{
  MsgLen              cCnt;
  ...
  if( sendTuclMsgToPrm(EVT_HELLO_ACK, sizeof(MsgModPrmHelloAck), (U8*)&helloAck) == ROK)
  {
    logMsgOut("Sent EVT_HELLO_ACK from TUCL to PRM seqNo [%d]", helloAck.helloAckSeqNum);
  }
}

void procRcvdTuclExitMsg(void)
{
