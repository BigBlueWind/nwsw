...
#ifdef PWAV_MEM2
  PUBLIC Void origCmFreeMem (Ptr  memPtr);
  PUBLIC S16 origCmAllocEvnt (Size evntSize, Size maxBlkSize, Mem *sMem, Ptr *ptr);
#endif
/*
*
*       Fun:   cmAllocEvnt
...
#ifdef PWAV_MEM2
PUBLIC S16 pwavCmAllocEvnt
(
Size            evntSize,    /* Size of the Event structure */
Size            maxBlkSize,  /* Chunk Memory size */
Mem             *sMem,       /* Static memory region and pool */
Ptr             *ptr,        /* Location to place allocated event ptr */
char            *file,       /* file from where it was called */
int             line         /* line in file from where it was called */
)
{
  S16 r = origCmAllocEvnt(evntSize, maxBlkSize, sMem, ptr);
  //logMem("AE|OK|%p|%d|%d|%d|%d|%s|%d", *ptr, evntSize, maxBlkSize, sMem->region, sMem->pool, file, line); 
  if (g_blockBufTable.isEnabled && (ROK == r)) pmRecordBufAlloc(&g_blockBufTable, *ptr, maxBlkSize, file, line);
  RETVALUE(r);
}

PUBLIC Void pwavCmFreeMem
(
Ptr            memPtr,    /* Link List CP */
char          *file,      /* file from where called */
int            line       /* line in file above from where called */
)
{
  origCmFreeMem(memPtr);
  //logMem("FM|OK|%p|%s|%d\n", memPtr, file, line);
  if (g_blockBufTable.isEnabled) pmRecordBufFree(&g_blockBufTable, memPtr, 0, file, line);
}
#endif /* PWAV_MEM2 */

/*
