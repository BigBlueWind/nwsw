...
typedef struct kwL2MeasCfmInfo
{
   ...
   union
   {
      ...
      {
          U32   dlIpThPut; /*!< Scheduled DL IP throughput */
          U32   ulIpThPut; /*!< Scheduled UL IP throughput */
      }ipThrput;
#endif
   }val;
}KwL2MeasCfmInfo;
...
EXTERN Void kwSendLmAlarm ARGS ((U16 category,
      U16  event,
      U16 cause,
      SuId suId,
      U32 ueId));
...
#endif /* __cplusplus */
#ifdef PWAV /* Function prototypes */
PUBLIC S16 sendRlcMsgToPrm(
    Event   event,
    U16     len,
    U8     *data
    );
void sendRlcStartIndToPrm(void);
void sendRlcHelloAckToPrm(Buffer  *mBuf);
void procRcvdRlcExitMsg(void);
#endif /*PWAV */
#endif /* __LKW_X__ */
...
/main/3      lkw_x_001.main_2 ap 1. Modified the content of the structure KwKwuSapSts
                                 2. Removed RGUSAP statistics from KwSts, as the same
                                    elements are reflecting in KwGenSts.
                              rd 3. Added support for L2 Measurement.
*********************************************************************91*/
