...
EXTERN  S16 SGetMsgPwav ARGS((Region region, Pool pool, Buffer **mBufPtr, U32 line,  U8 *fileName));
EXTERN  S16 SPutMsgPwav ARGS((Buffer *mBuf, U32 line,  U8 *fileName));
...
#define MSPD_DBG_RLC(...) 
#define MSPD_DBG(...) do { \
   sprintf(ys_global_printbuf, "DEBUG: %s():%u @%u ", __func__, __LINE__, ysGT); \
   sprintf(&ys_global_printbuf[strlen(ys_global_printbuf)], __VA_ARGS__); \
   SPrint(ys_global_printbuf); \
}while (0)
#define MSPD_DBG_RAW(...) do { \
   sprintf(ys_global_printbuf, __VA_ARGS__); \
   SPrint(ys_global_printbuf); \
} while (0)
#define MSPD_LOG(...) do { \
   sprintf(ys_global_printbuf, "STATS: %s():%u @%u ", __func__, __LINE__, ysGT); \
   sprintf(&ys_global_printbuf[strlen(ys_global_printbuf)], __VA_ARGS__); \
   SPrint(ys_global_printbuf); \
} while (0)
/*Added a new MACRO type for ERROR prints*/
#define MSPD_ERR(...) do { \
   sprintf(ys_global_printbuf, "ERROR: %s():%u @%u ", __func__, __LINE__, ysGT); \
   sprintf(&ys_global_printbuf[strlen(ys_global_printbuf)], __VA_ARGS__); \
   SPrint(ys_global_printbuf); \
} while (0)	
#else
...
#define MSPD_DBG_RAW(...)
#define MSPD_LOG(...)
#define MSPD_ERR(...)/*Added a new MACRO type for ERROR prints*/
#endif
...
#include "pm_mem.h"
#if 0
#define MB_MAX_LINES     300
#define MB_MAX_LINE_SIZE 100
extern char  g_memLogBuf[MB_MAX_LINES][MB_MAX_LINE_SIZE];
extern U32   g_numLinesInMemLogBuf;

#define logMem(...) do {\
 snprintf(&(g_memLogBuf[g_numLinesInMemLogBuf][0]), MB_MAX_LINE_SIZE,  __VA_ARGS__), \
 g_numLinesInMemLogBuf++;\
 if(g_numLinesInMemLogBuf == MB_MAX_LINES) {\
  fwrite(g_memLogBuf, MB_MAX_LINE_SIZE, MB_MAX_LINES, stdout),\
  g_numLinesInMemLogBuf = 0;\
 }\
} while (0)
#endif
#define logMem printf
#endif /* __SSIX__ */

PUBLIC S16 SGetSBufPwav (
