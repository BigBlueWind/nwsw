...
/* tfu_x_001.main_5 - Changes for MIMO feature addition */
/* tfu_x_001.main_6 - Removed dependency on LTEMAC_MIMO flag */
/** @name LTEMAC_MIMO */
/** @{ */
...
#ifdef LTEMAC_SPS
 /** @brief This structure contains release PDCCH information that is part
  * of DCI format 1A. 
  * @details This structure contains information needed for release PDCCH sent
  * using DCI format1A
  * @sa TfuDciFormat1AInfo
  */
typedef struct tfudciFormat1aRelPdcch
{
   U32                    alloc;         /*!< Set to all 1's for release PDCCH
                                          */
   U8                     mcs;           /*!< 5 bits for MCS */
   U8                     rv;            /*!< Redundancy version */
   TknU8                  harqProcId;    /*!< HARQ process ID. Reserved
...
typedef struct tfuDciFormat1aInfo
{
   /*tfu_x_001.main_6- Added for SPS support*/
#ifdef LTEMAC_SPS
   U8                    dciInfo;      /*!< Indicates if DCI format carries
                                          PDCCH order/PDSCH info/Release PDCCH
                                          info */
#else 
   Bool                 isPdcchOrder;
#endif
   union
   {
      ...
      /*tfu_x_001.main_6 - Added for SPS support*/
#ifdef LTEMAC_SPS
      Tfudciformat1aRelPdcch relPdcchInfo;  /*!< Release PDCCH information for
                                              the UE */
#endif
   }t;
} TfuDciFormat1aInfo;
...
{
   U8                     cqi;           /*!< CQI for this sub-band; the report 
                                              from UE must contain a 2-bit 
                                              Differential CQI, it is assumed
                                              that PHY must provide the 
                                              calculated value.*/
   TfuSubbandInfo         subBand;       /*!< Sub-band information start and 
                                              length. */
} TfuSubbandMode22;
#endif
/** @} */
...
/** @{ */
#ifndef LTEMAC_MIMO
   TfuSubbandMode22  subbandArr[TFU_MAX_DL_SUBBAND];  /*!< Sub-band information array */
#else
/** @} */
/** @name LTEMAC_MIMO */
/** @{ */
...
typedef struct tfuUeUlSchRecpInfo
{
   ...
   /* MS_WORKAROUND: Added since PHY API needs MCS in rxVector */                                          
#ifdef MSPD 
   U8                   mcs;          /*!< MCS */
#endif
} TfuUeUlSchRecpInfo;
...
/*tfu_x_001.main_6 - Added for SPS support*/
#ifdef LTEMAC_SPS
EXTERN S16 cmUnpkTfuDciformat1aRelPdcch ARGS((
Tfudciformat1aRelPdcch *param,
Buffer                 *mBuf
));
EXTERN S16 cmPkTfuDciformat1aRelPdcch ARGS((
Tfudciformat1aRelPdcch *param,
Buffer                 *mBuf
));
#endif
...
/** @{ */
#ifndef LTEMAC_MIMO
EXTERN S16 cmPkTfuSubbandMode22 ARGS((
   TfuSubbandMode22     *param,
   Buffer               *mBuf
));
EXTERN S16 cmUnpkTfuSubbandMode22 ARGS((
   TfuSubbandMode22     *param,
   Buffer               *mBuf
));
#endif
