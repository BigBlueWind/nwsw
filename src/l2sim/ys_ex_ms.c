...
PUBLIC S16 clCfgReq ( Pst* pst, SpId spId, CtfCfgTransId transId, CtfCfgReqInfo* cfgReqInfo );
PUBLIC S16 tfuProcDstatsCfgReq (Buffer *mBuf)
{
   MsgPmhTfuDStatsCfgReq msg;
   MsgLen              cCnt = 8;

   if (SCpyMsgFix( mBuf, 0, sizeof(MsgPmhTfuDStatsCfgReq), (Data*)&msg , &cCnt ) != ROK)   
  {
    logInfo("%s","tfuProcDstatsCfgReq(): SCpyMsgFix Failed\n" );
    RETVALUE(RFAILED);
  }

   ysCb.dStatsCfg.cmd = msg.cmd;
   ysCb.dStatsCfg.interval_ms = msg.interval_ms;
   logInfo("cmd:%d,interval:%d\n",ysCb.dStatsCfg.cmd ,ysCb.dStatsCfg.interval_ms);
   //printf("cmd:%d,interval:%d",ysCb.dStatsCfg.cmd ,ysCb.dStatsCfg.interval_ms);
}
...
#endif
{
   ...
   logInfo("ysActvTsk received message from Ent=%d", pst->srcEnt);
   switch(pst->srcEnt)
   {
      case ENT_PMH:
          switch (pst->event)
          {
            case EVT_PMH_TFU_DSTATS_CFG_REQ:
              logInfo("%s","in EVT_PMH_TFU_DSTATS_CFG_REQ\n");
              tfuProcDstatsCfgReq (mBuf);
              break;
            default:
             
              logError("ysActvTsk(): Invalid event (%d)!\n",pst->event);
              SPutMsg(mBuf);
              break;
          }
          break;
      ...
      case ENTSM:
         switch(pst->event)
         {
            ...
            case EVTLYSCFGREQ:
               ...
               ret = cmUnpkLysCfgReq(YsMiLysCfgReq, pst, mBuf);
               break;
            case EVTLYSCNTRLREQ:
               /* Process a control request */
               ret = cmUnpkLysCntrlReq(YsMiLysCntrlReq, pst, mBuf);
               break;
            case EVTLYSSSTAREQ:
               /* Process a status request  */
               ret = cmUnpkLysStaReq(YsMiLysStaReq, pst, mBuf);
               break;
            case EVTLYSSTSREQ:
      ...
      case ENTNH:
         switch (pst->event)
         {
            ...
            case EVTCTFUBNDREQ:
               ...
               break;
            case EVTCTFCFGREQ:
               ret = cmUnpkCtfCfgReq (clCfgReq, pst, mBuf);
               break;
/***************SEC_CHANGE_START*****************/	       
#if 0
#ifdef CTF_AS_SEC_ENB
            case EVTCTFDATREQ:
               ret = cmUnpkCtfDatReq (YsUiCtfDatReq, pst, mBuf);
               break;
#endif
#endif
/***************SEC_CHANGE_END*****************/	       
   ...
   SExitTsk();
   logInfo("ysActvTsk src=%d, dst=%d event=%d result=%d", pst->srcEnt, pst->dstEnt, pst->event, ret);
