...
#endif
{
   ...
   //MSPD_DBG("<Rahul>Setting kwLock to zero\n");
   kwCb.kwLock = 0;
   /*Initialize the UL Lock*/
   if (SInitLock(&kwCb.ulLock, SS_LOCK_MUTEX) != ROK)
   {
      //MSPD_DBG("<Rahul>Lock Initialization failed\n");
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0, "Lock initialization failed\n");
   ...
   /*Instance Id:0 for core 2, Instance Id:1 for core 1*/
   selfPstUl.dstInst = 0;
   ...
   cmLListInit(&kwCb.reasmblyRbList);
...
#endif
{
   ...
   TRC3(KwMiLkwCfgReq);
            ...
            logInfo("KwMiLkwCfgReq(post, elmId(%d))", cfg->hdr.elmId.elmnt);
   ...
   if (reason == LCM_REASON_NOT_APPL)
   {
      ...
      ret = ROK;
      logInfo("KwMiLkwCfgReq(post, elmId(%d)) success", elmnt);
   }
   else
   {
      rCfm.cfm.status = LCM_PRIM_NOK;
      ...
      ret = RFAILED;
      logError("KwMiLkwCfgReq(post, elmId(%d)) Failure", elmnt);
   }
...
#endif
{
   ...
   logInfo("KwMiLkwCntrlReq(post, elmId(%d))\n", cntrl->hdr.elmId.elmnt);
...
void sendKwStartIndToPrm(void)
{
  ...
  {
    logError("%s", "Sent EVT_START_IND from KW to PRM \n");
  }
}
...
void sendKwHelloAckToPrm(Buffer   *mBuf)
{
  ...
  {
    logError("sendKwHelloAckToPrm(): SCpyMsgFix Failed %d", cCnt );
    RETVOID;
  }
  ...
  {
    logInfo("Sent EVT_HELLO_ACK from KW to PRM for sequence number:%d \n", helloAck.helloAckSeqNum );
  }
  else
  {
    logInfo("Sent EVT_HELLO_ACK from KW to PRM failed for sequence number:%d \n", helloAck.helloAckSeqNum );
  }
}

void procRcvdKwExitMsg(void)
{
  logInfo("%s", "Exiting the KW \n" );
  exit(0);
}
#endif
/********************************************************************30**
