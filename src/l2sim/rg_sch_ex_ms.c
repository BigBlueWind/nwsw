...
PUBLIC S16 schedCfgReq ( Pst *pst, SpId spId, RgrCfgTransId transId, RgrCfgReqInfo *cfgReqInfo);
//PUBLIC S16 siCfgReq ( Pst *pst, SpId spId, RgrCfgTransId transId, RgrSiCfgReqInfo *cfgReqInfo);
...
{
   Mem sMem;
   /*rg003.201: Changed TRC3 to TRC2 */
   TRC2(schActvTsk)

   sMem.region = rgSchCb[pst->dstInst - RGSCH_INST_START].rgSchInit.region;
   sMem.pool = rgSchCb[pst->dstInst - RGSCH_INST_START].rgSchInit.pool;
   ...
   switch(pst->srcEnt)
   {
      ...
      case ENTSM:
	 //logInfo("%s", "Redeived config request from SM\n");
         switch(pst->event)
         {
#ifdef LCRGMILRG
     ...
     case ENTNX:
         switch(pst->event)
         {
            ...
            case EVTRGRUBNDREQ:
               ...
               break;
            case EVTRGRCFGREQ:
               cmUnpkRgrCfgReq(schedCfgReq, pst, mBuf);
               break;
      ...
      case ENTRG: /* When MAC sends a msg to Scheduler instance */
         switch(pst->event)
         {
            ...
            case EVTINFSFRECPIND:
               ...
               break;
               /*rg002.301 ccpu00119825 DEL RgMacSchSfDatInd handling is 
                * not present hence deleted*/
#ifdef LTE_L2_MEAS
