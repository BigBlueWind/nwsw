...
#endif
{
...
#ifdef DEBUGP
#ifdef RG_DEBUG
   /* rg005.201 disabling debugs by default */
   ...
   rgCb.rguSap.sapSta.sapState = LRG_NOT_CFG;
...
#endif    
{
   ...
   logInfo("RgMiLrgCfgReq(): pst->srcEnt=%d, pst->srcInst=%d", pst->srcEnt, pst->srcInst);
   /* Fill the post structure for sending the confirmation */
   rgLMMFillCfmPst(pst, &cfmPst, cfg);

   cmMemset((U8 *)&cfm, 0, sizeof(RgMngmt));

   /* rg009.201 -ccpu00116453: Adding transId */
#ifdef LMINT3
   ...
   //MSPD_DBG("<Rahul>Config received for element ID %d\n", cfg->hdr.elmId.elmnt);
   switch(cfg->hdr.elmId.elmnt)
   {
      case STGEN:
         //MSPD_DBG("<Rahul>Calling GenCfg\n");
         reason = rgLMMGenCfg(&cfg->t.cfg); 
         logInfo("RgMiLrgCfgReq(): Gen Cfg done with reason=%d.",reason);
         break;
      case STRGUSAP:
      case STCRGSAP:
      case STTFUSAP:
         ...
         logInfo("\nRgMiLrgCfgReq(): SAP(Elmnt=%d) Cfg done with reason=%d.",
                  cfg->hdr.elmId.elmnt, reason);
         break;
      default:
         //MSPD_DBG("<Rahul>Default case\n");
         ret = LCM_PRIM_NOK;
         reason = LCM_REASON_INVALID_ELMNT;
         logError("\nRgMiLrgCfgReq(): Invalid Elmnt=%d", cfg->hdr.elmId.elmnt);
         break;
   }
...
#endif    
{
   ...
   RGDBGPRM((rgPBuf, "\nRgMiLrgStsReq(): stsReq for elmnt= %d pst->srcEnt=%d, action=%d",
   sts->hdr.elmId.elmnt, pst->srcEnt, sts->t.sts.action));
   /* Fill the post structure for sending the confirmation */
   ...
   if(rgCb.rgInit.cfgDone != TRUE) 
   {
      ...
      RGDBGERR((rgPBuf, "\nRgMiLrgStsReq(): Gen Cfg not done"));
      RETVALUE(ROK);
   }
   ...
   switch(sts->hdr.elmId.elmnt)
   {
      ...
      default:
         ...
         RGDBGERR((rgPBuf, "\nRgMiLrgStsReq(): Invalid Elmnt = %d",sts->hdr.elmId.elmnt));
         break;     
   }
   RgMiLrgStsCfm(&cfmPst,&cfm);
   RETVALUE(ROK);
...
#endif    
{
   ...
   logInfo("\nRgMiLrgStaReq(): staReq for elmnt= %d pst->srcEnt=%d",
   ...
   if(rgCb.rgInit.cfgDone != TRUE) 
   {
      ...
      if (SGetSBuf(cfmPst.region, cfmPst.pool, 
         ...
         != ROK)
      {
         RGDBGERR((rgPBuf, "\nRgMiLrgCntrlReq(): Memory Unavailable \
                  for Confirmation"));
         RETVALUE(ROK);
      } 
      ...
      RGDBGERR((rgPBuf, "\nRgMiLrgStaReq(): Gen Cfg not done"));
      RETVALUE(ROK);
   }
   ...
   switch(sta->hdr.elmId.elmnt)
   {
      ...
      default:
         ...
         cfm.cfm.reason = LCM_REASON_INVALID_ELMNT;
         RgMiLrgStaCfm(&cfmPst, &cfm);
         logError("RgMiLrgStaReq(): Invalid elmnt=%d", sta->hdr.elmId.elmnt);
         break;     
   }
   RETVALUE(ROK);
}/*-- RgMiLrgStaReq --*/
...
#endif    
{
   ...
   logInfo("\nRgMiLrgCntrlReq():cntrlReq for elmnt= %d,action=%d,subaction=%d",
   ...
   if(rgCb.rgInit.cfgDone != TRUE)
   {
      ...
      logError("%s", "RgMiLrgCntrlReq(): Gen Cfg not done.");
      RETVALUE(ROK);
   }
   ...
   switch(cntrl->hdr.elmId.elmnt)
   {
      ...
      default:
         ...
         RgMiLrgCntrlCfm(&cfmPst, &cfm);
         logError("\nRgMiLrgCntrlReq(): invalid elmnt=%d",
...
#endif
{
   ...
   logInfo("rgLMMSapCfg(): SAP Config Requested for sapType=%d", sapType);
   ...
   {   
      case STRGUSAP:
         if ((cfg->s.rguSap.selector != RGU_SEL_TC) &&
             (cfg->s.rguSap.selector != RGU_SEL_LC))
         {
            ret = LCM_REASON_INVALID_PAR_VAL;
            logError("%s", "rgLMMSapCfg(): unsupported Selector value for RGU.");
            break;
         }
         if(rgCb.rguSap.sapSta.sapState == LRG_NOT_CFG)
         { 
      ...
      case STCRGSAP:
         ...
         {
            ret = LCM_REASON_INVALID_PAR_VAL;
            logError("%s", "\nrgLMMSapCfg(): unsupported Selector value for CRG.");
            break;
         }
      ...
      case STTFUSAP:
         ...
         {
            ret = LCM_REASON_INVALID_PAR_VAL;
            RGDBGERR((rgPBuf, "\nrgLMMSapCfg(): unsupported Selector value for TFU."));
            break;
         }
#endif
...
#endif
{
   ...
   logInfo("%s", "\nrgLMMGenCfg(): General Configuration Request");
   ...
   {
      logError("%s", "\nrgLMMSapCfg(): unsupported Selector value for RGU.");
   ...
   {
      RGDBGERR((rgPBuf, "\nrgGenCfg(): Failed to register timer."));
      RETVALUE(LCM_REASON_MEM_NOAVAIL);
   }   
#endif   
#if 0 /* TODO: need to check if this is really not requried */    
   /* cellCb hashList initialization */
   if (rgDBMInitCellLst(RG_NMB_CELL_HASHBIN) != ROK)
   {
      RGDBGERR((rgPBuf, "\nrgLMMGenCfg(): rgCb.cellLst Initialization failed"));
      RETVALUE(LCM_REASON_MEM_NOAVAIL);
   }
   ...
   {
      RGDBGERR((rgPBuf, "\nrgLMMGenCfg(): rgCb.inactvCellLst Initialization failed"));
      RETVALUE(LCM_REASON_MEM_NOAVAIL);
   }
#endif
...
#endif
{
   ...
   logInfo("%s", "\nrgLMMShutdown(): Layer Shutdown Requested");
...
#endif
{
   ...
   logInfo("%s", "rgLMMGenCntrl(): Control Req for STGEN");
   cfm->cfm.status = LCM_PRIM_OK;
   cfm->cfm.reason = LCM_REASON_NOT_APPL;
   ...
   switch(cntrl->t.cntrl.action)
   {
      case AENA:
         ...
         switch(cntrl->t.cntrl.subAction)
         {
            ...
            default:
               cfm->cfm.status = LCM_PRIM_NOK;
               cfm->cfm.reason = LCM_REASON_INVALID_PAR_VAL;
               logError("%s", "\nrgLMMGenCntrl(): invalid subaction=%d",
      ...
      default:
         cfm->cfm.status = LCM_PRIM_NOK;
         cfm->cfm.reason = LCM_REASON_INVALID_PAR_VAL;
         logError( "\nrgLMMGenCntrl(): invalid action=%d",
...
#endif
{
   ...
   logInfo("\nrgLMMSapCntrl(): Control Req for SAP (%d)", cntrl->hdr.elmId.elmnt);
         ...
         switch(cntrl->t.cntrl.action)
         {
            ...
            /* Bind Enable Request */
               ...
               else
               {
                  ...
                  /* rg003.201: Sending Status Indication to Layer Manager */
...
#endif
{
   ...
   UNUSED(tmrEvnt);
#ifndef ALIGN_64BIT
   logInfo("\nrgLMMStartTmr(): Timer start request for tmrEvnt=%d, tmrVal=%ld", 
   tmrEvnt, tmrVal);
#else
   logInfo("rgLMMStartTmr(): Timer start request for tmrEvnt=%d, tmrVal=%d", 
   tmrEvnt, tmrVal);
#endif
...
#endif
{
   ...
   logInfo("\nrgLMMStopTmr(): Timer stop request for tmrEvnt=%d",tmrEvnt);
   ret = RFAILED;
   ...
   logInfo("\nrgLMMStopTmr(): tmrEvnt=%d search result is ret=%d",tmrEvnt,ret);
...
#endif
{
   ...
   logInfo( "\nrgLMMTmrExpiry(): Timer Expired for tmrEvnt=%d",tmrEvnt);
   ...
   switch(tmrEvnt)
   {
      case RG_BNDREQ_TMR:
         ...
         if(tfuSap->numBndRetries > RG_MAX_BNDRETRY)
         {
            ...
            logInfo("%s", "\nrgLMMTmrExpiry(): Max Bind Retries Exceeded.");
         }
         else
         {
            logInfo("%s", "\nrgLMMTmrExpiry(): Retry Bind");
         ...
         break;
      default:
         logError("\nrgLMMTmrExpiry(): Invalid tmrEvnt=%d", tmrEvnt);
         ret = RFAILED;
         break;
   }
   RETVALUE(ret);
...
#endif
{
   ...
   RGDBGPRM((rgPBuf, "\nrgLMMStaInd(): category=%d, event=%d, cause=%d",
   category, event, cause));
   if(rgCb.rgInit.usta == FALSE)
   {
      RGDBGINFO((rgPBuf, "\nrgLMMTmrExpiry(): USTA Ind Disabled."));
      RETVALUE(ROK);
   }
...
#endif
{
   ...
   RGDBGPRM((rgPBuf, "\nrgLMMTrcInd(): Trace for event=%d, rgCb.trcLen=%d",event,
   rgCb.trcLen));

   if ((rgCb.trcLen == LRG_NO_TRACE) || (srcMbuf == NULLP))
   {
      RGDBGERR((rgPBuf, "\nrgLMMTrcInd(): Trace Disabled."));
      RETVOID;
   }
   ...
   else if(rgCb.trcLen > 0)
   {
      ...
      if(bufLen >= rgCb.trcLen)
      {
         ...
         {
            RGDBGERR((rgPBuf, "\nrgLMMTrcInd(): rgAllocSBuf Failed."));
            RETVOID;
         }
...
#endif
{
   ...
   UNUSED(pst);
   RGDBGPRM((rgPBuf, "\nrgLMMBndCfm(): suId=%d, status=%d, sapState=%d", 
   suId, status, rgCb.tfuSap.sapSta.sapState));

   /* Check if the suId is valid */
   if(rgCb.tfuSap.sapCfg.suId != suId)
   {
      RGDBGERR((rgPBuf, "\nrgLMMBndCfm(): Invalid SuId"));
      RETVALUE(RFAILED);
   }
   ...
   RGDBGINFO((rgPBuf, "\nrgLMMBndCfm(): RgMiLrgCntrlCfm  ret=%d.",ret));
   RETVALUE(ret);
}
    ...
    )
{
   ...
   */
  if( RFAILED == sendMsgToFrm( ENTRG, ENT_PRM, event, len, data ))
  {
    return RFAILED;
  }
  else
...
void sendRgStartIndToPrm(void)
{
  U8 version = 0;
  /*send start indication to PRM indicating successful startup*/
  if( sendRgMsgToPrm(EVT_START_IND, sizeof(U32), &version ) == ROK)
  {
    logError("%s", "Sent EVT_START_IND from RG to PRM \n");
  }
}

void sendRgHelloAckToPrm(Buffer   *mBuf)
{
  MsgLen              cCnt;
  ...
  {
    logError("%s", "sendRgHelloAckToPrm(): SCpyMsgFix Failed\n" );
    RETVOID;
  }
  helloAck.helloAckSeqNum = hello.helloSeqNum;

  if(sendRgMsgToPrm(EVT_HELLO_ACK, sizeof(MsgModPrmHelloAck), (U8*)&helloAck) == ROK)
  {
    logInfo("Sent EVT_HELLO_ACK from RG to PRM for sequence number:%d \n", helloAck.helloAckSeqNum);
  }
  else
  {
    logInfo("Sent EVT_HELLO_ACK from RG to PRM failed for sequence number:%d \n", helloAck.helloAckSeqNum);
  }
}

void procRcvdRgExitMsg()
{
  logInfo("%s", "Exiting the RG \n" );
  exit(0);
}
#endif

