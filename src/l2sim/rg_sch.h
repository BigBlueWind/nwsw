...
/* Tunable parameter */
#define RGSCH_DL_BW_PERCNT_HIKE  40   /* Percentage Hike in DL BW to optimize throughput
                                         in Frequency Selective scheduling */
...
/* rg005.201 Changing from 10 to 100 for optimization */
...
/* rg005.201 Changing from 31 to 128 for optimization - to support more [100] number of UEs */
...
/*rg002.301: [ccpu00118468]-ADD-a new hash define for TA.
 * Refer to 36.213, 4.2.3*/
...
#define RGSCHADDTOCRNTTIME(crntTime, toFill, incr)          \
...
/* rg002.301 - Mod - [ccpu00119331] changing variable type from
  unsigned to signed */
...
/* rg002.201 :ccpu00108815: Corrected timing for SIB2 handling: removed MACROs */

#define RGSCHSUBFRMCRNTTIME(crntTime, toFill, dcr)    \
{                                                  \
   if (crntTime.sfn == 0)                          \
{                                                  \
   if ((crntTime.subframe - (dcr)) < 0)              \
   {                                               \
      toFill.sfn = RGSCH_MAX_SFN - 1;                 \
   }                                               \
   else                                            \
   {                                               \
   toFill.sfn = crntTime.sfn;                      \
...
toFill.subframe = (RGSCH_NUM_SUB_FRAMES + crntTime.subframe - (dcr)) % (RGSCH_NUM_SUB_FRAMES);                                   \
}
...
/*rg009.201 - ccpu00116309: Support for iPhich=1 for TDD*/
...
/*rg009.201 [ccpu00115187]: Defining new MACRO for DL subframes */
...
/* rg006.201:[cccpu000111706, ccpu00112394]: Added MACRO RG_TFU_HQ_IND_DELTA */
/* 
...
/* rg009.201 change for CR ccpu00116486 */

/* rg002.301 -MOD-[ccpu00118351] 4 is replaced with a hash define
 * for Harq Interval */
...
/* rg004.201 patch - ccpu00109921 fix start */
/* rg005.201 for c++ compilation changed int to U8 */
...
#ifdef LTE_TDD
#define RG_SCH_DRX_UL_DELTA     (RG_UL_DELTA + 1) /*< look at timer queue
                                                    for UL
                                                    RG_SCH_DRX_UL_DELTA
                                                    in advance */
#else /* LTE_TDD */
/* rg002.301 -ADD-[ccpu00118351] Introduced UL control timing delta in FDD */
#define RG_SCH_DRX_UL_DELTA     (TFU_ULCNTRL_DLDELTA)
#endif /* LTE_TDD */
/*Assigning RG_SCH_DRX_MAX_DELTA to the higher delta of uplink delta and
 * downlink delta.*/
#if (RG_SCH_DRX_DL_DELTA < RG_SCH_DRX_UL_DELTA)
#define RG_SCH_DRX_MAX_DELTA RG_SCH_DRX_UL_DELTA
#else
#define RG_SCH_DRX_MAX_DELTA RG_SCH_DRX_DL_DELTA 
#endif
...
#if defined(LTEMAC_DRX) && defined(LTEMAC_R9)
#define RG_SCH_DRX_WILL_UE_RPRT_CQI(_ue,_res)                               \
do                                                                          \
{                                                                           \
 if(_ue->isDrxEnabled && _ue->drxCb)                                        \
   {                                                                        \
     if(_ue->drxCb->cqiMask.pres && _ue->drxCb->cqiMask.val == RGR_DRX_SETUP) \
      {/*cqiMask is setup by upper layers */                                \
       if((_ue->drxCb->drxUlInactvMask & RG_SCH_DRX_ONDUR_BITMASK) ==        \
     ...
     {/*cqiMask is not setup by upper layers */                             \
      if ((RG_SCH_DRX_UL_IS_UE_ACTIVE(_ue->drxCb))                          \
         )                                                                  \
      {/*UE is active, expect cqi/pmi/ri*/                                  \
        _res = TRUE;                                                        \
      }                                                                     \
      else                                                                  \
      {                                                                     \
     ...
     }/*else cqiMask NOT setup by UL */                                     \
   }/*_ue->isDrxEnabled*/                                                   \
 else                                                                       \
   {                                                                        \
      /*drx not enabled,expect cqi/pmi/ri as usual*/                        \
      _res = TRUE;                                                          \
   }                                                                        \
}while(0) 
#else
 ...
 ***********************************************************************/
 ...
 /* Periodic CQI  max periodicity =   160 Ref: 36.213 (Table 7.2.2-1A) */
...
#else
 /* Periodic CQI  max Np     =   160 Ref:36.213 (Table 7.2.2-1C) */
 /* SRS      max periodicity =   320 Ref: 36.213 (Table 8.2-2) */
 /* SR       max periodicity =   80  Ref: 36.213 (Table 10.1-5) */
#define  RG_SCH_PCQI_SRS_SR_TRINS_SIZE 320
...
#define RG_SCH_GET_SBCQI_J_K_VAL(_dlBw, _j, _k) \
{\
 if(_dlBw >= 64)\
   {\
   _j = 4;\
...
#define RG_SCH_GET_ACQI_K_VAL(_dlBw,  _k) \
{\
 if(_dlBw >= 64)\
   {\
   _k = 8;\
...
/*rg002.301 - REM - removed the MACRO RG_SCH_GET_PERIODICITY_TBL
