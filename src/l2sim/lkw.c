...
#endif
{
   ...
   if(ret1 != ROK)
   {
      SPutMsg(mBuf);
       if(ret1 != ROK)
       {
               logError("%s", "cmPkKwMngmt failure ......!");
       }
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   ret1 = cmUnpkKwMngmt(&cfg, LKW_EVT_CFG_REQ, mBuf);
    ...
    if(ret1 != ROK)
    {
       ...
       if(ret1 != ROK)
       {
          logError("%s", "cmUnpkKwMngmt failure ......!");
       }
       RETVALUE(RFAILED);
    }
