...
#endif
{
   ...
   if((ret1 = SGetSBuf(pst->region, pst->pool, (Data **)&measCfmEvt,\
               sizeof(PjL2MeasCfmEvt))) != ROK)
   {
#if (ERRCLASS & ERRCLS_ADD_RES)
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
            __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
            (ErrVal)ERRLPJ, (ErrVal)0, "SGetMsg() failed");
#endif /*  ERRCLASS & ERRCLS_ADD_RES  */
      RETVALUE(ret1);
   }

   if (SUnpkU32(&measCfmEvt->transId, mBuf) != ROK) {
      SPutSBuf(pst->region, pst->pool, (Data *)measCfmEvt, sizeof(PjL2MeasCfmEvt));
      SPutMsg(mBuf);
#if (ERRCLASS & ERRCLS_ADD_RES)
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
         __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
   ...
   TRC3(cmPkPjSSta)
      ...
      switch(elmnt) {
         ...
         case STCPJSAP:
            ...
            break;
         case STSID:
            /* lpj_c_001.main_2, fix for ccpu00120840 */
