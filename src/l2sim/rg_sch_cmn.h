...
#define RG_SCH_CMN_GET_MCS_FOR_RETX(tb, retxMcs) do {\
	 ...
	 if (tb->dlGrnt.iMcs < 29) {\
            U8 iTbs;\
            RG_SCH_CMN_MCS_TO_TBS(tb->dlGrnt.iMcs, iTbs);\
            RG_SCH_CMN_ITBS_TO_RETX_IMCS(iTbs, retxMcs); \
         } \
	 else {\
            retxMcs = tb->dlGrnt.iMcs; \
	 }\
...
#define RG_SCH_CMN_REFRESH_TIME 100 /* Refresh time/cycle in frames (10ms) */
