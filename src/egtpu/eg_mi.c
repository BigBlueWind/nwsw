...
void sendEgtpStartIndToPrm(void)
{
  unsigned int version = getSvnVersion(MODID_EGTP);
  printf  ("EGTP version: %u\n", version);
  logInfo ("EGTP version: %u", version);
  /*send start indication to PRM indicating successful startup*/
  sendEventToOam (ENTEG, ENT_PRM, EVT_START_IND, sizeof(version), (U8*)&version);
}

void sendEgtpHelloAckToPrm(Buffer   *mBuf)
{
  MsgLen              cCnt;
  ...
  sendEventToOam (ENTEG, ENT_PRM, EVT_HELLO_ACK, sizeof(MsgModPrmHelloAck), (U8*)&helloAck);
}

void procRcvdEgtpExitMsg(void)
{
...
#endif
{
    ...
    ************************************************/
   if (!egCb.tSapLst[TSrvCfg->tSapId]) 
#else
   if (!egCb.tSapCb) 
#endif
   {
...
#ifdef PWAV
                     /* Overwriting the existing dbgMask value */
                     EGDBGP(DBGMASK_MI, (egCb.init.prntBuf,
		           "DBG-Mask value :%02x\n", egCb.init.dbgMask));
#endif
                        break;
               ...
               case SADBG:
                  {
                     ...
                     /* Overwriting the existing dbgMask value */
                     EGDBGP(DBGMASK_MI, (egCb.init.prntBuf,
		           "DBG-Mask value :%02x resetting with ~%02x\n", egCb.init.dbgMask, genCntrl->u.cntrl.s.dbgCntrl.genDbgMask));
#endif
...
#endif
{
   ...
   /*---- eg006.201: Reset Echo timer start flag to FALSE; --*/
   egCb.echoTmrStarted = FALSE;

   /*********************************************** 
    *eg007.201: dstCbList already freed in TSapCb *
    ***********************************************/
   /******************************************************************** 
    *eg009.201: dstCbList for non-multi IP removeThis will not be here *
    *******************************************************************/
...
/* eg012.201 : Added code for no EG_MULTI_IP */
#ifndef EG_MULTI_IP
   /***************************
    * Check if the SAP Exists *
    ***************************/
...
/main/3      eg012.201   avenugop        1. Added SS_DIAG diagnostic changes
*********************************************************************91*/
