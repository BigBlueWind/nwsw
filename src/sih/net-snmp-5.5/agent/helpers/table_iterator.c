...
/*
 *  If SNMP Get Processing is failed then we will send the BSM Error Code to Manager.
 *
 */
#ifdef PWAV_SIH
	   	
	    if(errCode != SNMP_ERR_NOERROR && errTableId == requests->requestvb->name_loc[8]) {
		netsnmp_set_request_error(reqinfo, requests, errCode );
		bsmErrCode = errCode;
		errCode = SNMP_ERR_NOERROR;
		errTableId = 0;
		return bsmErrCode;
	    }
#endif
...
/*
 *  If SNMP Set Processing is failed then we will send the BSM Error Code to Manager.
 *
 */
#ifdef PWAV_SIH
