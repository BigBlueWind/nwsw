...
*
*/
SINT8 EtherPotTest(char *pEthername)
{
   int sock = -1;
   struct ifreq ifr;
   u_int32_t  ifAddr;
   ...
   if((ifr.ifr_flags & IFF_UP) == 0)
   {
    ...
    /* Self ping test is removed for now */
#if 0 
...
int bistmain
    ...
    switch(cmd)
        ...
        case 0: /* "h" */
            ...
            printf("\t gm  - get mac address ,Give 1 for eth1 ,2 for eth2 <port num >\n");
            printf("\t sm  - set mac address ,Give 1 for eth1 ,2 for eth2 <mac address > < port>\n");
        ...
        case 1: /* "gm" */
        {
             ...
             if(atoi(ppArgv[2]) != 1 && atoi(ppArgv[2])!= 2)
        ...
        case 2: /* "sm" */
        {
             ...
             if(atoi(ppArgv[3]) != 1 && atoi(ppArgv[3])!=2)
             printf("Invalid port num");
             ...
             if(setMacAddress((UINT8 *)ppArgv[2],portNum) == SETGETMACADDRESSRETURNCODE_SUCCESS)
             {
...
BistReturnCode bistTestStart(BistTestType bistTestType , EtherTestType etherTestType) 
{
   ...
   if(bistTestType == BISTTESTTYPE_BACKHAUL_ETHERNET_PORT || bistTestType == BISTTESTTYPE_DEBUG_ETHERNET_PORT || bistTestType == BISTTESTTYPE_ALL)
   {
       ...
       else if(bistTestType == BISTTESTTYPE_DEBUG_ETHERNET_PORT )
       {
       ...
       else if(bistTestType == BISTTESTTYPE_ALL)
       {
             statusBhPort= EtherPotTest(BACKHUAL_ETHERNET_PORT);
             statusDebugPort= EtherPotTest(DEBUG_ETHERNET_PORT);
