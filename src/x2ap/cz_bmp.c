...
#define CZ_BMP_FILL_UIM_ERR_INFO(_peerCb, _cType, _cVal, _msgType, _oldId, \
                                 _newId) \
{                                                                 \
   cmMemset((U8*)&czBmpErrInd, 0, sizeof(CztLclErrInd));                  \
...
PRIVATE S16 czBmpIsHoReqValid ARGS((CzPeerCb     *peerCb,
                                    CztX2AP_PDU  *pdu, 
                                    CzIeInfo     *ieInfo,
                                    U8           *causeType,
                                    U8           *causeVal));
...
/* cz001.301: Fix for the build warnings, all changes are guarded
...
#endif /* ANSI */
{
   ...
   /*x2ap build warning fixes*/
   /*CZ_SET_ZERO(errIeInfo, CZ_MAX_IDX * sizeof(CzIeInfo));*/
   cmMemset((U8*)&errIeInfo, 0, CZ_MAX_IDX * sizeof(CzIeInfo));
   ...
   if(ROK != (czBmpIsHoReqValid(peerCb, &(evnt->pdu), errIeInfo, 
                                     &causeType, &causeVal)))
   {
      /* Increment the incoming error counter for this message resulted in 
         error */
      ...
      czBmpSndFailRspToPeer(peerCb, evnt, &czUeInfo, errIeInfo, 
                            causeType, causeVal,
                            CZT_ERRCAUSE_PARAM_OUT_OF_BOUNDS);
      CZ_RETVALUE(RFAILED);
   }
...
staInd:
   ...
   /* Invoke a certain function in UIM to generate this */
   /*x2ap build warning fix*/
   /*CZ_SET_ZERO(&tmrExpStaInd, sizeof(CztStaInd));*/
   cmMemset((U8*)&tmrExpStaInd, 0, sizeof(CztStaInd));
...
/** @brief This function validates the Handover Request received from peer for logical
    range of IE set values against the configured ones locally.
 
    @details The IEs validated are:
    - Encryption IE.
    - Integrity Protection IE.
    The UE Security Capabilities IE defines the supported algorithms for encryption
    and integrity protection in the UE.
    ...
    "first bit" - 128-EEA1, 
    "second bit" - 128-EEA2, other bits reserved for future use. 
    Value '1' indicates support and value "0" indicates no support of the algorithm. 

    Integrity Protection Algorithms BIT STRING (16, ...)
    ...
    Algorithms IE in the UE Security Capabilities IE in the UE Context
    Information IE, plus the mandated support of EEA0 in all UEs, do not match
    any algorithms defined in the configured list of allowed encryption algorithms
    in the eNB, the eNB rejects the procedure using the HANDOVER PREPARATION
    FAILURE message.

    If the supported algorithms for integrity defined in the Integrity Protection
    Algorithms IE in the UE Security Capabilities IE in the UE Context Information
    IE, do not match any algorithms defined in the configured list of allowed
    integrity protection algorithms in the eNB or if all bits in Integrity
    Protection Algorithms IE are equal to 0, the eNB rejects the procedure
    using the HANDOVER PREPARATION FAILURE message.
   * 
   * @param[in] peerCb     The active peerCb from this eNodeB involved for an
                           UE handover.
   * @param[in] pdu        The pointer with X2AP ASN PDU structure received
                           from user or from peer for an UE handover.
   * @param[out] errIeInfo  The pointer to IE information set which triggered
                            to result in invalid received Handover Request.
                           from user or from peer for an UE handover.
   * @param[out] causeType  The result cause type when received Handover Request
                            is declared invalid.
   * @param[out] causeVal   The result cause value when received Handover Request
                            is declared invalid.
   * @return 
   * -# ROK if Handover Request validation is successful for IEs.
   * -# RFAILED otherwise.
...
#else /* ANSI */
PRIVATE S16 czBmpIsHoReqValid (peerCb, pdu, errIeInfo, causeType, causeVal)
CzPeerCb    *peerCb;
CztX2AP_PDU  *pdu;
CzIeInfo     *errIeInfo;
U8           *causeType;
...
{
   S16 ret = ROK;
   CztUE_CntxtInform *ueCntxtInfo = NULLP;
   CztUESecurCapabilities *ueSecCap;
   CztEncryptionAlgorithms  *encryptAlg = NULLP;
   CztIntegrityProtectionAlgorithms *intAlg = NULLP;
   U32 encryptAlgVal = 0; 
   U32 intAlgVal = 0; 
   S16 indx = 0;

   CZ_TRC2(czBmpIsHoReqValid);

   czUtlGetIE(pdu, Cztid_UE_CntxtInform, (TknU8 **)(&ueCntxtInfo)); 

   if((NULLP != ueCntxtInfo) && (PRSNT_NODEF == ueCntxtInfo->pres.pres))
   {
     /* Retrieve UE Security Capabilities */
      ueSecCap =  &(ueCntxtInfo->uESecurCapabilities);
      if(PRSNT_NODEF == ueSecCap->pres.pres)
      {
         /* Now take encryption and integrity algorithm(s) access */
         encryptAlg = &ueSecCap->encryptionAlgorithms;
         intAlg = &ueSecCap->integrityProtectionAlgorithms;
 
         if(PRSNT_NODEF == encryptAlg->pres)
         {
            for (indx = ((encryptAlg->len /8)-1);indx >= 0;indx--)
            { 
               encryptAlgVal |= ((U32)((encryptAlg->val[indx]) << (8 *indx))); 
            }
            /* Validate Encryption Algorithms IE value */
            /* CR: ccpu00118850 raised. It will be taken care in next
             * release. */
         }
         
         /* Now Validate Integrity Protection Algorithms IE value */
         
         if(PRSNT_NODEF == intAlg->pres)
         {
   ...
   /* Do not need to return failure, as Decoder is checked for presence, criticality and catch other 
      info for this IE when received */

   if(ROKPENDING == ret)
   {
      CZ_SET_ZERO(errIeInfo, sizeof(CzIeInfo));

      /* Fill the required triggering Error IE information */

      errIeInfo->pres = PRSNT_NODEF; 

      /* There is no separate ID for UE Security 
      Cap or Encryption Algo */ 
      errIeInfo->ieId = Cztid_UE_CntxtInform; 

      /* Criticality of this Triggering IE */
      errIeInfo->crit = CztCriticalityrejectEnum; 

      /* We didnt understand thie IE as per spec and rules */
   ...
   CZ_RETVALUE(ret);
} /* End of function czBmpIsHoReqValid */
...
/main/3   cz001.301  akaranth            1. Fix for the build warnings, all changes are guarded
                                            under CZTV1 flag, for backward compatability.
*********************************************************************91*/
