...
#endif /* End of else */
{
   ...
   if(waitIe != NULLP)
   {
      if(waitIe->pres == PRSNT_NODEF)
      {
         ...
         wait = (wait * SS_TICKS_SEC )/czCb.genCfg.timeRes;
         ...
         {
            CZ_DBG_INFO((CZ_PRNT_BUF, 
               "Started the Time to Wait timer with value %d.\n",  wait));
            peerCb->gpmCb.wait = TRUE;
            cztSapCb = peerCb->cztSapCb;
	    /*x2ap build warning fix*/
            /*CZ_SET_ZERO(&tmrStaInd, sizeof(CztStaInd)); */
	    cmMemset((U8*)&tmrStaInd, 0, sizeof(CztStaInd));
 ...
 * under CZTV1 flag, for backward compatability */
   ...
   else if(msgType == CZT_MSG_MOB_CHANGE_REQ)
   {
      ...
      /*x2ap kwork fixes*/
      if((ieTkn1!=NULL) && (ieTkn1->pres != PRSNT_NODEF))
      {
      ...
      /*x2ap kwork fixes*/
      if((ieTkn2!=NULL) && (ieTkn2->pres != PRSNT_NODEF)) 
      {
