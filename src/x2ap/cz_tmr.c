...
#endif /* SS_MULTIPLE_PROCS */
{
   ...
   if ((ret !=ROK)||
         (czCbPtr == NULLP))
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF,"czActvTmr() failed, cannot derive czCb "
               "with proc(%d),ent(%d),inst(%d) \n",
               proc, ent, inst));
      CZ_RETVALUE(RFAILED);
   }
#endif /* SS_MULTIPLE_PROCS */
...
#else
PUBLIC S16 czStartTmr(tmrType, wait, cb)
CzTmrType      tmrType;
S16            wait;
Void           *cb;
#endif
   ...
   switch(tmrEvnt)
   {
      case CZ_TMR_SCT_BIND:
      {
         ...
         if (lSapCb->state == LCZ_SAP_BINDING)
         {
	       ...
	       cmMemset((U8*)&dgn, 0, sizeof(CzUstaDgn));
