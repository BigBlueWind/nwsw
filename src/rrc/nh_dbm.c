...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Addition of New Transaction Info Block.\n"));
   ...
   if (NULLP == tmp)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "Memory Allocation Failure.\n"));
      RETVALUE(ROUTRES);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Deleting TransCb Block.\n"));
   /* There is already a link list existing in the UE control block
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Adding MUI Control Block.\n"));
   ...
   if (NULLP == tmp)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "Memory Allocation Failed!!.\n"));
      RETVALUE(ROUTRES);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Deletion of Mui Info Block.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, 
                "Fetch the Mui control Block: mui (%ld) \n", mui));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, 
                "Fetch the Mui control Block: mui (%d) \n", mui));
#endif /* ALIGN_64BIT */
   ...
   if (mui <= nhCb.genCfg.minMuiLstLen)
   {
      ...
      if (NULLP != tmp)
      {
         ...
         if ((NULLP != muiCb) && (muiCb->status != NH_STATUS_USED))
         {
            ...
            NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Mui ID.\n"));
            muiCb = NULLP;
         }
      }
      else
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Creating Cell control block.\n"));
   ...
   if (NULLP == tmp)
   {
      ...
      NH_DBG_FATAL((NH_PRNT_BUF, 
               "Memory Allocation failed for CmLList in Cell.\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
               "Memory Allocation failed for cellCb in Cell.\n"));
      NH_FREE(NH_REG, NH_POOL, tmp, sizeof(CmLList));
      RETVALUE(RFAILED);
   }
...
#ifdef PWAV
   /*Updating the Cell Stats CellId*/
   (*cellCb)->cellSts.cellId = cellId;
#endif
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Deleting Cell control Block.\n"));
...
#endif /* ANSI */
{
   ...
   if (ordrdCfgInfo->cellCfgInfo.cellCfgBitMsk & NHU_CELL_CFG_RB)
   {
      ...
      /* Allocate memory for NhuRbCfgInfo structure */
         ...
         {
            NH_DBG_FATAL((NH_PRNT_BUF, 
                     "Memory Allocation failed for RbCfg in Cell.\n"));
            NH_FREE(NH_REG, NH_POOL, tmp, sizeof(CmLList));
            NH_FREE(NH_REG, NH_POOL,
                  cellCb->cellTransCb.cellOrdrdCfg, sizeof(NhuCellCfgInfo));
            RETVALUE(ROUTRES);
      ...
      default:
         {
            ...
            NH_DBG_ERROR((NH_PRNT_BUF, "Unexpected config action.\n"));
            RETVALUE(RFAILED);
         }
   } /* switch(cellCb->cellTransCb.cfgAction) */
...
#endif /* ANSI */
{
   ...
   switch(action)
   {
      case NH_CFG_ACTN_ADD:
      {
         ...
         {
             NH_DBG_FATAL((NH_PRNT_BUF, 
                "Memory Allocation failed for adding Logical ChannelCfg.\n"));
      ...
      case NH_CFG_ACTN_MODIFY:
      {
         ...
         while(NULLP != tmpNode)
         {
            ...
            CM_LLIST_NEXT_NODE(&crntCfg->rbCfgList.rbCfgInfoList, tmpNode);
         }
         ...
         while(NULLP != tmpNode)
         {
                                             ...
                                             == rbCfgInfo->rbId.rbTyp)
            {
               ...
               NH_FREE(NH_REG, NH_POOL, tmpNode->node,
                                      sizeof(NhRbToLchMap));
               NH_FREE(NH_POOL, NH_POOL, tmpNode, sizeof(CmLList));
               break;
            }
            CM_LLIST_NEXT_NODE(&crntCfg->rbCfgList.rbCfgInfoList, tmpNode);
         }
      ...
      if(NH_UE_INFO_MOD_CRNTI == ordCfgInfo->ueInfo.crntiCfgType)
      {
         ...
         if (ROK != retVal)
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "Could not delete from hash list.\n"));
         }
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "Could not insert into hash list.\n"));
            RETVALUE(retVal);
         }
      }
   }
   ...
   for(i=0; i < ordCfgInfo->rbToModCfgList.numOfRbCfgInfo; i++)
   {
      ...
      while(NULLP != tmpNode)
      {
           ...
           && (((NhuRbCfgInfo *) tmpNode->node)->rbId.rbTyp == ordCfgInfo->rbToModCfgList.rbCfgInfoList[i].rbId.rbTyp))
         {
            ...
            if(ROK != retVal)
            {
               NH_DBG_ERROR((NH_PRNT_BUF,"nhDBMUpdModifiedRb failed\n"));
               RETVALUE(RFAILED);
            }
            nhDBMUpdRbToLchOrPdcp(crntCfgInfo,
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhDBMCommitOrdrdCfg \n"));
...
#endif /* ANSI */
{
   ...
   if(ROK != ret)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "UE with ID: %d not found in cell ID: %d.\n", 
                                       ueId, cellCb->cellId));
      RETVALUE(NULLP);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Initialising UE control block.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Creating UE control Block.\n"));
   ...
   if(ROK != ret)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "UE initialisation failed.\n"));
   ...
   if(ROK != ret)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "UE addition to cell hash list failed.\n"));
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, 
            "UE control Block Created and Initialized successfully.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "Deleting UE control Block.\n"));
