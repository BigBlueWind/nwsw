...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhSchedGenTmr \n"));
   ...
   switch(tmrEvnt)
   {
...
#endif /* RRC_PHY_CFG_SUPP */
      {
         if(action == NH_TMR_START)
         {
            ...
            else
            {
               NH_DBG_INFO((NH_PRNT_BUF, 
                          " nhSchedGenTmr: Cfg timer not enabled \n"));
               RETVALUE(ROK);
            }
         }
         else 
         {
            ...
            {
               NH_DBG_INFO((NH_PRNT_BUF, 
                          " nhSchedGenTmr: Bind timer not running \n"));
               RETVALUE(ROK);
            }
         }
      } 
      ...
      case NH_TMR_CELL_CFGCFM:
      {
         ...
         else
         {
            ...
            {
               NH_DBG_INFO((NH_PRNT_BUF, 
                          " nhSchedGenTmr: Cell Cfg timer not running \n"));
               RETVALUE(ROK);
            }
         }
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhStartLLyrCfgTmr \n"));
       ...
       && (nhCb.genCfg.cfgTmr.enb == TRUE))
   {
      ...
      if (NULLP == tmrCb)
      {
         NH_DBG_FATAL((NH_PRNT_BUF, "mem alloc for tmrCb failed \n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhStopLLyrCfgTmr \n"));
   ...
   if ((cellAndUeCb) && (cellAndUeCb->cellCb) && (cellAndUeCb->ueCb))
   {
      ...
      {
         NH_DBG_INFO((NH_PRNT_BUF, 
                    " nhStopLLyrCfgTmr: Cfg timer not running \n"));
         RETVALUE(ROK);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhProcGenTmrExpiry \n"));
   ...
   switch(tmrEvnt)
   {
...
#endif /* RRC_PHY_CFG_SUPP */
      {
         ...
         if (lSapCb == (NhLSapCb *)NULLP)
         {
            NH_DBG_ERROR((NH_PRNT_BUF, 
                          "Null lSapCb in nhProcGenTmrExpiry\n"));
            RETVOID;
         }
      ...
      case NH_TMR_CELL_CFGCFM:
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, 
                          "Null cellCb in nhProcGenTmrExpiry\n"));
            RETVOID;
         }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhProcLLyrCfgTmrExpiry \n"));

   tmrCb = (NhLLyrCfgTmrCb *)ctrlBlk;

   if (NULLP == tmrCb)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Timer control block NULL!\n"));
      RETVALUE(RFAILED);
   }
      ...
      transCb->ordrdCfg.ueInfo.crntiCfgType == NH_UE_INFO_MOD_CRNTI)
   {
      ...
      {
          NH_DBG_FATAL((NH_PRNT_BUF, "Could not delete from  list.\n"));
          RETVALUE(RFAILED);
      }
   }
