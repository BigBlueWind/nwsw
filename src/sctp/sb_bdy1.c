...
void sendSctpStartIndToPrm(void)
{
  U32 version = getSvnVersion(MODID_SCTP);
  logInfo ("SCTP svn version : %d", version);
  printf  ("SCTP svn version : %d\n", version);
  /*send start indication to PRM indicating successful startup*/
  if( sendSctpMsgToPrm(EVT_START_IND,  sizeof(U32), &version ) == ROK)
  {
    logMsgOut("%s", "Sent EVT_START_IND from SCTP to PRM" );
  }
}
inline PUBLIC S16 sendSctpMsgToPrm (Event event, MsgLen len, U8 *data) { return sendEventToOam (ENTSB /*sbGlobalCb.sbInit.ent*/, ENT_PRM, event, len, data); }
//inline PUBLIC void sendSctpHelloAckToPrm (void) { sendSctpMsgToPrm (EVT_HELLO_ACK, 0, NULL); }

void sendSctpHelloAckToPrm(
  Buffer  *mBuf
  )
{
  MsgLen              cCnt;
  ...
  if( sendSctpMsgToPrm(EVT_HELLO_ACK, sizeof(MsgModPrmHelloAck), (U8*)&helloAck) == ROK)
  {
    logMsgOut("%s", "Sent EVT_HELLO_ACK from SCTP to PRM" );
  }
}

void procRcvdSctpExitMsg(void)
{
