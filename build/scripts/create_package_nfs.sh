#!/bin/bash
# Creates a nfs image of eNB run-time software
...
cp -f  $buildDir/scripts/dump_lowerarm_256.sh  $setupDir/run/dump_lowerarm.sh
...
cp -f  $buildDir/scripts/pkg_installer_4gmx.sh     $setupDir/rel/inst_A/pkg_installer.sh
...
cp -f $buildDir/bin/4gmx/lowerarm/lowerarm_256.elf    $setupDir/rel/inst_A/bin/lowerarm.elf
find  $buildDir/bin/4gmx/ -name snmpset -exec cp '{}'        $setupDir/rel/inst_A/bin \;
...
tar -cvzf picobs.tgz $setupDir/*

rm -rf $setupDir

case "$?" in
  0) echo "--> Created NFS File:$installFile for 4gmx ";;
