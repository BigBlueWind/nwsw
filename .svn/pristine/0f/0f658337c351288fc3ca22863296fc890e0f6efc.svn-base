...
PRIVATE S16 pjLmmInitDlForBG ARGS ((Void));
PRIVATE S16 pjLmmInitDelRbForBG ARGS ((Void));
...
#endif
{
   ...
   PJDBGP((KW_PJ_DBGMASK_LMM | KW_DBGMASK_BRIEF), (_kwPBuf,
            "PjMiLpjCfgReq(post, elmId(%d))\n", cfg->hdr.elmId.elmnt));

  PJ_DIAG_LVL0(0x0c0d0001, PJ_DIAG_NA, SS_DIAG_INV_ARG,
         ...
         (cfg->hdr.elmId.elmnt != STGEN))
   {
      ...
      /*MSPD_DBG("<Rahul> Gen cfg is not done\n");*/
   ...
   PJDBGP((KW_PJ_DBGMASK_LMM | KW_DBGMASK_DETAIL), (_kwPBuf,
            "PjMiLpjCfgReq - cfm: status(%d) reason(%d)\n",
...
#endif
{
   ...
   PJDBGP((KW_PJ_DBGMASK_LMM | KW_DBGMASK_BRIEF), (_kwPBuf,
            "PjMiLpjCntrlReq(post, elmId(%d))\n", cntrl->hdr.elmId.elmnt));

  PJ_DIAG_LVL0(0x0c0d0002, PJ_DIAG_NA, SS_DIAG_INV_ARG,
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0d0003, PJ_DIAG_NA, SS_DIAG_INV_ARG,
               "Received StaReq from LM, element = %d",
                sta->hdr.elmId.elmnt, 0, 0, 0)

   PJDBGP((KW_PJ_DBGMASK_LMM | KW_DBGMASK_BRIEF), (_kwPBuf,
            "PjMiLpjStaReq(post, elmId(%d))\n", sta->hdr.elmId.elmnt));
   ...
   PJDBGP((KW_PJ_DBGMASK_LMM | KW_DBGMASK_DETAIL), (_kwPBuf,
            "PjMiLpjStaReq - cfm: status(%d) reason(%d)\n",
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0d0004, PJ_DIAG_NA, SS_DIAG_INV_ARG,
...
#endif
{
   ...
   cmPrcTmr(&(kwCb.pjTqCp), kwCb.pjTq, (PFV) pjTmrExpiry);
...
/**
 * @brief This funtion initializes PDCP for background processing
 *
 *  @param[in] Void        none
 *  @return    Void        none
 */
#ifdef ANSI
PRIVATE S16 pjLmmInitDlForBG
(
Void
)
#else
PRIVATE S16 pjLmmInitDlForBG(Void)
#endif
{
#ifdef MSPD_RLC_MULTICORE
   if (SInitLock(&kwCb.pjSelfPstReestLock, SS_LOCK_MUTEX) != ROK)
   {
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0,
                                    "Lock initialization failed\n");
      RETVALUE(LCM_REASON_ULLOCK_INIT_FAILED);
   }
   if (SInitLock(&kwCb.pjSelfPstDlLock, SS_LOCK_MUTEX) != ROK)
   {
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0,
                                    "Lock initialization failed\n");
      RETVALUE(LCM_REASON_ULLOCK_INIT_FAILED);
...
#endif
   /* Initialize selfPst  */
   KW_MEM_SET(&(pjSelfPstDl), 0, sizeof(Pst));
   pjSelfPstDl.dstProcId = SFndProcId();
   pjSelfPstDl.dstEnt  = ENTPJ;
   pjSelfPstDl.srcEnt  = ENTKW;
   pjSelfPstDl.dstInst = 0;
   pjSelfPstDl.srcInst = 0;
   pjSelfPstDl.prior   = PRIOR3;
   pjSelfPstDl.event   = EVTPJSELFMSGDL;
   /*One time allocation for the mBufDl used for DL Ack deletion*/
   /* Inititalize pjDrbReestLst in kwCb */
   cmLListInit(&kwCb.pjDrbReestLst);
   SGetMsg(kwCb.init.region, kwCb.init.pool, &kwCb.pjMBufDl);
 ...
 */
#ifdef ANSI
PRIVATE S16 pjLmmInitDelRbForBG
(
Void
)
#else
PRIVATE S16 pjLmmInitDelRbForBG(Void)
#endif
{
#ifdef MSPD_RLC_MULTICORE
   /* Initialize the DL lock for PDU release */
   if (SInitLock(&kwCb.pjDelRbLstLock, SS_LOCK_MUTEX) != ROK)
   {
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0,
                                    "Lock initialization failed\n");
      RETVALUE(LCM_REASON_ULLOCK_INIT_FAILED);
...
#endif
   /* Initialize selfPst  */
   KW_MEM_SET(&(pjSelfPstDelRb), 0, sizeof(Pst));
   pjSelfPstDelRb.dstProcId = SFndProcId();
   pjSelfPstDelRb.dstEnt  = ENTKW;
   pjSelfPstDelRb.srcEnt  = ENTKW;
#ifdef MSPD
   pjSelfPstDelRb.dstInst = 1;
#else
   pjSelfPstDelRb.dstInst = 0;
#endif
   pjSelfPstDelRb.srcInst = 0;
   pjSelfPstDelRb.prior   = PRIOR3;
   pjSelfPstDelRb.event   = EVTKWSELFMSGPJDELRB;
   /* Inititalize reestablishRbList in kwCb */
   cmLListInit(&kwCb.pjDelRbLst);
   /*One time allocation for the mBufReest used for Re-establishment procedure*/
   SGetMsg(kwCb.init.region, kwCb.init.pool, &kwCb.pjMBufDlRb);
   RETVALUE(ROK);
}

/* LOG_COUNT 0x60d00004 */
...
void sendPjStartIndToPrm(void)
{
  ...
  {
    MSPD_DBG("Sent EVT_START_IND from PJ to PRM \n");
  }
}
...
void sendPjHelloAckToPrm(Buffer   *mBuf)
{
  ...
  {
    MSPD_DBG( "sendPjHelloAckToPrm(): SCpyMsgFix Failed\n" );
    RETVOID;
  }
  ...
  {
    MSPD_DBG("Sent EVT_HELLO_ACK from PJ to PRM for sequence number:%d \n", helloAck.helloAckSeqNum );
  }
  else
  {
    MSPD_DBG("Sent EVT_HELLO_ACK from PJ to PRM failed for sequence number:%d \n", helloAck.helloAckSeqNum );
  }
}

void procRcvdPjExitMsg(void)
{
  MSPD_DBG("Exiting the PJ \n" );
  exit(0);
}
#endif
