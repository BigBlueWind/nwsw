...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0001, PJ_DIAG_NA, SS_DIAG_INV_ARG,
               "Received BndReq from RRC, spId = %d, suId = %d",
                spId, suId, 0, 0)
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0002, PJ_DIAG_NA, SS_DIAG_INV_ARG,
               "Receivd UnBnd Req from RRC, spId = %d",
                spId, 0, 0, 0)
   UNUSED(pst);
   UNUSED(reason);
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0003, PJ_DIAG_NA, SS_DIAG_INV_ARG,
            "Received CfgReq from RRC, rbId = %d, rbType = %d",
              entCfg->rbId, entCfg->rbType, 0, 0)
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0004, PJ_DIAG_UEID, ueInfo->ueId,
               "Received UE Id change req from RRC, oldCellId = %d, new UeId = %d, newCellid = %d",
                ueInfo->cellId, newUeInfo->ueId, newUeInfo->cellId, 0)
   ret = ROK;
   tUeInfo = NULLP;
   cpjSap = &(kwCb.cpjSap);
   ...
   {
      MSPD_ERR("Failed, old ue (%d, %d), new ue (%d, %d)\n", ueInfo->cellId, 
                ueInfo->ueId, newUeInfo->cellId, newUeInfo->ueId);
      PJDBGP((KW_PJ_DBGMASK_INF | KW_DBGMASK_CFG | KW_DBGMASK_ERROR),
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0005, PJ_DIAG_UEID, secCfg->ueId,
               "Received Security CfgReq from RRC, cellId = %d, reason = %d",
                secCfg->cellId, cfgCfm->reason, 0, 0)
...
#endif
{
  ...
  PJ_DIAG_LVL0(0x0c0f0006, PJ_DIAG_UEID, reEstReq->pdcpId.ueId,
               "Received ReEstReq from RRC, cellId = %d",
                reEstReq->pdcpId.cellId, 0, 0, 0)

   if ( pjCfgReEstReq(reEstReq,reEstCfm) != ROK )
   {
      MSPD_ERR("Failed, cell Id %d, ueId %d, reason %u\n", 
                reEstReq->pdcpId.cellId, reEstReq->pdcpId.ueId, reEstCfm->status);
      PJDBGP((KW_PJ_DBGMASK_INF | KW_DBGMASK_CFG | KW_DBGMASK_ERROR),
...
#endif
{
   ...
   {
      MSPD_ERR("Failed, cellId %d, ueId %d\n", staReq->cellId, staReq->ueId);
      PJDBGP((KW_PJ_DBGMASK_INF | KW_DBGMASK_CFG | KW_DBGMASK_ERROR),
...
#endif
{
   ...
   {
      MSPD_ERR("Fetching UeCb failed, cellId %d, ueId %d\n", pdcpId->cellId, pdcpId->ueId);
   ...
   if (pjRbCb == NULLP)
   {     
      MSPD_ERR("Fetching RbCb failed, cellId %d, ueId %d\n", pdcpId->cellId, pdcpId->ueId);
      PJDBGP((KW_PJ_DBGMASK_INF | KW_DBGMASK_ERROR), (_kwPBuf,
   ...
   if ( ret != ROK )
   {
      MSPD_ERR("[%d][%d][%d]pjDlmProcessSdus failed\n", pjRbCb->ueCb->key.ueId, 
                pjRbCb->rbType, pjRbCb->rbId);
#if (ERRCLASS & ERRCLS_INT_PAR)
