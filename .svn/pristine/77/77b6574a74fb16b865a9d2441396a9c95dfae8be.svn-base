 ...
 ******************************************************************/
PRIVATE S16 egUtilDropIeList ARGS( (CmLListCp *ieList));
 ...
 *****************************************/
PRIVATE S16 egUtilDupIe ARGS( (CmMemListCp *memCp, EgIe *oldIe, EgIe **dupIe));
...
#endif
{
   ...
   switch (oldIe->dataType)
   {
      ...
      case EG_FTEID:
      {
         ...
         {
            egUtilDupIpv4Addr(( (EgIe *)(*dupIe))->t.fTeid.u.ip4Addr, 
                                 oldIe->t.fTeid.u.ip4Addr);
         }
         else if(oldIe->t.fTeid.isIPv6Pres)
         {
            egUtilDupIpv6Addr(( (EgIe *)(*dupIe))->t.fTeid.u.ipv6Address, 
                                 oldIe->t.fTeid.u.ipv6Address);
         }
         break; 
      }
      ...
      case EG_PDN_CONN_SET_ID:
      {
         ...
         ( (EgIe *)(*dupIe))->t.csid.csid.length = oldIe->t.csid.csid.length;
         cmMemcpy( (U8* )&(((EgIe *)(*dupIe))->t.csid.csid.val), 
                     (U8 *)& oldIe->t.csid.csid.val, oldIe->t.csid.csid.length);
         break;
      }
...
#endif /* LEG_FILE_LOG */
{
   ...
   *********************************************/
   ...
   switch(egIe->dataType)
   {
      ...
      case EG_BEARER_FLAG:
         {
            sprintf(buff,"isPPCPres: %d\n", egIe->t.bearFlags.isPPCPres);  
            sprintf(tmpBuff,"isVBPres: %d\n", egIe->t.bearFlags.isVBPres);  
            EG_STRCAT(buff,tmpBuff);
            break; 
         }
      ...
      case EG_FTEID:
         {
            ...
            {
               egUtilDumpIpv4Addr(buff,egIe->t.fTeid.u.ip4Addr);
            }
            else if(egIe->t.fTeid.isIPv6Pres)
            {
               egUtilDumpIpv6Addr(buff,egIe->t.fTeid.u.ipv6Address);
            }
            break; 
         }
      ...
      case EG_PDN_CONN_SET_ID:
      {
         ...
         cmMemcpy( (U8* )tempBuff, egIe->t.csid.csid.val, 
               egIe->t.csid.csid.length);
         sprintf(tmpBuff," csid.length: %d\n csid.val: %s\n", 
               egIe->t.csid.csid.length, tempBuff);
         cmMemset( (U8* )tempBuff, 0, EG_UTIL_PRNT_BUFF_SIZE-5);
         EG_STRCAT(buff,tmpBuff);
         break;
      }
...
*/
         ...
         {
            printf( tmpBuff," ebi%d: %d\n", (temp+1), egIe->t.s103PDF.ebi.val[temp]);
         }
...
*/
...
#else
         sprintf( tmpBuff," upSubsUEAMBR: %ld\n", egIe->t.gsmKeyT.upSubsUEAMBR);
#endif
         EG_STRCAT(buff,tmpBuff);
      ...
      case EG_F_CAUSE:
      {
         ...
         cmMemcpy( (U8* )tempBuff, egIe->t.fCause.cause.val, 
               egIe->t.fCause.cause.length);
         sprintf(tmpBuff," cause.length: %d\n cause.val: %s\n", 
               egIe->t.fCause.cause.length, tempBuff);
         EG_STRCAT(buff,tmpBuff);
         break;
      }
      ...
      case EG_SRCID:
      {
         cmMemcpy( (U8* )tempBuff, egIe->t.cellId.targetCellId.val, 
               egIe->t.cellId.targetCellId.length);
         sprintf(buff," targetCellId.length: %d\n targetCellId.val: %s\n", 
               egIe->t.cellId.targetCellId.length, tempBuff);

         sprintf(tmpBuff," sourceType: %d\n", egIe->t.cellId.sourceType);
         EG_STRCAT(buff,tmpBuff);

         cmMemcpy( (U8* )tempBuff, egIe->t.cellId.sourceId.val, 
               egIe->t.cellId.sourceId.length);
         sprintf(tmpBuff," sourceId.length: %d\n sourceId.val: %s\n", 
               egIe->t.cellId.sourceId.length, tempBuff);
         EG_STRCAT(buff,tmpBuff);
         break;
      }
...
#else
         sprintf( buff," upSubsUEAMBR: %ld\n", egIe->t.ambr.uplAPNAMBR);
#endif

#ifdef ALIGN_64BIT
         sprintf( tmpBuff," dnlAPNAMBR: %d\n", egIe->t.ambr.dnlAPNAMBR);
#else
         sprintf( tmpBuff," dnlAPNAMBR: %ld\n", egIe->t.ambr.dnlAPNAMBR);
#endif
         EG_STRCAT(buff,tmpBuff);
         break;
      }      
...
EgMsg      *egMsg,         /* Diameter message */
EgIeLevelInfo *ieLvlInfo,    /* IE level information */
S16            *ieOccrCnt     /* IE count at a level */
)
#else
   ...
   *ieOccrCnt = 0;
   ...
   if (ieLvlInfo->ieInfo[trgtLvl].ieTypePres == TRUE)
   {
           ...
           egIe = (EgIe* ) CM_LLIST_NEXT_NODE(ieList, node))
      {
          ...
          * the occrrence count
          */
         if (egIe->ieType == ieLvlInfo->ieInfo[trgtLvl].ieType)
         {
            (*ieOccrCnt)++;
...
#endif
{
	/* ccpu00125923: klocwork fix , removed unnecessary variable initializations */
   EgUIe       *egIe; /* this is used to hold the temporary node address */
   CmLList     *node; /* initialize the occrrence count as zero initially */
   S16         retVal  = ROK; /* klocwork : return value initialized to failure */
   ...
   *ieOccrCnt = EG_ZERO; /* initialize the occrrence count as zero initially */
   ...
   /* klocwork fix: sanity check removed, egMsg is always verified in pack/unpack */
