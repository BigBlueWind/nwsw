...
typedef struct kwPduMap
{
  ...
  Bool         rcvd;              /*!< TRUE if an ACK is received for the PDU */
}KwPduMap;
...
typedef struct kwSdu                        
{
   ...
   union
   {
...
#ifdef MSPD		 
		 U32            sduRecTime;    /*!< This Indictres the Time when 
		                                 SDU was received this is for checking how long
										 it has stayed in the tm quque*/
#endif										 
      }tm;                          /*!< SDU for TM mode */
      ...
      struct   _u
      {
         ...
         Bool          isSegmented; /*!< TRUE if the SDU is segmented */
      }um;                          /*!< SDU for UM mode */
      ...
      struct   _a
      {
         ...
         U8            ackedPdus;   /*!< Number of PDUs acked for this SDU */
         KwPduMap      pduMap[KW_MAX_PDU_MAP]; /*!< Array of PDUs for the SDU */
         Bool          isSegmented; /*!< Flag to tell if the SDU is segmented */
      }am;                          /*!< SDU for AM mode */
...
typedef struct kwBoRep
{
   ...
   Bool        staPduPrsnt;          /*!< TRUE if the bo has control PDU */
} KwBoRep;
...
typedef struct kwUmDl
{
   ...
   S32         bo;             /*!< Buffer occupancy */
   S32         estHdrSz;       /*!< Estimated hdr size for the available BO */
   KwSn        vtUs;           /*!< VT(US) */
...
typedef struct kwUmRecBuf
{
   ...
   CmLList     reassemblyLnk;     /*!< Link to add to toBeReasmbldUmPduLst */
}KwUmRecBuf;
...
typedef struct kwUmUl
{
   ...
   KwSn        vrUh;                  /*!< VR(UH) - Highest rcvd SN + 1 */
   KwSn        vrUx;                  /*!< VR(UX) - Reordering state variable */
   CmTimer     reOrdTmr;              /*!< Reordering Timer */
   U8          reOrdTmrInt;           /*!< Timer Interval */
   Buffer      *partialSdu;           /*!< Partially reassembled SDU */
   CmLListCp   reassemblyLst;         /*!< List of UL PDUs to be reassembled */
   KwUmRecBuf  *partPdu;              /*!< Partially processed */
}KwUmUl;
/*@}*/
...
{
   KwSdu         *sdu;            /*!< Pointer to SDU */
   //U32           sduId;           /*!< SDU Identifier */
   MsgLen        actSz;           /*!< SDU Size */
   MsgLen        sduSz;           /*!< SDU Size */
   Bool          isSegmented;     /*!< Flag to indicate SDU is segmented */
}KwSduMap;
...
typedef struct kwRetx
{
   ...
   KwSduMap    sduMap[KW_MAX_DL_LI]; /*!< Array of SDUs mapped with PDU */
   CmLList     rlsReTxLnk;        /*!< Link to add to release ReTx lst */
}KwRetx;
...
typedef struct kwTx
{
   ...
   KwAmHdr     amHdr;             /*!< AM header Info */
   U8          numSdu;            /*!< Number of PDUs in sduMap */
   KwSduMap    sduMap[KW_MAX_DL_LI]; /*!< Array of SDUs for this PDU */
...
typedef struct kwAmDl
{
   ...
   KwSdu       *partialSdu;      /*!< SDU that is partially included */
   CmLListCp   txedSduQ;         /*!< SDUs already transmitted */
   ...
   U32         estHdrSz;         /*!< Estimated hdr size for available BO */
   ...
   KwSn        vtMs;             /*!< VT(MS) - Max send state variable */
   KwSn        vtA;              /*!< VT(A) - ACK state variable */ 
   ...
   CmLList     staPduLnk;        /*!< Link for status PDU list   */
   Buffer      *staPdu;          /*!< Status PDU for slow path   */
   U8          fByte;            /*!< First byte of status pdu   */
   Bool        isRbstalled;      /*!< set when the DL MAX Retx exceeded */
   U8          staPduFirstByte;  /*!< For background STA PDU processing */
}KwAmDl;
...
typedef struct kwAmRecBuf
{
   ...
   Bool        allRcvd;          /*!< All bytes received flag for this PDU */
   CmLList     reassemblyLnk;    /*!< Link to add to toBeReasmbldAmPduLst */
}KwAmRecBuf;
...
typedef struct kwAmUl
{
   ...
   KwSn        vrMs;           /*!< VR(MS) - Maximum STATUS transmit 
                                                 state variable */
   Bool        staTrg;         /*!< Flag to indicate status trigger occured */
...
#ifdef KW_BG_UL_PROC
   CmLListCp   reassemblyLst;  /*!< List of PDUs for reassembly */
   KwAmRecBuf  *partPdu;       /*!< Partially processed */
#endif
#ifdef MSPD
   U16         lastGt;         /*!< Time at which last PDU was received */
#endif
}KwAmUl;
...
typedef struct _kwRbCb
{
...
#endif /* KW_PDCP */
   CmLList        deleteRbLnk;         /*!< Link to add to deleteRbLst */
   CmLList        pduRlsRbLnk;         /*!< Link to add to pduRlsRbLst */
   CmLList        reassemblyRbLnk;     /*!< Link to add to reassemblyRbLst */
   CmLList        reestablishRbLnk;    /*!< Link to add to reestablishRbLst */
   CmLList        pjDelRbLnk;
   ...
   U8             rbOp;                /*!< Multipurpose flag for both RB
                                            ...
                                            and UL PDUs are completed.*/
   U8             reestInProgress;
   U16            staProStartTime;
#ifdef SIM_MAXRETX
   U16            tempStaPduCnt;      /*!< Added for simulating DL MAX RETX*/
#endif
#ifdef SIM_REEST
   U8             wrapCnt;            /*!< Added for simulating re-establishment */
#endif
}KwRbCb;
...
typedef struct _kwCb
{
   ...
   CmTqCp        kwTqCp;              /*!< RLC Timer queue control point */
   CmTqType      pjTq[PJ_TMR_LEN];    /*!< Timer queue */
   CmTqCp        pjTqCp;              /*!< PDCP Timer queue control point */
   CmHashListCp  cellLstCp;           /*!< Hashlist of CellCb */
   CmHashListCp  ueLstCp;             /*!< Hashlist of UeCb */
#ifdef LTE_L2_MEAS
   KwL2Cb        kwL2Cb;              /*!< CB for L2 Measurements in RLC */
#endif
#ifdef KW_PDCP
   ...
   Buffer        *mBufReest;          /*!< Buffer for self post message */
   Buffer        *mBufDelete;         /*!< Buffer for self post message */
   Buffer        *pjMBufDl;         /*!< Buffer for self post message */
   Buffer        *pjMBufDlRb;         /*!< Buffer for self post message */
   U32           numDlUmSdus;         /*!< Current number of DL UM SDUs */
#ifdef MSPD_RLC_MULTICORE
   SLockId       dbmLock;             /*!< Locking the DBM operations */
   SLockId       deleteSelfMsgPosted; /*!< This flag indicates presence of msg */
   SLockId       reestSelfMsgPosted;  /*!< This flag indicates presence of msg */
   SLockId       dlSelfMsgPosted;     /*!< Flag set if message is posted */
   SLockId       ulSelfMsgPosted;     /*!< This flag indicates presence of msg */
   SLockId       ulLock;              /*!< Mutex for queue opeations PDUs */
   SLockId       selfPstUlLock;       /*!< Mutex for posting the message */
   SLockId       dlLock;              /*!< Mutex for PDU list operations */
   SLockId       selfPstDlLock;       /*!< Mutex for simultaneous posts */
   SLockId       sduLock;             /*!< Used while adding SDU */
   SLockId       deleteRbLstLock;     /*!< Used while adding RB to delete list */
   SLockId       selfPstDeleteLock;
   SLockId       reestRbLstLock;      /*!< Used while adding RB to reest list */
   SLockId       selfPstReestLock;
   SLockId       pjSelfPstReestLock;   
   SLockId       pjSelfPstDlLock;         
   SLockId       pjDelRbLstLock;
   SLockId       pjDelRbSelfMsgPosted; /*!< This flag indicates presence of msg */
#else
   U8            deleteSelfMsgPosted; /*!< This flag indicates presence of msg */
   U8            reestSelfMsgPosted;  /*!< This flag indicates presence of msg */
   U8            dlSelfMsgPosted;     /*!< Flag set if message is posted */
   U8            ulSelfMsgPosted;     /*!< This flag indicates presence of msg */
   U8            pjDelRbSelfMsgPosted; /*!< This flag indicates presence of msg */
#endif
   CmLListCp     staPduRbLst;
   CmLListCp     reassemblyRbLst;     /*!< RB list with PDUs to be processed */
   CmLListCp     pduRlsRbLst;         /*!< RB List with DL PDUs to be released */
   CmLListCp     reestablishRbLst;    /*!< List of RBs to be re-established */
   CmLListCp     deleteRbLst;         /*!< List of RBs to be deleted */
   CmLListCp     pjDrbReestLst;         /*!< List of RBs to be deleted */   
   CmLListCp     pjDelRbLst;         /*!< List of RBs to be deleted */   
}KwCb;
...
EXTERN Pst selfPstReest;              /*!< Pst structure to post the message to 
                                      rlc itself for reestablishment */
EXTERN Pst selfPstDelete;             /*!< Pst structure to post the message to 
                                      rlc itself for UE delete */
EXTERN Pst pjSelfPstDl; 
EXTERN Pst pjSelfPstDelRb; 
/****************************************************************************
 *                      EXTERN Declarations
 ***************************************************************************/
...
EXTERN Void  kwUtlSetReestInProgress ARGS ((KwUeCb *ueCb));
...
EXTERN Void kwUtlUlAddRbToReassemblyLst ARGS ((KwRbCb   *rbCb));
EXTERN Void kwUtlUlRemRbFrmReassemblyLst ARGS ((KwRbCb   *rbCb));
EXTERN Void kwUtlPostSelfMsgReest ARGS ((Void));
EXTERN Void kwUtlPostSelfMsgDelete ARGS ((Void));
EXTERN Void kwUtlProcessSelfMsgDelete ARGS ((Void));
EXTERN Void kwUtlAddRbToReestablishLst ARGS ((KwRbCb *rbCb));
...
EXTERN S16 kwAmmReEstablish     ARGS(( KwRbCb *rbCb));
...
EXTERN Void kwAmmProcessSelfMsgReest ARGS ((KwRbCb *rbCb));
EXTERN Void kwAmmGetRbFrmDeleteLst ARGS ((KwRbCb **rbCb));
EXTERN Void kwAmmProcessSelfMsgDelete ARGS ((KwRbCb *rbCb));
EXTERN Void kwAmmUlProcessSelfMsg ARGS ((KwRbCb *rbCb, U16 *numSdus));
EXTERN Void kwAmmGetNumRbInDeleteLst ARGS ((U16 *numRb));
EXTERN Void kwAmmDlRemSdu ARGS ((KwRbCb *rbCb, KwSdu *sdu));
EXTERN Void kwAmmResetAllBo ARGS ((KwRbCb *rbCb));
EXTERN Void kwUmmResetAllBo ARGS ((KwRbCb *rbCb));
/* CFG Module */
...
EXTERN Void kwDbmDelSingleRb ARGS ((RbCb *rbCb));
...
EXTERN S16 kwDbmFetchAndCreateUeCb ARGS ((CmLteRnti ueId, CmLteCellId cellId, KwUeCb **ueCb));
EXTERN S16 kwDbmFetchAndCreateRbCb ARGS ((U8 rbId, U8 rbType, KwUeCb *ueCb, RbCb **rbCb, U8 rb));
...
EXTERN Void pjDlmAddRbToReestLst ARGS((
PjRbCb                  *rbCb
));
EXTERN Void pjDlmGetRbFrmReestLst ARGS((
PjRbCb                  **rbCb
));

EXTERN Void pjDlmProcessSelfMsg ARGS ((
void
));
EXTERN Void pjDlmGetNumRbInReestLst ARGS((
U16                     *numRb
));
EXTERN Void pjDlmPostSelfMsg ARGS((
Void
));


EXTERN Void pjDlmRemRbFrmReestLst ARGS((
PjRbCb                  *pjRbCb
));
