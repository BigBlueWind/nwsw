...
# This is the script for 4gmx environment. The difference in pkg_installer_x86.sh and pkg_installer_4gmx.sh are:
# 1. in list of exes, exes = { radioup.exe/lowerarm.exe } rest all executables will remain same.
# NOTE:: Any change in this file should also be replicated in pkg_installer_x86.sh
...
exes=(bsm.exe cp.exe gtp.exe lowerarm.elf deh.exe sih.exe fth.exe pmh.exe cih.exe  wih.exe lighttpd.exe logutil.exe);
...
topLevelFiles=(pkg_installer.sh patch_installer.sh config_installer.sh rh_installer.sh bs.sh dump_lowerarm.sh bootarm_bs.sh rdmem_arm);
...
if [[ $ckeckLevel == FirstLevel ]];then
  ...
  tar -x -z --strip-components=1 -f $picobsHome/download/$fileName -C $picobsHome/rel/inst_C > /dev/null 2>&1     #untar the package
  if [ $? != 0 ];then
