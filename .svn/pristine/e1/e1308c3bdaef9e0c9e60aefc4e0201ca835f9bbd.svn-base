...
typedef struct rgUlSf
{
   ...
   U32             totalUlBwAvail; /*!< Total uplink band width available in
                                     this subframe */
} RgUlSf;
...
struct rgL2MeasCb
{
   ...
   U32                totalUlBwAvail; /*!< Cumultive uplink bandwidth for this 
                                          measCb */
};
/** @brief
...
struct rgQciCb
{
   ...
   U8               prbCount; /*!< Cummulative PRB count */
...
typedef struct rgTbInfo{
   ...
   /*rg009.201 - Changed as a result of CR timer implementation*/
...
#ifdef LTE_L2_MEAS
   RguSnMapInfo  *rguSnMapInfo[RGU_MAX_TB]; /*!< Pointer received from RLC in
                                               DDatReq for TB1 */
...
{
   U8           numHqProcs;
   RgDlHqProcCb *procs; /*!< Downlink harq processes */
};
...
struct rgUeCb
{
   ...
   /*rg008.201 - Added suport for SPS*/
#ifdef LTEMAC_SPS
...
#ifdef LTE_L2_MEAS
EXTERN S16 rgUIMSndharqStaInd ARGS ((CmLteCellId     cellId,
...
/* rg007.201 - Changes for MIMO feature addition */
/* rg008.201 - Removed dependency on MIMO compile-time flag */
EXTERN S16 rgDHMSndDatReq ARGS((RgDlSf *dlSf, RgTfuDatReqInfo *datInfo, 
                                RgDlHqProcCb *hqProc, RgErrInfo *err));
EXTERN S16 rgDHMHndlDedDatReq ARGS((RgDlHqProcCb *hqProc, 
                              RgRguDedDatReq *datReq, RgErrInfo *err));
/*rg006.201 - Changed for contention resolution timer implementation*/
EXTERN S16 rgDHMHndlCmnDatReq ARGS((RgDlHqProcCb *hqProc, 
                RgRguCmnDatReq *datReq, RgErrInfo *err));
/* rg007.201 - Changes for MIMO feature addition */
/* rg008.201 - Removed dependency on MIMO compile-time flag */
EXTERN S16 rgDHMSndStaInd ARGS((RgCellCb *cell, RgUeCb *ue,
      RgInfUeAlloc *allocInfo, CmLteTimingInfo timingInfo, 
      RgDlHqProcCb   *hqProc, RgErrInfo *err));
