      ...
      case CTF_DELETE:
         {
            ...
            {
               case CTF_CELL_CFG:
		       MSPD_DBG(" PST: Calling ysMsCfgDelCellCfg cellId: %d \n",cfgReqInfo->u.release.u.cellRel.cellId);
...
#endif
{
   ...
   YS_MS_ALLOC(kdfCfmInf , sizeof(CtfKdfCfmInfo));
   ...
   {
      case CTF_KDF_TYPE1:
        kdfCfmInf->keyDerType = CTF_KDF_TYPE1;
	  	ret = YsMsUlmPrcKenbCfg(&(kdfReqInfo->u.kEnbInf), &(kdfCfmInf->u.kEnbInf));
		/*for(i=0;i<16;i++)
		MSPD_DBG("%x ",kdfCfmInf->u.kEnbInf.cpCiphKey[i]);
		MSPD_DBG("\n");
				for(i=0;i<16;i++)
		MSPD_DBG("%x ",kdfCfmInf->u.kEnbInf.upCiphKey[i]);
		MSPD_DBG("\n");
				for(i=0;i<16;i++)
		MSPD_DBG("%x ",kdfCfmInf->u.kEnbInf.intKey[i]);
		MSPD_DBG("\n");*/
	  	break;
      case CTF_KDF_TYPE2:

                /*MSPD_DBG("KKKDFFFFFFFF TYPPPPPPEEEEE 22222222222\n");
		for(i=0;i<32;i++)
		MSPD_DBG("%x ",kdfReqInfo->u.kEnbStarInf.secKey[i]);
        	MSPD_DBG("\n");*/
             
                kdfCfmInf->keyDerType = CTF_KDF_TYPE2;
	  	ret = YsMsUlmPrcKenbStarCfg(&(kdfReqInfo->u.kEnbStarInf), &(kdfCfmInf->u.kEnbStarInf));
		/*for(i=0;i<32;i++)
		MSPD_DBG("%x ",kdfCfmInf->u.kEnbStarInf.secKey[i]);
        	MSPD_DBG("\n");*/
   ...
   if(ret != ROK)
   {
      /*YS_LOGERR_INTPAR(EYSXXX, (ErrVal)ReqInfo->u.reCfg.cfgElem,
            "YsUiCtfCfgReq failed\n");*/
      YsUiCtfKdfCfm(&ysCb.ctfSap.sapPst, ysCb.ctfSap.suId,
            transId, kdfCfmInf, CTF_CFG_CFM_NOK);
   }// Sudhanshu WR Change
   else
   {
      /*if(cfgReqInfo->u.reCfg.u.dedRecfg.kenbInfo.pres == FALSE)
      {
      MSPD_DBG("\n Sending UECFG success\n");
      YsUiCtfKdfCfm(&ysCb.ctfSap.sapPst, ysCb.ctfSap.suId,
            transId, CTF_CFG_CFM_OK);
    ...
    YsUiCtfKdfCfm(&ysCb.ctfSap.sapPst, ysCb.ctfSap.suId,
                      transId, kdfCfmInf, CTF_CFG_CFM_OK);
   }
...
#endif
{
   ...
   {
      YS_LOGERR_INTPAR(EYSXXX, (ErrVal)spId,
            "tfuSap is NULL:YsUiTfuBndReq failed\n");
   ...
   switch (tfuSap->sapState)
   {
      ...
      default: /* Should Never Enter here */
         YS_LOGERR_INTPAR(EYSXXX, (ErrVal)tfuSap->sapState,
...
#endif
{
   ...
   YS_DBG_INFO((_ysp, "YsUiTfuCntrlReq(): spId = %d;\n", spId));
...
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_CL_DAT_REQ, RESOURCE_LARM, t, MacGetTick());
#endif
...
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_CL_RECP_REQ, RESOURCE_LARM, t, MacGetTick());
#endif
   RETVALUE(ret);
}  /* YsUiTfuRecpReq*/
