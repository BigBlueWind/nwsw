...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), 
            "Cell config for MAX C/I DL cell %d\n", cellCfg->cellId));
   ...
   {
      iTbs     = (*(RgSchCmnCqiToTbs *)(cmnDlCell->cqiToTbsTbl[0]))[RG_SCH_MAXCI_MIN_CQI_VAL];
      minEff   = (*(RgSchCmnTbSzEff *)(cmnDlCell->cqiToEffTbl[0]))[iTbs];
      iTbs     = (*(RgSchCmnCqiToTbs *)(cmnDlCell->cqiToTbsTbl[0]))[RG_SCH_MAXCI_MAX_CQI_VAL];
   ...
   else /* cellCfg->numTxAntPorts == 4 */
   {
      iTbs     = (*(RgSchCmnCqiToTbs *)(cmnDlCell->cqiToTbsTbl[0]))[RG_SCH_MAXCI_MIN_CQI_VAL];
      minEff   = (*(RgSchCmnTbSzEff *)(cmnDlCell->cqiToEffTbl[0]))[iTbs];
      iTbs     = (*(RgSchCmnCqiToTbs *)(cmnDlCell->cqiToTbsTbl[1]))[RG_SCH_MAXCI_MAX_CQI_VAL];
...
#endif
{
   ...
   if (maxciDlCell)
   {
      RGSCHDBGPRM(instIdx, (rgSchPBuf(instIdx),
               "Cell delete for MAX C/I DL cell %d\n", cell->cellId));
...
#else
PUBLIC S16 rgSCHMaxciDlUeHqEntInit(cell, ue)
   RgSchCellCb      *cell;
   RgSchUeCb        *ue;
#endif
{
   ...
   if ((ret = rgSCHUtlAllocSBuf(cell->instIdx,
               (Data**)&dlHqEnt->sch, 
               (dlHqEnt->numHqPrcs * sizeof(RgSchMaxciDlHqProc)))) != ROK)
   {
       /* ccpu00117052 - MOD - Passing double pointer
       for proper NULLP assignment*/
...
#endif
{
   ...
   RgSchMaxciDlUe *maxciDlUe = RG_SCH_MAXCIDL_GET_UE(ue);
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx),
            "UE re-config for MAX C/I DL ue %d\n", ue->ueId));
...
#endif
{
   ...
   RGSCHDBGPRM(instIdx, (rgSchPBuf(instIdx),
            "LC Cfg for MAX C/I DL LC %d\n", dlLc->lcId));
   
   if((ret = rgSCHUtlAllocSBuf(cell->instIdx, (Data**)&(cmnDlLc->schSpfc),
      (sizeof(RgSchMaxciDlLc)))) != ROK)
   {
      RGSCHDBGERR(instIdx, (rgSchPBuf(instIdx), 
               "Memory allocation FAILED for MAX C/I DL LC %d\n",
               dlLc->lcId));
      err->errCause = RGSCHERR_MAXCI_MEMERR;
      RETVALUE(ret);
   }

   maxciDlLc = RG_SCH_MAXCIDL_GET_LC(dlLc);
...
#endif
{
   RgSchMaxciDlLc  *maxciDlLc = RG_SCH_MAXCIDL_GET_LC(dlLc);
   RgSchCmnDlSvc   *cmnDlLc   = RG_SCH_CMN_GET_DL_SVC(dlLc);
#ifdef DEBUGP
   Inst            instIdx    = cell->instIdx;
#endif
...
#else
PUBLIC Void rgSCHMaxciDlCqiInd(cell, ue, cqiInfo)
RgSchCellCb          *cell;
RgSchUeCb            *ue;
TfuDlCqiRpt          *cqiInfo;
#endif
...
/* rg007.201 - Changes for MIMO feature addition */
 /* rg008.201 - Removed dependncy on MIMO compile-time flag */
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx),
            "DL Priority level updation for MAX C/I DL UE: ueId %d"
            "\n", ue->ueId));
/* rg007.201 - Changes for MIMO feature addition */
 /* rg008.201 - Removed dependncy on MIMO compile-time flag */
...
/**
 * @brief DL scheduling by Max C/I
 *
 * @details
 *
 *     Function : rgSCHMaxciDlSched
 *
 *     Processing Steps:
 *     - Schedule BCCH, PCCH [common module]
 *     - Schedule RA responses [common module]
 *     - Schedule SRBs [common module]
 *     - Schedule TAs
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHMaxciDlSched
(
RgSchCellCb           *cell,
RgSchCmnDlRbAllocInfo *allocInfo
)
#else
PUBLIC Void rgSCHMaxciDlSched(cell, allocInfo)
RgSchCellCb           *cell;
RgSchCmnDlRbAllocInfo *allocInfo;
#endif
{
   TRC2(rgSCHMaxciDlSched);

   /* Schedule TA */
   rgSCHMaxciDlTaSchd(cell, allocInfo);

   /* Scheduling the re-transmitting UEs */
   rgSCHMaxciDlRetx(cell, allocInfo);

   /* Schedule the transmitting UEs */
...
#endif
{
   ...
   Inst                 instIdx       = cell->instIdx;
#endif
   /*rg002.301-ADD-Added handling for updating totBytes.*/
   ...
   if (!(RG_SCH_CMN_DL_IS_UE_ACTIVE(ue)))
   {
      RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx),
               "rgSCHMaxciDlTaSchdForUe(): Could not schedule TA since UE is"
               "inactive %d \n", ue->ueId));
      RETVOID;
   }
   ...
   {
      RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx),
               "rgSCHMaxciDlTaSchdForUe(): Could not get HARQ proc for UE %d \n",
               ue->ueId));
      RETVOID;
   }
   /* rg007.201 - Changes for MIMO feature addition */
...
#endif
{
   ...
   for (lnk = alloc->dedAlloc.schdTxRetxUeLst.first; lnk;)
   {
      ...
      /* rg008.201 - Removed dependncy on MIMO compile-time flag */
      lnk                          = lnk->next;
      maxciUe = RG_SCH_MAXCIDL_GET_UE(ue);
...
#endif
{
   ...
   if (RG_SCH_MAXCIDL_IS_SRB(dlLc))
   {
      ...
      RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx), "Updating bo for SRB %d\n",
               dlLc->lcId));
      if (!remBo)
      {
         /* Handling for bo = 0 */
         ...
         {
            RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx), "Removing UE %d from"
                     "SRB transmitting list \n", ue->ueId));
            rgSCHMaxciDlDelUeFrmTxSrbLst(maxciDlUe);
         }
      }
   ...
   /* Handling BO for DRB */
   RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx), "Updating bo for DRB %d\n",
            dlLc->lcId));
   if (RG_SCH_CMN_SVC_IS_GBR(dlLc))
   {
...
#endif
{
   ...
   if (bo)
   {
      ...
      if (!maxciDlLc->lcLst)
      {
         ...
         {
            RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx), "Adding UE %d to SRB"
                     "transmitting list \n", ue->ueId));
            rgSCHMaxciDlAddUeToTxSrbLst(maxciDlCell, maxciDlUe);
         }
      }
...
#endif
{
   ...
   if (bo && allocLimit)
   {
      ...
      if ((!maxciDlLc->lcLst))
      {
         ...
         {
            RGSCHDBGINFO(instIdx, (rgSchPBuf(instIdx), "Adding UE %d to UE"
                     "transmitting list \n", ue->ueId));
            rgSCHMaxciDlAddUeToTxUeLst(maxciDlCell, maxciDlUe);
         }
      }
...
#endif
{
   ...
   for (hqProcIdx = 0; hqProcIdx < ue->dl.hqEnt->numHqPrcs; hqProcIdx++)
   {
      ...
      {
         if ((hqP->tbInfo[tbCnt].subFrm != NULLP) && 
               (hqP->tbInfo[tbCnt].sfLnk.node != NULLP))
         {
...
#endif
{
   ...
   {
      maxciDlUe = RG_SCH_MAXCIDL_GET_UE((RgSchUeCb *)inactvUeLnk->node);
      if (maxciDlUe->txSrbLstPtr)
      {
         rgSCHMaxciDlDelUeFrmTxSrbLst(maxciDlUe);
      }
...
#endif
{
   ...
   for (lnk = retxProcs->first; lnk; lnk = lnk->next)
   {
      ...
      /* rg007.201 - Changes for MIMO feature addition */
      /* rg008.201 - Removed dependncy on MIMO compile-time flag */
      RgSchMaxciDlUe     *maxciUe   = RG_SCH_MAXCIDL_GET_UE(ue);
      /* rg007.201 - Changes for MIMO feature addition */
      /* rg008.201 - Removed dependncy on MIMO compile-time flag */
...
#endif
{
   ...
   /* rg007.201 - Changes for MIMO feature addition */
   /* rg008.201 - Removed dependncy on MIMO compile-time flag */
   /* rg003.301[ccpu00122038]-MOD- Check for the return value */ 
   ...
   if ((ret != ROK) || (bytes == 0))
   {
      if (cmnDlProc->totBytes == 0)
      {
          ...
          * UE from txUeLst and resetting the outStndAlloc.*/
   ...
   cmnUe->proc = proc;
   /*rg002.301 - ADD -[ccpu00120941] Added the handling for updating 
    * totBytes.*/
   cmnDlProc->totBytes += bytes;
   /* rg007.201 - Changes for MIMO feature addition */
   /* rg008.201 - Removed dependncy on MIMO compile-time flag */
   /* rg003.301[ccpu00122038]-DEL- *allocBytes = bytes */ 
   /* rg007.201 - Changes for MIMO feature addition */
   /* rg008.201 - Removed dependncy on MIMO compile-time flag */
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), "rgSCHMaxciUlCellCfg: CellId: %d \n", 
            cell->cellId));
   ...
   {
      RGSCHDBGERR(cell->instIdx, (rgSchPBuf(cell->instIdx),
               "Memory allocation FAILED for MAX-CI UL cell %d\n", 
               cell->cellId));
      err->errCause = RGSCHERR_MAXCI_MEMERR;
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   {
      RGSCHDBGERR(cell->instIdx, (rgSchPBuf(cell->instIdx),
               "Memory allocation FAILED for MAX-CI UL UE CB CellId = %d\n", 
               cell->cellId));
      err->errCause = RGSCHERR_MAXCI_MEMERR;
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   RgSchMaxciUlUe       *maxciUe = RG_SCH_MAXCIUL_GET_UE(ue);
...
#endif
{
         ...
         ((cqiIdx >= 0 ) && *remUe); cqiIdx--)
   {
      ...
      while (node && *remUe)
      {
         ...
         cmnUe = RG_SCH_CMN_GET_UL_UE(ue);
         maxciUe = RG_SCH_MAXCIUL_GET_UE(ue);
         /* If SR is received allocate accordingly */
         if (maxciUe->srRcvd)
         {
            cmnUe->alloc.reqBytes = cmnUe->effBsr ? 
               cmnUe->effBsr: RG_SCH_MAXCI_SR_BYTES;
         }
         else
         {
            cmnUe->alloc.reqBytes = cmnUe->effBsr;
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), 
            "rgSCHMaxciUlUpdBsrShort: CellId: %d lcgId: %d Bsr:%d \n", 
            cell->cellId, lcg->lcgId, bsr));
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), 
            "rgSCHMaxciUlUpdBsrTrunc: CellId: %d lcgId: %d Bsr: %d\n", 
            cell->cellId, lcg->lcgId, bsr));
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), 
            "rgSCHMaxciUlUpdBsrLong: CellId: %d \n", 
            cell->cellId));
   /* This UE could be inactive for Uplink due to Measurement gap and ack nack
    * repetition. In such cases just return 
    */
...
#endif
{
   ...
   if (cmnUe->effBsr || maxciUe->srRcvd)
   {
      ...
      {
#ifdef TFU_UPGRADE
    queue = &(maxciCell->ueLst[cmnUe->validUlCqi - 1]);
#else
         /* rg007.201 - Changes for MIMO feature addition */
         /* rg008.201 - Removed dependncy on MIMO compile-time flag */
...
#ifdef TFU_UPGRADE
      cmLListAdd2Tail(&(maxciCell->ueLst[cmnUe->validUlCqi - 1]), 
            &maxciUe->prioLnk);
      maxciUe->crntQ = &(maxciCell->ueLst[cmnUe->validUlCqi - 1]); 
#else
      /* rg007.201 - Changes for MIMO feature addition */
      /* rg008.201 - Removed dependncy on MIMO compile-time flag */
