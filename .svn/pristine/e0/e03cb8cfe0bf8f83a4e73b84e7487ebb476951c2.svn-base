...
# This is the script for x86 environment. The difference in patch_install_x86.sh and patch_install_4gmx.sh are: 
...
# Any change in this file should also be replicated in patch_install_4gmx.sh
#set -x
...
exes=(bsm.exe cp.exe gtp.exe radioup.exe deh.exe sih.exe fth.exe pmh.exe wih.exe lighttpd.exe lth.exe spawn-fcgi.exe);
...
if [[ $ckeckLevel == FirstLevel ]];then
  ...
  tar -xzf $picobsHome/download/$fileName -C $picobsHome/rel/inst_C     #untar the package
  if [ $? != 0 ];then
