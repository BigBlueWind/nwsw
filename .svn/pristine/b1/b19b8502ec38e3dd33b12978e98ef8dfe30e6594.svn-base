...
# 150 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_GET_EVENT_KEY(_keyVal,_hoMsgEvnt,_msgDirectn,_keyPrefix,_key) _keyPrefix = ((CZ_DIR_OUT == _msgDirectn) ? (czBmpEvntKeyMap[_hoMsgEvnt][0]): (czBmpEvntKeyMap[_hoMsgEvnt][1])); _key = PutHiWord(_keyVal, _keyPrefix);
# 160 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_GET_EVENT_KEY_PREFIX(_hoMsgEvnt,_msgDirectn,_keyPrefix) _keyPrefix = ((CZ_DIR_OUT == _msgDirectn) ? (czBmpEvntKeyMap[_hoMsgEvnt][0]): (czBmpEvntKeyMap[_hoMsgEvnt][1]));
...
#define CZ_BMP_GET_KEY(_keyVal,_keyPrefix,_key) _key = PutHiWord(_keyVal, _keyPrefix);




#define CZ_BMP_GET_KEY_PREFIX(_key,_keyPrefix) _keyPrefix = GetHiWord(_key); _keyPrefix = GetLoByte(_keyPrefix);




#define CZ_BMP_GET_KEY_VAL(_key,keyVal) _keyVal = GetLoWord(_key, _keyVal);





#define CZ_BMP_IS_VALID_UE_X2AP_ID(_ueId) ((_ueId < CZ_MAX_UE_X2AP_ID)?(TRUE):(FALSE))




#define CZ_BMP_FILL_UE_INFO(_oldId,_newId) { CZ_SET_ZERO((&czUeInfo), sizeof(CztUeInfo)); czUeInfo.oldUEX2APId = _oldId; czUeInfo.newUEX2APId = _newId; }
# 197 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_FILL_GEN_ERR_INFO(_peerId,_cType,_cVal,_msgType) { CZ_SET_ZERO((&czBmpErrInd), sizeof(CztLclErrInd)); czBmpErrInd.peerId = _peerId; czBmpErrInd.causeType = _cType; czBmpErrInd.causeValue = _cVal; czBmpErrInd.msgType = _msgType; }
# 209 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_FILL_UIM_ERR_INFO(_peerCb,_cType,_cVal,_msgType,_oldId,_newId) { CZ_SET_ZERO((&czBmpErrInd), sizeof(CztLclErrInd)); czBmpErrInd.peerId = (_peerCb->peerId); czBmpErrInd.causeType = _cType; czBmpErrInd.causeValue = _cVal; czBmpErrInd.msgType = _msgType; czBmpErrInd.u.ueInfo.oldUEX2APId = _oldId; czBmpErrInd.u.ueInfo.newUEX2APId = _newId; }
# 223 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_FILL_UIM_GEN_ERR_INFO(_peerCb,_cType,_cVal,_msgType) { CZ_SET_ZERO((&czBmpErrInd), sizeof(CztLclErrInd)); czBmpErrInd.peerId = (_peerCb->peerId); czBmpErrInd.causeType = _cType; czBmpErrInd.causeValue = _cVal; czBmpErrInd.msgType = _msgType; }
# 233 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_FILL_UIM_UE_ERR_INFO(_peerCb,_msgType,_oldId,_newId) { CZ_SET_ZERO((&czBmpErrInd), sizeof(CztLclErrInd)); czBmpErrInd.peerId = (_peerCb->peerId); czBmpErrInd.msgType = _msgType; czBmpErrInd.u.ueInfo.oldUEX2APId = _oldId; czBmpErrInd.u.ueInfo.newUEX2APId = _newId; }
# 243 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_FILL_UIM_MSG_INFO(_peerCb,_msgType) { CZ_SET_ZERO((&czBmpErrInd), sizeof(CztLclErrInd)); czBmpErrInd.peerId = (_peerCb->peerId); czBmpErrInd.msgType = _msgType; }
...
#define CZ_BMP_FILL_UIM_CAUSE_INFO(_cType,_cVal) { czBmpErrInd.causeType = _cType; czBmpErrInd.causeValue = _cVal; }
...
# 370 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_ENTRY_INFO(_peerCb,_bmpMsgEvnt,_msgDirectn) { CZ_DBG_INFO((CZ_PRNT_BUF, "Handling (%s) message from (%s) through BMP for" "peer(%ld)\n", czBmpEvntPrintMap[_bmpMsgEvnt], ((_msgDirectn == CZ_DIR_OUT) ? (CZ_BMP_MSG_FRM_USER):(CZ_BMP_MSG_FRM_PEER)) ,(_peerCb->peerId))); }
# 381 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_ENTRY_OUT_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt) { CZ_DBG_INFO((CZ_PRNT_BUF, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId))); }
...
#define CZ_BMP_LOG_OUT_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt,_streamId) { CZ_DBG_INFO((CZ_PRNT_BUF, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId), _streamId)); }
# 398 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_OUT_HDLR_ERR(_peerCb,_oldId,_newId,_bmpMsgEvnt,_cause) { CZ_DBG_ERROR((CZ_PRNT_BUF, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId), _cause)); }
# 407 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_EXIT_OUT_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt,_streamId) { CZ_DBG_INFO((CZ_PRNT_BUF, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId), _streamId)); }
# 419 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_TMR_EXPIRY_INFO(_ueCb,_tmr) { CZ_DBG_INFO((CZ_PRNT_BUF, "Timer(%d) expired for UE(%d:%d) with peer (%ld)", _tmr, (_ueCb->oldUEX2APId), (_ueCb->newUEX2APId), (ueCb->peerCb->peerId)));}
...
#define CZ_BMP_LOG_ERRINFO_TO_USR(_peerCb,_oldId,_newId,_bmpMsgEvnt,_causeVal,_lclCauseVal) { CZ_DBG_INFO((CZ_PRNT_BUF, "Generating Error Indication on (%s) for an" "UE(%d:%d) to send to peer(%ld): Cause(%d), local Cause(%d)", czBmpEvntPrintMap[_bmpMsgEvnt],_oldId, _newId, (_peerCb->peerId), _causeVal, _lclCauseVal)); }
# 439 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_ENTRY_IN_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt) { CZ_DBG_INFO((CZ_PRNT_BUF, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId))); }
...
#define CZ_BMP_LOG_IN_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt) { CZ_DBG_INFO((CZ_PRNT_BUF, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[_bmpMsgEvnt], (_peerCb->peerId), _oldId, _newId)); }
...
#define CZ_BMP_LOG_IN_HDLR_ERR(_peerCb,_oldId,_newId,_bmpMsgEvnt,_cause) { CZ_DBG_ERROR((CZ_PRNT_BUF, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[_bmpMsgEvnt], _oldId, _newId, (_peerCb->peerId), _cause)); }
# 464 "../..//x2ap/cz_bmp.c"
#define CZ_BMP_LOG_EXIT_IN_HDLR_INFO(_peerCb,_oldId,_newId,_bmpMsgEvnt) { CZ_DBG_INFO((CZ_PRNT_BUF, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[_bmpMsgEvnt], (_peerCb->peerId), _oldId, _newId)); }
# 480 "../..//x2ap/cz_bmp.c"
...
static S16 czBmpIsHoReqValid (CzPeerCb *peerCb, CztX2AP_PDU *pdu, CzIeInfo *ieInfo, U8 *causeType, U8 *causeVal);
# 661 "../..//x2ap/cz_bmp.c"
...

{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlHoEvnt"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",853, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   if(0 != ret)
   {
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",869, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",877, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Handling (%s) message from (%s) through BMP for" "peer(%ld)\n", czBmpEvntPrintMap[hoMsgEvnt], ((msgDirectn == CZ_DIR_OUT) ? ("User"):("Peer")) ,(peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
       ...
       (!(((oldUEX2APId->val < 4096)?(1):(0))))))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",902, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid old UE X2AP ID(%d)" " when sending mobility event(%s) to peer(%ld)", oldUEX2APIdVal, czBmpEvntPrintMap[hoMsgEvnt], (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = hoMsgType; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = 0xFFFF; };


      czBmpGenLclErrInd(peerCb, evnt, 0x10,
                        (0x40 + 0x02));

      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",914, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
   ...
   if(CZ_DIR_OUT == msgDirectn)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",953, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing event (%d), through BMP in state(%d)" "for peer(%ld)", hoMsgEvnt, state, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   else if( CZ_DIR_IN == msgDirectn)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",976, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Incoming event(%d), through BMP in state(%d)" "from peer(%ld)", hoMsgEvnt, state, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",990, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...

{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpIsPeerValid"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1049, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } };
# 1059 "../..//x2ap/cz_bmp.c"
      ...
      ((CZ_GP_CONNECTING == peerCb->state) && (CZ_DIR_OUT == msgDirectn)))
   {
      ...
      if(CZ_DIR_OUT == msgDirectn)
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1074, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility event(%s) received from user" "when peer (%ld) not in active state(%d)\n", czBmpEvntPrintMap[hoMsgEvnt], (peerCb->peerId), (peerCb->state)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
         ...
         { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = hoMsgType; };
      ...
      else
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1099, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility event(%s) received from peer" "(%ld) while not in active state(%d)\n", czBmpEvntPrintMap[hoMsgEvnt], (peerCb->peerId), (peerCb->state)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
         ...
         if(0 == hoMsgType)
         {
            ...
            if(oldUEX2APId != 0L)
            {
               { do { if (0L != ((&czUeInfo))) { (Void)cmMemset((U8*)((&czUeInfo)), 0, (Size)sizeof(CztUeInfo)); } } while(0); czUeInfo.oldUEX2APId = oldUEX2APId->val; czUeInfo.newUEX2APId = 0xFFFF; };
            }
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1130, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }

   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1133, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
}
# 1184 "../..//x2ap/cz_bmp.c"
 S16 czBmpHdlErrInd
(
...

{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlErrInd"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1207, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1225, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending / Receiving Error Indication for peer                              peer(%ld): ", peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1232, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1293, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
}
# 1330 "../..//x2ap/cz_bmp.c"
...
# 1346 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutErrInd"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1354, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1363, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing Error Indication on BMP event(%s)" " to peer(%ld)\n", czBmpEvntPrintMap[hoMsgEvnt], peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 1371 "../..//x2ap/cz_bmp.c"
   if((0xFFFF != oldUEX2APIdVal) && (!ueCb))
   {


      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 7; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = 0xFFFF; };


      czBmpGenLclErrInd(peerCb, evnt, 0x10,
                        (0x40 + 0x02));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1380, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
...
# 1445 "../..//x2ap/cz_bmp.c"
   ...
   newUEX2APIdVal = ((0L != newUEX2APId) &&
                     (1 == newUEX2APId->pres))? (U16)(newUEX2APId->val):(0xFFFF);
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1471, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending Error Indication on BMP event (%s)" "for an UE(%d:%d) to peer(%ld) on stream(%d)", czBmpEvntPrintMap[hoMsgEvnt],oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), (peerCb->peerCfg.globalStreamId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 1477 "../..//x2ap/cz_bmp.c"
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1486, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Error                Indication failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 1494 "../..//x2ap/cz_bmp.c"
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 7; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1505, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 1560 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInErrInd"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1565, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1566, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "For peer with peerId (%ld)", peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
...
# 1658 "../..//x2ap/cz_bmp.c"
   ...
   if(0 != ret)
   {
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1673, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-Error                Indication failed for UE(%d:%d)\n", oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1683, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 1736 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutHoReq"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1741, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1742, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "For peer with peerId (%ld)", peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);;

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1745, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(!ueCb)
   {
      ...
      if(0 != ret)
      {
         do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1765, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Failed to allocate UeCb. UE(%d:%d)\n ", oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);





         { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 0; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = 0xFFFF; };


         czBmpGenLclErrInd(peerCb, evnt, 0x70,
                        (0x70 + 0x03));
         do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1775, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
      }
   }
...
# 1794 "../..//x2ap/cz_bmp.c"
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1799, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), (ueCb->streamId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1814, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Handover                Request failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1821, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1843, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), ueCb->streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1851, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 1899 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoReq"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1909, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   do { if (0L != (errIeInfo)) { (Void)cmMemset((U8*)(errIeInfo), 0, (Size)5 * sizeof(CzIeInfo)); } } while(0);

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1914, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   { do { if (0L != ((&czUeInfo))) { (Void)cmMemset((U8*)((&czUeInfo)), 0, (Size)sizeof(CztUeInfo)); } } while(0); czUeInfo.oldUEX2APId = oldUEX2APIdVal; czUeInfo.newUEX2APId = 0xFFFF; };




   if(0 != (czBmpIsHoReqValid(peerCb, &(evnt->pdu), errIeInfo,
                                     &causeType, &causeVal)))
   {



      czBmpSndFailRspToPeer(peerCb, evnt, &czUeInfo, errIeInfo,
                            causeType, causeVal,
                            (0x40 + 0x0B));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1930, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
   ...
   if(!ueCb)
   {
      ...
      if(0 != ret)
      {
         do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1954, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Failed to allocate UeCb. UE(%d:%d)\n ", oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
         ...
         czBmpSndFailRspToPeer(peerCb, evnt, &czUeInfo, 0L,
                               ...
                               CztCauseMiscom_interventionEnum,
                               (0x70 + 0x03));
         do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1963, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
      }
   }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1976, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",1994, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-Handover                Request failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2001, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      czBmpSndFailRspToPeer(peerCb, evnt, &czUeInfo, 0L,
                            ...
                            CztCauseMiscom_interventionEnum,
                            (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2015, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2019, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_REQ], (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2021, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 2069 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutHoAck"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2074, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2077, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      (1 != newUEX2APId->pres))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2097, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid new UE X2APId for an UE(%d:%d) when " "sending Handover Request Acknowledge to peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 1; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };

      czBmpGenLclErrInd(peerCb, evnt, 0x10,
                        (0x40 + 0x03));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2106, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2117, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, (ueCb->newUEX2APId), (peerCb->peerId), ueCb->streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2130, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Handover Ack               failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, ueCb->newUEX2APId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2142, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
# 2150 "../..//x2ap/cz_bmp.c"
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 1; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = (newUEX2APId->val); };

      czBmpGenLclErrInd(peerCb, evnt, 0x50, (U8)ret);
   }
   else
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2157, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, (ueCb->newUEX2APId), (peerCb->peerId), ueCb->streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2164, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 2212 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoAck"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2217, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   { do { if (0L != ((&czUeInfo))) { (Void)cmMemset((U8*)((&czUeInfo)), 0, (Size)sizeof(CztUeInfo)); } } while(0); czUeInfo.oldUEX2APId = oldUEX2APIdVal; czUeInfo.newUEX2APId = newUEX2APIdVal; };



   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2232, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
...
# 2255 "../..//x2ap/cz_bmp.c"
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2263, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2277, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-Handover Ack                failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 2284 "../..//x2ap/cz_bmp.c"
      ueCb->newUEX2APId = 0xFFFF;
      ueCb->state = CZ_BMP_HO_PROGRESS;
      ...
      ret = czStartTmr(CZ_TMR_TRELOCPREP, czCb.nodeCb.protCfg.tRELOCprep,
            (U32 *) ueCb);
# 2305 "../..//x2ap/cz_bmp.c"
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2306, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           ...
                           CztCauseMiscom_interventionEnum,
                           (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2313, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2317, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_REQ_ACK], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };

   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2318, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 2370 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutHoCancel"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2376, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2379, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if((0L != newUEX2APId) && (1 == newUEX2APId->pres))
   {
      ...
      if(newUEX2APIdVal != ueCb->newUEX2APId)
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2399, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when sending Handover Cancel to peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 2407 "../..//x2ap/cz_bmp.c"
         { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 3; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };

         czBmpGenLclErrInd(peerCb, evnt, 0x10,
                           (0x40 + 0x03));

         do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2412, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
      }
   }
# 2425 "../..//x2ap/cz_bmp.c"
   czStopUeTmrs(ueCb);
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2437, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2449, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Handover                Cancel failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2456, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2470, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2478, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 2529 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoCancel"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2534, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2537, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if((0L != newUEX2APId) && (newUEX2APId->val != ueCb->newUEX2APId))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2558, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when received Handover Cancel from peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
            ...
            CztCauseRadioNwunknown_new_eNB_UE_X2AP_IDEnum,
            (0x40 + 0x03));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2567, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
# 2577 "../..//x2ap/cz_bmp.c"
   czNmmDelUeCb(ueCb);
   ueCb= 0L;
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2584, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], (peerCb->peerId), oldUEX2APIdVal, (czUeInfo.newUEX2APId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2598, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-Handover Cancel               failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, czUeInfo.newUEX2APId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2605, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, (czUeInfo.newUEX2APId), (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
# 2613 "../..//x2ap/cz_bmp.c"
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           3,
                           3,
                           CztCauseMiscom_interventionEnum,
                           (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2618, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2622, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], (peerCb->peerId), oldUEX2APIdVal, (czUeInfo.newUEX2APId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2624, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 2672 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutHoFail"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2676, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2679, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2691, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2707, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Handover                Failure failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2714, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 2; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = 0xFFFF; };

      czBmpGenLclErrInd(peerCb, evnt, 0x50, (U8)ret);
   }
   else
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2729, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2737, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 2786 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoFail"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2789, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
...
# 2807 "../..//x2ap/cz_bmp.c"
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2814, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2829, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf,"Indication of the incoming X2-Handover Failure               failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2836, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], oldUEX2APIdVal, 0xFFFF, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           ...
                           CztCauseMiscom_interventionEnum,
                           (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2848, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2852, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_HO_PREP_FAIL], (peerCb->peerId), oldUEX2APIdVal, 0xFFFF); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2854, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 2905 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutSnStTrnsfr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2910, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2920, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
       ...
       (newUEX2APIdVal != ueCb->newUEX2APId))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2936, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when sending SN Status Transfer to peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 2944 "../..//x2ap/cz_bmp.c"
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 4; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };

      czBmpGenLclErrInd(peerCb, evnt, 0x10,
                        (0x40 + 0x03));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2948, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2952, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ueCb->streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2964, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-SN State                Transfer failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 2976 "../..//x2ap/cz_bmp.c"
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2977, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2991, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ueCb->streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",2999, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 3049 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInSnStTrnsfr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3054, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3067, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
# 3076 "../..//x2ap/cz_bmp.c"
   if ((0xFFFF == newUEX2APIdVal) ||
            (newUEX2APIdVal != ueCb->newUEX2APId))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3086, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when received SN Status Transfer from peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 3094 "../..//x2ap/cz_bmp.c"
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           4,
                           0,
                           CztCauseRadioNwunknown_new_eNB_UE_X2AP_IDEnum,
                           (0x40 + 0x03));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3099, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
# 3109 "../..//x2ap/cz_bmp.c"
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3110, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3125, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-SN Transfer                failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3132, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
# 3140 "../..//x2ap/cz_bmp.c"
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           4,
                           3,
                           CztCauseMiscom_interventionEnum,
                           (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3145, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3149, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_SN_STATUS_TRFR], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3151, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 3202 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutUeCnxtRelease"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3208, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3218, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Outgoing BMP event (%s) for an UE(%d:%d) to " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
        ...
        (newUEX2APIdVal != ueCb->newUEX2APId))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3234, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when sending UE Context Release to peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 3242 "../..//x2ap/cz_bmp.c"
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = 5; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };

      czBmpGenLclErrInd(peerCb, evnt, 0x10,
                        (0x40 + 0x03));

      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3247, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3260, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3272, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-UE Context                Release failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3279, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3293, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3301, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
...
# 3353 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInUeCnxtRelease"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3358, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3371, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Received BMP event (%s) for an UE(%d:%d) from " "peer(%ld)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
           ...
           (newUEX2APIdVal != ueCb->newUEX2APId))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3388, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid newUEX2APId for an UE(%d:%d)" "when received UE Context Release from peer(%ld)", oldUEX2APIdVal, newUEX2APIdVal, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 3397 "../..//x2ap/cz_bmp.c"
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           5,
                           0,
                           CztCauseRadioNwunknown_new_eNB_UE_X2AP_IDEnum,
                           (0x40 + 0x03));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3402, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
   }
   ...
   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3423, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3438, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Indication of the incoming X2-UE Context                Release failed from the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3445, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering BMP event (%s) to user for an " "UE(%d:%d) received from peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
# 3453 "../..//x2ap/cz_bmp.c"
      czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                           5,
                           3,
                           CztCauseMiscom_interventionEnum,
                           (0x50 + 0x05));
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3458, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
   }

   { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3462, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivered BMP event (%s) from peer(%ld) for an" "UE(%d:%d) to user", czBmpEvntPrintMap[CZ_EVNT_UE_CXT_RELEASE], (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };


   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3464, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...

{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlTmrEvnt"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3512, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }

   switch(tmrEvnt)
   {
      case CZ_TMR_TRELOCPREP:
      {
         { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3518, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Timer(%d) expired for UE(%d:%d) with peer (%ld)", tmrEvnt, (ueCb->oldUEX2APId), (ueCb->newUEX2APId), (ueCb->peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);};
      ...
      {
         { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3531, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Timer(%d) expired for UE(%d:%d) with peer (%ld)", tmrEvnt, (ueCb->oldUEX2APId), (ueCb->newUEX2APId), (ueCb->peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);};
      ...
      default:
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3545, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Unknown timer for BMP(%d) with UE(%p)", tmrEvnt, (Void *)ueCb); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);

         do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3546, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
      }
   }
...
# 3564 "../..//x2ap/cz_bmp.c"
   ...
   if(CZ_GP_CONNECTING == peerCb->state)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3577, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Peer(%ld) in Reset state with timer(%d) " "expiry on UE(%d:%d), aborting...", peerId, lclStatVal, oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   if(0L != cancelEvnt)
   {
      { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3587, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3599, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Transmission of the outcoming X2-Handover                Cancel failed towards the peer(%ld) for UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
         ...
         { do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3606, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending BMP event (%s) for an UE(%d:%d)" "to peer(%ld) failed, cause(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), ret); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
      ...
      {
         { do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3621, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sent BMP event (%s) for an UE(%d:%d)" "to peer(%ld) on stream(%d)", czBmpEvntPrintMap[CZ_EVNT_HO_CANCEL], oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId), streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0); };
   ...
   else
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3637, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Building HO Cancel with timer(%d) " "expiry on UE(%d:%d) to peer(%ld) failed\n", lclStatVal, oldUEX2APIdVal,newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
...
staInd:
   ...
   if(0 != ret)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3663, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Delivering Status Indication with timer(%d) " "expiry on UE(%d:%d), peer(%ld) failed", lclStatVal, oldUEX2APIdVal, newUEX2APIdVal, peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3667, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 3716 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlOutEvntErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3722, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
# 3731 "../..//x2ap/cz_bmp.c"
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3762, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility event(%s) received in state(%s) " "for an UE(%d:%d) from user to send to peer(%ld) \n", czBmpEvntPrintMap[hoMsgEvnt], ((state <= CZ_BMP_HO_PREPARED)? (czBmpStatePrintMap[state]):("Invalid")), oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = hoMsgType; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3772, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
...
# 3825 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoReqErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3829, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3853, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Handover Request received in state(%s) " "for an UE(%d:%d) from peer(%ld)\n", ((state <= CZ_BMP_HO_PREPARED)? (czBmpStatePrintMap[state]):("Invalid")), oldUEX2APIdVal, 0xFFFF, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3862, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
}
# 3890 "../..//x2ap/cz_bmp.c"
...
# 3907 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInHoRspEvntErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3913, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
...
# 3941 "../..//x2ap/cz_bmp.c"
   do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3945, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility response event(%s) received in wrong" "state (%s) for an UE(%d:%d) from peer(%ld) \n", czBmpEvntPrintMap[hoMsgEvnt], ((state <= CZ_BMP_HO_PREPARED)? (czBmpStatePrintMap[state]):("Invalid")), oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 3956 "../..//x2ap/cz_bmp.c"
   { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.msgType = hoMsgType; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",3961, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
}
# 3989 "../..//x2ap/cz_bmp.c"
...
# 4005 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInReqErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4011, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4042, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility event(%s) received in wrong state (%s)" " for an UE(%d:%d) from peer(%ld) \n", czBmpEvntPrintMap[hoMsgEvnt], ((state <= CZ_BMP_HO_PREPARED)? (czBmpStatePrintMap[state]):("Invalid")), oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 4054 "../..//x2ap/cz_bmp.c"
   do { cmFreeMem((evnt)); (evnt) = 0L; } while(0);

   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4056, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
}
# 4085 "../..//x2ap/cz_bmp.c"
...
# 4101 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpHdlInEvntErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4106, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4143, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Mobility event(%s) received in wrong state (%s)" " for an UE(%d:%d) from peer(%ld) \n", czBmpEvntPrintMap[hoMsgEvnt], ((state <= CZ_BMP_HO_PREPARED)? (czBmpStatePrintMap[state]):("Invalid")), oldUEX2APIdVal, czUeInfo.newUEX2APId, peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
# 4148 "../..//x2ap/cz_bmp.c"
   czBmpSndErrIndToPeer(peerCb, evnt, &czUeInfo,
                        hoMsgType,
                        2,
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4154, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
}
# 4180 "../..//x2ap/cz_bmp.c"
...

{
   S16 lclCauseVal = 0;
    ...
    Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpIsErrIndValid"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4195, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } };
      ...
      ((CZ_GP_CONNECTING == peerCb->state) && (CZ_DIR_OUT == msgDirectn)))
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4215, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Error Indication from(%s) " "when peer (%ld) in not active state(%d)\n", ((msgDirectn == CZ_DIR_OUT) ? ("User"):("Peer")), (peerCb->peerId), (peerCb->state)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   else if(0 == hoMsgType)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4230, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Error Indication for Handover Request" "is invalid for peer (%ld)\n", (peerCb->peerId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);


      lclCauseVal = (0x60 + 0x02);
   }

   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4235, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(lclCauseVal); }while(0);
...
{
   S16 ret;
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpGenErrIndEvntErr"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4280, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } };
   ...
   if(CZ_DIR_OUT == msgDirectn)
   {
      ...
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.causeType = 0x60; czBmpErrInd.causeValue = causeVal; czBmpErrInd.msgType = 7; };
   ...
   else
   {
      ...
      czUtlUpdInErrSts(peerCb, 7);
# 4307 "../..//x2ap/cz_bmp.c"
      { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.causeType = 0x80; czBmpErrInd.causeValue = causeVal; czBmpErrInd.msgType = 7; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4317, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
}
# 4341 "../..//x2ap/cz_bmp.c"
...

{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpGenLclErrInd"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4358, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4368, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
}
# 4418 "../..//x2ap/cz_bmp.c"
...
# 4434 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpSndFailErrToPeer"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4441, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4442, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "For peer with peerId (%ld)", peerCb->peerId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);;
# 4455 "../..//x2ap/cz_bmp.c"
   ...
   switch(hoMsgType)
   {
      case 0:
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4478, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Generating HO Preparation Failure" "on HO Request to peer(%ld) with UE(%d:%d)" "on stream %d\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal, streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      case 5:
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4504, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Generating Error Indication " "on %d to peer(%ld) with UE(%d:%d) on stream(%d)\n", hoMsgType, (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal, streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      default:
      {
         do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4510, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Invalid Message Type\n"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
         do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4511, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(1); }while(0);
      }
   }
   if(0L != rspErrEvnt)
   {
      ...
      if(0 != ret)
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4535, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending Handover Error " "Response/Indication %d to peer(%ld) failed on UE(%d:%d)\n", rspMsgType, (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   else
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4553, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Building Handover Error " "Response/Indication %d to peer(%ld) failed on UE(%d:%d)\n", rspMsgType, (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.causeType = 0x80; czBmpErrInd.causeValue = (0x60 + 0x01); czBmpErrInd.msgType = hoMsgType; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4571, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 4629 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpSndFailRspToPeer"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4636, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
# 4645 "../..//x2ap/cz_bmp.c"
   oldUEX2APIdVal = bmpUeInfo->oldUEX2APId;
   newUEX2APIdVal = bmpUeInfo->newUEX2APId;
   ...
   if(0L != rspErrEvnt)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4662, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Generating Handover Preparation Failure " "to peer(%ld) with UE(%d:%d) on stream(%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal, streamId); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      else
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4685, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending Handover Preparation Failure" "to peer(%ld) failed on UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   else
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4703, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Building Failure Response" "to peer(%ld) failed on UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   {
      do { cmFreeMem((evnt)); (evnt) = 0L; } while(0);
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4713, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
   }



   { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.causeType = 0x80; czBmpErrInd.causeValue = lclCauseVal; czBmpErrInd.msgType = 0; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4728, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
...
# 4787 "../..//x2ap/cz_bmp.c"
{
   ...
   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpSndErrorIndToPeer"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4793, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } }
   ...
   if(0L != rspErrEvnt)
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 3))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4820, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"INFO\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Generating Error Indication " "on %d to peer(%ld) with UE(%d:%d) on stream(%d)\n", hoMsgType, (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal, (peerCb->peerCfg.globalStreamId)); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
      ...
      if(0 != ret)
      {
         ...
         do { if (czCb.init.dbgMask & ((0x00000100 << 7))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4843, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"FATAL\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Sending Error Indication" "to peer(%ld) failed on UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   else
   {
      ...
      do { if (czCb.init.dbgMask & ((0x00000100 << 6))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4862, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " [" "\"ERROR\"" "]" ); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, ":"); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "Building Error Indication" "to peer(%ld) failed on UE(%d:%d)\n", (peerCb->peerId), oldUEX2APIdVal, newUEX2APIdVal); SPrint(czCb.init.prntBuf); }; { sprintf (czCb.init.prntBuf, "\n"); SPrint(czCb.init.prntBuf); }; } }while(0);
   ...
   {
      do { cmFreeMem((evnt)); (evnt) = 0L; } while(0);
      do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4871, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);
   }



   { do { if (0L != ((&czBmpErrInd))) { (Void)cmMemset((U8*)((&czBmpErrInd)), 0, (Size)sizeof(CztLclErrInd)); } } while(0); czBmpErrInd.peerId = (peerCb->peerId); czBmpErrInd.causeType = 0x80; czBmpErrInd.causeValue = lclCauseVal; czBmpErrInd.msgType = hoMsgType; czBmpErrInd.u.ueInfo.oldUEX2APId = oldUEX2APIdVal; czBmpErrInd.u.ueInfo.newUEX2APId = newUEX2APIdVal; };
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4886, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(0); }while(0);

}
# 4949 "../..//x2ap/cz_bmp.c"
static S16 czBmpIsHoReqValid
(
CzPeerCb *peerCb,
CztX2AP_PDU *pdu,
CzIeInfo *errIeInfo,
...
{
   S16 ret = 0;
   CztUE_CntxtInform *ueCntxtInfo = 0L;
   CztUESecurCapabilities *ueSecCap;
   CztEncryptionAlgorithms *encryptAlg = 0L;
   CztIntegrityProtectionAlgorithms *intAlg = 0L;
   U32 encryptAlgVal = 0;
   U32 intAlgVal = 0;
   S16 indx = 0;

   Txt __tapa_fun[255]; sprintf(__tapa_fun,"czBmpIsHoReqValid"); { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",4975, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf," Entering\n"); SPrint(czCb.init.prntBuf); }; ;; } };

   czUtlGetIE(pdu, 14, (TknU8 **)(&ueCntxtInfo));

   if((0L != ueCntxtInfo) && (1 == ueCntxtInfo->pres.pres))
   {

      ueSecCap = &(ueCntxtInfo->uESecurCapabilities);
      if(1 == ueSecCap->pres.pres)
      {

         encryptAlg = &ueSecCap->encryptionAlgorithms;
         intAlg = &ueSecCap->integrityProtectionAlgorithms;

         if(1 == encryptAlg->pres)
         {
            for (indx = ((encryptAlg->len /8)-1);indx >= 0;indx--)
            {
               encryptAlgVal |= ((U32)((encryptAlg->val[indx]) << (8 *indx)));
            }
         ...
         if(1 == intAlg->pres)
         {
            for (indx = ((intAlg->len /8)-1);indx >= 0;indx--)
            {
               intAlgVal |= ((U32)((intAlg->val[indx]) << (8 *indx)));
            }
   ...
   if(16 == ret)
   {
      do { if (0L != (errIeInfo)) { (Void)cmMemset((U8*)(errIeInfo), 0, (Size)sizeof(CzIeInfo)); } } while(0);
      ...
      errIeInfo->crit = CztCriticalityrejectEnum;


      errIeInfo->typOfErr = CztTypOfErrnot_understoodEnum;
   ...
   do { if (czCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (czCb.init.prntBuf, " [%s 0x%x:%x]", "LTE-X2AP", czCb.init.ent,czCb.init.inst); SPrint(czCb.init.prntBuf); }; } { { sprintf (czCb.init.prntBuf, " [%s:%d:%s]","../..//x2ap/cz_bmp.c",5039, __tapa_fun); SPrint(czCb.init.prntBuf); }; } { sprintf (czCb.init.prntBuf, " Exiting\n"); SPrint(czCb.init.prntBuf); }; } return(ret); }while(0);
}
