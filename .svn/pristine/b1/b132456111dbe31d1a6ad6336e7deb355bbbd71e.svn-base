/* $Id: chunkHandler.c,v 1.4 2012/05/30 13:28:43 nedosis Exp $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
                ...
                gboolean ignore_dups, gboolean ignoreLast)
{
            ...
            if (IS_IPV6_ADDRESS_PTR_NBO(address)) {
...
#endif                          /* SIN6_LEN */
                _memcpy(&(tmp_su.sin6.sin6_addr.s6_addr),&(address->dest_addr.sctp_ipv6), 16);
                ...
                if (discard == FALSE) {
                    ...
                    if (new_found == TRUE) {
                        ...
                        _memcpy(addresses[nAddresses].sin6.sin6_addr.s6_addr,
    ...
    if (ignoreLast == FALSE) {
        ...
        if (last_found == FALSE) {
            _memcpy(&addresses[nAddresses], lastSource, sizeof(union sockunion));
    ...
    for (i = 0; i < noOfAddresses; i++) {
        ...
        switch (sockunion_family(&(sock_addresses[i]))) {
            ...
            case AF_INET6:
                ...
                _memcpy(address->dest_addr.sctp_ipv6,
   ...
   if (chunks[initCID] == NULL || chunks[initAckCID] == NULL) {
    ...
    while (curs < vlp_totalLength) {
        ...
        if (pType == VLPARAM_PRSCTP) {
            ...
            _memcpy(ack_string, vl_initPtr, pLen);
...
#endif
                    _memcpy(&preferredDest->sin6.sin6_addr.s6_addr,
                           address->dest_addr.sctp_ipv6, sizeof(struct in6_addr));
                    ...
                    unsigned int *supportedTypes, union sockunion* lastSource)
{
    ...
    if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_INIT_ACK ||
        ...
        _memcpy(&tmp, &addresses[0], sizeof(union sockunion));
        _memcpy(&addresses[0], &addresses[1], sizeof(union sockunion));
        _memcpy(&addresses[1], &tmp, sizeof(union sockunion));
...
ChunkID ch_makeCookie(SCTP_cookie_param * cookieParam)
{
    ...
    _memcpy(&(cookieChunk->cookie), &(cookieParam->ck.z_side_initAck),
                             ...
                             union sockunion* lastSource)
{
    ...
    if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_COOKIE_ECHO) {
        ...
        _memcpy(addresses, &temp_addresses[no_loc_ipv4_addresses],
               no_remote_ipv4_addresses * sizeof(union sockunion));

        if (no_remote_ipv6_addresses != 0)
            _memcpy(&addresses[no_remote_ipv4_addresses],
...
boolean ch_goodCookie(ChunkID chunkID)
{
    ...
    if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_COOKIE_ECHO) {
        ...
        /* store HMAC */
        _memcpy(cookieSignature, cookie->hmac, HMAC_LEN);
...
gboolean ch_verifyHeartbeat(ChunkID chunkID)
{
    ...
    if (chunks[chunkID]->chunk_header.chunk_id == CHUNK_HBACK) {
        ...
        if (key == NULL) abort();
        /* store HMAC */
        _memcpy(hbSignature, heartbeatChunk->hmac, HMAC_LEN);
...

{
    ...
    if (length > 0) {
        _memcpy(&ec->cause_information, data, length);
    }
                           ...
                           unsigned short dataLength, unsigned char* data)
{
    ...
    if (dataLength>0) _memcpy(vlPtr->the_params, data, dataLength);
