...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLEncAndSnd called.\n"));
   ...
   if(TRUE == datReqParam->isPdcpDatReq)
   {
      ...
      if (ROK != retVal)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"nhDBMAddMuiCb Failed.\n"));
   ...
   if (ROK != retVal) 
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhPIMSendUeMsg Failed.\n"));
      if(TRUE == datReqParam->isPdcpDatReq)
      {
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLEncAndStoreMsg called.\n"));
   ...
   * nhEncBuf is allocated internally by nhEDMEncReq */
   ...
   if(NULLP == transCtrlBlk->lowerLyrParam)
   {
      NH_DBG_FATAL((NH_PRNT_BUF,
           "Memory Allocation failed for storing PIM information.\n")); 
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLSendStoredMsg called.\n"));
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Incorrect SDU type received"));
      RETVALUE(RFAILED);
   }
   RETVALUE(ROK);
} /* nhUTLGetRrcTransId */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"Updating the PDU Statistics \n"));
   ...
   switch(mesgType)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"Invalid message type.\n"));
         break;
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLUpdNhCntr called.\n"));
   ...
   switch(evntCause)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"Invalid Cause.\n"));
         break;      
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"Updating the Cell PDU Statistics.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"Updating the General Statistics.\n"));
   ...
   switch(stsType)
   {
      ...
      case NH_STS_CAUSE_RLC_ERR:
         ...
         break;
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"Invalid Statistics Type.\n"));
         break;      
...
#endif /* ANSI */
{
   ...
   if ((NULLP != transCb) && (transCb->status == NH_STATUS_USED))
   {
      ...
      NH_DBG_ERROR((NH_PRNT_BUF,
            "Duplicate NX transaction(Id:%ld) triggered from RRC user \n", 
               datReqSdus->hdr.transId));
#else /* ALIGN_64BIT */
      NH_DBG_ERROR((NH_PRNT_BUF,
            "Duplicate NX transaction(Id:%d) triggered from RRC user \n", 
               datReqSdus->hdr.transId));
#endif /* ALIGN_64BIT */
   ...
   {
      NH_DBG_INFO((NH_PRNT_BUF,"Msg is already encoded by RRC user.\n"));
      if(TRUE == datReqSdus->sdu.isUeCfgPres)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"UE configuration info present.\n"));
         /* fill the error info and return */
   ...
   if (ROK != ret)
   {
      ...
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "Invalid message type in the UE message"));
...
#ifndef ALIGN_64BIT           
          NH_DBG_ERROR((NH_PRNT_BUF,
            "Duplicate message transaction(Id:%ld) triggered from RRC user \n",
                msgTransKey));
#else /* ALIGN_64BIT */
          NH_DBG_ERROR((NH_PRNT_BUF,
            "Duplicate message transaction(Id:%d) triggered from RRC user \n",
                msgTransKey));
#endif /* ALIGN_64BIT */
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,
...
#endif /* ANSI */
{
   ...
   if(nhIdentity.isPdcpId)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, 
               "PDCP radio bearer with ID(%d) cannot be found\n", 
               nhIdentity.rbId));
         /* fill the error info and return */
   ...
   else
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, 
               "RLC entity with ID(%d) cannot be found \n", nhIdentity.rbId));
         /* Fill the error info and return */
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_SDU_PARAM, 
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
               "Maximum Limit of UE Connections Reached.\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                       "CellCb doesn't Exist with CellId (%d).\n",
                       datRspSdus->hdr.cellId));
      /* Fill the error info and return */
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateDatRsp: Validation for DLCCH Msg failed.\n"));
      RETVALUE(RFAILED);
   }
   ...
   else if(datRspSdus->sdu.msgCategory == NHU_MSG_DLDCCH)
   {
      ...
      if(dlDcchMsg && dlDcchMsg->choice.val == DL_DCCH_MSGTYP_C4)
      {
         ...
         switch(dlDcchMsg->val.c1.choice.val)
         {
            case C4_CSFBPARAMSRSPCDMA2000:
            {
               ...
               {
                      /* Unknown Message Type, error */
                  NH_DBG_ERROR((NH_PRNT_BUF, "Invalid UeId in Header %d\n", 
                                datRspSdus->hdr.ueId));
                  NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,
...
/* Conditional IE validation */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
              "nhUTLValidateDatRsp: conditional IE validation \
              failed\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      /* return on error */
      NH_DBG_ERROR((NH_PRNT_BUF, "RbId is not Configured \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   *cellCb = tmpCellCb;
   ...
   if(cfgReqSdus->hdr.ueId == 0) 
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                 "UE ID config not present in UE Cfg \n"));
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
...
#endif /* ANSI */
{
   ...
   if(messageType == NH_MSGTYPE_CONNRECFG)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "SRB1 is not configured.\n"));
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG, 
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
      "Security not yet configured, Can't send Sec Mode Command Message\n"));
...
/* Conditional IE validation */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
              "nhUTLValidateDatReq: conditional IE validation \
              failed\n"));
      RETVALUE(RFAILED);
   }
#endif /* NH_ASN_COND_VALIDATE */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
              "nhUTLValidateRbOrPdcpId: conditional IE validation \
              failed\n"));
      /* return on error */
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   if(NULLP == ueCb)
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF,"ueCb does not exist.\n"));
      RETVALUE(ROK);
   }
   ...
   if(ueCfg->ueCfgBitMsk & NHU_EXP_CFG_SEC_CFG)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
         "SRB1 Not yet configured, Can't initiate Security Activation\n"));
         /* Fill the error info and return */
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG,
                              NHU_ERRCAUSE_SRB1_NOT_CONFIGURED);
         RETVALUE(RFAILED);
      ...
      if(!(ueCb->rbSecCfgBitMask & NH_SEC_CFG_DONE))
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
            "Both intg and Cipher cfg must be provided for first sec cfg\n"));
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG,
   ...
   if (ueCfg->ueCfgBitMsk & NHU_EXP_CFG_RB_CFG)
   {
      ...
      if (ret != ROK)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "Validation for RB Addition failed\n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: KWU sapID %d not configured\n",
                    cellCfgInfo->llyrSapIdInfo.kwuSapId));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: CKW sapID %d not configured\n",
                    cellCfgInfo->llyrSapIdInfo.ckwSapId));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: CRG sapID %d not configured\n", 
                    cellCfgInfo->llyrSapIdInfo.crgSapId));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: CPJ sapID %d not configured\n",
                    cellCfgInfo->llyrSapIdInfo.cpjSapId));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: PJU sapID %d not configured\n",
                    cellCfgInfo->llyrSapIdInfo.pjuSapId));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateMsapInfo: CTF sapID %d not configured\n",
                    cellCfgInfo->llyrSapIdInfo.ctfSapId));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   for (rbId = 0; rbId < cellCfgInfo->rbToAddCfgList.numOfRbCfgInfo; rbId++)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"Invalid RLC Mode\n"));
         RETVALUE(RFAILED);
      }
         ...
         rbCfgInfo->dlRbCfg.dlRbBitMsk & NHU_RB_CFG_LOGCH_MAP_DL)
      {
         ...
         {
             NH_DBG_ERROR((NH_PRNT_BUF,
                           "Invalid LogCh to Trch mapping in cell cfg \n"));
             RETVALUE(RFAILED);
         }
      } /* rbCfgInfo->rbCfgBitMsk & NHU_RB_CFG_DL */
...
#endif /* ANSI */
{
   ...
   {                                                                            
      NH_DBG_ERROR((NH_PRNT_BUF,                                                
         "Cell Setup: duplicate cell(id:%d) found \n",                          
         cellCfgReqSdus->hdr.cellId));                                          
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,                      
   ...
   {                                                                            
      ...
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,                          
                                  NHU_ERRCAUSE_INVALID_CFG_INFO);               
      printf("cellCfgInfo->cellCfgBitMsk : %d",cellCfgInfo->cellCfgBitMsk);
      RETVALUE(RFAILED);                                                        
   }                                                                            
   ...
   {                                                                            
      NH_DBG_ERROR((NH_PRNT_BUF,                                                
         "Invalid Add RB config in cell cfg \n"));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                  NHU_ERRCAUSE_INV_RB_CFG_INFO);
      RETVALUE(RFAILED);
   }
      ...
      ((cellCfgInfo->sysBwCfg.ulSysBw != NHU_SYS_BW_N6) &&
       ...
       (cellCfgInfo->sysBwCfg.ulSysBw != NHU_SYS_BW_N100)))
             
         {
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                     NHU_ERRCAUSE_INVALID_CFG_INFO);
            printf("cellCfgInfo->sysBwCfg.dlSysBw : %d cellCfgInfo->sysBwCfg.ulSysBw :%d",cellCfgInfo->sysBwCfg.dlSysBw,cellCfgInfo->sysBwCfg.ulSysBw);
            RETVALUE(RFAILED);
         }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid PHY config for cell \n"));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
...
#endif /* ANSI */
{
   ...
   switch(cellCfgReqSdus->sdu.cfgType)
   {
      ...
      case NHU_CFGTYPE_MODIFY:
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Cell Modify/Release: cell(id:%d) not found \n",
               cellCfgReqSdus->hdr.cellId));
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,
                                        NHU_ERRCAUSE_CELL_NOT_CONFIGURED);
            RETVALUE(RFAILED);
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Cell Modify: cellId:%d has ongoing transaction\n",
               cellCfgReqSdus->hdr.cellId));
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Invalid cell config bitmask(%d) in cell cfg \n",
      ...
      case NHU_CFGTYPE_RELEASE:
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Cell Release: cell(id:%d) not found \n",
               cellCfgReqSdus->hdr.cellId));
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_NHU_HDR,
                                        NHU_ERRCAUSE_CELL_NOT_CONFIGURED);
            RETVALUE(RFAILED);
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Cell Modify/Release: cellId:%d has ongoing transaction\n",
               cellCfgReqSdus->hdr.cellId));
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                        NHU_ERRCAUSE_CFG_IN_PROGRESS);
            RETVALUE(RFAILED);
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, 
            "Invalid config type(cfgType: %d) in cell cfg \n", 
   ...
   switch(sapType)
   {
      case STNHKWUSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbKwuSaps) || (lSapId < 0))
         {
             ...
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid KWU \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      case STNHCKWSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbCkwSaps) || (lSapId < 0))
         {
             ...
             * Drop message, return failure.
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid CKW \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      case STNHCRGSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbCrgSaps) || (lSapId < 0))
         {
             ...
             * Drop message, return failure.
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid CRG \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      case STNHCPJSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbCpjSaps) || (lSapId < 0))
         {
             ...
             * Drop message, return failure.
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid CPJ \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      case STNHPJUSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbPjuSaps) || (lSapId < 0))
         {
             ...
             * Drop message, return failure.
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid PJU \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      case STNHCTFSAP:
         ...
         if ((lSapId >= (SuId)nhCb.genCfg.nmbCtfSaps) || (lSapId < 0))
         {
             ...
             * Drop message, return failure.
             */
            NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLFindLlyrSapFrmSuId() invalid CTF \
                     spId\n"));
            RETVALUE(NULLP);
         }
#endif /* ERRCLASS */
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,
                     "nhUTLFindLlyrSapFrmSuId: Invalid sapType \n"));
         RETVALUE(NULLP);
   }
...
#endif /* ANSI */
{
   ...
   switch(msgType)
   {
      ...
      case C6_MEASUREMENTREPORT:
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "SRB validation failed for message:%d \n",
                     msgType));
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "received in RB ID: %d.\n", rbId));
            RETVALUE(RFAILED);
      ...
      case C6_PROXIMITYIND_R9:
         ...
         break;
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"Unknown message received.\n"));
         RETVALUE(RFAILED);
   } /* end of switch */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((nhCb.init.prntBuf,"nhUTLGetRbToRestb called.\n"));
   ...
   while(NULLP != crntRbInf)
   {
       ...
       */
       ...
       if(found == FALSE)
       {
          ...
          {
             NH_DBG_FATAL((NH_PRNT_BUF,
                   "Memory Allocation failed for adding radio bearer.\n"));
             RETVALUE(ROUTRES);
          }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhUTLFreeUlBitMapFrmCrntRbCfg called  \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhUTLFreeUlBitMapFrmOrdCfg called  \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLValidateAsnCondIes called.\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
              "nhUTLValidateAsnCondIes:NULLP Cell Cb\n"));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_SDU_PARAM,
   ...
   switch(msgType)
   {
      case NH_MSGTYPE_CONNSETUP:
            ...
            if (retVal != ROK)
            {
               NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateAsnCondIes:Valdiation \
                                          for RRCConnSetup failed.\n"));
            }
            break;
      ...
      case NH_MSGTYPE_HOFRMEUTRAPREPREQ:
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateAsnCondIes:Valdiation\
                                      for HO-FRM-EUTRA-PREP-REQ failed.\n"));
            }
            break;
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateAsnCondIes: conditional IE validation \
                    failed\n"));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_SDU_PARAM,
                        NHU_ERRCAUSE_COND_IE_FAIL);
      RETVALUE(RFAILED);
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLValidateCondIesReCfgHo called.\n"));
   ...
   if( ((NHU_MODE_FDD == duplexMode) &&
       ...
       ((NHU_MODE_TDD == duplexMode) && 
        (NH_IS_TKN_PRSNT(
            reCfgIEs->mobilityCntrlInfo.carrierFreq.ul_CarrierFreq))) )
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesReCfgHo: \
               Failed NHU_ERRCAUSE_UL_ARFCN_VALUEUTRA\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesReCfgHo: \
               Failed NHU_ERRCAUSE_DEDTED_INF_NAS\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhUTLValidateCondIesReCfgHo: conditional IE validation \
                     for radio resource dedicated failed\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesReCfgHo: \
               Failed NHU_ERRCAUSE_SCRTY_CFG_HO\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLValidateCondIesRadResDed called.\n"));
   ...
   if(TRUE == isHo)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
        "nhUTLValidateCondIesRadResDed: Failed NHU_ERRCAUSE_SRB_TOADDMDLST\n"));
         RETVALUE(RFAILED);
      }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
        "nhUTLValidateCondIesRadResDed: Failed NHU_ERRCAUSE_DRB_TOADDMDLST\n"));
         RETVALUE(RFAILED);
      }
   }
   else
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                 "nhUTLValidateCondIesRadResDed: conditional IE validation \
                  for RBs failed\n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
       "nhUTLValidateCondIesRadResDed: Validation for Physical config dedicated\
        failed \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   if (NH_IS_TKN_PRSNT(radioResDed->physicalConfigDedicated.antennaInfo.choice))
   {
          ...
          (NhuAntennaInfoDedicatedtransmissionModetm6Enum == txMode) )
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRadResPhyCfgDed: \
                     Failed NHU_ERRCAUSE_CODEBK_SUBSET_RSTRCN\n"));
            RETVALUE(RFAILED);
         }
      }
      ...
      if(NhuAntennaInfoDedicatedtransmissionModetm8_v920Enum != txMode)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRadResPhyCfgDed: \
                     Failed NHU_ERRCAUSE_CODEBK_SUBSET_RSTRCN_R9\n"));
            RETVALUE(RFAILED);
         }
      }
#endif /* LTERRC_REL9 */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRadResPhyCfgDed: \
               Failed NHU_ERRCAUSE_TDD_ACKNACKFB_MOD\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (NH_IS_TKN_PRSNT(radioResDed->physicalConfigDedicated.extaddgrp_r9.pres))
   {
                  ...
                  cqi_ReportConfig.cqi_ReportModeAperiodic)))
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRadResPhyCfgDed: \
                     Failed NHU_ERRCAUSE_PMI_RI_REPORT\n"));
            RETVALUE(RFAILED);
         }
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhUTLValidateCondIesMeasCfg called.\n"));
   ...
   if(NH_IS_IE_PRSNT(measCfg))
   {
      if(NH_IS_IE_PRSNT(measCfg.preRegistrationInfoHRPD))
      {
              ...
              val))
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesMeasCfg: \
                        Failed NHU_ERRCAUSE_PREREG_ZONE_ID\n"));
               RETVALUE(RFAILED);
            }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesMeasCfg: \
                  Failed NHU_ERRCAUSE_RPRT_CFG_TOADDMODLST\n"));
         RETVALUE(RFAILED);
      }
   ...
   if(NH_IS_TKN_PRSNT(reportCfg.choice))
   {
     ...
     if(!(NH_IS_TKN_PRSNT(reportCfg.val.reportConfigEUTRA.triggerTyp.val.\
             ...
             NhuReportConfigInterRATtriggerTypperiodicalpurposereportCGIEnum))
        {
           ...
           si_RqstForHO_r9))
           {
              NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesReportCfg: \
                       Failed NHU_ERRCAUSE_SI_REQSTFORHO_R9\n"));
              RETVALUE(RFAILED);
           }
        }
   } /* end of measurement config pres check */
...
#endif /* ANSI */
{
   ...
   if (NH_IS_IE_PRSNT(ies->radioResourceConfigDedicated))
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRRCConnSetup: \
                  Failed NHU_ERRCAUSE_SRB_TOADDMDLST\n"));
         RETVALUE(RFAILED);
      }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRRCConnSetup: \
                  Failed NHU_ERRCAUSE_DRB_TOADDMDLST\n"));
         RETVALUE(RFAILED);
      }
      ...
      for (idx = 0; idx < count; idx ++)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRRCConnSetup: \
                     Failed NHU_ERRCAUSE_LGCL_CHANNEL_CFG\n"));
            RETVALUE(RFAILED);
         }
      }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRRCConnSetup: \
                     Validation for RadResDedCom faield \n"));
         RETVALUE(RFAILED);
      }
   }/* if (NH_IS_IE_PRSNT(ies->radioResourceConfigDedicated)) */
      ...
      case C1_SYSTEMINFOBLOCKTYP1:
         ...
         */
         if(duplexMode == NHU_MODE_TDD)
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesSibs: \
                        Failed NHU_ERRCAUSE_TDD_CFG\n"));
               RETVALUE(RFAILED);
            }
         }
         break;
            ...
            for (idx = 0; idx < count; idx++)
            {
                  ...
                  sib_TypAndInfo->member[idx].choice.val == MEMBER_SIB4 )
               {
                  if (isCSGCell)
                  {
                     ...
                     {
                        NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesSibs: \
                                 Failed NHU_ERRCAUSE_CSG_PHYSCELLIDRNGE\n"));
                        RETVALUE(RFAILED);
                     }
                  } /* if (isCSGCell)  */
            ...
            for (idx = 0; idx < count; idx++)
            {
                  ...
                  sib_TypAndInfo->member[idx].choice.val == MEMBER_SIB8 )
               {
                           ...
                           parameters1XRTT.csfb_RegistrationParam1XRTT.pres))
                  {
                     ...
                     {
                        NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesSibs: \
                                  Failed NHU_ERRCAUSE_AC_BARRINGCONFIG1XRTT_R9\n"));
                        RETVALUE(RFAILED);
            ...
            for (idx = 0; idx < count; idx++)
            {
                  ...
                  sib_TypAndInfo->member[idx].choice.val == MEMBER_SIB11 )
               {
                        ...
                        warningMsgSegmentNumber.val == 0)
                  {
                     ...
                     {
                        NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesSibs: \
                                  Failed NHU_ERRCAUSE_DATACODINGSCHM\n"));
                        RETVALUE(RFAILED);
                     } /* if(NH_IS_TKN_NOTPRSNT(sib_TypAndInfo.membe */
                  } /* if (sib_TypAndInfo.member[idx].val.sib8.p*/
               }/* if(NH_IS_TKN_PRSNT(sib_TypAnd  */
   ...
   */
   ...
   for (idx = 0; idx < count; idx ++)
   {
                              ...
                              FALSE, ueCb) == FALSE)
      {
         if (NH_IS_TKN_NOTPRSNT(srb_toAddMod.rlc_Config.choice))
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_RLC_CFG\n"));
            RETVALUE(RFAILED);
         }
      }
   ...
   */
   ...
   for (idx = 0; idx < count; idx ++)
   {
               ...
               explicitValue.ul_SpecificParams.pres))
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_LGCL_CHANNELSR_MASK_R9"));
            RETVALUE(RFAILED);
         }
      }
#endif /* LTERRC_REL9 */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs\
               Failed: nhUTLValidateCondIesRb_DRB_PdcpCfg\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs\
               Failed: nhUTLValidateCondIesRb_DRB_RLCCfg\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs\
               Failed: nhUTLValidateCondIesRb_DRB_LgclCfg\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs\
               Failed: nhUTLValidateCondIesRb_DRB_Ul_SpParam\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs\
               Failed: nhUTLValidateCondIesRb_DRB_Othr\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
       ...
       (NH_IS_IE_PRSNT(mobIEs->purpose.val.handover)) )
   {
                        ...
                        mobIEs->purpose.val.handover.targetRAT_Typ.val) )
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesMobFrmEutra: \
                     Failed NHU_ERRCAUSE_NAS_SCRTY_PARAM"));
            RETVALUE(RFAILED);
         }
      }
                        ...
                        mobIEs->purpose.val.handover.targetRAT_Typ.val)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesMobFrmEutra: \
                     Failed NHU_ERRCAUSE_SI_OR_PSI_GERAN"));
            RETVALUE(RFAILED);
         }
      }
   }
...
#endif /* ANSI */
{
       ...
       (NhuCDMA2000_Typtype1XRTTEnum == hoPrepIEs->cdma2000_Typ.val) )
   {
      ...
      {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesHoFrmEutraPrepTfr: \
                     Failed NHU_ERRCAUSE_RAND_CDMA2000"));
            RETVALUE(RFAILED);
      }
      else if(NH_IS_TKN_NOTPRSNT(hoPrepIEs->mobilityParams))
      {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesHoFrmEutraPrepTfr: \
                     Failed NHU_ERRCAUSE_MBLTY_PARAM_CDMA2000"));
            RETVALUE(RFAILED);
      }
   }
   RETVALUE(ROK);
...
#endif /* ANSI */
{
   ...
   if (retVal != ROK)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesHoPrepInfo:\
               Failed: conditional IEs within Radio Resource Dedicated IE\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesHoPrepInfo:\
               Failed: conditional IEs within Measurement IE\n"));
      RETVALUE(RFAILED);
   }
   ...
   */
   ...
   for (idx = 0; idx < count; idx ++)
   {
                             ...
                             TRUE, ueCb) == FALSE)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_LGCL_CHANNEL_ID"));
            RETVALUE(RFAILED);
         }
      }
   ...
   */
   for (idx = 0; idx < count; idx ++)
   {
      ...
      if (NH_IS_IE_PRSNT(drb_toAddMod.rlc_Config.val.um_Uni_Directional_UL))
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_UL_SPCFCPARAMS"));
            RETVALUE(RFAILED);
         }
      }
   }
   ...
   */
   for (idx = 0; idx < count; idx ++)
   {
                             ...
                             TRUE, ueCb) == FALSE)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_EPS_BEARERIDENTITY"));
            RETVALUE(RFAILED);
         }
      }
   }
   ...
   */
   for (idx = 0; idx < count; idx ++)
   {
                             ...
                             TRUE, ueCb) == FALSE)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "nhUTLValidateCondIesRbs: \
                     Failed NHU_ERRCAUSE_DISCRD_TMR"));
            RETVALUE(RFAILED);
         }
      }
   }
...
#endif /* ANSI */
{
   ...
   switch(elmntId)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,
                     "nhUTLValidateLSapSuId: Invalid sapType \n"));
...
#endif /* ANSI */
{
   ...
   *mesgType = NH_IGNORE;
   ...
   if(datRspSdus->sdu.msgCategory == NHU_MSG_DLCCCH)
   {
      ...
      if(dlCcchMsg && dlCcchMsg->choice.val == DL_CCCH_MSGTYP_C3)
      {
         ...
         switch(dlCcchMsg->val.c1.choice.val)
         {
            case C3_RRCCONSETUP:
            {
               ...
               {
                  NH_DBG_ERROR((NH_PRNT_BUF,
                                 "Temporary Crnti is not present.\n"));
                  NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                    NHU_ERRCAUSE_CRNTI_NOT_CFG);
                  RETVALUE(RFAILED);
               }
               ...
               if(datRspSdus->hdr.ueId == NH_ZERO_VAL) 
               {
                  ...
                  {
                     NH_DBG_ERROR((NH_PRNT_BUF,
                             "UE ID config not present in the UE CFG.\n"));
                     NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                                 NHU_ERRCAUSE_CRNTI_NOT_CFG);
                     RETVALUE(RFAILED); 
                  }
               ...
               else
               {
                  ...
                  {
                     NH_DBG_ERROR((NH_PRNT_BUF,
                          "UE ID mismatch in config and message SDU %d.\n", 
                          datRspSdus->sdu.m.dlCcchMsg.tmpCrnti ));
                     NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                           NHU_ERRCAUSE_CRNTI_MISMATCH);
                     RETVALUE(RFAILED);
            ...
            case C3_RRCCONREESTB:
            {
               ...
               if((TRUE == datRspSdus->sdu.isUeCfgPres) && (
                 ((datRspSdus->sdu.ueCfg.ueCfgBitMsk & NHU_EXP_CFG_UE_CFG) 
                                                                     == 0) ||
                 ((datRspSdus->sdu.ueCfg.ueCfgBitMsk & NHU_EXP_CFG_SEC_CFG) 
                                                                     == 0)))
               {
                  NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Configuration Info %d\n",
                                             datRspSdus->hdr.ueId));
                  NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                              NHU_ERRCAUSE_INVALID_CFG_INFO);
                  RETVALUE(RFAILED);
                     ...
                     (datRspSdus->sdu.ueCfg.ueCfgBitMsk & NHU_EXP_CFG_UE_CFG) &&
                      (datRspSdus->sdu.ueCfg.ueInfo.newCrnti !=
                                          datRspSdus->sdu.m.dlCcchMsg.tmpCrnti))
               {
                  NH_DBG_ERROR((NH_PRNT_BUF, "UE ID mismatch in config and \
                  message SDU %d.\n", datRspSdus->sdu.m.dlCcchMsg.tmpCrnti ));
                  NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_INV_CFG,
                                    NHU_ERRCAUSE_CRNTI_MISMATCH);
                  RETVALUE(RFAILED);
...
#endif /* ANSI */
{
         ...
         && (ueCb->crntCfg.bitMskPres & NH_CRNT_CFG_RB))
   {
      ...
      for(i=0; i<(rbToAddList->numOfRbCfgInfo); i++)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                        "Logical channel Configuration with Invalid \
                                                  Ul/Dl BitMask \n"));
            /* Fill the error info and return */
            NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG,
                               NHU_ERRCAUSE_INV_RB_CFG_INFO);
         ...
         if (rbCfgInfo->rbCfgBitMsk & NHU_RB_CFG_PDCP_INFO)
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF,
                        "RB(PDCP) Configuration with \
                         Invalid Ul/Dl(RLC) BitMask \n"));
               /* Fill the error info and return */
               NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG,
                               NHU_ERRCAUSE_INV_RB_CFG_INFO);
               RETVALUE(RFAILED);
