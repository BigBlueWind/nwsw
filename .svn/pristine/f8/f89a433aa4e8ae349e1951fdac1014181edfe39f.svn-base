 ...
 */
RESULTCODE GemacWrite(U32 size, PUNKNOWN data)
{
    GEMAC_FRAME *frm;
    U32 *src = (U32*) data;
    U32 *dest;
    ...
    if (OsalAllocMemArrayBlock(&Gemac0Tx, (OSAL_PADDRESS*)&frm) != SUCCESS)
        return FAILURE;
    ...
    {
        frm->control.next = (PUNKNOWN)Gemac0TxHead;
    }
 ...
 * @return Standard result code
 */
RESULTCODE GemacAddBDesc(GEMAC_FRAME *frame, U32 index, U32 size, PUNKNOWN data)
{
    // FIXME: Other fields
 ...
 * @return Standard result code
 */
RESULTCODE GemacWriteBlocks(U32 blkNum, U32 size, PUNKNOWN data, ...)
{
    GEMAC_FRAME *frm;
    U32 i;
    U32 sizeVar;
    ...
    for (i = 1; i < blkNum; i++)
    {
        ...
        dataVar = va_arg(paramList, PUNKNOWN);
        if (sizeVar > GEM_MAX_FRAME)
        {
            OsalFreeMemArrayBlock(&Gemac0Tx, (OSAL_PADDRESS*)frm);
 ...
 */
void Gemac0RxTask(PUNKNOWN taskParm)
{
    while (1)
    {
 ...
 * @param taskParm Not used
 */
void Gemac0TxTask(PUNKNOWN taskParm)
{
    ...
    while (1)
    {
        if (OsalWaitForSema(&Gemac0TxSema, OSAL_WAIT_INFINITE) == SUCCESS)
        {
            ...
            while (frmList)
            {
                ...
                if (OsalFreeMemArrayBlock(&Gemac0Tx, frmList) != SUCCESS)
                {
                    // TODO: Alert
                    break;
                }
 ...
 * @param taskParm Not used
 */
void Gemac1RxTask(PUNKNOWN taskParm)
{
    while (1)
    {
 ...
 * @param taskParm Not used
 */
void Gemac1TxTask(PUNKNOWN taskParm)
{
    GEMAC_FRAME *frmList, *frmTmp;
    ...
    while (1)
    {
        if (OsalWaitForSema(&Gemac1TxSema, OSAL_WAIT_INFINITE) == SUCCESS)
        {
            ...
            if (OsalCheckSema(&Gemac1TxSema) == SUCCESS)
            {
                ...
                {
                    frmTmp = (GEMAC_FRAME*) frmList->control.next;
                    if (OsalFreeMemArrayBlock(&Gemac1Tx, frmList) != SUCCESS)
                    {
                        // TODO: Alert
                        break;
                    }
...
#if 0
/** Callback for Quadnet's [OPEN] interface
 *
 *  @param interfaceHandle
 *  @return
 */
int gemacOpen(ttUserInterface interfaceHandle)
{
    GemStartTx(&Gemac0);
    GemEnableRx(&Gemac0);
...
/** Callback for Quadnet's [CLOSE] interface
 *
 *  @param interfaceHandle
 *  @return
 */
int GemacClose(ttUserInterface interfaceHandle)
{
    IdmaStop(IDMA_EMAC0);
