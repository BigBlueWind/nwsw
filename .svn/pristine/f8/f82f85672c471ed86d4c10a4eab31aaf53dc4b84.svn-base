...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, 
              "Updating Explicit Configuration in Trans Control Block \n"));

   if (ueCfg->ueCfgBitMsk & NHU_EXP_CFG_UE_CFG)
   { 
      NH_DBG_INFO((NH_PRNT_BUF,
                   "Updating UE CFG in ordered cfg \n"));
   ...
   { 
      NH_DBG_INFO((NH_PRNT_BUF,
                    "Updating Rb CFG in ordered cfg \n"));
      if (ueCfg->rbCfgAllList.rbCfgTypBitMsk & NHU_CFG_RB_ADD)
      {
         NH_DBG_INFO((NH_PRNT_BUF,
             "Updating Rb ADD CFG in ordered cfg \n"));
         ...
         if (NULLP == transCtrlBlk->ordrdCfg.rbToAddCfgList.rbCfgInfoList)
         {
            NH_DBG_FATAL((NH_PRNT_BUF, 
                     "Cannot allocate mem for Add RB list"));
            RETVALUE(RFAILED);
         }
      ...
      {
         NH_DBG_INFO((NH_PRNT_BUF,
                "Updating Rb MOD CFG in ordered cfg \n"));
         ...
         {
            NH_DBG_FATAL((NH_PRNT_BUF, 
                     "Cannot allocate mem for modified RB list"));
            RETVALUE(RFAILED);
         }
         ...
         if(ROK != nhCGMUpdPdcpSduStaUeCbExpCfg(transCtrlBlk, ueCfg))
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                         "nhCGMUpdPdcpSduStaUeCbExpCfg function failed\n"));
            RETVALUE(RFAILED);
         }
     ...
     {
        NH_DBG_INFO((NH_PRNT_BUF,
                 "Updating Rb RLS CFG in ordered cfg \n"));
        ...
        {
           NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Cannot allocate mem for release RB list"));
           RETVALUE(RFAILED);
        }
   ...
   {
      NH_DBG_INFO((NH_PRNT_BUF, 
          "Updating TrCh CFG in ordered cfg\n"));
   ...
   { 
      NH_DBG_INFO((NH_PRNT_BUF, 
           "Updating Security CFG in ordered cfg \n"));
   ...
   {
      NH_DBG_INFO((NH_PRNT_BUF, 
           "Updating Physical layer CFG in ordered cfg \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"Release lower layer Cfg.\n"));
   ...
   {
       NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhLIMRlsLowerLyrCfg failed for UEID %d CellId %d \n", 
                    nhIdentity.ueId, nhIdentity.cellId));
       RETVALUE(RFAILED);
   }

   NH_DBG_DEBUG((NH_PRNT_BUF, "delete UE CB.\n"));
   /* call UDM module to delete the UE control block */
   if (RFAILED == nhDBMDeleteUeCb(cellCb, &ueCb))
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhDBMDeleteUeCb failed \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   if (ROK != ret)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Update of ordered configuration \
                                    in UeCb failed.\n"));
      RETVALUE(ret);
   }
   ...
   if(cfgReqd)
   {
         ...
         transCtrlBlk->ordrdCfg.ueInfo.crntiCfgType == NH_UE_INFO_MOD_CRNTI) 
      {
         ...
         if(ROK != ret)
         {
            NH_DBG_ERROR((NH_PRNT_BUF, "UE addition to cell \
                                       hash list failed.\n"));
            RETVALUE(ret);
         }
      }
      ...
      if(ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"LIM Send Cfg Failed\n"));
         RETVALUE(ret);
      }
   }
...
#endif /* ANSI */
{
   ...
   switch(cellCfgSdu->sdu.cfgType)
   {
      ...
      default:
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, 
            "Incorrect cell config type detected after validation \n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   switch (cellCfgSdu->sdu.cfgType)
   {
      case NHU_CFGTYPE_SETUP:
      {
         ...
         if (cellCfgFrmSdu->cellCfgBitMsk & NHU_CELL_CFG_RB)
         {
            ...
            {
               NH_DBG_FATAL((NH_PRNT_BUF, 
                        "Cannot allocate mem for Add RB config \n"));
               RETVALUE(RFAILED);
            }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
               "Cannot allocate mem for Ordered configuration"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   if (transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_UEID)
   {
      ...
      else
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, "Invalid ue cfgtype"));
         RETVOID;
      }
   } /* if (transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_UEID) */
   else
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, 
           "nhCGMDeleteTrans: Release lower layer Cfg.\n"));
   ...
   */
   if (transCtrlBlk->ordrdCfg.actionMask)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
              "Ongoing configuration, cannot delete transCtrlBlk \n"));
    ...
    */
   NH_DBG_DEBUG((NH_PRNT_BUF,"delete Trans CB. \n"));

   /* call UDM module to delete trans control block */
   if (RFAILED == nhDBMDelTransCb(ueCb, &transCtrlBlk))
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhUDMDelTransInfoBlock Failed \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
    ...
    ******************************************************/
    ...
    {
       NH_DBG_FATAL((NH_PRNT_BUF,
             "Memory Allocation failed for adding CmLList.\n"));
       RETVALUE(ROUTRES);
    }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF," nhCGMPdcpDiscReqdCfgReq called.\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF,
                    "Cannot allocate mem for Mod RB list"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
              "Updating Explicit Configuration in Trans Control Block for \
   ...
   for(rbCnt = 0; rbCnt < noRbToCfg; rbCnt++)
   {
      ...
      if (pdcpCfgBitMsk & NHU_PDCP_SDU_STA_INFO)
      {
             ...
             (hoPdcpSduStaInfo->numBits > 0) )
         {
            ...
            {
               NH_DBG_FATAL((NH_PRNT_BUF,
                  "Cannot allocate mem for ulBitMap in ordered config"));
