/** @file ctrlmsg.c
 *
 * @brief Control messages handling
...
MsgFreeBufferCb     MsgFreeBuffer  = MsgInitFreeBuffer;
MsgAllocBufferCb    MsgAllocBuffer = MsgInitAllocBuffer;
...
void PackToHostTask(PTR ptr)
{
    ...
    while (1)
    {
        if (OsalWaitForSema(&PackToHostSema, OSAL_WAIT_INFINITE) == SUCCESS)
        {
            while(OsalDequeue(&PackQueue,&msg) == SUCCESS)
            {
                 pHdr = (PMSGHEADER) msg;
                 HostSendMsg(pHdr->length + sizeof(MSGHEADER), msg);
                 OsalFreeMemArray(&MsgArrayPack,msg);
            }
        }
    }
}
EXPORT RESULTCODE MsgDispatcherInit(void)
{
    ...
    OsalCreateMemArray(&MsgArrayPack, MsgArrayMemPack, MSG_MAXSIZE * num, MSG_MAXSIZE);

	
    OsalCreateTask(&PackToHost,
        (OSAL_PADDRESS) &PackToHostTask,
        NULL, "PackToHostTask",
...
//        printf("MsgID:   0x%04x\n", pHdr->msgID);
//        printf("RealCRC: 0x%08x\n", RealCRC);               
//        printf("RefCRC:  0x%08x\n", RefCRC);
...
//		pHdr->control, pHdr->length, *(U32*)((U32)pHdr+16), *(U32*)((U32)pHdr+ 20));
    ...
    if (route == MSGS_UARM)
    {
        ...
        {
            case MSGS_SVSR:
                ret = SvsrDispatch(size, msg);
                break;
            case MSGS_HOST:
                if (HostSendMsg)
               	{
                    ret = HostSendMsg(size, msg);
               	}
                else
              	{
                    MsgFreeBuffer(msg);
                ...
                break;
            case MSGS_MAC:
                ret = MacDispatch(size, msg);
                break;
//          case MSGS_MSMAC:
            case MSGS_AUX:
                // Forward message to AUX interface
                if (AuxSendMsg)
                    ret = AuxSendMsg(size, msg);
                else
                    ret = MacDispatch(size, msg);
                break;
            case MSGS_RRC:
#if defined(ENODEB) || defined(UE) || defined (EHDMAC)
                ret = RrcDispatch(size, msg);
#else
                MsgFreeBuffer(msg);
#endif
                break;
    ...
    else if (route == MSGS_LARM)
    {
        ...
        case MSGS_UARM:
            ...
            break;
        case MSGS_LARM:
#if defined (UE) && defined (SIMUPC)
            // MAC-2-MAC mode
            PhyTransfer(size, msg);
#else
            MsgFreeBuffer(msg);
#endif
            break;
        case MSGS_AUX:
            PhyDispatch(size, msg);
            break;
        case MSGS_HOST:
#if defined (ENODEB)&& defined (SIMUPC) && defined (USE_WINDOWS)
            // recv Host "PHY_LTE_PHYTICK_MSG"
            if (IcpuMode == IMODE_HBI)
                IcpuSendMsg(size, msg);
            else
                PhyTransfer(size, msg);
#else
            // Directly send msg to LowARM
            if (IcpuSendMsg)
                IcpuSendMsg(size, msg);
            else
                MsgFreeBuffer(msg);
...
#ifdef _MSC_VER
        if ((int)((__int8*)pNext - (__int8*)msg) < (int) msg->length + (int) sizeof(MSGHEADER))
#else
...
EXPORT PMSGHEADER MsgInitAllocBuffer(int flags)
{
    ...
    if (flags == MSG_FAST) 
    {
    MsgAllocBuffer = ((U32)IcpuGetPhys != ((U32)Icpu1GetPhys)) ? Msg2AllocBuffer : Msg1AllocBuffer;    
    }
    else
    {        
        return Msg1AllocBuffer(flags);
    }
#else
    MsgAllocBuffer = Msg1AllocBuffer;    
#endif
    return MsgAllocBuffer(flags);
}

EXPORT void MsgInitFreeBuffer(PMSGHEADER pMsg)
{
#if defined (USE_LINUX) && defined (C4KEVM)
    if (MsgAllocBuffer != MsgInitAllocBuffer)
    {
    MsgFreeBuffer = ((U32)IcpuGetPhys != ((U32)Icpu1GetPhys)) ? Msg2FreeBuffer : Msg1FreeBuffer;
    }
    else
    {
        Msg1FreeBuffer(pMsg);
