...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 118 "../..//lterrc/nh_edm.c" 2
# 1 "../..//cm/cm5.x" 1
# 119 "../..//lterrc/nh_edm.c" 2
# 1 "../..//cm/cm_tkns.x" 1
# 91 "../..//cm/cm_tkns.x"
#define __CMTKNSX 
# 121 "../..//cm/cm_tkns.x"
...
# 132 "../..//lterrc/nh_edm.c" 2
# 1 "../..//lterrc/nh_3gdb.x" 1
# 89 "../..//lterrc/nh_3gdb.x"
#define __NH3GDBX__ 
...
extern CmPAsnElmDef *nhUTRANInterRATHovrInfo_Pdu[];
extern CmPAsnElmDef *nhUTRANUE_RadioAccessCapInfo_Pdu[];
# 133 "../..//lterrc/nh_edm.c" 2
# 1 "../..//mt/ss_diag.h" 1
# 90 "../..//mt/ss_diag.h"
...

{
   ...
   switch(msgCategory)
   {
      ...
      default:
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_edm.c", 225); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid message category \n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(1);
   }


   { if(nhCb.init.logMask & 0x20) { Buffer *_newBuf; SAddMsgRef(mBuf, nhCb.init.region, nhCb.init.pool, &_newBuf); ssDiagFix(0x004b0001, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x20, SS_DIAG_MSG_TYPE_HEXDUMP, _newBuf, 0, 0, 0,0, "Received RRC Message :"); } };


   ret = cmPAsnDecMsgUalign ((Void*)pdu, 1, (Void **)&mBuf,
                            &numDecOct, 0, elmDef, 3, &err,
                            4096, (CmMemListCp *)memCp,
                            dbgFlag);

   if (0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_edm.c", 241); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "PASN Decoding Failed for msg category %d \n", msgCategory); SPrint((&nhCb.init)->prntBuf); } }; };


      nhUTLUpdGenSts((CntrType)6);
      return(ret);
   }
...

{
   ...
   NhuDL_CCCH_Msg *dlCcchPdu = 0L;
   NhuDL_DCCH_Msg *dlDcchPdu = 0L;
   NhuPCCH_Msg *pcchPdu = 0L;
   NhuBCCH_BCH_Msg *bcchBchPdu = 0L;
   NhuBCCH_DL_SCH_Msg *bcchDlschPdu = 0L;
   NhuHovrCmd *hovrCmd = 0L;
   NhuHovrPrepInfo *hovrPrepInfo = 0L;
   NhuUERadioAccessCapInfo *ueRadioAccCapInfo = 0L;
   NhuUECapInfo *ueCapInfo = 0L;
   ...
   do { if (SGetMsg((nhCb.init.region), (nhCb.init.pool), (Buffer **)(mBuf)) != 0) { (*(mBuf)) = 0L; } } while(0);

   if (0L == mBuf)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_edm.c", 308); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Unable to Allocate Buffer \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   switch(msgCategory)
   {
      case 0x03:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)dlCcchPdu, 1,
               (Void*)(*mBuf), 0, nhDL_CCCH_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x01:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)dlDcchPdu, 1,
              (Void*)(*mBuf), 0, nhDL_DCCH_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x05:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)pcchPdu, 1,
             (Void*)(*mBuf), 0, nhPCCH_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x06:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)bcchBchPdu, 1,
             (Void*)(*mBuf), 0, nhBCCH_BCH_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x07:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)bcchDlschPdu, 1,
            (Void*)(*mBuf), 0, nhBCCH_DL_SCH_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x08:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)hovrCmd, 1,
            (Void*)(*mBuf), 0, nhHovrCmd_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x09:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)ueRadioAccCapInfo, 1,
      ...
      case 0x0A:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)ueCapInfo, 1,
            (Void*)(*mBuf), 0, nhUECapInfo_Pdu, &err, length, dbgFlag, 0);
         break;
      ...
      case 0x0B:
         ...
         ret = cmPAsnEncMsgUalign ((TknU8*)hovrPrepInfo, 1,
                                  (Void*)(*mBuf), 0, nhHovrPrepInfo_Pdu,
                                  &err, length, dbgFlag, 0);
         break;
# 397 "../..//lterrc/nh_edm.c"
      default:
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_edm.c", 399); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Message Category \n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(1);
   }
   { if(nhCb.init.logMask & 0x20) { Buffer *_newBuf; SAddMsgRef(*mBuf, nhCb.init.region, nhCb.init.pool, &_newBuf); ssDiagFix(0x004b0002, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x20, SS_DIAG_MSG_TYPE_HEXDUMP, _newBuf, 0, 0, 0,0, "Sending RRC Message :"); } };


   if (ret != 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_edm.c", 408); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "PASN Encoding Failed for Message category %d \n", msgCategory); SPrint((&nhCb.init)->prntBuf); } }; };



      { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004b0003, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0, msgCategory, 0, 0, 0, "ASN Decoding failed, msgCategory = %d"); } };


      nhUTLUpdGenSts((CntrType)7);

      do{ if ((*mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_edm.c", 414, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((*mBuf)); (*mBuf) = 0L; } }while(0);
      return(ret);
   }
