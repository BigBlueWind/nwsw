...
#ifdef PWAV_SIH
    /* Initialize the SIH Module */ 
    if (!sihInit(argc, argv)) 
       snmp_log(LOG_INFO, "SIH Initialised\n"); 
#endif /* PWAV_SIH */
...
receive(void)
{
   ...
   Msg     *msgIn = NULL;
   UINT16   ec;
   ModAddr * modAddr = getModAddr(MODID_SIH);
#endif /* PWAV_SIH */    
...
#endif
/*
 * When BSM Send's Config request to SIH then SIH will make reconfig to 1. which makes 
 * Reconfiguring of net-snmp
 */
            reconfig = 0;
...
#ifdef PWAV_SIH
        if (modAddr->recvFd >= 0) {
            NETSNMP_LARGE_FD_SET(modAddr->recvFd, &readfds);
            numfds = modAddr->recvFd >= numfds ? modAddr->recvFd + 1 : numfds;
        }
#endif /* PWAV_SIH */    
    ...
    reselect:
        ...
        if (modAddr->recvFd >= 0) 
        {
         if (NETSNMP_LARGE_FD_ISSET(modAddr->recvFd, &readfds))
         {
          msgIn = recvMsg(MODID_SIH, &ec);
          if ( msgIn != NULL )
          {
           sihMain(msgIn);
          }          
