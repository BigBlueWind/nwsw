...
#include "lnh.x"
#include "nh.x"
#include "ss_diag.h"        /*LOG Definiton*/
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, 
          "NhMiLnhCfgReq(elementId (%d))\n", cfg->hdr.elmId.elmnt));
  NH_DIAG_LVL0(0x060e0001, NH_DIAG_NA, SS_DIAG_INV_ARG,
        "Received CfgReq from SM element = %d",
        cfg->hdr.elmId.elmnt,0,0,0);

   /* This macro checks for NULL pointers being passed */
   if (NH_CHK_INPUT_PARAM(cfg))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cfg\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, 
                "NhMiLnhCntrlReq ( elementId (%d))\n",
                cntrl->hdr.elmId.elmnt));
  NH_DIAG_LVL0(0x060e0002, NH_DIAG_NA, SS_DIAG_INV_ARG,
        "Received CntrlReq from SM element = %d",
        cntrl->hdr.elmId.elmnt,0,0,0);

   /* This macro checks for NULL pointers being passed */
   if (NH_CHK_INPUT_PARAM(cntrl))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cntrl\n"));
      RETVALUE(RFAILED);
   }
   ...
   switch(cntrl->hdr.elmId.elmnt)
   {
      ...
      default:
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, 
                       "Invalid Element in control request %d\n",
                       cntrl->hdr.elmId.elmnt));
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "NhMiLnhStsReq(action(%d), elmnt(%d)) \n",
                action, sts->hdr.elmId.elmnt));
 
  NH_DIAG_LVL0(0x060e0003, NH_DIAG_NA, SS_DIAG_INV_ARG,
        "Received StsReq from SM action = %d, element = %d",
        action, sts->hdr.elmId.elmnt,0,0);
   
 
   /* This macro checks for NULL pointers being passed */
   if (NH_CHK_INPUT_PARAM(sts))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL sts\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "NhMiLnhStaReq(pst, elmnt(%d))\n", sta->hdr.elmId.elmnt));
 
  NH_DIAG_LVL0(0x060e0004, NH_DIAG_NA, SS_DIAG_INV_ARG,
        "Received StaReq from SM element = %d",
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL sta\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "Configuration Received. ElmId(%d)\n", 
                cfg->hdr.elmId.elmnt));
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  " nhValidateGenCfg: timer enabled but wait is 0 \n"));
      RETVALUE(LCM_REASON_INVALID_PAR_VAL);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "General Configuration Received\n"));
   memSize = NULLP;
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "General configuration Invalid Parameters.\n"));
   ...
   if (NULLP == nhCb.uSapCb)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for uSap.\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "Memory Allocation Failed for Lower Sap.\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Timer Registration Failed\n"));
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for KWU sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "KWU Sap(s) Initialized\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for CKW sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "CKW SAP(s) Initialized\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for CRG sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "CRG SAP(s) Initialized\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for PJU sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "PJU SAP(s) Initialized\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for CPJ sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "CPJ SAP(s) Initialized\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, 
                    "Memory Allocation Failed for CTF sap(s).\n"));
      RETVALUE(RFAILED);
   }
   else
   {
      ...
      NH_DBG_INFO((NH_PRNT_BUF, "CTF SAP(s) Initialized\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "Protocol Configuration Received\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "Upper SAP Configuration Received\n"));
   status->status = LCM_PRIM_OK;
   status->reason = LCM_REASON_NOT_APPL;
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhValidateLSapCfg: In nhValidateLSapCfg\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  " nhValidateLSapCfg: timer enabled but wait is 0\n"));
      RETVALUE(LCM_REASON_INVALID_PAR_VAL);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,
                "Lower SAP Configuration Received\n"));
   lSapCb = NULLP;
...
#endif /* RRC_PHY_CFG_SUPP */
      default:
         NH_DBG_ERROR((NH_PRNT_BUF, "Invalid SAP ElmId.\n"));
         status->status = LCM_PRIM_NOK;
         RETVALUE(RFAILED);
   } /* end of switch */
...
#else
               nhCb.init.dbgMask |= cntrl->u.dbg.dbgMsk;
               NH_DBG_INFO((NH_PRNT_BUF, "nhGenCntrl: DBG-Mask value :%02x", nhCb.init.dbgMask));               
#endif
#ifdef NH_LOG_TO_FILE
               nhCb.fileLogEnb = cntrl->u.dbg.fileLogEnb;
               cmMemcpy((U8*)nhCb.filePath,
                        (U8*)cntrl->u.dbg.filePath,
                        LNH_MAX_FILE_PATH);
               nhCb.cfgDbgLines = cntrl->u.dbg.nmbDbgLines;
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                "LSap Control Request Failed for KWU.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "LSap Control Request Failed for CKW.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "LSap Control Request Failed Failed for CRG.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "LSap Control Request Failed for PJU.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "LSap Control Request Failed for CPJ.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "LSap Control Request Failed for CTF.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "UE statistics validation Failed.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "CELL statistics validation Failed.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                   "LSAP Status Request validation Failed.\n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                    "USap Status Request validation Failed \n"));
      status->status = LCM_PRIM_NOK;
      status->reason = ret;
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   switch (type)
   {
      ...
      case TSSTA:
         ...
         break;
      default:
         NH_DBG_ERROR((NH_PRNT_BUF, "Invalid msgType.\n"));
         break;
   } /* end of switch */
   RETVALUE(ROK);
} /* nhLMMSndLmCfm */
...
#endif /* ANSI */
{
   ...
   while(NULLP != tmpNode)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"NULL logChCb\n"));
         RETVALUE(RFAILED);
      }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, 
             "Deleting CELL Control Block Failed "));
      }
