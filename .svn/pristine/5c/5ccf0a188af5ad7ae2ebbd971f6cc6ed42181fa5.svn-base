...
/**********************************************************************
     ...
     Sid:      ve_sm_init_merged.c@@/main/2 - Wed Dec  1 08:44:33 2010
...
PUBLIC S16 smBuildCzGenCntrl ARGS((Void));
PUBLIC S16 smBuildRgSchDbgCntrl ARGS((Void));
...
EXTERN S16 veSkipSteps ARGS (( U8 procId, U8 nxtEvnt));
#else
EXTERN S16 tfActvTsk ARGS((Pst *pst, Buffer *mBuf));
...
#ifdef DEBUGP
   VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "RLC/PDCP Tapa Task successfully\
...
#ifdef DEBUGP
   VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "LTE MAC Tapa Task successfully \
            registered and attached to %d\n",sysTskId));
#endif
...
#ifdef DEBUGP
   VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "LTE SCH Tapa Task successfully \
            registered and attached to %d\n",sysTskId));
#endif
...
#ifdef DEBUGP
   VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "CL Tapa Task successfully \
            registered and attached to %d\n",sysTskId));
#endif
...
/* Intializing log file information */
#if (VE_SM_LOG_TO_FILE && DEBUGP)
   smCb.dbgFp = NULLP;
   smCb.nmbDbgLines = 0;
   smCb.cfgDbgLines = 1000;
   cmMemset( (U8* )smCb.filePath, 0, VE_SM_MAX_FILE_PATH);
   ...
   {
#ifdef DEBUGP
      VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "RLC/PDCP Tapa Task \
   ...
   {
#ifdef DEBUGP
      VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "LTE MAC Tapa Task \
               initialization failed"));
#endif
      RETVALUE(RFAILED);
   } /* end of if statement */
   ...
   {
#ifdef DEBUGP
      VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "CL Tapa Task \
               initialization failed"));
#endif
      RETVALUE(RFAILED);
   } /* end of if statement */
...
#ifdef DEBUGP
   VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "Stack Manager Tapa Task successfully \
            registered and attached to %d\n",sm_taskId));
...
#endif
{
   ...
   if(SGetMsg(smCb.init.region, smCb.init.pool, &mBuf) != ROK)
   {
      RETVALUE(RFAILED);
   } /* end of if statement */
...
#endif
{
   ...
   strLen = (U16) strlen((const S8*)input);
...
#endif
{
   ...
   strLen = (U16)strlen((const S8*)tagVal);
   while (loopCnt < strLen)
   {
   ...
   while( feof(fp) == 0 )
   {
      ...
      switch(tagNum)
      {
         ...
         case VE_TAG_MOD_TYPE:
            {
               smCfgCb.modType = (U8) atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_DUP_MODE:
            {
               smCfgCb.duplexMode = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MAX_UE_SUPPORT:
            {
               smCfgCb.maxUeSupp = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MCC_0:
            {
               smCfgCb.mcc[0].pres = TRUE;
               smCfgCb.mcc[0].val = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MCC_1:
            {
               smCfgCb.mcc[1].pres = TRUE;
               smCfgCb.mcc[1].val = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MCC_2:
            {
               smCfgCb.mcc[2].pres = TRUE;
               smCfgCb.mcc[2].val = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MNC_0:
            {
               smCfgCb.mnc[0].pres = TRUE;
               smCfgCb.mnc[0].val = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MNC_1:
            {
               smCfgCb.mnc[1].pres = TRUE;
               smCfgCb.mnc[1].val = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_MNC_2:
            {
               if (-1 == atoi((const S8*)tagVal))
               {
                  ...
                  smCfgCb.mnc[2].val = (U8)atoi((const S8*)tagVal);
               }
               else
               {
         ...
         case VE_TAG_TA_CODE:
            {
               smCfgCb.trackAreaCode = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_FREQ_BAND_IND:
            {
               smCfgCb.freqBandInd   = (U8)atoi((const S8*)tagVal);
            }
            break;   
         ...
         case VE_TAG_HI_DBG:
            {
               smCfgCb.hiDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_SB_DBG:
            {
               smCfgCb.sbDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_SZ_DBG:
            {
               smCfgCb.szDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_EG_DBG:
            {
               smCfgCb.egDbg = (U8)atoi((const S8*)tagVal);               
            }
            break;   
         case VE_TAG_VE_DBG:
            {
               smCfgCb.veDbg  = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_NH_DBG:
            {
               smCfgCb.nhDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_KW_DBG:
            {
               smCfgCb.kwDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_RG_DBG:
            {
               smCfgCb.rgDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_YS_DBG:
            {
               smCfgCb.ysDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         case VE_TAG_SM_DBG:
            {
               smCfgCb.smDbg = (U8)atoi((const S8*)tagVal);
            }
            break;   
         ...
         case VE_TAG_MAX_RRM_RNTIS:
            { 
               smCfgCb.maxRrmRntis = (U8)atoi((const S8*)tagVal);
            }
            break;
         case VE_TAG_MAC_RNTI_STRT:
            {
               smCfgCb.macRntiStart = (U8)atoi((const S8*)tagVal);
            }
            break;
         case VE_TAG_MAX_MAC_RNTIS:
            {
               smCfgCb.maxMacRntis = (U8)atoi((const S8*)tagVal);
            }
            break;
         case VE_TAG_RRM_NO_OF_DED_PREMBL:
            {
               smCfgCb.rrmNumDedPrm = (U8)atoi((const S8*)tagVal);
            }
            break;
         case VE_TAG_RRM_DED_PREMBL_STRT:
            {
               smCfgCb.rrmDedPrmStart = (U8)atoi((const S8*)tagVal);
            }
            break;
         case VE_TAG_MAC_NO_OF_PREMBL:
            {
               smCfgCb.numOfMacPrm = (U8)atoi((const S8*)tagVal);
            }
            break;

         case VE_TAG_SCTP_SRVC_TYPE:
            {
               smCfgCb.sctpUdpServiceType = (U8)atoi((const S8*)tagVal);
            }
            break;   
       ...
       case VE_TAG_OP_MODE:
         {
            smCfgCb.opMode  = (U8)atoi((const S8*)tagVal);
         }
         break;
       case VE_TAG_COUNTER:
         {
...
*       Fun:  veSkipSteps
*
*       Desc: Skips the steps of state machine 
*
*       Ret:   ROK, RFAILED
*
*       File:  ve_sm_init.c
...
#ifdef CNE_UE_SIM_TEST
#ifdef ANSI
PUBLIC S16 veSkipSteps
(
U8 procId,
U8 nxtEvnt
)
#else
PUBLIC S16 veSkipSteps(procId,nxtEvnt)
U8 procId;
U8 nxtEvnt;
#endif
{
   ...
   VE_FILL_PST(tmpPst, procId, ENTSM, ENTVE, nxtEvnt, VE_POOL, SM_SELECTOR_LC);

   if(veSmSelfPstMsg(&tmpPst) != ROK)
   {
#ifdef DEBUGP
      VEDBGP(DBGMASK_SM, (smCb.init.prntBuf, "Last Operation\
               SUCCESSFULLY PERFORMED. Failure at Self Pst.\n"));
#endif
   }
   else
...
/***********************************************************
 *
 *     Func : veSmCreateDbgFile
 *
 *     Desc : Creates the file for logging SM debug logs
 ...
 *     Notes: None
 *
 *     File : ve_sm_init_merged.c
 *
 **********************************************************/
#ifdef ANSI
PUBLIC Void veSmCreateDbgFile
(
Void
)
#else
PUBLIC Void veSmCreateDbgFile(Void)
#endif
{
   Txt   fName[VE_SM_MAX_FILE_PATH]; /* to hold the Date & Time in File name */
   Txt   outfName[VE_SM_MAX_FILE_PATH * 2]; /* file name with prefix, Date & Time and Suffix */
   Txt   *prefName = "sm_"; /* file prefix */
   Txt   *sufFname = ".log"; /* file suffix */
   S16   strLen;
   S16   cnt;
   SystemId sysId;
   S16   retVal;
   Txt   sysIdInfo[VE_SM_MAX_FILE_PATH];
   time_t tt;
   struct tm tmStmp;

   /* Added trace macro */
   TRC2(veSmCreateDbgFile)

   strLen = 0; cnt = 0;

   cmMemset((U8*)fName, 0, VE_SM_MAX_FILE_PATH);
   cmMemset((U8*)outfName, 0, (VE_SM_MAX_FILE_PATH * 2));
   cmMemset((U8*)&sysId, 0, sizeof(SystemId));
   cmMemset((U8*)sysIdInfo, 0, VE_SM_MAX_FILE_PATH);
   ...
   veUtlStrcpy(outfName, smCb.filePath);

   /* copy the file prefix, into the file name */
   veUtlStrcat(outfName, prefName);
   ...
   if(smCb.dbgFp)
   {
      sprintf(sysIdInfo, "--------- ENB Stack Manager Main Version    : %d ---------\n", sysId.mVer);
      fwrite(sysIdInfo, cmStrlen((U8*)sysIdInfo), 1, smCb.dbgFp);
      sprintf(sysIdInfo, "--------- ENB Stack Manager Main Revision   : %d ---------\n", sysId.mRev);
      fwrite(sysIdInfo, cmStrlen((U8*)sysIdInfo), 1, smCb.dbgFp);
      sprintf(sysIdInfo, "--------- ENB Stack Manager Branch Version  : %d ---------\n", sysId.bVer);
      fwrite(sysIdInfo, cmStrlen((U8*)sysIdInfo), 1, smCb.dbgFp);
      sprintf(sysIdInfo, "--------- ENB Stack Manager Branch Revision : %d ---------\n", sysId.bRev);
      fwrite(sysIdInfo, cmStrlen((U8*)sysIdInfo), 1, smCb.dbgFp);
      if(sysId.ptNmb)
      {
         sprintf(sysIdInfo, "--------- ENB Stack Manager Part Number     : %s ---------\n\n", sysId.ptNmb);
         fwrite(sysIdInfo, cmStrlen((U8*)sysIdInfo), 1, smCb.dbgFp);
      }
   }
...
/********************************************************************30**
         ...
         End of file:     ve_sm_init_merged.c@@/main/2 - Wed Dec  1 08:44:33 2010
