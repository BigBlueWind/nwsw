...
#ifdef PWAV
   g_exeName = basename(argv[0]);
#endif
   /* mt028.201 */
#ifndef PWAV
   while ((ret = SGetOpt(argc, argv, "o:f:s:m:")) != EOF)
#else
   while ((ret = SGetOpt(argc, argv, "o:f:s:m:r:v")) != EOF)
#endif
   {
      switch (ret)
      {
...
/* mt001.301 : Additions */
...
#ifdef PWAV
         case 'v':
           if(0 == strncmp(g_exeName, "cp.exe", strlen("cp.exe")))
           {
              printf("** EXE[%s] svn version[%d] **\n","MODID_UEH",getSvnVersion(MODID_UEH));
           }
           if(0 == strncmp(g_exeName, "gtp.exe", strlen("gtp.exe")))
           {
             printf("** EXE[%s] svn version[%d] **\n","MODID_UDH",getSvnVersion(MODID_UDH));
           }
         ...
         case 'r':
            g_rootDir = msOptArg;
            printf("%s: started with argv[0]=[%s]\n", __FUNCTION__, g_exeName);

            if (NULL == g_rootDir)
            {
              printf("%s: rootDir NOT set. Exiting\n", __FUNCTION__);
              exit(EXIT_FAILURE);
            }
            else
            {
              printf("%s: Using rootDir=%s\n", __FUNCTION__, g_rootDir);
            }
            break;
#endif /*PWAV*/
         case 'f':
...
#ifdef PWAV_MEM2
   if (origSGetMsg(SS_DFLT_REGION, SS_DFLT_POOL, &mBuf) != ROK)
#else
   if (SGetMsg(SS_DFLT_REGION, SS_DFLT_POOL, &mBuf) != ROK)
#endif /* PWAV_MEM2 */
#else
