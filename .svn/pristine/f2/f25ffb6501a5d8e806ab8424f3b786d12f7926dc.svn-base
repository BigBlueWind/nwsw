...
# 2207 "../..//cm/ssi.h"
#define PID_STK(h) (h)
# 98 "../..//lterrc/nh_rtm.c" 2
# 1 "../..//cm/cm5.h" 1
# 99 "../..//lterrc/nh_rtm.c" 2
# 1 "../..//cm/cm_hash.h" 1
# 91 "../..//cm/cm_hash.h"
#define __CMHASHH__ 
...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 118 "../..//lterrc/nh_rtm.c" 2
# 1 "../..//cm/cm5.x" 1
# 119 "../..//lterrc/nh_rtm.c" 2
# 1 "../..//cm/cm_lib.x" 1
# 94 "../..//cm/cm_lib.x"
#define __CMLIBX__ 
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 219); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhSchedGenTmr \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   switch(tmrEvnt)
   {
...

      {
         if(action == 1)
         {
            ...
            if ((lSapCb) && (lSapCb->bndTmr.enb == 1))
            {
               wait = lSapCb->bndTmr.val;
            }
            else
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 248); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhSchedGenTmr: Cfg timer not enabled \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         ...
         {
            for (tmrNum = 0; tmrNum < 255; tmrNum++)
            {
            ...
            if (tmrNum >= 255)
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 264); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhSchedGenTmr: Bind timer not running \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      case 6:
      {
         if(action == 1)
         {
            ...
            if ((cellCb) && (nhCb.genCfg.cfgTmr.enb == 1))
            {
               wait = nhCb.genCfg.cfgTmr.val;
            }
            else
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 283); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhSchedGenTmr: Cfg timer not enabled \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         ...
         else
         {
            ...
            if (tmrNum >= 255)
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 301); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhSchedGenTmr: Cell Cfg timer not running \n"); SPrint((&nhCb.init)->prntBuf); } }; };
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 373); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhStartLLyrCfgTmr \n"); SPrint((&nhCb.init)->prntBuf); } }; };
       ...
       && (nhCb.genCfg.cfgTmr.enb == 1))
   {
      ...
      do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&tmrCb), (sizeof(NhLLyrCfgTmrCb))) == 0) { do { (Void)cmMemset((U8*)((*(&tmrCb))), 0, (Size)((sizeof(NhLLyrCfgTmrCb)))); } while(0); } else { (*(&tmrCb)) = 0L; } } while(0);
      if (0L == tmrCb)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 390); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "mem alloc for tmrCb failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 450); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhStopLLyrCfgTmr \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if ((cellAndUeCb) && (cellAndUeCb->cellCb) && (cellAndUeCb->ueCb))
   {
      for (tmrNum = 0; tmrNum < 64; tmrNum++)
      {
         ...
         if ((0L != tmrCb) && (tmrCb->ueCb == cellAndUeCb->ueCb) &&
            (tmrCb->transId == nxTransId))
         {
            break;
         }
      ...
      if (tmrNum >= 64)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 470); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhStopLLyrCfgTmr: Cfg timer not running \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      { if ((tmrCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_rtm.c", 488, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmrCb), (Size)(sizeof(NhLLyrCfgTmrCb))); (tmrCb) = 0L; } };
   }

   return(0);
}
# 513 "../..//lterrc/nh_rtm.c"
 Void nhProcGenTmrExpiry
(
Ptr ctrlBlk,
S16 tmrEvnt
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 530); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhProcGenTmrExpiry \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   switch(tmrEvnt)
   {
      ...
      {
         lSapCb = (NhLSapCb*)ctrlBlk;
         if (lSapCb == (NhLSapCb *)0L)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 549); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Null lSapCb in nhProcGenTmrExpiry\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            return;
         }
         if (lSapCb->state == 3)
         {
            if (lSapCb->bndRetryCnt < 3)
            {

               lSapCb->bndRetryCnt++;


               (Void) nhSchedGenTmr((Void*)lSapCb, (S16)tmrEvnt, 1);
            ...
            else
            {
               ...
               do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(NhUstaDgn))); } while(0);
               dgn.type = 3;
...
# 595 "../..//lterrc/nh_rtm.c"
         ...
         if (1 == nhUIMSndCellCfgCfm(cellCb->cellTransCb.cfmStatus,
                                        cellCb, 0x05))
         {
             { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 609); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhUIMSndCellCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };

         }



         if(0x02 != cellCb->cellTransCb.cfgAction)
         {
            if(0 != nhDBMDeleteCellCb(&cellCb))
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 618); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhDBMDeleteCellCb failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            }
         }
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 668); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhProcLLyrCfgTmrExpiry \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   tmrCb = (NhLLyrCfgTmrCb *)ctrlBlk;

   if (0L == tmrCb)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 674); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Timer control block NULL!\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if (transCb == 0L)
   {
     ...
     { if ((tmrCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_rtm.c", 682, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmrCb), (Size)(sizeof(NhLLyrCfgTmrCb))); (tmrCb) = 0L; } };
     do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(NhUstaDgn))); } while(0);
     dgn.type = 4;
     return(1);
   }
   ...
   { if ((tmrCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_rtm.c", 691, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmrCb), (Size)(sizeof(NhLLyrCfgTmrCb))); (tmrCb) = 0L; } };
   ...
   if(transCb->ordrdCfg.actionMask & 0x0040 &&
      transCb->ordrdCfg.ueInfo.crntiCfgType == 0x02)
   {
      if (0L == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 704); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULLP Cell Cb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
      if (0 != nhHashListDelete(&(cellCb->newUeHlCp), (U32)(ueCb)))
      {
          { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 709); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Could not delete from  list.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
          return(1);
      }
   }
   ...
   if ((transCb->cause == 17) &&
       (transCb->ordrdCfg.actionMask == 0x0002))
   {
   ...
   if (1 == nhUIMSndCfgCfm(nhIdentity, transId,
                             cfgMask, cfmStatus, 0x05))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_rtm.c", 749); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhUIMSndCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
