...
#ifndef PWAV
PRIVATE S16 cmUnpkKwMngmt ARGS ((KwMngmt *param, Event eventType, Buffer * mBuf));
#else
EXTERN S16 cmUnpkKwMngmt ARGS ((KwMngmt *param, Event eventType, Buffer * mBuf));
#endif
...
#else
PUBLIC S16 cmUnpkKwMngmt(param ,eventType, mBuf)
KwMngmt *param;
Event eventType;
Buffer *mBuf;
#endif
...
#endif
{
   ...
   {
     CMCHKPK(cmPkLteCellId, param->val.ipThMeas.cellId, mBuf);
     CMCHKPK(cmPkLteRnti,   param->val.ipThMeas.ueId, mBuf);
     for(idx = 0; idx < param->val.ipThMeas.numQci; idx++)
     {
        CMCHKPK(SPkU8, param->val.ipThMeas.qci[idx], mBuf);
     }
     CMCHKPK(SPkU8, param->val.ipThMeas.numQci, mBuf);
...
#endif
{
   ...
   if((param->measType & LKW_L2MEAS_DL_IP) ||
      (param->measType & LKW_L2MEAS_UL_IP))
   {
      CMCHKUNPK(SUnpkU8, &param->val.ipThMeas.numQci, mBuf);
      for(idx = param->val.ipThMeas.numQci; idx > 0; idx--)
      {
         CMCHKUNPK(SUnpkU8, &param->val.ipThMeas.qci[idx - 1], mBuf);
      }
      CMCHKUNPK(cmUnpkLteRnti, &param->val.ipThMeas.ueId, mBuf);
      CMCHKUNPK(cmUnpkLteCellId, &param->val.ipThMeas.cellId, mBuf);
      RETVALUE(ROK);
   }
#endif
...
#endif
{
   ...
   {
      if (cmPkKwL2MeasCfmInfo(&measCfmEvt->measCfm[idx], mBuf, measCfmEvt->measType) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)
         SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
               __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
               (ErrVal)ERRLKW, (ErrVal)0, "Packing failed");
   ...
   if (SPkU16(measCfmEvt->numCfm, mBuf) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
         __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
         (ErrVal)ERRLKW, (ErrVal)0, "Packing failed");
...
#ifdef LTE_RLC_R9
   CMCHKPK(cmPkLteCellId, measCfmEvt->cellId, mBuf);
   CMCHKPK(cmPkLteRnti,   measCfmEvt->ueId, mBuf);
#endif
...
#endif
{
   ...
   {
      CMCHKPK(SPkU32, param->val.ipThrput.ulIpThPut, mBuf);
      CMCHKPK(SPkU32, param->val.ipThrput.dlIpThPut, mBuf);
...
#endif
{
...
#ifdef LTE_RLC_R9
   CMCHKUNPK(cmUnpkLteRnti, &measCfmEvt.ueId, mBuf);
   CMCHKUNPK(cmUnpkLteCellId, &measCfmEvt.cellId, mBuf);
#endif
   if (SUnpkU16(&measCfmEvt.numCfm, mBuf) != ROK) {
      SPutMsg(mBuf);
#if (ERRCLASS & ERRCLS_ADD_RES)
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
         __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
   ...
   for(idx = measCfmEvt.numCfm; idx > 0; idx--)
   {
      if (cmUnpkKwL2MeasCfmInfo(&measCfmEvt.measCfm[idx - 1], mBuf, 
          measCfmEvt.measType) != ROK) {
         SPutMsg(mBuf);
#if (ERRCLASS & ERRCLS_ADD_RES)
         SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
               __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
      ...
      (measType & LKW_L2MEAS_UL_IP))
   {
      ...
      CMCHKUNPK(SUnpkU32, &param->val.ipThrput.dlIpThPut, mBuf);
      CMCHKUNPK(SUnpkU32, &param->val.ipThrput.ulIpThPut, mBuf);
      RETVALUE(ROK);
   }
#endif
   CMCHKUNPK(SUnpkU8,  &param->qci, mBuf);
