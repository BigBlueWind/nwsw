...
#define SS_MT_TMR 1
# 1 "../..//enbapp/lterrm/rm_ccm_cell.c"
# 97 "../..//enbapp/lterrm/rm_ccm_cell.c"
...
# 98 "../..//enbapp/lterrm/rm_ccm_cell.c" 2
# 122 "../..//enbapp/lterrm/rm_ccm_cell.c"
static S16 rmCcmProcCellAdd
(
RrmWrCellCfgReqInfo *cellCfgInfo,
RrmWrCause *cause
)
{
   RmCellCb *cellCbPtr;

   cellCbPtr = rmUtlGetCellCb(cellCfgInfo->cellId);
   if (cellCbPtr != 0L)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 134); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcCellAdd: Cell is already exists" "with cell ID (%u)", cellCfgInfo->cellId); SPrint((&(rmCb.init))->prntBuf); } }; };

      *cause = RRM_WR_CAUSE_CELL_ALREADY_EXISTS;
      return(1);
   }


   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&(cellCbPtr)), (sizeof(RmCellCb))) == 0) { { cmMemset((U8 *)((*(&(cellCbPtr)))), 0, ((sizeof(RmCellCb)))); }; } else { (*(&(cellCbPtr))) = 0L; } };
   if (cellCbPtr == 0L)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 144); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcCellAdd: Failed to allocate CellCb" "(%u)", cellCfgInfo->cellId); SPrint((&(rmCb.init))->prntBuf); } }; };

      *cause = RRM_WR_CAUSE_OUT_OF_RESOURCE;
      return(1);
   }

   cellCbPtr->cellId = cellCfgInfo->cellId;
   cellCbPtr->cellLstLnk.node = (U32)cellCbPtr;
   cellCbPtr->cellCfg = cellCfgInfo->cellCfg;



   if(0 != rmCacAddCellCfg(cellCbPtr, cause))
   {

      { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (cellCbPtr), (Size) (sizeof(RmCellCb))); (cellCbPtr) = 0L; };
      return(1);
   }


   if(0 != rmRbcAddCellCfg(cellCbPtr, cause))
   {
      rmCacDelCellCfg(cellCbPtr);
   ...
   if(0 != rmRcmAddCellCfg(cellCbPtr, cause))
   {
      rmCacDelCellCfg(cellCbPtr);
      rmRbcDelCellCfg(cellCbPtr);
   ...
   cmLListInit(&cellCbPtr->ueCbLst);


   cmLListAdd2Tail(&rmCb.enbCb.cellCbLst, &cellCbPtr->cellLstLnk);

   return(0);
}
# 215 "../..//enbapp/lterrm/rm_ccm_cell.c"
static S16 rmCcmProcCellMod
(
RrmWrCellCfgReqInfo *cellReCfgInfo,
RrmWrCause *cause
)
{
   U32 rcmMask = 0;
   ...
   cellCb = rmUtlGetCellCb(cellReCfgInfo->cellId);
   if(0L == cellCb)
   {
      *cause = RRM_WR_CAUSE_CELLID_UNKNOWN;
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 232); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcCellMod:" "Cell control bloack is not found for cellID:(%d)", (cellReCfgInfo->cellId)); SPrint((&(rmCb.init))->prntBuf); } }; };


      return(1);
   }

   if(cellReCfgInfo->reCfgType & 0x0002)
   {
      cellCb->cellCfg.moDataAcbTime = cellReCfgInfo->cellCfg.moDataAcbTime;
      cacMask |= 0x0002;
   }

   if(cellReCfgInfo->reCfgType & 0x0004)
   {
      cellCb->cellCfg.moSigAcbTime = cellReCfgInfo->cellCfg.moSigAcbTime;
      cacMask |= 0x0004;
   }
   ...
   if (rcmMask)
   {
      rmRcmModCellCfg(cellCb, rcmMask);
   }


   if (cacMask)
   {
      rmCacModCellCfg(cellCb, cacMask);
   }
   return(0);
}
# 285 "../..//enbapp/lterrm/rm_ccm_cell.c"
static S16 rmCcmProcCellDel
(
RrmWrCellCfgReqInfo *cellDelInfo,
RrmWrCause *cause
)
{
   RmCellCb *cellCbPtr = 0L;

   cellCbPtr = rmUtlGetCellCb(cellDelInfo->cellId);
   if(0L == cellCbPtr)
   {
      *cause = RRM_WR_CAUSE_CELLID_UNKNOWN;
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 299); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcCellDel: " "Failed to Find CellCb with cellId:(%u)", cellDelInfo->cellId); SPrint((&(rmCb.init))->prntBuf); } }; };
   ...
   rmRcmDelCellCfg(cellCbPtr);


   cmLListDelFrm(&(rmCb.enbCb.cellCbLst), &cellCbPtr->cellLstLnk);
...
# 344 "../..//enbapp/lterrm/rm_ccm_cell.c"
 S16 rmCcmProcCellCfg
(
 RrmWrCellCfgReqInfo *cellCfgInfo
)
{
   RrmWrCellCfgRespInfo *cellCfgRespPtr;
   S16 ret = 1;


   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&(cellCfgRespPtr)), (sizeof(RrmWrCellCfgRespInfo))) == 0) { { cmMemset((U8 *)((*(&(cellCfgRespPtr)))), 0, ((sizeof(RrmWrCellCfgRespInfo)))); }; } else { (*(&(cellCfgRespPtr))) = 0L; } };
   if (0L == cellCfgRespPtr)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 357); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " Failed to allocate memory for" "cell config response "); SPrint((&(rmCb.init))->prntBuf); } }; };

      return(ret);
   }

   cellCfgRespPtr->statusInfo.status = RRM_WR_CFG_SUCCESS;
   cellCfgRespPtr->statusInfo.cause = RRM_WR_CAUSE_NOT_APP;
   cellCfgRespPtr->cellId = cellCfgInfo->cellId;

   switch(cellCfgInfo->addModOrDel)
   {
      case RRM_WR_CFG_ADD:
      {
         ret = rmCcmProcCellAdd(cellCfgInfo, &(cellCfgRespPtr->statusInfo.cause));
         if(0 != ret)
         {
            cellCfgRespPtr->statusInfo.status = RRM_WR_CFG_FAILURE;
         }
         break;
      }
      case RRM_WR_CFG_MOD:
      {
         ret = rmCcmProcCellMod(cellCfgInfo, &(cellCfgRespPtr->statusInfo.cause));
         if(0 != ret)
         {
            cellCfgRespPtr->statusInfo.status = RRM_WR_CFG_FAILURE;
         }
      ...
      case RRM_WR_CFG_DEL:
      {
         ret = rmCcmProcCellDel(cellCfgInfo, &(cellCfgRespPtr->statusInfo.cause));
         if(0 != ret)
         {
            cellCfgRespPtr->statusInfo.status = RRM_WR_CFG_FAILURE;
         }
      ...
      default:
      {
         { { if ((&(rmCb.init))->dbgMask & (2)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 398); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcCellCfg: " "Received %u invalid action request received", cellCfgInfo->addModOrDel); SPrint((&(rmCb.init))->prntBuf); } }; };


         cellCfgRespPtr->statusInfo.status = RRM_WR_CFG_FAILURE;
         cellCfgRespPtr->statusInfo.cause = RRM_WR_CAUSE_OTHER;
         ret = 1;
      }
   }

   RmWrCellCfgResp(cellCfgRespPtr);
   return(ret);
}
# 425 "../..//enbapp/lterrm/rm_ccm_cell.c"
 S16 rmCcmSendCellRecfg(RrmWrCellRecfgIndInfo *cellRecfgInfo)
{

  if(0 != RmWrCellRecfgInd(cellRecfgInfo))
  {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_cell.c", 431); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmSendCellRecfg:" "Reconfiguration Indication is not processed successfully at FSM"); SPrint((&(rmCb.init))->prntBuf); } }; };
