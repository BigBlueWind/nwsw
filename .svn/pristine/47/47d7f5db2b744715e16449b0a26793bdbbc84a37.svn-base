...
#endif
{
   ...
   if( ((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr)) 
                 != ROK) || (czCbPtr == NULLP) )
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF,"failed, cannot derive czCb"));
      CZ_RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if( ((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr))
                 != ROK) || (czCbPtr == NULLP) )
   {
              CZ_DBG_ERROR((CZ_PRNT_BUF,    "failed, cannot derive czCb"));
      CZ_RETVALUE(RFAILED);
   }
   CZ_DBG_INFO((CZ_PRNT_BUF,
...
#endif
{
   ...
   /* MS_WORKAROUND for HO ccpu00115102*/
...
/*         CZ_DBG_ERROR((CZ_PRNT_BUF,"Reached maximum instances.\n"));*/
   ...
   /*CZ_SET_ZERO(&czCb,sizeof(CzCb));*/
   cmMemset((U8*)&czCb, 0, sizeof(CzCb));
   if(reason == SHUTDOWN)
   {
      czCb.shutdown = TRUE;
   }
