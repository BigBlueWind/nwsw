/*
 *  $Id: adaptation.c,v 1.8 2010/03/25 10:45:35 gadelzoi Exp $
 *
...
static int fdnum;
HANDLE            hEvent, handles[3];
static HANDLE  stdin_thread_handle;
WSAEVENT       stdinevent;
HANDLE SctpShutdownEvent;
...
#if defined (WIN32)
          if(adl_setReceiveBufferSize(sfd, 128*1024) < 0)
          {
              error_log(ERROR_FATAL, "setsockopt: SO_RCVBUF failed !");
              *myRwnd = -1;            
          }
#elif defined (LINUX)
...
#ifdef WIN32
    // Do not wait, just test
    if (WaitForSingleObject(SctpShutdownEvent, 0) == WAIT_OBJECT_0)
    {
        result = -1;
    }
#endif
...
int adl_eventLoop()
{
   ...
   n = MsgWaitForMultipleObjects(3, handles, FALSE, msecs, QS_KEY);
      if (n==1 && idata.len>0)
      {
         for (i=0; i< NUM_FDS; i++)
         {
...
int adl_init_adaptation_layer(int * myRwnd)
{
   ...
   stdinevent = WSACreateEvent();
    ...
    SctpShutdownEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
    if (SctpShutdownEvent == NULL)
    {
        error_log(ERROR_FATAL, "CreateEvent() of SctpShutdownEvent failed!");
        return -1;
    }

   handles[0]=hEvent;
   handles[1]=stdinevent;
   handles[2]=SctpShutdownEvent;
#endif
