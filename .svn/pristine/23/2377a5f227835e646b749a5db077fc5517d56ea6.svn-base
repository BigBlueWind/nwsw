/* $Id: distribution.c,v 1.6 2012/05/30 13:28:43 nedosis Exp $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
#include  <errno.h>
#include "socket.h"
#include  "inet_aton.h"         /* for inet_ntoa() under both SOLARIS/LINUX */
...
typedef struct _sctp_sync_t
{
	VUINT32     	lock;
	VUINT32     	irq[CPU_NUM];
}sctp_sync_t;

static sctp_sync_t sctp_sync_ctx;

void sctp_lock(sctp_sync_t *ctx)
{
	while(ctx->lock){MxDelay(10); /*uart_printf("\r\nLock!!!\r\n"); */};
...
void sctp_unlock (sctp_sync_t *ctx)
{	
	ctx->irq[MxGetCpuID ()] = ARM_INT_disable ();	
...
BOOL sctp_trylock(sctp_sync_t *ctx)
{
	ctx->irq[MxGetCpuID ()] = ARM_INT_disable ();
	if(!ctx->lock)
	{
...
UINT32 sctp_delayed_callback(LPVOID param)
{ 	
	if(sctp_sync_ctx.lock)
	{
		return MX_OK;
	}
...
#define SCTP_LOCK() sctp_lock(&sctp_sync_ctx);
#define SCTP_UNLOCK() sctp_unlock(&sctp_sync_ctx);
...
#define CHECK_LIBRARY           if(sctpLibraryInitialized == FALSE) {SCTP_UNLOCK(); return SCTP_LIBRARY_NOT_INITIALIZED;}
#define ZERO_CHECK_LIBRARY      if(sctpLibraryInitialized == FALSE) {SCTP_UNLOCK(); return 0;}

//#define LIBRARY_DEBUG

#ifdef LIBRARY_DEBUG
 #define ENTER_LIBRARY(fname)	uart_printf("\r\nEntering sctplib  (%s)\r\n", fname);// fflush(stdout);
 #define LEAVE_LIBRARY(fname)	uart_printf("\r\nLeaving  sctplib  (%s)\r\n", fname);// fflush(stdout);
 #define ENTER_CALLBACK(fname)	uart_printf("\r\nEntering callback (%s)\r\n", fname);// fflush(stdout);
 #define LEAVE_CALLBACK(fname)	uart_printf("\r\nLeaving  callback (%s)\r\n", fname);// fflush(stdout);
#else
                                                   ...
                                                   unsigned short toPort)
{
    ...
    case AF_INET6:
        ...
        _memcpy(&(tmpAssoc.destinationAddresses[0].sin6.sin6_addr.s6_addr),
...
int sctp_initLibrary(void)
{
    ...
    rc = MxCreateThread(sctp_delayed_callback, NULL, NULL, 0, &cb_tread);
    if(FAILED(rc))
    	error_logi(ERROR_MAJOR, "CreateThread error %d\r\n", rc);
    rc = MxCreateEvent(0, NULL, &cb_evnt);
    if(FAILED(rc))
        error_logi(ERROR_MAJOR, "CreateEvent error %d\r\n", rc);
    rc = MxCreateTimer(TIMER_MODE_PERIOD , 10, NULL, cb_evnt, &cb_timer);
    if(FAILED(rc))
       error_logi(ERROR_MAJOR, "MxCreateTimer error %d\r\n", rc);
    rc = MxScheduleThreadByEvent(cb_tread, cb_evnt, THREAD_EVENT_AUTO_RESET | THREAD_EVENT_SMP);
    if(FAILED(rc))
        error_logi(ERROR_MAJOR, "ScheduleThreadByEvent %d\r\n", rc);
    rc = MxStartTimer(cb_timer);
    if(FAILED(rc))
        error_logi(ERROR_MAJOR, "MxStartTimer %d\r\n", rc);
        
    //TimerDrvSetProc (10, 0, sctp_delayed_callback, NULL);
...
int sctp_freeLibrary(void)
{
	SCTP_LOCK();
#ifdef ON4GMX
    MxStopTimer(cb_timer);
    MxRemoveTimer(cb_timer);
    MxRemoveEvent(cb_evnt);
    MxRemoveThread(cb_tread);
...
#ifdef ON4GMX
    {    	    	
    	SCTP_controlData *cd = mdi_readSCTP_control();
    	int start_time = MxGetTime();  
    	unsigned int assID = assocID;
    	assocID = 0;
    	currentAssociation = retrieveAssociationForced(assID);
    	
    	//TODO Add timeout    	
    	
    	while ((cd->association_state != ESTABLISHED && cd->association_state != CLOSED ) )
    	{
    		MxDelay(50);
    		//uart_printf("\r\nwait for sctp connection\r\n");    		
    		currentAssociation = retrieveAssociationForced(assID);
    		cd = mdi_readSCTP_control();    		
    	};
    	
    	if(cd->association_state == ESTABLISHED)
    		assocID = assID;
    }
#endif
    sctpInstance = old_Instance;
               ...
               void* ulp_data)
{
    ...
    _memcpy(dAddress, destinationAddress, SCTP_MAX_IP_LEN);
...
int sctp_shutdown(unsigned int associationID)
{
    ...
    {    	    	
        ...
        while ((cd->association_state != CLOSED) && (MxGetTime() - start_time < SCTP_TIMEOUT) )
        {
        	MxDelay(50);
        	//uart_printf("\r\nwait for sctp connection\r\n");    		
        	currentAssociation = retrieveAssociationForced(associationID);
...
#ifdef ON4GMX    
    else if (result == NO_DATA_AVAILABLE) {
    	MxDelay(10);
    	goto recv_retr;
    }
#endif
...
int sctp_startTimer(unsigned int seconds , unsigned int microseconds,
...
int sctp_restartTimer(unsigned int timer_id, unsigned int seconds, unsigned int microseconds)
{
    int result;
...
int mdi_send_message(SCTP_message * message, unsigned int length, short destAddressIndex)
{
    ...
    if (currentAssociation == NULL) {
        ...
        if (lastFromAddress == NULL) {
            ...
            _memcpy(&dest_su, lastFromAddress, sizeof(union sockunion));
            dest_ptr = &dest_su;
...
void mdi_communicationUpNotif(unsigned short status)
{
    ...
    if (currentAssociation != NULL) {
        ...
        if(sctpInstance && sctpInstance->ULPcallbackFunctions.communicationUpNotif) {
...
int mdi_readLastFromAddress(union sockunion* fromAddress)
{
    if (lastFromAddress == NULL) {
        ...
        _memcpy(fromAddress, lastFromAddress, sizeof(union sockunion));
        return 0;
    }
    return 1;
}
...
int mdi_readLastDestAddress(union sockunion* destAddress)
{
    if (lastDestAddress == NULL) {
        ...
        _memcpy(destAddress, lastDestAddress, sizeof(union sockunion));
        return 0;
    }
    return 1;
}
...
void mdi_writeDestinationAddresses(union sockunion addresses[MAX_NUM_ADDRESSES], int noOfAddresses)
{
    ...
    if (currentAssociation == NULL) {
        ...
        _memcpy(currentAssociation->destinationAddresses, addresses,
                ...
                case AF_INET :
                    if ((addressTypes & SUPPORT_ADDRESS_TYPE_IPV4) != 0) {
                        if ( adl_filterInetAddress(&(myAddressList[tmp]), filterFlags) == TRUE) {
                            _memcpy(&(laddresses[count]), &(myAddressList[tmp]),sizeof(union sockunion));
                            count++;
                        }
                    }
                    break;
                ...
                case AF_INET :
                    if ((addressTypes & SUPPORT_ADDRESS_TYPE_IPV4) != 0) {
                        if ( adl_filterInetAddress(&(sctpInstance->localAddressList[tmp]), filterFlags) == TRUE) {
                            _memcpy(&(laddresses[count]), &(sctpInstance->localAddressList[tmp]),
...
int mdi_getSupportedAddressTypes(void)
{
    if (sctpInstance == NULL) return -1;
    else
                   ...
                   union sockunion *destinationAddressList)
{
    ...
    if (instance->has_IN6ADDR_ANY_set) {
        ...
        _memcpy(currentAssociation->localAddresses, myAddressList,
        ...
        for (ii = 0; ii <  myNumberOfAddresses; ii++) {
            if (sockunion_family(&(myAddressList[ii])) == AF_INET) {
                _memcpy(&(currentAssociation->localAddresses[currentAssociation->noOfLocalAddresses]),
        ...
        _memcpy(currentAssociation->localAddresses, instance->localAddressList,
    ...
    _memcpy(currentAssociation->destinationAddresses, destinationAddressList,
                    ...
                    gboolean assocSupportsPRSCTP, gboolean assocSupportsADDIP)
{
    ...
    event_logii(INTERNAL_EVENT_0, "ASSOCIATION RESTART: remote initial TSN:  %u, local initial TSN:  %u",
