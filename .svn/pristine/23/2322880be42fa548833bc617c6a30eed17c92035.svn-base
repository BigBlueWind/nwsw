...
static int uarm_fd = -1;
#define ICPU2_USE_KERNEL_DRV

#define __NR_cacheflush_all	0xF0006
...
void icpu2_cache_clean(struct icpu2_mm *mm, void *virt, unsigned int size)
{
	...
	if (uarm_fd < 0) {
		uarm_fd = open("/dev/u-arm", O_RDWR);
		if (uarm_fd < 0)
			abort();
	}

	params[0] = (unsigned int)virt;
	params[1] = params[0] + size;
	params[2] = icpu2_virt_to_phys(mm, virt);

	if (ioctl(uarm_fd, UARM_IOCTL_CACHE_CLEAN, &params[0])) {
		printf("errno: %d\n", errno);
		abort();
	}
}
...
void icpu2_cache_invalidate(struct icpu2_mm *mm, void *virt, unsigned int size)
{
	...
	if (ioctl(uarm_fd, UARM_IOCTL_CACHE_INVALIDATE, &params[0])) {
		printf("errno: %d\n", errno);
		abort();
	}
}
...
volatile U32 *ApbTimerReg2 = NULL;

RESULTCODE Icpu2Init(void)
{

    icpu2 = icpu2_init(ICPU2_DEVNAME, 0);
    if (icpu2 == NULL)
    {
        return FAILURE;
    }
    else
    {
	    ...
	    if (map != MAP_FAILED)
           {   
                ApbTimerReg2 = (volatile U32*)((unsigned long)map + 4);
                DiagPrint(DIAG_LEVEL1, "ICPU", "DirectMode: ApbTimerReg2 %X\n", ApbTimerReg2);
           }
        }
...
#if 0
	U32 max_priority;
    struct sched_param resched;

    max_priority = sched_get_priority_max(SCHED_RR);
    printf("ICPU2: max priority for SCHED_RR is %d\n", max_priority);

    resched.sched_priority = 90*max_priority/100; // 60% from max priority
    printf("ICPU2: changing priority for %s is %d\n", __FUNCTION__, resched.sched_priority);

    if( sched_setscheduler( 0, SCHED_RR, &resched ) == -1 ) {
        printf("ICPU2: SCHED_RR doesn't support\n");
    }
#endif

    while (1)
    {
        rc = icpu2_wait(icpu2, 0); /* was icpu2_wait(icpu2, ICPU2_WAIT_TYPE) */
        if (rc)
        {
            pBlock = NULL;
...
//	struct icpu2 *icpu2 = NULL;
	...
	icpu2->context_mm = icpu2_mmap(context_phys, 1024*1024, 0);
    if (!icpu2->context_mm)
        goto err2;
    ...
    /* map tx/rx storages */
	printf("Mapping TX storage as %s memory\n", cache&1 ? "cacheable" : "non-cacheable");
	icpu2->tx_storage_mm = icpu2_mmap(icpu2->context->tx_storage, icpu2->txq_free.queue->num * icpu2->tx_block_size, cache&1);
    if (!icpu2->tx_storage_mm)
        goto err4;

	printf("Mapping RX storage as %s memory\n", cache&2 ? "cacheable" : "non-cacheable");
	icpu2->rx_storage_mm = icpu2_mmap(icpu2->context->rx_storage, icpu2->rxq_free.queue->num * icpu2->rx_block_size, cache&2);
    if (!icpu2->rx_storage_mm)
        goto err5;

    /* map INTC regs, used to notify opposite CPU */
	icpu2->intc_mm = icpu2_mmap(0xFE430000, 4096, 0);
    if (!icpu2->intc_mm)
        goto err6;
...
void *icpu2_mmap_helper(unsigned int phys_start, unsigned int length, int flag)
{
    int fd;
    void *map = NULL;

	if (flag)
		fd = open("/dev/u-arm", O_RDWR);
	else
...
struct icpu2_mm *icpu2_mmap(unsigned int phys_addr, unsigned int length, int flag)
{
    ...
    if (mm) {
        mm->phys = phys_addr;
        mm->length = length;
		mm->virt = icpu2_mmap_helper(mm->phys, mm->length, flag);
        if (!mm->virt) {
            free(mm);
            mm = NULL;
        }
...
static void *__icpu2_rx(struct icpu2 *icpu2, struct icpu2_local_queue *q)
{
	...
	virt = icpu2->rx_storage_mm->virt + block_index*icpu2->rx_block_size;
