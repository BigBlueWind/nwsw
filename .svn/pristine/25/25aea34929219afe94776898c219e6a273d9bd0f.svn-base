...
# 2207 "../..//cm/ssi.h"
#define PID_STK(h) (h)
# 102 "../..//lterrc/nh_dbm.c" 2
# 1 "../..//cm/cm5.h" 1
# 103 "../..//lterrc/nh_dbm.c" 2
# 1 "../..//cm/cm_tkns.h" 1
# 91 "../..//cm/cm_tkns.h"
#define __CMTKNSH 
# 115 "../..//cm/cm_tkns.h"
...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 114 "../..//lterrc/nh_dbm.c" 2
# 1 "../..//cm/cm5.x" 1
# 115 "../..//lterrc/nh_dbm.c" 2
# 1 "../..//cm/cm_lib.x" 1
# 94 "../..//cm/cm_lib.x"
#define __CMLIBX__ 
...
# 123 "../..//lterrc/nh_dbm.c" 2
# 132 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitRbAddCfg ( NhOrdrdCfgInfo *ordCfgInfo, NhCrntCfgInfo *crntCfgInfo);

static S16 nhDBMCommitRbModCfg ( NhOrdrdCfgInfo *ordCfgInfo, NhCrntCfgInfo *crntCfgInfo);

static S16 nhDBMCommitRbRlsCfg ( NhOrdrdCfgInfo *ordCfgInfo, NhCrntCfgInfo *crntCfgInfo);

static S16 nhDBMCommitUeCfg ( NhActvUeCb *ueCb, TransCb *transCtrlBlk);


static S16 nhDBMFillLogchPdcpInfo ( NhRbToLchMap *dstCfgInfo, NhuRbCfgInfo *srcRbCfgInfo, U8 action);



static S16 nhDBMUpdRbToLchOrPdcp ( NhCrntCfgInfo *crntCfg, NhuRbCfgInfo *rbCfgInfo, U8 action);



static S16 nhInitUeCb ( NhActvUeCb *ueCb );

static S16 nhDBMCommitCellOrdrdCfgAdd ( NhCellCb *cellCb);
# 174 "../..//lterrc/nh_dbm.c"
 S16 nhDBMAddTransCb
(
NhActvUeCb *ueCb,
...

{
   ...
   S16 ret;
   CmLList *tmp = 0L;
   TransCb *locTransCb=0L;

   ;

   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 194); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Addition of New Transaction Info Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
# 204 "../..//lterrc/nh_dbm.c"
   ...
   for (i=0; i < nhCb.genCfg.minTransLstLen; i++)
   {
      ...
      if (0 == locTransCb->status)
      {
         locTransCb->status = 1;
         locTransCb->msgTransKey = msgTransKey;

         ret = nhHashListInsert(&ueCb->msgTransHlCp, (U32)locTransCb,
   ...
   do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&tmp), (sizeof(CmLList))) == 0) { do { (Void)cmMemset((U8*)((*(&tmp))), 0, (Size)((sizeof(CmLList)))); } while(0); } else { (*(&tmp)) = 0L; } } while(0);
   if (0L == tmp)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 230); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(4);
   }

   do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&locTransCb), (sizeof(TransCb))) == 0) { do { (Void)cmMemset((U8*)((*(&locTransCb))), 0, (Size)((sizeof(TransCb)))); } while(0); } else { (*(&locTransCb)) = 0L; } } while(0);
   if (0L == locTransCb)
   {
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 237, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 238); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(4);
   }
   ...
   locTransCb->transId = 0xffffffff;
   { if((locTransCb)->transId >= nhCb.genCfg.minTransLstLen) { SGetSysTime((&(locTransCb->transId))); } }

   locTransCb->msgTransKey = msgTransKey;

   tmp->node = (U32)(locTransCb);
   ...
   if (0 != ret)
   {
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 262, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      { if ((locTransCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 263, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(locTransCb), (Size)(sizeof(TransCb))); (locTransCb) = 0L; } };
      return(ret);
   }
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 306); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Deleting TransCb Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
# 318 "../..//lterrc/nh_dbm.c"
   if ((*transCtrlBlk)->transId < nhCb.genCfg.minTransLstLen)
   {
      ...
      nhHashListDelete(&ueCb->msgTransHlCp, (U32) *transCtrlBlk);


      if ((*transCtrlBlk)->ordrdCfg.actionMask & 0x0001)
      {
         { if (((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 331, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList), (Size)(((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.numOfRbCfgInfo *sizeof(NhuRbCfgInfo)))); ((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList) = 0L; } };
      ...
      if ((*transCtrlBlk)->ordrdCfg.actionMask & 0x0002)
      {
         ...
         { if (((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 341, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList), (Size)(((*transCtrlBlk)->ordrdCfg.rbToModCfgList.numOfRbCfgInfo *sizeof(NhuRbCfgInfo)))); ((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList) = 0L; } };
      ...
      {
         { if (((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 348, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel), (Size)(((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.numOfRbCfgInfo *sizeof(NhuRbId)))); ((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel) = 0L; } };
      ...
      {
         ((((tmp) = ((&(*transCtrlBlk)->ordrdCfg.rbToRestbCfgList.rbCfgInfoList)->crnt = (&(*transCtrlBlk)->ordrdCfg.rbToRestbCfgList.rbCfgInfoList)->first)) != 0L) ? (tmp)->node : 0L);

         while(0L != tmp)
         {
            cmLListDelFrm(&(*transCtrlBlk)->ordrdCfg.rbToRestbCfgList.rbCfgInfoList, tmp);
            { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 358, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(NhuRbCfgInfo))); (tmp->node) = 0L; } };
            { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 359, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
            ((((tmp) = ((&(*transCtrlBlk)->ordrdCfg.rbToRestbCfgList.rbCfgInfoList)->crnt = (&(*transCtrlBlk)->ordrdCfg.rbToRestbCfgList.rbCfgInfoList)->first)) != 0L) ? (tmp)->node : 0L);
         }
      }
      ...
      {
         do{ if (((*transCtrlBlk)->storedMsg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 367, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg(((*transCtrlBlk)->storedMsg)); ((*transCtrlBlk)->storedMsg) = 0L; } }while(0);
      }

      if ((*transCtrlBlk)->lowerLyrParam)
      {
         { if (((*transCtrlBlk)->lowerLyrParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 373, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->lowerLyrParam), (Size)(sizeof(NhDatReqParam))); ((*transCtrlBlk)->lowerLyrParam) = 0L; } };

      }

      do { (Void)cmMemset((U8*)((U8 *)*transCtrlBlk), 0, (Size)(sizeof(TransCb))); } while(0);


      (*transCtrlBlk)->status = 0;
   ...
   else
   {
      ...
      while(0L != tmp)
      {
         ...
         if ((*transCtrlBlk)->transId == node->transId)
         {
            ...
            nhHashListDelete(&ueCb->msgTransHlCp, (U32)node);
            ...
            if ((*transCtrlBlk)->ordrdCfg.actionMask & 0x0001)
            {
               { if (((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 414, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList), (Size)(((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.numOfRbCfgInfo *sizeof(NhuRbCfgInfo)))); ((*transCtrlBlk)->ordrdCfg.rbToAddCfgList.rbCfgInfoList) = 0L; } };
            ...
            if ((*transCtrlBlk)->ordrdCfg.actionMask & 0x0002)
            {
               ...
               { if (((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 425, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList), (Size)(((*transCtrlBlk)->ordrdCfg.rbToModCfgList.numOfRbCfgInfo *sizeof(NhuRbCfgInfo)))); ((*transCtrlBlk)->ordrdCfg.rbToModCfgList.rbCfgInfoList) = 0L; } };
            ...
            if ((*transCtrlBlk)->ordrdCfg.actionMask & 0x0004)
            {
               { if (((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 432, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel), (Size)(((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.numOfRbCfgInfo *sizeof(NhuRbId)))); ((*transCtrlBlk)->ordrdCfg.rbToRlsCfgList.rbsToRel) = 0L; } };
            ...
            if ((*transCtrlBlk)->storedMsg)
            {
               do{ if (((*transCtrlBlk)->storedMsg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 438, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg(((*transCtrlBlk)->storedMsg)); ((*transCtrlBlk)->storedMsg) = 0L; } }while(0);
            }

            if ((*transCtrlBlk)->lowerLyrParam)
            {
               { if (((*transCtrlBlk)->lowerLyrParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 444, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*transCtrlBlk)->lowerLyrParam), (Size)(sizeof(NhDatReqParam))); ((*transCtrlBlk)->lowerLyrParam) = 0L; } };

            }


            { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 448, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(TransCb))); (tmp->node) = 0L; } };
            { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 449, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
            (*transCtrlBlk) = 0L;
            break;
         }
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 502); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Adding MUI Control Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

   ((((tmp) = ((&(ueCb->muiInfoLst))->crnt = (&(ueCb->muiInfoLst))->first)) != 0L) ? (tmp)->node : 0L);
   ...
   for (i=0; i < nhCb.genCfg.minMuiLstLen; i++)
   {
      ...
      if ((muiInfo != 0L ) && (0 == muiInfo->status))
      {
         muiInfo->status = 1;
      ...
      ((((tmp) = ((&(ueCb->muiInfoLst))->crnt = ((&(ueCb->muiInfoLst))->crnt ? (&(ueCb->muiInfoLst))->crnt->next : (&(ueCb->muiInfoLst))->first)))!= 0L) ? (tmp)->node : 0L);
   }
   ...
   if (0L == tmp)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 526); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failed!!.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(4);
   }

   do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&muiInfo), (sizeof(MuiInfo))) == 0) { do { (Void)cmMemset((U8*)((*(&muiInfo))), 0, (Size)((sizeof(MuiInfo)))); } while(0); } else { (*(&muiInfo)) = 0L; } } while(0);
   if (0L == muiInfo)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 533); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failed!!.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 534, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      return(4);
   }

   muiInfo->status = 1;
   muiInfo->nxTransId = nxTransId;
   muiInfo->mui = 0xffffffff;

   { if ((muiInfo)->mui >= nhCb.genCfg.minMuiLstLen) { SGetSysTime((&(muiInfo->mui))); } };

   tmp->node = (U32)(muiInfo);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 588); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Deletion of Mui Info Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
# 601 "../..//lterrc/nh_dbm.c"
   ...
   else
   {
      ...
      while(0L != tmp)
      {
         ...
         if ((*muiCb)->mui == node->mui)
         {
            ...
            { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 627, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(MuiInfo))); (tmp->node) = 0L; } };
            { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 628, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
            (*muiCb) = 0L;
            break;
         }
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 679); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Fetch the Mui control Block: mui (%ld) \n", mui); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (mui <= nhCb.genCfg.minMuiLstLen)
   {
      ...
      if (0L != tmp)
      {
         ...
         if ((0L != muiCb) && (muiCb->status != 1))
         {

            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 701); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Mui ID.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
            muiCb = 0L;
         }
      }
      else
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 768); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Creating Cell control block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0L == tmp)
   {
      ...
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 778); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for CmLList in Cell.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

      return(1);
   }


   do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(cellCb), (sizeof(NhCellCb))) == 0) { do { (Void)cmMemset((U8*)((*(cellCb))), 0, (Size)((sizeof(NhCellCb)))); } while(0); } else { (*(cellCb)) = 0L; } } while(0);
   if (0L == *cellCb)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 787); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for cellCb in Cell.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 788, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      return(1);
   }

   (*cellCb)->cellId = cellId;

   ret = nhHashListInit(&(*cellCb)->actvUeHlCp, 256,
                        0, 0,
                        0, nhCb.init.region, nhCb.init.pool);

   if(0 != ret)
   {
      { if ((*cellCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 800, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(*cellCb), (Size)(sizeof(NhCellCb))); (*cellCb) = 0L; } };
      return(ret);
   }
   ...
   ret = nhHashListInit(&(*cellCb)->newUeHlCp, 32,
                        sizeof(CmHashListEnt), 0,
                        0, nhCb.init.region, nhCb.init.pool);

   if(0 != ret)
   {
      { if ((*cellCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 813, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(*cellCb), (Size)(sizeof(NhCellCb))); (*cellCb) = 0L; } };
      return(ret);
   }

   cmInitTimers((*cellCb)->cfgTimers, (U8)64);
...

{
   CmLList *tmp = 0L;
   CmLList *cellTmp = 0L;
   CmLListCp *cellCbList = 0L;
   CmLListCp *rbCfgInfoList = 0L;
   NhActvUeCb *ueCb = 0L;
   NhLLyrCfgTmrCb *tmrCb = 0L;
   U8 tmrNum;
   CmTmrArg arg;
   S16 ret;
   U8 i;
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 865); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Deleting Cell control Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (nhCb.eNodeBCb.nhCellCbList.numOfCellCb)
   {
      ...
      i = 1;
      ((((cellTmp) = ((cellCbList)->crnt = (cellCbList)->first)) != 0L) ? (cellTmp)->node : 0L);
      while (0L != cellTmp)
      {
         ...
         i++;
         ((((cellTmp) = ((cellCbList)->crnt = ((cellCbList)->crnt ? (cellCbList)->crnt->next : (cellCbList)->first)))!= 0L) ? (cellTmp)->node : 0L);
      }
   ...
   if ((*cellCb)->crntCellCfgInfo.rbCfgList.numOfRbCfgInfo)
   {
      ...
      while (0L != tmp)
      {
         ...
         { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 902, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(NhuRbCfgInfo))); (tmp->node) = 0L; } };

         { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 903, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
         ((((tmp) = ((rbCfgInfoList)->crnt = (rbCfgInfoList)->first)) != 0L) ? (tmp)->node : 0L);
      }
   }


   if(0L != (*cellCb)->cellTransCb.cellOrdrdCfg)
   {
      if((*cellCb)->cellTransCb.cellOrdrdCfg->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo)
      {
         { if (((*cellCb)->cellTransCb.cellOrdrdCfg->cellCfgInfo.rbToAddCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 916, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*cellCb)->cellTransCb.cellOrdrdCfg->cellCfgInfo.rbToAddCfgList.rbCfgInfoList), (Size)((*cellCb)->cellTransCb.cellOrdrdCfg->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo * sizeof(NhuRbCfgInfo))); ((*cellCb)->cellTransCb.cellOrdrdCfg->cellCfgInfo.rbToAddCfgList.rbCfgInfoList) = 0L; } };



      }
      { if (((*cellCb)->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 919, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)((*cellCb)->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhuCellCfgInfo))); ((*cellCb)->cellTransCb.cellOrdrdCfg) = 0L; } };

   }


   for (tmrNum = 0; tmrNum < 64; tmrNum++)
   {
      if ((*cellCb)->cfgTimers[tmrNum].tmrEvnt != -1)
      {
         ...
         { if ((tmrCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 940, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmrCb), (Size)(sizeof(NhLLyrCfgTmrCb))); (tmrCb) = 0L; } };
      }
   }
   ...
   for(;;)
   {
      ret = nhHashListGetNext(&(*cellCb)->newUeHlCp, 0L,
                  (U32 *) &ueCb);
      if ((ret != 0) || (0L == (NhActvUeCb *)ueCb))
      {
         break;
      }
      else
      {
         nhHashListDelete(&(*cellCb)->newUeHlCp, (U32)ueCb);
      }
   }

   for(;;)
   {
      ret = nhHashListGetNext(&(*cellCb)->actvUeHlCp, 0L,
                  (U32 *) &ueCb);
      if ((ret != 0) || (0L == (NhActvUeCb *)ueCb))
      {
         break;
      ...
      {
         nhHashListDelete(&(*cellCb)->actvUeHlCp, (U32)ueCb);
   ...
   { if ((*cellCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 987, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(*cellCb), (Size)(sizeof(NhCellCb))); (*cellCb) = 0L; } };
   { if ((cellTmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 988, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellTmp), (Size)(sizeof(CmLList))); (cellTmp) = 0L; } };
   nhCb.eNodeBCb.nhCellCbList.numOfCellCb--;

   return(0);
}
# 1011 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitCellOrdrdCfgAdd
(
NhCellCb *cellCb
)
...
{
   NhCellOrdrdCfg *ordrdCfgInfo = 0L;
   NhCellCfgInfo *crntCfgInfo = 0L;
   CmLList *tmp = 0L;
   ...
   if (ordrdCfgInfo->cellCfgInfo.cellCfgBitMsk & 0x01)
   {
      ...
      for(i=0; i<ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo; i++)
      {
         ...
         if (0L == tmp)
         {
            ...
            { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1052); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for CmLList in Cell.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            { if ((cellCb->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1054, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellCb->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhuCellCfgInfo))); (cellCb->cellTransCb.cellOrdrdCfg) = 0L; } };

            return(4);

         }

         do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&rbCfgInfo), (sizeof(NhuRbCfgInfo))) == 0) { do { (Void)cmMemset((U8*)((*(&rbCfgInfo))), 0, (Size)((sizeof(NhuRbCfgInfo)))); } while(0); } else { (*(&rbCfgInfo)) = 0L; } } while(0);
         if (0L == rbCfgInfo)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1063); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for RbCfg in Cell.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1064, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
            { if ((cellCb->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1066, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellCb->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhuCellCfgInfo))); (cellCb->cellTransCb.cellOrdrdCfg) = 0L; } };

            return(4);
         }
         do { (Void)cmMemcpy((U8*)(rbCfgInfo),(const U8*)(&(ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.rbCfgInfoList[i])), (Size)(sizeof(NhuRbCfgInfo)));} while(0);



         tmp->node = (U32)(rbCfgInfo);
   ...
   if (ordrdCfgInfo->cellCfgInfo.cellCfgBitMsk & 0x08)
   {
      do { (Void)cmMemcpy((U8*)(&(crntCfgInfo->macCfg.rachCfg)),(const U8*)(&(ordrdCfgInfo->cellCfgInfo.macCmnCfg.rachCfg)), (Size)(sizeof(NhuRachCfg)));} while(0);


   }
# 1096 "../..//lterrc/nh_dbm.c"
   if(ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo)
   {
      { if ((ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1100, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.rbCfgInfoList), (Size)((sizeof(NhuRbCfgInfo)*(ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo)))); (ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.rbCfgInfoList) = 0L; } };


      ordrdCfgInfo->cellCfgInfo.rbToAddCfgList.numOfRbCfgInfo = 0;
   }

   { if ((cellCb->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1105, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellCb->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhCellOrdrdCfg))); (cellCb->cellTransCb.cellOrdrdCfg) = 0L; } };

   return(0);
}
# 1127 "../..//lterrc/nh_dbm.c"
 S16 nhDBMCommitCellOrdrdCfg
(
NhCellCb *cellCb
)
...
# 1223 "../..//lterrc/nh_dbm.c"
            { if ((cellCb->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1224, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellCb->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhuCellCfgInfo))); (cellCb->cellTransCb.cellOrdrdCfg) = 0L; } };

         }
         break;
      case 0x04:
         {
            { if ((cellCb->cellTransCb.cellOrdrdCfg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1230, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(cellCb->cellTransCb.cellOrdrdCfg), (Size)(sizeof(NhuCellCfgInfo))); (cellCb->cellTransCb.cellOrdrdCfg) = 0L; } };
      ...
      default:
         {
            ...
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1236); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Unexpected config action.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(1);
         }
   }
...
# 1264 "../..//lterrc/nh_dbm.c"
static S16 nhDBMFillLogchPdcpInfo
(
...

{
   ...
   switch (action)
   {
      case 0x01:
      {
         ...
         do { (Void)cmMemcpy((U8*)(&(dstCfgInfo->lchId)),(const U8*)(&(srcRbCfgInfo->lcId)), (Size)(sizeof(NhuLogChId)));} while(0);


         if(srcRbCfgInfo->rbCfgBitMsk & 0x01)
         {
            if(srcRbCfgInfo->ulRbCfg.ulRbBitMsk & 0x01)
            {
               dstCfgInfo->logChCfg.lChCfgBitMsk |= 0x01;
               do { (Void)cmMemcpy((U8*)(&(dstCfgInfo->logChCfg.logChCfg)),(const U8*)(&(srcRbCfgInfo->ulRbCfg.ulLogChMap.lcCfg)), (Size)(sizeof(NhuLogChCfg)));} while(0);


               dstCfgInfo->logChCfg.lChCfgBitMsk |= 0x02;
         ...
         if(srcRbCfgInfo->rbCfgBitMsk & 0x02)
         {
            if(srcRbCfgInfo->dlRbCfg.dlRbBitMsk & 0x01)
            {
               dstCfgInfo->logChCfg.lChCfgBitMsk |= 0x04;
               dstCfgInfo->logChCfg.dlTrchType =
                  srcRbCfgInfo->dlRbCfg.dlLogChMap.trChType;

               if(dstCfgInfo->dir == 0x02)
                  dstCfgInfo->dir = (0x01 | 0x02);
               else
                  dstCfgInfo->dir = 0x01;

            }
         }
         if (srcRbCfgInfo->rbCfgBitMsk & 0x04)
            dstCfgInfo->isPdcpConfigured = 1;
      }
      break;
      case 0x02:
      {
...
# 1363 "../..//lterrc/nh_dbm.c"
static S16 nhDBMUpdRbToLchOrPdcp
(
NhCrntCfgInfo *crntCfg,
...

{
   ...
   switch(action)
   {
      case 0x01:
      {
         ...
         if(0L == tmpNode)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1392); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for adding radio bearer.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            return(4);
         }
         do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&tmpCfgInfo), (sizeof(NhRbToLchMap))) == 0) { do { (Void)cmMemset((U8*)((*(&tmpCfgInfo))), 0, (Size)((sizeof(NhRbToLchMap)))); } while(0); } else { (*(&tmpCfgInfo)) = 0L; } } while(0);
         if(0L == tmpCfgInfo)
         {
             { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1399); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for adding Logical ChannelCfg.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

             { if ((tmpNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1400, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmpNode), (Size)(sizeof(CmLList))); (tmpNode) = 0L; } };
             return(4);
         }

         nhDBMFillLogchPdcpInfo(tmpCfgInfo,rbCfgInfo, action);
         tmpNode->node = (U32) tmpCfgInfo;
      ...
      case 0x02:
      {
         ...
         ((((tmpNode) = ((&crntCfg->logChCbLst)->crnt = (&crntCfg->logChCbLst)->first)) != 0L) ? (tmpNode)->node : 0L);
         ...
         while(0L != tmpNode)
         {
            ...
            ((((tmpNode) = ((&crntCfg->rbCfgList.rbCfgInfoList)->crnt = ((&crntCfg->rbCfgList.rbCfgInfoList)->crnt ? (&crntCfg->rbCfgList.rbCfgInfoList)->crnt->next : (&crntCfg->rbCfgList.rbCfgInfoList)->first)))!= 0L) ? (tmpNode)->node : 0L);
         }
      ...
      case 0x04:
      {
         ...
         while(0L != tmpNode)
         {
                                             ...
                                             == rbCfgInfo->rbId.rbTyp)
            {
               ...
               { if ((tmpNode->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1445, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmpNode->node), (Size)(sizeof(NhRbToLchMap))); (tmpNode->node) = 0L; } };

               { if ((tmpNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1446, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.pool), (nhCb.init.pool), (Data *)(tmpNode), (Size)(sizeof(CmLList))); (tmpNode) = 0L; } };
               break;
            }
            ((((tmpNode) = ((&crntCfg->rbCfgList.rbCfgInfoList)->crnt = ((&crntCfg->rbCfgList.rbCfgInfoList)->crnt ? (&crntCfg->rbCfgList.rbCfgInfoList)->crnt->next : (&crntCfg->rbCfgList.rbCfgInfoList)->first)))!= 0L) ? (tmpNode)->node : 0L);
         }
...
# 1477 "../..//lterrc/nh_dbm.c"
 S16 nhDBMUpdModifiedRb
(
...

{
   ...
   if(srcRbCfgInfo->rbCfgBitMsk & 0x04)
   {
      ...
      return(nhDBMUpdModifiedRbPdcpInfo(dstRbCfgInfo, srcRbCfgInfo));
   }

   return(0);

}
# 1525 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitUeCfg
(
NhActvUeCb *ueCb,
TransCb *transCtrlBlk
)
...
{
   NhCrntCfgInfo *crntCfgInfo = 0L;
   NhOrdrdCfgInfo *ordCfgInfo = 0L;
   S16 retVal = 0;
   NhCellCb *cellCb = 0L;
   ...
   if(ordCfgInfo->actionMask & 0x0008)
   {
      crntCfgInfo->bitMskPres |= 0x0004;
      crntCfgInfo->trChCfg.trChBitMsk = ordCfgInfo->trChCfg.trChBitMsk;

      if(ordCfgInfo->trChCfg.trChBitMsk & 0x01)
      {
         do { (Void)cmMemcpy((U8*)(&crntCfgInfo->trChCfg.ulHarqCfg),(const U8*)(&ordCfgInfo->trChCfg.ulHarqCfg), (Size)(sizeof(NhuUlHarqCfg)));} while(0);


      }
   }

   if(ordCfgInfo->actionMask & 0x0020)
   {
      crntCfgInfo->bitMskPres |= 0x0008;
      crntCfgInfo->secCfg.secCfgBitMsk = ordCfgInfo->secCfg.secCfgBitMsk;

      do { (Void)cmMemcpy((U8*)(&crntCfgInfo->secCfg.intgCfgInfo),(const U8*)(&ordCfgInfo->secCfg.intgCfgInfo), (Size)(sizeof(NhuIntgKeyAlgoInfo)));} while(0);


      do { (Void)cmMemcpy((U8*)(&crntCfgInfo->secCfg.cipherCfgInfo),(const U8*)(&ordCfgInfo->secCfg.cipherCfgInfo), (Size)(sizeof(NhuCiphKeyAlgoInfo)));} while(0);


   }

   if(ordCfgInfo->actionMask & 0x0200)
   {
      crntCfgInfo->bitMskPres |= 0x0008;
      crntCfgInfo->secCfg.secCfgBitMsk = ordCfgInfo->secCfg.secCfgBitMsk;

      if(ordCfgInfo->secCfg.secCfgBitMsk & 0x02)
      {
         do { (Void)cmMemcpy((U8*)(&crntCfgInfo->secCfg.intgCfgInfo.intgKey),(const U8*)(&ordCfgInfo->secCfg.intgCfgInfo.intgKey), (Size)(16 * sizeof(U8)));} while(0);


      }

      if(ordCfgInfo->secCfg.secCfgBitMsk & 0x01)
      {
         do { (Void)cmMemcpy((U8*)(&crntCfgInfo->secCfg.cipherCfgInfo.cipherCpKey),(const U8*)(&ordCfgInfo->secCfg.cipherCfgInfo.cipherCpKey), (Size)(16 * sizeof(U8)));} while(0);



         do { (Void)cmMemcpy((U8*)(&crntCfgInfo->secCfg.cipherCfgInfo.cipherUpKey),(const U8*)(&ordCfgInfo->secCfg.cipherCfgInfo.cipherUpKey), (Size)(16 * sizeof(U8)));} while(0);


      }
   }

   if(ordCfgInfo->actionMask & 0x0040)
   {
      if(0x02 == ordCfgInfo->ueInfo.crntiCfgType)
      {
         cellCb = ueCb->cellCb;

         retVal = nhHashListDelete(&(cellCb->newUeHlCp), (U32)ueCb);
         if (0 != retVal)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1606); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Could not delete from hash list.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

         }

         retVal = nhHashListDelete(&(cellCb->actvUeHlCp), (U32)ueCb);
         if (0 != retVal)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1613); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Could not delete from hash list.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         ...
         retVal = nhHashListInsert (&(cellCb->actvUeHlCp), (U32)ueCb,
               (U8 *)&(ueCb->ueCbKey), sizeof (NhUeCbKey));
         if (0 != retVal)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1624); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Could not insert into hash list.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            return(retVal);
         }
      }
   }
# 1639 "../..//lterrc/nh_dbm.c"
   return(0);
}
# 1657 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitRbRlsCfg
(
...

{
                   ...
                   == ordCfgInfo->rbToRlsCfgList.rbsToRel[i].rbTyp))
         {
            ...
            nhDBMUpdRbToLchOrPdcp(crntCfgInfo,
                                  ((NhuRbCfgInfo *) tmpNode->node),
                                   0x04);
            { if ((tmpNode->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1690, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmpNode->node), (Size)(sizeof(NhuRbCfgInfo))); (tmpNode->node) = 0L; } };
            { if ((tmpNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1691, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.pool), (nhCb.init.pool), (Data *)(tmpNode), (Size)(sizeof(CmLList))); (tmpNode) = 0L; } };
            break;
         }
   ...
   if(crntCfgInfo->rbCfgList.numOfRbCfgInfo == 0)
   {
      crntCfgInfo->bitMskPres &= ~0x0001;
   }
   ...
   if ((crntCfgInfo->rbCfgList.numOfRbCfgInfo) < 2)
   {


      crntCfgInfo->bitMskPres &= ~0x0040;
      crntCfgInfo->bitMskPres &= ~0x0080;
   }
   else if ((crntCfgInfo->rbCfgList.numOfRbCfgInfo) == 2)
   {
...
# 1740 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitRbModCfg
(
NhOrdrdCfgInfo *ordCfgInfo,
NhCrntCfgInfo *crntCfgInfo
)
...
{
   CmLList *tmpNode = 0L;
   NhuRbCfgInfo *srcRbCfgInfo = 0L;
   NhuRbCfgInfo *dstRbCfgInfo = 0L;
   S16 i;
   S16 retVal;
   ...
   for(i=0; i < ordCfgInfo->rbToModCfgList.numOfRbCfgInfo; i++)
   {
      ...
      while(0L != tmpNode)
      {
         ...
         {
            do { (Void)cmMemcpy((U8*)((U8*) tmpNode->node),(const U8*)((U8*) &ordCfgInfo->rbToModCfgList.rbCfgInfoList[i]), (Size)(sizeof(NhuRbCfgInfo)));} while(0);
            ...
            if(0 != retVal)
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1779); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"nhDBMUpdModifiedRb failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
               return(1);
            }
            nhDBMUpdRbToLchOrPdcp(crntCfgInfo,
...
# 1809 "../..//lterrc/nh_dbm.c"
static S16 nhDBMCommitRbAddCfg
(
NhOrdrdCfgInfo *ordCfgInfo,
NhCrntCfgInfo *crntCfgInfo
)
...

{
   ...
   for(i=0; i < ordCfgInfo->rbToAddCfgList.numOfRbCfgInfo; i++)
   {
      ...
      if(0L == tmpNode)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1833); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for adding radio bearer.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(4);
      }

      do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&tmpRbCfgInfo), (sizeof(NhuRbCfgInfo))) == 0) { do { (Void)cmMemset((U8*)((*(&tmpRbCfgInfo))), 0, (Size)((sizeof(NhuRbCfgInfo)))); } while(0); } else { (*(&tmpRbCfgInfo)) = 0L; } } while(0);
      if(0L == tmpRbCfgInfo)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1841); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for adding radio bearer.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

         { if ((tmpNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 1842, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmpNode), (Size)(sizeof(CmLList))); (tmpNode) = 0L; } };
         return(4);
      }

      do { (Void)cmMemcpy((U8*)((U8*) tmpRbCfgInfo),(const U8*)((U8*) &ordCfgInfo->rbToAddCfgList.rbCfgInfoList[i]), (Size)(sizeof(NhuRbCfgInfo)));} while(0);



      tmpNode->node = (U32) tmpRbCfgInfo;

      cmLListAdd2Tail(&crntCfgInfo->rbCfgList.rbCfgInfoList, tmpNode);

      nhDBMUpdRbToLchOrPdcp(crntCfgInfo, tmpRbCfgInfo, 0x01);
...
# 1897 "../..//lterrc/nh_dbm.c"
 S16 nhDBMCommitOrdrdCfg
(
NhActvUeCb *ueCb,
TransCb *transCtrlBlk
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1914); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhDBMCommitOrdrdCfg \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   crntCfgInfo = (NhCrntCfgInfo *)&ueCb->crntCfg;
   ordCfgInfo = (NhOrdrdCfgInfo *)&transCtrlBlk->ordrdCfg;

   if(ordCfgInfo->actionMask & 0x0001)
   {
      crntCfgInfo->bitMskPres |= 0x0001;
   ...
   if(ordCfgInfo->actionMask & 0x0002)
   {
      crntCfgInfo->bitMskPres |= 0x0001;
   ...
   if(ordCfgInfo->actionMask & 0x0004)
   {
   ...
   return(nhDBMCommitUeCfg(ueCb, transCtrlBlk));

}
# 1976 "../..//lterrc/nh_dbm.c"
 NhCellCb* nhDBMGetCellCb
(
U16 cellId
)
...

{
   ...
   while(0L != tmpNode)
   {
      if (0L == tmpNode->node)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 1997); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NULL logChCb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(0);
      }
...
# 2032 "../..//lterrc/nh_dbm.c"
 NhActvUeCb* nhDBMGetUeCb
(
NhCellCb *cellCb,
U16 ueId
)
...

{
   ...
   if(0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2055); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "UE with ID: %d not found in cell ID: %d.\n", ueId, cellCb->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 2079 "../..//lterrc/nh_dbm.c"
 NhActvUeCb* nhDBMGetNewUeCb
(
NhCellCb *cellCb,
U16 ueId
)
...

{
   ...
   do { (Void)cmMemset((U8*)(&ueCbKey), 0, (Size)(sizeof(NhUeCbKey))); } while(0);
   ueCbKey.crnti = ueId;
   ...
   if(0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2105); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "UE with ID: %d not found in cell ID: %d.\n", ueId, cellCb->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 2127 "../..//lterrc/nh_dbm.c"
static S16 nhInitUeCb
(
NhActvUeCb *ueCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2144); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Initialising UE control block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   ret = nhHashListInit(&ueCb->msgTransHlCp, 256, 0,
                  0, 0, nhCb.init.region,nhCb.init.pool);
   ...
   for (i=0; i < nhCb.genCfg.minTransLstLen; i++)
   {
      do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&tempNode), (sizeof(CmLList))) == 0) { do { (Void)cmMemset((U8*)((*(&tempNode))), 0, (Size)((sizeof(CmLList)))); } while(0); } else { (*(&tempNode)) = 0L; } } while(0);
      if (0L == tempNode)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2168); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(4);
      }

      do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&transCtrlBlk), (sizeof(TransCb))) == 0) { do { (Void)cmMemset((U8*)((*(&transCtrlBlk))), 0, (Size)((sizeof(TransCb)))); } while(0); } else { (*(&transCtrlBlk)) = 0L; } } while(0);
      if (0L == transCtrlBlk)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2175); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         { if ((tempNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2176, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tempNode), (Size)(sizeof(CmLList))); (tempNode) = 0L; } };
         return(4);
      }
      ...
      tempNode->node = (U32)(transCtrlBlk);
   ...
   for (i=0; i < nhCb.genCfg.minMuiLstLen; i++)
   {
      ...
      if (0L == tempNode)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2199); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(4);
      }
      ...
      if (0L == muiInfo)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2206); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         { if ((tempNode) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2207, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tempNode), (Size)(sizeof(CmLList))); (tempNode) = 0L; } };
         return(4);
      }



      do { (Void)cmMemset((U8*)((U8 *)muiInfo), 0, (Size)(sizeof(MuiInfo))); } while(0);
      muiInfo->mui = i;

      tempNode->node = (U32)(muiInfo);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2258); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Creating UE control Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };


   do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(ueCb), (sizeof(NhActvUeCb))) == 0) { do { (Void)cmMemset((U8*)((*(ueCb))), 0, (Size)((sizeof(NhActvUeCb)))); } while(0); } else { (*(ueCb)) = 0L; } } while(0);
   if (0L == *ueCb)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2264); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation Failure.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(4);
   }
   ...
   if(0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2275); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "UE initialisation failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((*ueCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2276, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(*ueCb), (Size)(sizeof(NhActvUeCb))); (*ueCb) = 0L; } };
      return(ret);
   }
   ...
   ret = nhHashListInsert(&cellCb->actvUeHlCp, (U32) (*ueCb),
                          (U8*) &((*ueCb)->ueCbKey),
                          sizeof (NhUeCbKey));
   if(0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2291); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "UE addition to cell hash list failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2304); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "UE control Block Created and Initialized successfully.\n"); SPrint((&nhCb.init)->prntBuf); } }; };


   return(0);
}
# 2325 "../..//lterrc/nh_dbm.c"
 S16 nhDBMDeleteUeCb
(
NhCellCb *cellCb,
NhActvUeCb **ueCb
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2344); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Deleting UE control Block.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   while(0L != tmp)
   {
      ...
      {
         { if ((transCtrlBlk->ordrdCfg.rbToAddCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2370, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(transCtrlBlk->ordrdCfg.rbToAddCfgList.rbCfgInfoList), (Size)(transCtrlBlk->ordrdCfg.rbToAddCfgList.numOfRbCfgInfo * sizeof(NhuRbCfgInfo))); (transCtrlBlk->ordrdCfg.rbToAddCfgList.rbCfgInfoList) = 0L; } };
      ...
      if(transCtrlBlk->ordrdCfg.rbToModCfgList.numOfRbCfgInfo)
      {
         ...
         { if ((transCtrlBlk->ordrdCfg.rbToModCfgList.rbCfgInfoList) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2379, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(transCtrlBlk->ordrdCfg.rbToModCfgList.rbCfgInfoList), (Size)(transCtrlBlk->ordrdCfg.rbToModCfgList.numOfRbCfgInfo * sizeof(NhuRbCfgInfo))); (transCtrlBlk->ordrdCfg.rbToModCfgList.rbCfgInfoList) = 0L; } };

      }
      if(transCtrlBlk->ordrdCfg.rbToRlsCfgList.numOfRbCfgInfo)
      {
         { if ((transCtrlBlk->ordrdCfg.rbToRlsCfgList.rbsToRel) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2384, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(transCtrlBlk->ordrdCfg.rbToRlsCfgList.rbsToRel), (Size)(transCtrlBlk->ordrdCfg.rbToRlsCfgList.numOfRbCfgInfo * sizeof(NhuRbId))); (transCtrlBlk->ordrdCfg.rbToRlsCfgList.rbsToRel) = 0L; } };

      }

      if (transCtrlBlk->storedMsg)
      {
         do{ if ((transCtrlBlk->storedMsg) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2389, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((transCtrlBlk->storedMsg)); (transCtrlBlk->storedMsg) = 0L; } }while(0);
      }

      if (transCtrlBlk->lowerLyrParam)
      {
         { if ((transCtrlBlk->lowerLyrParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2395, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(transCtrlBlk->lowerLyrParam), (Size)(sizeof(NhDatReqParam))); (transCtrlBlk->lowerLyrParam) = 0L; } };

      }


      { if ((transCtrlBlk) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2399, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(transCtrlBlk), (Size)(sizeof(TransCb))); (transCtrlBlk) = 0L; } };
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2400, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      ((((tmp) = ((&((*ueCb)->transCbLst))->crnt = (&((*ueCb)->transCbLst))->first)) != 0L) ? (tmp)->node : 0L);
   }
   ...
   while (0L != tmp)
   {
      ...
      cmLListDelFrm(&((*ueCb)->muiInfoLst), tmp);
      { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2426, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(MuiInfo))); (tmp->node) = 0L; } };
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2427, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      ((((tmp) = ((&((*ueCb)->muiInfoLst))->crnt = (&((*ueCb)->muiInfoLst))->first)) != 0L) ? (tmp)->node : 0L);
   }


   ((((tmp) = ((&(*ueCb)->crntCfg.logChCbLst)->crnt = (&(*ueCb)->crntCfg.logChCbLst)->first)) != 0L) ? (tmp)->node : 0L);

   while(0L != tmp)
   {
      cmLListDelFrm(&(*ueCb)->crntCfg.logChCbLst, tmp);
      { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2437, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(NhRbToLchMap))); (tmp->node) = 0L; } };
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2438, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      ((((tmp) = ((&(*ueCb)->crntCfg.logChCbLst)->crnt = (&(*ueCb)->crntCfg.logChCbLst)->first)) != 0L) ? (tmp)->node : 0L);
   }
   ...
   ((((tmp) = ((&(*ueCb)->crntCfg.rbCfgList.rbCfgInfoList)->crnt = (&(*ueCb)->crntCfg.rbCfgList.rbCfgInfoList)->first)) != 0L) ? (tmp)->node : 0L);

   while(0L != tmp)
   {
      cmLListDelFrm(&(*ueCb)->crntCfg.rbCfgList.rbCfgInfoList, tmp);
      { if ((tmp->node) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2451, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp->node), (Size)(sizeof(NhuRbCfgInfo))); (tmp->node) = 0L; } };
      { if ((tmp) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2452, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmp), (Size)(sizeof(CmLList))); (tmp) = 0L; } };
      ((((tmp) = ((&(*ueCb)->crntCfg.rbCfgList.rbCfgInfoList)->crnt = (&(*ueCb)->crntCfg.rbCfgList.rbCfgInfoList)->first)) != 0L) ? (tmp)->node : 0L);
   }
   ...
   for (tmrNum = 0; tmrNum < 64; tmrNum++)
   {
      ...
      if ((0L != tmrCb) && (tmrCb->ueCb == (*ueCb)))
      {
         ...
         { if ((tmrCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2476, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(tmrCb), (Size)(sizeof(NhLLyrCfgTmrCb))); (tmrCb) = 0L; } };
      }
   }


   { if ((*ueCb) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_dbm.c", 2481, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(*ueCb), (Size)(sizeof(NhActvUeCb))); (*ueCb) = 0L; } };

   return(0);
}
# 2502 "../..//lterrc/nh_dbm.c"
 S16 nhDBMUpdModifiedRbCfgUl
(
NhuRbCfgInfo *dstRbCfgInfo,
NhuRbCfgInfo *srcRbCfgInfo
)
...

{
   ...
   if(srcRbCfgInfo->ulRbCfg.ulRbBitMsk & 0x02)
   {
      if(srcRbCfgInfo->ulRbCfg.ulRlcMode.mode == 0x02)
         do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->ulRbCfg.ulRlcMode.m.umRlc),(const U8*)((U8*) &srcRbCfgInfo->ulRbCfg.ulRlcMode.m.umRlc), (Size)(sizeof(NhuUlUmRlcModeInfo)));} while(0);


      else if(srcRbCfgInfo->ulRbCfg.ulRlcMode.mode == 0x03)
         do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->ulRbCfg.ulRlcMode.m.amRlc),(const U8*)((U8*) &srcRbCfgInfo->ulRbCfg.ulRlcMode.m.amRlc), (Size)(sizeof(NhuUlAmRlcModeInfo)));} while(0);
...
# 2556 "../..//lterrc/nh_dbm.c"
 S16 nhDBMUpdModifiedRbCfgDl
(
NhuRbCfgInfo *dstRbCfgInfo,
NhuRbCfgInfo *srcRbCfgInfo
)
...

{
   ...
   if(srcRbCfgInfo->dlRbCfg.dlRbBitMsk & 0x02)
   {
      if(srcRbCfgInfo->dlRbCfg.dlRlcMode.mode == 0x02)
         do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->dlRbCfg.dlRlcMode.m.umRlc),(const U8*)((U8*) &srcRbCfgInfo->dlRbCfg.dlRlcMode.m.umRlc), (Size)(sizeof(NhuDlUmRlcModeInfo)));} while(0);


      else if(srcRbCfgInfo->dlRbCfg.dlRlcMode.mode == 0x03)
         do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->dlRbCfg.dlRlcMode.m.amRlc),(const U8*)((U8*) &srcRbCfgInfo->dlRbCfg.dlRlcMode.m.amRlc), (Size)(sizeof(NhuDlAmRlcModeInfo)));} while(0);
...
# 2602 "../..//lterrc/nh_dbm.c"
 S16 nhDBMUpdModifiedRbPdcpInfo
(
NhuRbCfgInfo *dstRbCfgInfo,
NhuRbCfgInfo *srcRbCfgInfo
)
...

{
   ...
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x01)
      dstRbCfgInfo->pdcpCfg.discardTmr = srcRbCfgInfo->pdcpCfg.discardTmr;
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x02)
      dstRbCfgInfo->pdcpCfg.flushTmr = srcRbCfgInfo->pdcpCfg.flushTmr;
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x04)
      dstRbCfgInfo->pdcpCfg.snSize = srcRbCfgInfo->pdcpCfg.snSize;
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x08)
      dstRbCfgInfo->pdcpCfg.statusRptReq =
                               srcRbCfgInfo->pdcpCfg.statusRptReq;
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x10)
      do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->pdcpCfg.hdrComprInfo),(const U8*)((U8*)&srcRbCfgInfo->pdcpCfg.hdrComprInfo), (Size)(sizeof(NhuPdcpHdrComprInfo)));} while(0);



   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x20)
   {
      do { (Void)cmMemcpy((U8*)((U8*)&dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo),(const U8*)((U8*)&srcRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo), (Size)(sizeof(NhuHoPdcpSduInfo)));} while(0);



      if(0 != srcRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.numBits)
      {

         { ulBitMapSize = srcRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.numBits / 8; if(srcRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.numBits % 8) { ulBitMapSize++; } };




         do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap), (sizeof(U8) * ulBitMapSize)) == 0) { do { (Void)cmMemset((U8*)((*(&dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap))), 0, (Size)((sizeof(U8) * ulBitMapSize))); } while(0); } else { (*(&dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap)) = 0L; } } while(0);



         if(0L == dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap)
         {
             { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_dbm.c", 2653); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for ulBitMap.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

             return(1);
         }
         do { (Void)cmMemcpy((U8*)((U8*)dstRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap),(const U8*)((U8*)srcRbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.ulBitMap), (Size)(sizeof(U8) * ulBitMapSize));} while(0);


      }
   }
   if(srcRbCfgInfo->pdcpCfg.pdcpCfgBitMsk & 0x40)
   {
