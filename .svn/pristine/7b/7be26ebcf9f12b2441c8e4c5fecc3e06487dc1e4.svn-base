...
#endif    
{
   ...
   RGDBGPRM((rgPBuf, "\nRgMiLrgCfgReq(): pst->srcEnt=%d, pst->srcInst=%d", 
  ...
  RG_DIAG_LVL0(0x0a0b0001, RG_DIAG_NA, SS_DIAG_INV_ARG,
   ...
   switch(cfg->hdr.elmId.elmnt)
   {
      case STGEN:
         ...
         RGDBGINFO((rgPBuf, "\nRgMiLrgCfgReq(): Gen Cfg done with reason=%d.",reason));
         break;
      case STRGUSAP:
      case STCRGSAP:
      case STTFUSAP:
      ...
      case STGEN:
         {
...
#ifdef MAC_SCH_STATS
            {
               ...
               U8       cqi = 0;
#ifdef MSPD
               MSPD_DBG("<======= Updating the MAC SCH Statistics ========>\n");
#endif
...
#endif    
{
   ...
   switch(sta->hdr.elmId.elmnt)
   {
      ...
      default:
         ...
         cfm.cfm.reason = LCM_REASON_INVALID_ELMNT;
         RgMiLrgStaCfm(&cfmPst, &cfm);
         RGDBGERR((rgPBuf, "\nRgMiLrgStaReq(): Invalid elmnt=%d", sta->hdr.elmId.elmnt));
         MSPD_ERR("\nRgMiLrgStaReq(): Invalid elmnt=%d", sta->hdr.elmId.elmnt);
         break;     
   }
   RETVALUE(ROK);
...
#endif    
{
   ...
   if(rgCb.rgInit.cfgDone != TRUE)
   {
      ...
      RGDBGERR((rgPBuf, "\nRgMiLrgCntrlReq(): Gen Cfg not done."));
      MSPD_ERR("\nRgMiLrgCntrlReq(): Gen Cfg not done.");
      RETVALUE(ROK);
   }
...
#endif
{
   ...
   RGDBGPRM((rgPBuf, "\nrgLMMSapCfg(): SAP Config Requested for sapType=%d", sapType));
...
#endif
{
   ...
   RGDBGPRM((rgPBuf, "\nrgLMMGenCntrl(): Control Req for STGEN"));
   cfm->cfm.status = LCM_PRIM_OK;
   cfm->cfm.reason = LCM_REASON_NOT_APPL;
...
#endif
{
   ...
   tmrEvnt, tmrVal));
#else
   RGDBGPRM((rgPBuf, "rgLMMStartTmr(): Timer start request for tmrEvnt=%d, tmrVal=%d", 
   tmrEvnt, tmrVal));
#endif
...
void sendRgStartIndToPrm(void)
{
  ...
  {
    MSPD_DBG("Sent EVT_START_IND from RG to PRM \n");
  }
}
...
void sendRgHelloAckToPrm(Buffer   *mBuf)
{
  ...
  {
    MSPD_DBG( "sendRgHelloAckToPrm(): SCpyMsgFix Failed\n" );
    RETVOID;
  }
  ...
  {
    MSPD_DBG("Sent EVT_HELLO_ACK from RG to PRM for sequence number:%d \n", helloAck.helloAckSeqNum);
  }
  else
  {
    MSPD_DBG("Sent EVT_HELLO_ACK from RG to PRM failed for sequence number:%d \n", helloAck.helloAckSeqNum);
  }
}

void procRcvdRgExitMsg()
{
  MSPD_DBG("Exiting the RG \n" );
#ifdef PWAV
  stop_printf("physhutdown\n");
#endif
  exit(0);
}
#endif
