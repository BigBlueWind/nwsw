...
#endif    
{
   ...
   YS_DBG_INFO((_ysp, "\nYsMiLysCfgReq(): pst->srcEnt=%d, pst->srcInst=%d", 
  ...
  YS_DIAG_LVL0(0x0d060001, YS_DIAG_NA, SS_DIAG_INV_ARG,
   ...
   if (SGetSBuf(cfmPst.region, cfmPst.pool, (Data **)&cfm, sizeof(YsMngmt))
      != ROK)
   {
      YS_DBG_ERR((_ysp, "\nYsMiLysCfgReq(): Memory Unavailable for \
               Confirmation"));
      RETVALUE(ROK);
   }
   ...
   switch(cfg->hdr.elmId.elmnt)
   {
      case STGEN:
         ...
         YS_DBG_INFO((_ysp, "\nYsMiLysCfgReq(): Gen Cfg done with reason=%d.",
                                 reason));
         break;
      case STCTFSAP:
      case STTFUSAP:
         reason = ysLMMSapCfg(&cfg->t.cfg, cfg->hdr.elmId.elmnt);
         YS_DBG_INFO((_ysp, "\nYsMiLysCfgReq(): SAP(Elmnt=%d) Cfg done with \
                          reason=%d.", cfg->hdr.elmId.elmnt, reason));
         break;
      default:
         ret = LCM_PRIM_NOK;
         reason = LCM_REASON_INVALID_ELMNT;
         YS_DBG_ERR((_ysp, "\nYsMiLysCfgReq(): Invalid Elmnt=%d",
                        cfg->hdr.elmId.elmnt));
         break;
   }
...
#endif
{
   ...
   YS_DBG_INFO((_ysp, "\nysLMMGenCfg(): General Configuration Request"));
   ...
   if ((SGetSMem(ysCb.ysInit.region, memSize, &ysCb.ysInit.pool)) != ROK)
...
#endif
{
   ...
   YS_DBG_INFO((_ysp, "\nysLMMSapCfg(): SAP Config Requested for sapType=%d",
                        sapType));
...
/*
               ...
               */
               ...
               break;
            case SALOG:
               ysCb.ysInit.logMask =  cntrl->t.cntrl.s.logMask;
               break;
