...
#ifdef PWAV
Queue       *g_udh_prio0_q;
Queue       *g_tucl_prio0_q;
Queue       *g_gtpu_prio0_q;
#endif
#ifdef ANSI
PUBLIC S16 SPstTsk
(
...
/*ss014.301 SSI-4GMX specific changes*/   
   ...
   if(( NULL == g_udh_prio0_q ) && ( tTsk->ent == ENTUDH ))
     g_udh_prio0_q = &tTsk->sTsk->dQ.queue[0];
   if(( NULL == g_tucl_prio0_q ) && ( tTsk->ent == ENTHI ))
     g_tucl_prio0_q = &tTsk->sTsk->dQ.queue[0];
   if(( NULL == g_gtpu_prio0_q ) && ( tTsk->ent == ENTEG ))
     g_gtpu_prio0_q = &tTsk->sTsk->dQ.queue[0];
#else
   if (osCp.procId == 1)
   {
     r = tTsk->actvTsk(pst, mBuf);
   }
   else
   {
      r = ssDmndQPutLast(&tTsk->sTsk->dQ, mBuf,
