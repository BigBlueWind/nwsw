...
EXTERN  S16 pwavCmAllocEvnt ARGS(( Size  evntSize, Size  maxBlkSize, Mem  *sMem, Ptr  *ptr, char *file, int   line));

#define cmAllocEvnt(evntSize, maxBlkSize, sMem, ptr) \
        pwavCmAllocEvnt(evntSize, maxBlkSize, sMem, ptr, __FILE__, __LINE__)

EXTERN  Void pwavCmFreeMem ARGS(( Ptr   memPtr, char *file, int   line));

#define cmFreeMem(memPtr)\
        pwavCmFreeMem(memPtr, __FILE__, __LINE__)

#else
EXTERN  S16 cmAllocEvnt ARGS(( Size  evntSize, Size  maxBlkSize, Mem  *sMem, Ptr  *ptr));
EXTERN  Void cmFreeMem ARGS(( Ptr   memPtr));
#endif /* PWAV_MEM2 */

EXTERN  S16 cmGetMem ARGS(( Ptr   memPtr, Size  size, Ptr  *allocPtr));
