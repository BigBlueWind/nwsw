...
g_mem_profile (void)
{
  ...
  _memcpy (local_data, profile_data,
		 ...
		 gint          type)
{
  ...
  G_LOCK(mem_chunks_lock);
  mem_chunk->next = mem_chunks;
  mem_chunk->prev = NULL;
  if (mem_chunks)
    mem_chunks->prev = mem_chunk;
  mem_chunks = mem_chunk;
  G_UNLOCK(mem_chunks_lock);
		 ...
		 guint        n_preallocs)
{
  ...
  allocator->name = g_new0(gchar, strlen(name) + 1);
...
g_mem_init (void)
{
  ...
  g_profile_mutex = NULL;//g_mutex_new ();
#endif
}
