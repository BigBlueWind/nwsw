  ...
  )
{
  ...
  /* Read the file. If there is an error, report it and exit. */
   if(! config_read_file(&cfg, fileToRead))
   {
     ...
     config_destroy(&cfg);
     printf("\n Please enter valid Path for Process Start Configuration file \n");
     *error = TRUE;
     return FALSE;
   }

   setting = config_lookup(&cfg, "process_start_table");
   if(setting NOTEQ NULL)
   {
     ...
     for(i = 0; i < count; ++i)
     {
       ...
       const char *exePath = NULL, *exeName = NULL ;

       config_setting_lookup_string(table, "exePath", &exePath );
       config_setting_lookup_string(table, "exeName", &exeName );
       strncpy( ptrProcessStart->path, exePath, strlen(exePath));
       strncpy( ptrProcessStart->exeName, exeName, strlen(exeName));
       ptrProcessStart++;
     }
     config_destroy(&cfg);
     return count;
