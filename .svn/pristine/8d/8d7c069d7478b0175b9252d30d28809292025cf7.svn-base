...
PRIVATE Void rgSCHDrxCalcNxtTmrExpry ARGS((RgSchCellCb     *cell,
 ...
 PRIVATE S16 rgSCHDrxGetNxtTmrExpry ARGS((RgSchUeCb* ue,U16 curTime,
                                          U32 duration, 
...
#endif
{
...
#ifdef LTE_TDD
   /*For TDD HARQ RTT timer is 4 subframes from HARQ feedback time. Refer 7.7
    * in 36.321-930. We are adding 1 subframe so that UE is scheduled for
    * retransmission in the next subframe*/
   harqRTTExpIndx = ((hqP->tbInfo[tbCnt].fdbkTime.sfn * 10) +
         hqP->tbInfo[tbCnt].fdbkTime.subframe + 4 + 1)
      % RG_SCH_MAX_DRXQ_SIZE;
   ...
   harqRTTExpIndx = ((hqP->tbInfo[tbCnt].timingInfo.sfn * 10) +
         hqP->tbInfo[tbCnt].timingInfo.subframe + RG_SCH_MIN_HARQ_RTT + 1)
      % RG_SCH_MAX_DRXQ_SIZE;
...
#endif
{
   ...
   {
      RGSCHDBGERR(cell->instIdx, (rgSchPBuf(cell->instIdx),"rgSCHDrxUeCfg():"
               "Invalid params."
               "cell or ue or ueCfg is NULL"));
      RETVALUE(RFAILED);
   }
#endif

   MSPD_DBG("UE configuration for DRX UE %d\n", ue->ueId);  
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx),
            "rgSCHDrxUeCfg():UE configuration for DRX UE %d\n", ue->ueId));
   ...
   {
      RGSCHDBGERR(cell->instIdx, (rgSchPBuf(cell->instIdx),
               "Memory allocation FAILED for DRX UE Cb\n"));
      RETVALUE(ret);
   }
   ...
   ueDrxCb->distance              = DRX_TMR_EXPRD;
      ...
      /* Mark the UE in long cycle initially */
   ...
   {
      MSPD_ERR("DRXUE. Invalid value for distance, %d\n", ueDrxCb->distance);
   }
...
#endif
{
   ...
   MSPD_DBG("rgSCHDrxUeReCfg: UE configuration for DRX UE %d\n", ue->ueId);  /* sachin */
   RGSCHDBGPRM(instIdx, (rgSchPBuf(instIdx),
               "rgSCHDrxUeReCfg():UE Re-Configuration for DRX UE %d\n", 
                ue->ueId));
...
#endif
{
   ...
   MSPD_DBG("rgSCHDrxUeDel():UE DEL Reqfor DRX UE %d\n", ue->ueId);

   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx),
            "rgSCHDrxUeDel():UE DEL Reqfor DRX UE %d\n", ue->ueId));

   if((ueDrxCb = ue->drxCb) == NULLP)
   {
      /*If DRX not enabled for UE, then return*/
      RETVALUE(ROK);
   }
...
#endif
{
   ...
   RGSCHDBGPRM(instIdx, (rgSchPBuf(instIdx), "rgSCHDrxCellCfg()"));
   ...
   {
      RGSCHDBGERR(instIdx, (rgSchPBuf(instIdx),
                "rgSCHDrxCellCfg():Invalid Params. cell/cellCfg"
                "is NULL"));
      RETVALUE(RFAILED);
   }
#endif
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx, (rgSchPBuf(cell->instIdx), "rgSCHDrxSrInd()"));
...
{
#ifdef DEBUGP
   Inst              instIdx      = cellCb->instIdx;
#endif /*DEBUGP*/
   RgSchDrxUeCb      *drxCb;

   TRC2(rgSCHDrxDedRa);

   RGSCHDBGPRM(instIdx, (rgSchPBuf(instIdx), "rgSCHDrxDedRa()"));
   ...
   /* Call the schedulers even if no other
    * conditions is keeping the UE inactive */

   if ( RG_SCH_CMN_UL_IS_UE_ACTIVE(ueCb))
   {
      rgSCHUtlUlActvtUe(cellCb, ueCb);
   }
   if ( RG_SCH_CMN_DL_IS_UE_ACTIVE(ueCb))
   {
      rgSCHUtlDlActvtUe(cellCb, ueCb);
   }
   RETVOID;
...
#else
PRIVATE S16 rgSCHDrxGetNxtTmrExpry (ue,curTime,duration,tmrExpryIdx)
 RgSchUeCb           *ue;
 U16                 curTime;
 U32                 duration;
 CmLteTimingInfo     *tmrExpryIdx;
#endif
{
   RgSchCellCb*    cell;            /*!< cyclic prefix info is in cell cb */
   ...
   /* Figure out whether this TDD configuration has DwPTS
    * counted as DL subframe */
   if ( cell->isCpDlExtend )
   {
...
#endif
{
   ...
   rgSCHDrxGetNxtTmrExpry(ue,curTimeInSf,tmrLen,&tmrExpry);
...
#endif
{
   ...
   while (node)
   {
      ...
      /*reset the inactivity bit in Uplink */
      ue->ul.ulInactvMask &= ~(RG_DRX_INACTIVE);
      ...
      if ( RG_SCH_CMN_UL_IS_UE_ACTIVE(ue))
      {
         /*CALL Schedulers to activate UE*/
         if( ROK != rgSCHUtlUlActvtUe (cell, ue))
         {
            RGSCHDBGERR(cell->instIdx,
                  (rgSchPBuf(cell->instIdx),
                   "rgSCHDrxTtiHdlOnDurUl() Unable to Inactivate UE\n"));
         }
      }
...
#endif
{
   ...
   while (node)
   {
      ...
      {
         /*CALL Schedulers to activate UE*/
         if (ROK != rgSCHUtlDlActvtUe (cell, ue))
         {
            RGSCHDBGERR(cell->instIdx,
                  (rgSchPBuf(cell->instIdx),
                   "rgSCHDrxTtiHdlOnDurDl() Unable to add UE \n"));
         }
      }
...
#endif /* ANSI */
{
   ...
   while (node)
   {
      ...
      reTxExpIndx =  ((dlIndex +
               drxUe->drxRetransTmrLen) % RG_SCH_MAX_DRXQ_SIZE);
#endif /*LTE_TDD*/
      ...
      /* add the UE to the cell's retransmission queues-*/
      ...
      if ( RG_SCH_CMN_DL_IS_UE_ACTIVE(ue))
      {
         ...
         if (ROK != rgSCHUtlDlActvtUe (cell, ue))
         {
            RGSCHDBGERR(cell->instIdx,
                  (rgSchPBuf(cell->instIdx),
                  "rgSCHDrxTtiHdlDlHarqRTT() Unable to add UE \n"));
         }
         }
      }
      else
...
#endif /* ANSI */
{
   ...
   while (node)
   {
      ...
      if ( RG_SCH_CMN_UL_IS_UE_ACTIVE(ue))
      {
         /*Call the schedulers to Activate UE*/
         rgSCHUtlUlActvtUe (cell, ue);
      }
   }
