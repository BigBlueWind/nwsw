...
#endif
{
   ...
   {
      RGDBGERR((rgPBuf, "rgDUXExtSduLen(): \
                  SUnpkU8 failed\n"));
      MSPD_ERR("rgDUXExtSduLen(): \
                  SUnpkU8 failed\n");
...
#endif
{
   ...
   if(ret != ROK)
   {
      RGDBGERR((rgPBuf, " rgDUXInsSdu(): \
               Allocation of RgSubHdr failed\n")); 
      MSPD_ERR(" rgDUXInsSdu(): \
               Allocation of RgSubHdr failed\n"); 
...
#endif
{
   ...
   *subHdrChkCmp = TRUE;
   do
   {
      if(SUnpkU8(&byte,mBuf) != ROK)
      {
         RGDBGERR((rgPBuf, "rgDUXExtCeSubHdrs(): \
                           SUnpkU8 failed\n"));
         MSPD_ERR("rgDUXExtCeSubHdrs(): \
                           SUnpkU8 failed\n");
         err->errCause = RGERR_DUX_UNPACK_FAILURE;
         RETVALUE(RFAILED);
      }
      ...
      RGDBGPRM((rgPBuf, "rgDUXExtCeSubHdrs(): lcId(%u)\n",lcId));
      ...
      if(lcId == RG_PAD_LCID)
      {
         if(*extnBit)
         {
            ...
            {
               RGDBGERR((rgPBuf, "rgDUXExtCeSubHdrs(): \
                           padSubHdrs received more than 2\n"));
               MSPD_ERR("rgDUXExtCeSubHdrs(): \
                           padSubHdrs received more than 2\n");
   ...
   *subHdrChkCmp = TRUE;
   ...
   do
   {
      ...
      if(lcId == RG_PAD_LCID)
      {
         if(*extnBit)
         {
            RGDBGERR((rgPBuf, "rgDUXExtSduSubHdrs(): \
                        invalid padSubHdrs received\n"));
            MSPD_ERR("rgDUXExtSduSubHdrs(): \
                        invalid padSubHdrs received\n");
            err->errCause = RGERR_DUX_INV_PDU_RX;
            RETVALUE(RFAILED);      
         }
      ...
      if ((lcId >= RG_PHR_LCID) && (lcId <= RG_LONG_BSR_LCID))
      {
         ...
         {
            RGDBGERR((rgPBuf, " rgDUXExtCeSubHdrs(): \
                     Allocation of RgMacCeSubHdr failed\n"));
            MSPD_ERR(" rgDUXExtCeSubHdrs(): \
                     Allocation of RgMacCeSubHdr failed\n");
            err->errCause = RGERR_DUX_MEM_EXHAUST;
            RETVALUE(RFAILED);
...
#endif
{
   ...
   do 
   {
      /* Extract the lcid */
      RG_EXT_LCID(lcId, byte);
      RGDBGPRM((rgPBuf, "rgDUXExtSduSubHdrs(): lcId(%u)\n",lcId));
      RG_EXT_EXTN_BIT(extnBit, byte);
      ...
      if(lcId <= RG_DEDLC_MAX_LCID)
      {
        /* ++numSduSubHdrs;*/
         ...
         if(extnBit)
         {
            ...
            RGDBGPRM((rgPBuf, "rgDUXExtSduSubHdrs(): sduLen(%u)\n",sduLen));
      ...
      {
         RGDBGERR((rgPBuf, " rgDUXExtSduSubHdrs(): Invalid LCID(%u) received\n"
                                                                     ,lcId)); 
         MSPD_ERR(" rgDUXExtSduSubHdrs(): Invalid LCID(%u) received\n"
                                                                     ,lcId); 
   ...
   while(tmpNode)
   {
      ...
      switch(ceSubHdr->lcId)
      {
         case RG_PHR_LCID:
               {
                  ...
                  if(ret != ROK)
                  {
                     RGDBGERR((rgPBuf, "rgDUXExtCes(): \
                              Unpacking of PHR failed\n"));
                     MSPD_ERR("rgDUXExtCes(): \
                              Unpacking of PHR failed\n");
                     err->errCause = RGERR_DUX_UNPACK_FAILURE;
                     RETVALUE(RFAILED);
                  }
         ...
         case RG_CRNTI_LCID:
               ...
               break;
         default:
            RGDBGERR((rgPBuf, " rgDUXExtCes(): Invalid LCID(%u) received\n"
                                                           , ceSubHdr->lcId)); 
            MSPD_ERR(" rgDUXExtCes(): Invalid LCID(%u) received\n"
                                                           , ceSubHdr->lcId); 
            err->errCause = RGERR_DUX_INV_LCID_RX;
            RETVALUE(RFAILED);
      }
...
#endif
{
   ...
   while(tmpNode)
   {
      ...
      if(!sdu->len)
      {
         ...
         {
            RGDBGERR((rgPBuf, "rgDUXExtSdus(): \
                              mBuf length check failed\n"));
            MSPD_ERR("rgDUXExtSdus(): \
                              mBuf length check failed\n");
            err->errCause = RGERR_DUX_UNPACK_FAILURE;
            RETVALUE(RFAILED);
         }
...
#endif
{
   ...
   if((!extnBit) && (subHdrChkCmp))
   {
      if(SPutMsg(mBuf) != ROK)
      {
         RGDBGERR((rgPBuf, "rgDUXDemuxData(): \
                            SPutMsg failed\n"));
         MSPD_ERR( "rgDUXDemuxData(): \
                            SPutMsg failed\n");
         err->errCause = RGERR_DUX_UNPACK_FAILURE;
         RETVALUE(RFAILED);
      }
   ...
   if(rgDUXExtCeSubHdrs(pdu, mBuf,&numPadSubHdrs, 
         &unPkByte, &extnBit, &subHdrChkCmp, err) != ROK)
   {
      MSPD_ERR( " rgDUXExtCeSubHdrs failed\n");
      RETVALUE(RFAILED);
   }
   ...
   {
      MSPD_ERR( " rgDUXExtSduSubHdrs failed\n");
      RETVALUE(RFAILED);
   }
