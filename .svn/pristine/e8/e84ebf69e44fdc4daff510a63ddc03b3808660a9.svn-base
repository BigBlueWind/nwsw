...
const ADRADPROFILE Ad261Lte10Mhz_b5[] = {
//************************************************************
...
{SPIWR,0x2AB,0x07},// Set RF PLL reflclk scale to REFCLK * 2
{SPIWR,0x2AC,0xFF},// Set RF PLL reflclk scale to REFCLK * 2
{SPIWR,0x009,0x17},// Enable Clocks
{DELAY,0,20},// waits 20 ms
//************************************************************
// Set BBPLL Frequency: 983.040000
//************************************************************
{SPIWR,0x045,0x03},// Set BBPLL reflclk scale to REFCLK * 2
...
{SPIWR,0x043,0x00},// BBPLL Freq Word (Fractional[7:0])
{SPIWR,0x042,0x00},// BBPLL Freq Word (Fractional[15:8])
{SPIWR,0x041,0x00},// BBPLL Freq Word (Fractional[23:16])
{SPIWR,0x044,0x10},// BBPLL Freq Word (Integer[7:0])
...
{WCALDONE,BBPLL,2000},// Wait for BBPLL to lock, Timeout 2sec, Max BBPLL VCO Cal Time: 225.000 us (Done when 0x05E[7]==1)
...
//************************************************************
{SPIWR,0x010,0xC8},// PPORT Config 1
...
{SPIWR,0x01C,0x10},// AuxADC Setup (Clock Div)
{SPIWR,0x01D,0x01},// AuxADC Setup (Decimation/Enable)
//************************************************************
// Setup Control Outs
//************************************************************
{SPIWR,0x035,0x01},// Ctrl Out index
{SPIWR,0x036,0xFF},// Ctrl Out [7:0] output enable
//************************************************************
// Setup GPO
//************************************************************
{SPIWR,0x03A,0x1E},// Set number of REFCLK cycles for 1us delay timer
{SPIWR,0x020,0x00},// GPO Auto Enable Setup in RX and TX
{SPIWR,0x027,0x03},// GPO Manual and GPO auto value in ALERT
...
{WCALDONE,RXCP,2000},// Wait for CP cal to complete, Max RXCP Cal time: 600.000 (us)(Done when 0x244[7]==1)
{SPIWR,0x27D,0x04},// Start TX CP cal
{WCALDONE,TXCP,2000},// Wait for CP cal to complete, Max TXCP Cal time: 600.000 (us)(Done when 0x284[7]==1)
//************************************************************
// FDD RX/TX Synth Frequency: 834.000000,879.000000 MHz
//************************************************************
//************************************************************
// Setup Synthesizer
//************************************************************
{SPIWR,0x23A,0x4A},// Set VCO Output level[3:0]
{SPIWR,0x239,0xC1},// Set Init ALC Value[3:0] and VCO Varactor[3:0]
...
{SPIWR,0x251,0x0E},// Set VCO Varactor Reference[3:0]
{SPIWR,0x250,0x70},// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
{SPIWR,0x23B,0x9D},// Set Synth Loop Filter charge pump current (Icp)
{SPIWR,0x23E,0xF4},// Set Synth Loop Filter C2 and C1
{SPIWR,0x23F,0xDF},// Set Synth Loop Filter  R1 and C3
{SPIWR,0x240,0x0A},// Set Synth Loop Filter R3
//************************************************************
// Setup Synthesizer
//************************************************************
{SPIWR,0x27A,0x4A},// Set VCO Output level[3:0]
{SPIWR,0x279,0xC1},// Set Init ALC Value[3:0] and VCO Varactor[3:0]
{SPIWR,0x282,0x17},// Set VCO Bias Tcf[1:0] and VCO Bias Ref[2:0]
{SPIWR,0x278,0x78},// Set VCO Cal Offset[3:0]
{SPIWR,0x285,0x00},// Set VCO Cal Ref Tcf[2:0]
{SPIWR,0x291,0x0E},// Set VCO Varactor Reference[3:0]
{SPIWR,0x290,0x70},// Set VCO Varactor Ref Tcf[2:0] and VCO Varactor Offset[3:0]
{SPIWR,0x27B,0x9A},// Set Synth Loop Filter charge pump current (Icp)
{SPIWR,0x27E,0xF4},// Set Synth Loop Filter C2 and C1
{SPIWR,0x27F,0xDF},// Set Synth Loop Filter  R1 and C3
{SPIWR,0x280,0x0A},// Set Synth Loop Filter R3
{SPIWR,0x233,0xF7},// Write Rx Synth Fractional Freq Word[7:0]
{SPIWR,0x234,0xFF},// Write Rx Synth Fractional Freq Word[15:8]
{SPIWR,0x235,0x4B},// Write Rx Synth Fractional Freq Word[22:16]
{SPIWR,0x232,0x00},// Write Rx Synth Integer Freq Word[10:8]
{SPIWR,0x231,0x6C},// Write Rx Synth Integer Freq Word[7:0]
{SPIWR,0x005,0x22},// Set LO divider setting
{SPIWR,0x273,0xF9},// Write Tx Synth Fractional Freq Word[7:0]
{SPIWR,0x274,0xFF},// Write Tx Synth Fractional Freq Word[15:8]
{SPIWR,0x275,0x39},// Write Tx Synth Fractional Freq Word[22:16]
{SPIWR,0x272,0x00},// Write Tx Synth Integer Freq Word[10:8]
{SPIWR,0x271,0x72},// Write Tx Synth Integer Freq Word[7:0] (starts VCO cal)
{SPIWR,0x005,0x22},// Set LO divider setting
...
{SPIWR,0x100,0x8E},// Max Digital Gain
...
{SPIWR,0x106,0x25},// ADC Overload Step Sizes
{SPIWR,0x107,0x3F},// Large LMT Overload Threshold
{SPIWR,0x108,0x1F},// Small LMT Overload Threshold
{SPIWR,0x111,0x0A},// Settling Delay
{SPIWR,0x11A,0x1C},// Initial LMT Gain Limit
{SPIWR,0x120,0x12},// Prevent Inc & AGC Inner Low Thresh
{SPIWR,0x121,0x44},// LMT Overload Exceeded Counters
{SPIWR,0x122,0x44},// ADC Overload Exceeded Counters
{SPIWR,0x123,0x99},// AGC Inner High & Low Step Sizes
{SPIWR,0x124,0x00},// Gain Update Counter<7:0>
{SPIWR,0x125,0x04},// Gain Update Counter<15:8>
{SPIWR,0x128,0x03},// Digital Sat Exceeded Counter
{SPIWR,0x129,0x55},// AGC Outer High & Low Thresholds
...
{WCALDONE,RXFILTER,2000},// Wait for RX filter to tune, Max Cal Time: 11.169 us (Done when 0x016[7]==0)
...
{WCALDONE,TXFILTER,2000},// Wait for TX filter to tune, Max Cal Time: 5.778 us (Done when 0x016[6]==0)
...
{SPIWR,0x0A1,0x7B},// Set TxQuadcal M[1:0]
{SPIWR,0x0A9,0xFF},// Set Tx Quad Cal Count
{SPIWR,0x0A2,0x7F},// Set Tx Quad Cal Kexp
{SPIWR,0x0A5,0x01},// Set Tx Quad Cal Magnitude Threshhold
{SPIWR,0x0A6,0x01},// Set Tx Quad Cal Magnitude Threshhold
{SPIWR,0x0AA,0x25},// Set Tx Quad Cal Gain Table index
{SPIWR,0x0A4,0xF0},// Set Tx Quad Cal Setting Count
{SPIWR,0x0AE,0x00},// Set Tx Quad Cal LPF Gain index incase Split table mode used
...
{WCALDONE,BBDC,2000},// BBDC Max Cal Time: 13151.042 us. Cal done when 0x016[0]==0
{SPIWR,0x185,0x20},// Set RF DC offset Wait Count
{SPIWR,0x186,0x32},// Set RF DC Offset Count[7:0]
{SPIWR,0x187,0x24},// Settings for RF DC cal
{SPIWR,0x18B,0x83},// Settings for RF DC cal
{SPIWR,0x188,0x05},// Settings for RF DC cal
{SPIWR,0x189,0x30},// Settings for RF DC cal
{SPIWR,0x016,0x02},// RFDC Max Cal Time: 59923.828 us
{WCALDONE,RFDC,2000},// Wait for cal to complete (Done when 0x016[1]==0)
{SPIWR,0x016,0x10},// TXQuad Max Cal Time: 6150.000 us
{WCALDONE,TXQUAD,2000},// Wait for cal to complete (Done when 0x016[4]==0)
...
//************************************************************
{SPIWR,0x150,0x0E},// RSSI Measurement Duration 0, 1
...
{SPIWR,0x002,0xDE},// Set # transmitters enabled
{SPIWR,0x003,0xDE},// Set # receivers enabled
