...
#ifdef MSPD
   MSPD_DBG("Received LRG config request\n");
#endif
...
#endif    
{
   ...
   #ifdef MSPD
   MSPD_DBG("\n RgMiLrgSchL2MeasReq Recevied with TransId=[%d]",measInfo->hdr.transId);
   #endif
...
#endif
{
   ...
   {
      /* Handle Cell fetch failure */
      RGSCHLOGERROR(inst, ERRCLS_INT_PAR,ERG009,(ErrVal)ueDelInd->cellId,
                            "rgMacSchUeDelInd(): Invalid cell Id");
      RETVALUE(ROK);
   }
#endif

   /* Update Bo in the given logical channel of the UE */
   if ((ue = rgSCHDbmGetUeCb(cell, ueDelInd->rnti)) != NULLP)
   {
      /*UE is deleted at MAC, so no more scheduling for that UE*/
      MSPD_DBG("UE %d deleted at MAC. Marking Inactive in SCH\n", ueDelInd->rnti);
      ue->dl.dlInactvMask |= RG_MACUEDEL_INACTIVE;
      ue->ul.ulInactvMask |= RG_MACUEDEL_INACTIVE;
	   
   }
   else
   {
      /*MS_FIX:UE not present in Scheduler, RNTI should be in the Guard Pool*/
      MSPD_DBG("UE %d deleted at MAC, not present in SCH\n", ueDelInd->rnti);
	  ...
	  while(tmp)
	  {
		 ...
		 if(rntiLnk->rnti == ueDelInd->rnti)
		 {
		    ...
		    MSPD_DBG("\n Released RNTI %d from the Guard pool", ueDelInd->rnti);
		    break;
		 }
	  ...
	  if(tmp == NULLP)
	  {
	     MSPD_ERR("RNTI %d not present in the Guard Pool", ueDelInd->rnti);
	  }
