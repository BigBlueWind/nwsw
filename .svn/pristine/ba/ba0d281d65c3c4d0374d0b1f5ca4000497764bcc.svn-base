...
/*rg002.301 ccpu00118423  MOD added extra input param cell */
...
PRIVATE S8 rgSCHPwrGetDeltaFrmAbsTpc ARGS((
         U8                    tpc
         ));
...
/* rg009.201. Added changes of TFU_UPGRADE */
   ...
   S8                  availPwr;
   U8                  maxRb;
#ifdef PWAV /* Temporary change for ULLA analysis */
   U8                  minRBforScheduling = 6;
#endif
...
#ifdef MSPD
	  if (uePwr->phVal < 0)
	  MSPD_DBG("Ph Val %d, AvailPwr %d\n", uePwr->phVal, availPwr);
#endif
   }
   ...
   /*rg002.301 ccpu00118423 MOD modified func call according to new definition*/
   maxRb = rgSCHPwrGetMaxRb(cell,availPwr);
#ifdef MSPD
#ifdef PWAV /* Temporary change for ULLA analysis */
   if (maxRb < 50)
   MSPD_DBG("Max Rb (%d) phVal (%d) AvailPwr (%d)\n", 
   maxRb, uePwr->phVal, availPwr);
   if(minRBforScheduling >= maxRb)
     maxRb = minRBforScheduling;
#endif
   if (maxRb < 50)
   MSPD_DBG("Max Rb (%d) phVal (%d) AvailPwr (%d)\n", 
   maxRb, uePwr->phVal, availPwr);
...
#endif
{
   ...
   RETVALUE((trgCqi - crntCqi) * RG_SCH_UL_CQI_DB_STEP_2);
...
#endif
{
   /*rg002.301 ccpu00118423 ADD added new param cellUl*/
   ...
   if (pwr <= 0)
   {
      /*rg002.301 - Del - [ccpu00118788] give 4 RBS so that UE can report changed power status*/
   ...
   /*rg002.301 ccpu00118423 MOD replaced #def for DLBw with configured val*/
   if (pwr > rgSchPwrRbToPwrTbl[cellUl->maxUlBwPerUe])
   {
      RETVALUE(cellUl->maxUlBwPerUe);
   }
   /*rg002.301 ccpu00118423 cap rbs to maximum available RBs */
...
#endif
{
   ...
   /* rg005.201 removed unused linked list */
   CmLListCp             *lst;
   CmLList               *lnk;
   RgSchPdcch            *pdcch;
...
#ifdef MSPD
#ifndef PWAV
		 MSPD_DBG("<GRP_PWR>PUCCH Grp power TFU_DCI_FORMAT_3A: ue (%u) at idx (%u) tpc (%u)\n",
			ue->ueId, uePwr->pucchIdx, tpc);
#endif
#endif
         atleastOne = TRUE;
...
#ifdef MSPD
#ifndef PWAV
		 MSPD_DBG("<GRP_PWR>PUCCH Grp power TFU_DCI_FORMAT_3: ue (%u) at idx (%u) tpc (%u)\n",
		 	ue->ueId, uePwr->pucchIdx, tpc);
#endif
#endif
         atleastOne = TRUE;
...
#ifdef MSPD
#ifndef PWAV
       MSPD_DBG("<GRP_PWR>TPC-RNTI Cb (%u) removed from tobeScheduled list\n", cb->tpcRnti);
#endif
#endif
...
#ifdef MSPD
#ifndef PWAV
   MSPD_DBG("<GRP_PWR>Rem PUCCH power after group power control(ueId %u) %d rmvUe %u\n",  
   	ue->ueId, uePwr->remPucchPwr, rmvUe);
#endif
#endif
   if (rmvUe)
   {
...
#ifdef MSPD
#ifndef PWAV
   MSPD_DBG("PUCCH power Delta %d ue %u\n", uePwr->remPucchPwr, ue->ueId);
#endif
#endif
...
#ifdef MSPD
   if (uePwr->phVal < 0)
   MSPD_DBG("Reported PHR %d MaxUePwr %d for cqi%u allocRb %u\n", 
   	uePwr->phVal, uePwr->maxUePwr, allocInfo->cqi, allocInfo->numRb);
#endif
   
/*rg002.301 ccpu00118423 MOD modified function call according to definition*/
...
/* rg009.201. Added changes of TFU_UPGRADE */
#ifdef TFU_UPGRADE
      rgSCHPwrGetDelta2FrmCqi(ueUl->validUlCqi, uePwr->trgCqi) / 2;
#else
      /* rg007.201 - Changes for MIMO feature addition */
      /* rg008.201 - Removed dependncy on MIMO compile-time flag */
...
#ifdef MSPD
#ifndef PWAV
    MSPD_DBG("Cell-wide Group Power Config pucchFmt3(%u %u) pucchFmt3A(%u %u)"
    "puschFmt3(%u %u) puschFmt3a(%u %u)\n", 
   	cfg->pwrCfg.pucchPwrFmt3.startTpcRnti, cfg->pwrCfg.pucchPwrFmt3.size,
   		cfg->pwrCfg.pucchPwrFmt3a.startTpcRnti, cfg->pwrCfg.pucchPwrFmt3a.size,
   	cfg->pwrCfg.puschPwrFmt3.startTpcRnti, cfg->pwrCfg.puschPwrFmt3.size,
   		cfg->pwrCfg.puschPwrFmt3a.startTpcRnti, cfg->pwrCfg.puschPwrFmt3a.size);
#endif
#endif
...
#ifdef MSPD
#ifndef PWAV
	   MSPD_DBG("<GRP_PWR>PucchRntiCb cfgdUes(%u %0x %0x) isFmt3a(%u) ueNode(%u) "
	   	"toBeSchdUes(%u %0x %0x) tpcRnti(%u)\n",
	  	pucchRntiCb->cfgdUes.count,pucchRntiCb->cfgdUes.first, pucchRntiCb->cfgdUes.last,
	  	pucchRntiCb->isFmt3a, pucchRntiCb->schdLnk.node, 
	  	pucchRntiCb->toBeSchdUes.count, pucchRntiCb->toBeSchdUes.first, pucchRntiCb->toBeSchdUes.last,
	  	pucchRntiCb->tpcRnti);
#endif
#endif
   }
...
#ifdef MSPD
#ifndef PWAV
   MSPD_DBG("Power Configuration Ue(%u) isAccumulated(%u) deltaMcsEnbld(%u)"
   	"trgCqi(%u) p0UePusch(%u) p0UePucch(%u) grpPwrPucch(%u %u %u) grpPwrPusch(%u %u %u)\n", 
   	ue->ueId, uePwr->isAccumulated, uePwr->deltaMcsEnbld, uePwr->trgCqi,
   	uePwr->p0UePusch, uePwr->p0UePucch,
...
#ifdef MSPD
#ifndef PWAV
   MSPD_DBG("<GRP_PWR>Added UE(%u %0x) for scheduling (%u %0x %0x) intoCb(%0x)\n",
   	ue->ueId, ue, cb->toBeSchdUes.count, cb->toBeSchdUes.first, cb->toBeSchdUes.last,
   	cb);
#endif
#endif
...
#ifdef MSPD
#ifndef PWAV
   MSPD_DBG("<GRP_PWR>Added tpcRntiCb(%0x,%u) for scheduling into cellLst(%u %0x %0x)\n",
   	cb, cb->tpcRnti, cellPwr->pucchGrpPwr.count, cellPwr->pucchGrpPwr.first, cellPwr->pucchGrpPwr.last);
#endif
#endif
   RETVOID;
}
...
/***********************************************************
 *
 *     Func : rgSCHPwrGetDeltaFrmAbsTpc
 *
 *     Desc : Get delta corresponding to given
 *            absolute TPC command
 *
 *     Ret  : S8
 *
 *     Notes:
 ...
 **********************************************************/
#ifdef ANSI
PRIVATE S8 rgSCHPwrGetDeltaFrmAbsTpc
(
U8                    tpc
)
#else
PRIVATE S8 rgSCHPwrGetDeltaFrmAbsTpc(tpc)
U8                    tpc;
#endif
{
   S8           deltas[4] = {-4, -1, 1, 4};
   TRC2(rgSCHPwrGetDeltaFrmAbsTpc);
