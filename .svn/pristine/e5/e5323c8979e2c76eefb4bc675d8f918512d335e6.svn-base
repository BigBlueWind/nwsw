...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConSetupHdlr called.\n"));
   ...
   nhUTLGetRrcTransId((Void *) & (datRspSdu->sdu.m.dlCcchMsg), C3_RRCCONSETUP, 
   ...
   if(TRUE == datRspSdu->sdu.isUeCfgPres)
   {
      ...
      else
      {
         ...
         if (ROK != retVal)
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "Cannot create UE control block.\n"));
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "Unable to create transaction control block.\n"));
      ...
      if (ROK != retVal)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"nhUTLEncAndStoreMsg failed.\n"));
         NH_CCM_DEL_UECB_TRANSCB(isTransCbCreated, &transCb, isUeCbCreated,
               cellCb, &ueCb);
         RETVALUE(retVal);
      }
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "Cannot perform lower layer config.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConRstbRejHdlr called.\n"));
   ...
   if(datRspSdu->hdr.ueId > 0)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                       "UE Control block does not exist.\n"));
         NH_DBG_ERROR((NH_PRNT_BUF,
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                        "nhCGMCnclUe failed for CRNTI: %d and CellId: %d.\n",
                        datReqParam.nhId.ueId, datReqParam.nhId.cellId));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConRejHdlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConSetupCmpHndlr called.\n"));
   ...
   if (ROK != retVal)
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhUIMDatInd failed.\n"));
      RETVALUE(retVal);
   }
...
#endif/* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConRlsHndlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConRlsCmpHndlr called.\n"));
   ...
   if(NULLP == transCb) 
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"Unable to retrieve transCb for UE ID:%d \n",
                     cellAndUeCb->ueCb->ueCbKey.crnti));
      NH_DBG_ERROR((NH_PRNT_BUF,"with transId: %d.\n", msgTransId));
      RETVALUE(RFAILED);
   }
   ...
   if(ROK != retVal)
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhCGMCnclUe failed.\n"));
      RETVALUE(retVal);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMReCfgReqHndlr called.\n"));
         ...
         datReqSdu->sdu.ueCfg.conReCfgCause & NHU_CONN_RECFG_FOR_MESR_CFG))
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "Message Cannot be sent unprotected.\n"));
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG, 
   ...
   { 
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "SRB2 is not establisged.\n"));
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG, 
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "Atleast one DRB is not established.\n"));
         NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG, 
   ...
   if (TRUE == datReqSdu->sdu.isUeCfgPres)
    {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                       "Cannot send lower layer configuration.\n"));
         NH_CCM_DEL_UECB_TRANSCB(isTransCbCreated, &transCb, FALSE,
                                 NULLP, &(cellAndUeCb->ueCb));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMReCfgCmpHndlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMSecModCmdHndlr called.\n"));
   ...
   if (TRUE == datReqSdu->sdu.isUeCfgPres)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "Could not store lower layer config.\n"));
         RETVALUE(retVal);
      }
      ...
      else
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
               "Security not configured, Can't send Sec Mode Command\n"));
...
#endif/* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMSecModCmpHndlr called.\n"));
   ...
   if(NULLP == transCb)
   {
      ...
      if (ROK != retVal)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"nhCCMSecModCmpHndlr: NULL transCb found .\n"));
         RETVALUE(RFAILED);
      }
   ...
   if(ROK != retVal)
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"DiscReqd cfg request failed .\n"));
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMSecModFailHndlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMUlCmnHndlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMCountChkHndlr called.\n"));
   ...
   if(ROK != retVal)
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"nhCCMCountChkHndlr: nhUTLEncAndSnd Failed.\n"));
      NH_CCM_DEL_UECB_TRANSCB(isTransCbCreated, &transCb, FALSE,
            NULLP, &(cellAndUeCb->ueCb));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMUlDedMsgHndlr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCCMConRestbHdlr called.\n"));
   ...
   /* Get the message transaction key */
   nhUTLGetRrcTransId((Void *) & (datRspSdu->sdu.m.dlCcchMsg), C3_RRCCONREESTB, 
                    NH_SDU_DL_CCCH, &msgTransId);
   NH_GET_TRANSCB_KEY(msgTransKey, msgType, msgTransId);
   ...
   if(TRUE == datRspSdu->sdu.isUeCfgPres)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "Cannot store lower layer config.\n"));
         /* Delete UE and Trans CB only if Lower Layer Config is not sent */
         if((!transCb->numOfMacCfms) && (!transCb->numOfRlcCfms) &&
               (!transCb->numOfPdcpCfms))
         {
   ...
   else
   {
      /*Check if RRC user has issued config requuest before sending the message*/
       ...
       if(ROK != retVal)
       {
          NH_DBG_ERROR((NH_PRNT_BUF,"nhCCMConRestbHdlr: nhUTLEncAndSnd Failed.\n"));
          NH_CCM_DEL_UECB_TRANSCB(isTransCbCreated, &transCb, FALSE,
                NULLP, &(cellAndUeCb->ueCb));
