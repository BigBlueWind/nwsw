...
RESULTCODE OsalWaitForTaskClose(OSAL_TASK *pTask, OSAL_DELAYU timeout, char forceClose);
...
RESULTCODE OsalCreateHeap(OSAL_HEAP *pHeap, OSAL_PADDRESS pHeapMemory,
                          OSAL_SIZE HeapSize, char CacheManagementNeeded);
...
RESULTCODE OsalCyclicBuffPutData_spinlock(OSAL_QUEUE *pQueue, U8* buf_in, U32 wr_size);
RESULTCODE OsalCyclicBuffGetData_spinlock(OSAL_QUEUE *pQueue, U8* buf_out, U32 rd_size);
U32 OsalCyclicBuffGetFullSize_spinlock(OSAL_QUEUE *pQueue);
U32 OsalCyclicBuffGetFreeSize_spinlock(OSAL_QUEUE *pQueue);
...
RESULTCODE OsalStartTimer(OSAL_TIMER *pTimer, OSAL_DELAYU delayMicroSeconds,
                          OSAL_SEMAPHORE *pSemaphore, char autoRestart);
