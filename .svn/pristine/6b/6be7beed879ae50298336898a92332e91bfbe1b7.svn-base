...
U32 dldrops_kwu_um, dlsdudrops_pju, dlpkt_um, buffer_occ, ulCqiRptCnt[16], dlCqiRptCnt[16];
U32 totUlCqiRpt, totDlCqiRpt;
U32 ysTotalPkts, ysPktsDropped, ysPktsDrpdMem;
U32 ysMsDlAmTxPdus = 0, ysMsDlAmRetxPdus = 0;

MsgTfuPmhDStatsInd statusInd;

void ysSendDstatsIndToPmh (void)
{
   UINT32 index = 0;
   SupervisorCtx *pCtx= &SvsrCtx [3];
   #define STAT_PERIOD  1
   #define to_kb(x) (((x)*8)/(1000*STAT_PERIOD))
	 statusInd.counterList[index].id    = CTRID_MAC_DL_RATE;
	 statusInd.counterList[index++].val = to_kb(dlrate_rgu);

	 statusInd.counterList[index].id    = CTRID_MAC_UL_RATE;
	 statusInd.counterList[index++].val = to_kb(ulrate_rgu);

	 statusInd.counterList[index].id    = CTRID_RLC_DL_RATE;
	 statusInd.counterList[index++].val = to_kb(dlrate_kwu);

	 statusInd.counterList[index].id    = CTRID_RLC_UL_RATE;
	 statusInd.counterList[index++].val = to_kb(ulrate_kwu);

	 statusInd.counterList[index].id    = CTRID_PDCP_DL_RATE;
	 statusInd.counterList[index++].val =to_kb(dlrate_pju);

	 statusInd.counterList[index].id    = CTRID_PDCP_UL_RATE;
	 statusInd.counterList[index++].val =to_kb(ulrate_pju);

	 statusInd.counterList[index].id    = CTRID_RLC_NUM_SDU_DROP;
	 statusInd.counterList[index++].val = dldrops_kwu_um;
	 
	 statusInd.counterList[index].id    = CTRID_ICPU_NUM_PKTS_RX;
	 statusInd.counterList[index++].val = pCtx->ICpuStat.RxMsgNum;

	 statusInd.counterList[index].id    = CTRID_ICPU_NUM_PKTS_TX;
	 statusInd.counterList[index++].val =  pCtx->ICpuStat.TxMsgNum;

	 statusInd.counterList[index].id    = CTRID_TFU_DL_RATE;
	 statusInd.counterList[index++].val = to_kb(dlrate_tfu);

	 statusInd.counterList[index].id    = CTRID_TFU_UL_RATE;
	 statusInd.counterList[index++].val =  to_kb(ulrate_tfu);

	 statusInd.counterList[index].id    = CTRID_PDCP_NUM_SDU_DROP;
	 statusInd.counterList[index++].val = dlsdudrops_pju;

   statusInd.n = index;
   if (RFAILED == sendMsgToFrm (ENTTF, ENT_PMH, EVT_TFU_PMH_DSTATS_IND, sizeof(MsgTfuPmhDStatsInd), (U8 *)&statusInd))
   {
     RB_PRINTF("%s","sendMsgToFrm failed Event: EVT_TFU_PMH_DSTATS_IND\n");
   }
...
#ifdef PWAV
   if ((ysCb.dStatsCfg.cmd == STCMD_START) && (ysCb.dStatsCfg.interval_ms != 0) && ( ysCb.dStatsCfg.interval_ms <= count))
   {
     ysSendDstatsIndToPmh();
     count = 0;
   }
   count++;
#endif
