...
#include "ss_diag.h"        /*LOG Dependent*/
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlLSapBndCfm \n"));
   ...
   switch(sapId)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF, "Invalid sapId suId (%d) \n", sapId));
         RETVALUE(RFAILED);
   }  /* end of switch */

   if( ROK != (ret = nhSchedGenTmr(lSapCb, event, NH_TMR_STOP)))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "stop timer failed for event (%d) \n", event));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCkwBndCfm: suId(%d), status(%d)\n", 
                             suId, status));

   ckwSapCb = nhUTLFindLlyrSapFrmSuId(STNHCKWSAP, suId);
  NH_DIAG_LVL0(0x060d0001, NH_DIAG_SPID, ckwSapCb->spId,
            "Received BndCfm from RLC for control SAP suId = %d sap state = %d",
                  suId, ckwSapCb->state,0,0);

#if (ERRCLASS & ERRCLS_INT_PAR)
   if (ckwSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiCkwBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Sap state (%d) \n",ckwSapCb->state));
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiKwuBndCfm: suId(%d), status(%d)\n", 
                             suId, status));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiKwuBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Sap state \n"));
      NH_SET_ZERO (&dgn, sizeof(dgn));
      dgn.type = LNH_USTA_DGNVAL_SAP;
  ...
  NH_DIAG_LVL0(0x060d0002, NH_DIAG_SPID, kwuSapCb->spId,
              "Received BndCfm from RLC for data SAP suId = %d sap status = %d",
                                           suId, kwuSapCb->state,0,0);
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCrgBndCfm: suId(%d), status(%d)\n", 
                             suId, status));

   crgSapCb = nhUTLFindLlyrSapFrmSuId(STNHCRGSAP,suId);

  NH_DIAG_LVL0(0x060d0003, NH_DIAG_SUID, suId,
                    "Received BndCfm from MAC suId = %d sap status = %d",
                                     suId,status,0,0);
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiCrgBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjBndCfm: suId(%d), status(%d)\n", 
                             suId, status));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiCpjBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
  ...
  NH_DIAG_LVL0(0x060d0004, NH_DIAG_NA, SS_DIAG_INV_ARG,
               "RRC Recived Sap Bind Cfm from PDCP suId = %d, status = %d",
               suId, status,0,0);           
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiPjuBndCfm: suId(%d), status(%d)\n", 
                             suId, status));
  NH_DIAG_LVL0(0x060d0005, NH_DIAG_NA, SS_DIAG_INV_ARG,
               "Received BndCfm from PDCP suId = %d, sap status = %d",
               suId, status,0,0);
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiPjuBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCtfBndCfm: suId(%d), status(%d)\n", 
                             suId, status));

  NH_DIAG_LVL0(0x060d0006, NH_DIAG_SUID, suId,
                     "Received BndCfm from PHY suId = %d, sap status = %d",
                                                 suId,kwuSapCb->state,0,0);
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "NhLiCtfBndCfm: Invalid suId from lower layer \n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiKwuDatInd: suId(%d) \n", suId));

   /* Error Check - check for null pointer passed by RLC */
   if (NH_CHK_INPUT_PARAM(mBuf))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL mBuf\n"));
      if (!(NH_CHK_INPUT_PARAM(diParam)))
      {
         NH_FREE(pst->region, pst->pool, diParam, sizeof(KwuDatIndInfo));
      }
      RETVALUE(RFAILED);
   }
   if (NH_CHK_INPUT_PARAM(diParam))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL diParam\n"));
      NH_FREEMBUF(mBuf);
      RETVALUE(RFAILED);
   }
   ...
   if (kwuSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiKwuDatInd\n"));
...
#endif /* CCPU_OPT */
  NH_DIAG_LVL0(0x060d0007, NH_DIAG_UEID, nhIdentity.ueId,
                "Received DatInd from RLC suId = %d, rbId = %d",
                                suId, nhIdentity.rbId, 0, 0);
   
   
   /* Call common datInd handler. Same handler is called for PJU datInd */
   if (ROK != nhLIMHndlDatInd(&nhIdentity, mBuf, tCrnti))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlDatInd Failed \n"));
      NH_FREEMBUF(mBuf);
      NH_FREE(pst->region, pst->pool, diParam, sizeof(KwuDatIndInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, " NhLiPjuDatInd: suId(%d), pdcpId(%d) \n", suId,
                pdcpId->rbId));
   ...
   if (NH_CHK_INPUT_PARAM(mBuf))
   {
      ...
      {
         NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      }
      RETVALUE(RFAILED);
   }
   if (NH_CHK_INPUT_PARAM(pdcpId))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL pdcpId\n"));
      NH_FREEMBUF(mBuf);
      RETVALUE(RFAILED);
   }
   ...
   if (pjuSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiPjuDatInd\n"));
      NH_FREEMBUF(mBuf);
      NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      RETVALUE(RFAILED);
   }
  ...
  NH_DIAG_LVL0(0x060d0008, NH_DIAG_UEID, nhIdentity.ueId,
               "Received DatInd from PDCP suId = %d, pdcp UeId = %d",
               suId, pdcpId->ueId,0,0);          
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, " NhLiPjuDatCfm: suId(%d) status(%d)\n", 
                             suId, datCfm->cfmSta[0].status));


   if (NH_CHK_INPUT_PARAM(datCfm))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL datCfm\n"));
      if (!(NH_CHK_INPUT_PARAM(pdcpId)))
      {
         NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      }
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL pdcpId\n"));
      NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
      RETVALUE(RFAILED);
   }
   ...
   if (datCfm->numSdus == 0)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid interface param\n"));
      NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
      RETVALUE(RFAILED); 
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "NULL CELL CB \n"));
      NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
      RETVALUE(RFAILED); 
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "NULL UE CB \n"));
      NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
      RETVALUE(RFAILED); 
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "mui doesnt exist \n"));
      NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
      NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
      RETVALUE(RFAILED);
   }
  ...
  NH_DIAG_LVL0(0x060d0009, NH_DIAG_CRNTI, ueCb->ueCbKey.crnti,
          "Received DatCfm from PDCP suId = %d, status = %d",
          suId, datCfm->cfmSta[0].status,0,0);
   /*
    ...
    */
   if (PJU_DAT_CFM_SUCCESS == datCfm->cfmSta[0].status)   /* pls check which status u want to check
                                                      - just used 0 index for compilation */
   {
      /* Data is transmitted by PDCP successfully, remove MUI 
       * entry from muiList in ueCb.
       */
      NH_DBG_DEBUG((NH_PRNT_BUF, 
                  "NhLiPjuDatCfm: status is SUCCESS, deleting mui\n"));
      ...
      if (transCtrlBlk != NULLP) 
      {
         if (transCtrlBlk->cause == NH_MSGTYPE_CONNRLS)
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF, 
                           "Lower state Machine Function Failed \n"));
               NH_FREE(pst->region, pst->pool, pdcpId, sizeof(CmLtePdcpId));
               NH_FREE(pst->region, pst->pool, datCfm, sizeof(PjuDatCfmInfo));
               RETVALUE(RFAILED);
            }
   ...
   else
   {
      ...
      /* pls check which status u want to check - just used 0 index 
       * for compilation */
      NH_MAP_PDCP_DAT_CFM_CAUSE(datCfm->cfmSta[0].status, nhuCause);
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjCfgCfm: suId(%d), \n", suId));

   if (NH_CHK_INPUT_PARAM(pdcpCfgCfm))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL pdcpCfgCfm\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (cpjSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, " INVALID SAP ID in NhLiCpjCfgCfm\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm failed \n"));
      NH_FREE(pst->region, pst->pool, pdcpCfgCfm, sizeof(CpjCfgCfmInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL ueInfo\n"));
      RETVALUE(RFAILED);
   }

   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjUeIdChgCfm: \
                for UeId (%d) and status %d \n", ueInfo->ueId, status.status));
   ...
   if (cpjSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiCpjUeIdChgCfm \n"));
  ...
  NH_DIAG_LVL0(0x060d000a, NH_DIAG_UEID, nhIdentity.ueId,
               "Received UeIdChgCfm from PDCP suId = %d, status = %d",
               suId, status.status,0,0);  
   ...
   if (ROK != nhLIMHndlUeIdChgCfm(&nhIdentity, transId, status, 
                                  NH_PDCP_CONFIGURED))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlUeIdChgCfgCfm failed\n"));
      NH_FREE(pst->region, pst->pool, ueInfo,sizeof(CpjUeInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCtfCfgCfm: suId(%d), status(%d)\n", 
                             suId, status));
  NH_DIAG_LVL0(0x060d000b, NH_DIAG_SUID, suId,
                        "Received CfgCfm from PHY status = %d",
                           status,0,0,0);
   ...
   if (ctfSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NjLiCtfCfgCfm\n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlPhyCfgCfm failed\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
...
#ifndef ALIGN_64BIT   
   NH_DBG_DEBUG((NH_PRNT_BUF, "NhLiCtfUeIdChgCfm: transId (%ld) status(%d)\
            bitmask(%d) \n", nxTransId, status.status, phyActionBitMask));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, "NhLiCtfUeIdChgCfm: transId (%d) status(%d)\
            bitmask(%d) \n", nxTransId, status.status, phyActionBitMask));
#endif /* ALIGN_64BIT */
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCtfUeIdChgCfm: \
                for UeId (%d) and status %d \n", ueInfo->ueId, status.status));

  NH_DIAG_LVL0(0x060d000c, NH_DIAG_UEID, ueInfo->ueId,
             "Received UeIdChgCfm from PHY suId = %d status = %d transId = %d",
                suId, staus.status,nxTransId,0);
   ...
   if (ctfSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiCtfUeIdChgCfm \n"));
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL restbCfm\n"));
      RETVALUE(RFAILED);
   }

   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjReEstCfm: \
        for UeId (%d) and status %d \n", restbCfm->ueId, restbCfm->status));
   ...
   if (cpjSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiCpjReEstCfm \n"));
  ...
  NH_DIAG_LVL0(0x060d000d, NH_DIAG_UEID, nhIdentity.ueId,
                "Received ReEstCfm from PDCP, re-establish Request only for SRB1 suId = %d status = %d",
                suId, restbCfm->status,0,0);
   /* Call LIM Module function */
   if (ROK != nhLIMHndlRestbCfgCfm(&nhIdentity, restbCfm->transId, 
                                   restbCfm->status ))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlRestbCfgCfm failed\n"));
      NH_FREE(pst->region, pst->pool, restbCfm,sizeof(CpjReEstCfmInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjSecCfgCfm: suId(%d), \n", suId));

   if (NH_CHK_INPUT_PARAM(secCfgCfm))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL secCfgCfm\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlPdcpSecCfgCfm failed \n"));
      NH_FREE(pst->region, pst->pool, secCfgCfm, sizeof(CpjSecCfgCfmInfo));
      RETVALUE(RFAILED);
   }

  NH_DIAG_LVL0(0x060d000e, NH_DIAG_NA, SS_DIAG_INV_ARG,
                "Received SecCfgCfm from PDCP suId = %d, status = %d",
                                        suId, 0,0,0);
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL countCfmInfo\n"));
      RETVALUE(RFAILED);
   }

   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjCountCfm: \
                for UeId (%d) and status %d \n", countCfmInfo->ueId, 
                countCfmInfo->status));

   cpjSapCb = nhUTLFindLlyrSapFrmSuId(STNHCPJSAP, suId);
  NH_DIAG_LVL0(0x060d000f, NH_DIAG_UEID, countCfmInfo->ueId,
                "Received CountCfm, COUNT values of all established DRB from PDCP layer suId = %d",
                                                              suId, 0,0,0);
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "NhLiCpjCountCfm failed\n"));
      NH_FREE(pst->region, pst->pool, countCfmInfo,sizeof(CpjCountCfmInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCpjSduStaCfm: suId(%d), \n", suId));

   if (NH_CHK_INPUT_PARAM(cpjSduStaCfm))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cpjSduStaCfm\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (cpjSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, " INVALID SAP ID in NhLiCpjSduStaCfm\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "transId doesnt exist \n"));
      nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Pdcp Cfg Cfm Rcvd \n"));
      RETVALUE(ROK);
   }
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF,"NhuPdcpSduStaCfm memory allocation\
                    failed.\n"));
      nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMRptCfgCfm failed \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCkwCfgCfm: \n"));
  NH_DIAG_LVL0(0x060d0010, NH_DIAG_SUID, suId,
                       "Received CfgCfm from RLC",0,0,0,0);
   

   if (NH_CHK_INPUT_PARAM(cfmInfo))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cfmInfo\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (ckwSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiCkwCfgCfm\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm failed\n"));
      NH_FREE(pst->region, pst->pool, cfmInfo, sizeof(CkwCfgCfmInfo));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCkwUeIdChgCfm: \
                for UeId (%d) and status %d \n", ueInfo->ueId, status.status));
   ...
   if (ckwSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in NhLiCkwUeIdChgCfm \n"));
   ...
   nhIdentity.ueId  = ueInfo->ueId;
   nhIdentity.cellId = ueInfo->cellId;
  NH_DIAG_LVL0(0x060d0011, NH_DIAG_UEID, nhIdentity.ueId,
                    "Received UeIdChgCfm from RLC suId = %d status = %d",
                                              suId, 0,0,0);
   ...
   if (ROK != nhLIMHndlUeIdChgCfm(&nhIdentity, transId, status, 
                                  NH_RLC_CONFIGURED))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlUeIdChgCfm failed\n"));
      NH_FREE(pst->region, pst->pool, ueInfo,sizeof(CkwUeInfo));
      RETVALUE(RFAILED);
   }
   NH_FREE(pst->region, pst->pool, ueInfo,sizeof(CkwUeInfo));
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, "NhLiCrgCfgCfm: suId(%d), status(%d)\n", 
                             suId, status));

   crgSapCb = nhUTLFindLlyrSapFrmSuId(STNHCRGSAP, suId);

  NH_DIAG_LVL0(0x060d0012, NH_DIAG_SUID, suId,
                "Received CfgCfm from MAC status = %d",
                                        status,0,0,0);

#if (ERRCLASS & ERRCLS_INT_PAR)
   if (crgSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "SAP CHECK Failed in RrLiCmkCfgCfm\n"));
      RETVALUE(RFAILED);
   }
#endif /* (ERRCLASS & ERRCLS_INT_PAR) */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm failed\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRvrtCfg \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_ERROR((NH_PRNT_BUF, "Decoding failed.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlStatusInd \n"));
   ...
   if (NULLP == cellCb)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, " NULL cellCb \n"));
      RETVALUE(RFAILED);
   }
   ...
   */
   if (staInd->cause == PJU_INTVER_FAILED)
   {
      ...
      NH_DBG_DEBUG((NH_PRNT_BUF, 
            "nhLIMHndlStatusInd: UE MSG RCVD: event is (%d)\n ", nhEvnt));
      if(nhMsgType == NH_MSGTYPE_SECMODFAIL)
      {
                     ...
                     ulDcchPdu->message.val.c1.choice.val))
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF, 
                           "nhLIMHndlStatusInd: Send to UIM Failed \n"));
               NH_FREEEVNT(datIndSdu);
               RETVALUE(RFAILED);
            }
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                "nhLIMHndlStatusInd: nhCCMSecModFailHndlr failed \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlDatInd \n"));
   ...
   if (NULLP == datIndSdu)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "nhLIMHndlDatInd: cmAllocEvnt failed \n"));
      RETVALUE(RFAILED);
   }
   ...
   if (NULLP == cellCb)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlDatInd: NULL cellCb \n"));
      NH_FREEEVNT(datIndSdu);
      RETVALUE(RFAILED);
   }

   NH_DBG_PARAM((NH_PRNT_BUF, "nhLIMHndlDatInd: cellId(%d)\n", 
                              nhIdentity->cellId));

   if (TRUE == nhIdentity->isPdcpId)
   {
      NH_DBG_PARAM((NH_PRNT_BUF, "nhLIMHndlDatInd: ueId(%d)\n", 
                                 nhIdentity->ueId));
      ...
      if (NULLP == ueCb)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlDatInd: NULL ueCb \n"));
         NH_FREEEVNT(datIndSdu);
         RETVALUE(RFAILED);
      }
      datIndSdu->hdr.ueId = nhIdentity->ueId;
   ...
   if (datIndSdu->sdu.msgCategory == NHU_MSG_ULDCCH)
   {
                                  ...
                                  ulDcchPdu->message.val.c1.choice.val))
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, 
                        "nhLIMHndlDatInd: Send to UIM Failed \n"));
            NH_FREEEVNT(datIndSdu);
            RETVALUE(RFAILED);
         }
         RETVALUE(RFAILED); 
      ...
      nhUTLGetMsgTypeAndEvntFrmSdu (ulDcchPdu->message.val.c1.choice.val, 
                        NH_SDU_UL_DCCH, &nhMsgType, &nhEvnt);
      NH_DBG_DEBUG((NH_PRNT_BUF, 
                  "nhLIMHndlDatInd: UE MSG RCVD: event is (%d)\n ", nhEvnt));
      nhUTLGetRrcTransId((Void*)ulDcchPdu,
                        ulDcchPdu->message.val.c1.choice.val, 
                        NH_SDU_UL_DCCH, &rrcTransId);
   } /* if (datIndSdu->sdu.msgCategory == NHU_MSG_ULDCCH) */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Lower state Machine Function Failed \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCellCfgCfm \n"));
  ...
  NH_DIAG_LVL0(0x060d0013, NH_DIAG_NA, SS_DIAG_INV_ARG,
                        "Received CellCfgCfm from RLC Status = Failed, RAB-ID = %d",
                        rbId, 0, 0, 0);
               }
               break;
            }
   ...
   else
   {
      ...
      NH_DBG_DEBUG((NH_PRNT_BUF, \
         "nhLIMHndlRlcCellCfgCfm: status=%d numOfRlcCfms=%d\n", \
         ...
         (NH_ZERO_VAL == cellTransCb->rlcMapTbl[0].rbId)) 
      {
         ...
         {
  NH_DIAG_LVL0(0x060d0014, NH_DIAG_NA, SS_DIAG_INV_ARG,
                  "Received CellCfgCfm from RLC, Releasing of RAB failed  RAB-ID = %d",
                  cellTransCb->rlcMapTbl[0].rbId, 0, 0, 0);
   ...
   if (cellTransCb->numOfRlcCfms == 0)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMRptCellCfgCfm failed\n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcUECfgCfm \n"));
   ...
   if ((NhActvUeCb*)NULLP == (ueCb = nhDBMGetUeCb(cellCb, cfmInfo->ueId)))
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlRlcUECfgCfm:NULL UE CB \n"));
         RETVALUE(RFAILED); 
      }
   }
    /* Find trans node in list with specified transId value */
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  " nhLIMHndlRlcUECfgCfm: transId doesnt exist \n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Rlc Cfg Cfm Rcvd \n"));
      RETVALUE(ROK);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm failed\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
...
#ifndef ALIGN_64BIT    
   NH_DBG_INFO((NH_PRNT_BUF, " transId(%ld) ueId(%d) cellId      \
                (%d)\n", transId, nhIdentity->ueId, nhIdentity->cellId));
#else /* ALIGN_64BIT */
   NH_DBG_INFO((NH_PRNT_BUF, " transId(%d) ueId(%d) cellId      \
                (%d)\n", transId, nhIdentity->ueId, nhIdentity->cellId));
#endif /* ALIGN_64BIT */

   if (NULLP == (cellCb = nhDBMGetCellCb(nhIdentity->cellId)))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlRestbCfgCfm:NULLP Cell Cb\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                  "nhLIMHndlRestbCfgCfm:transId doesnt exist \n"));
      RETVALUE(RFAILED);
   }
   cfmStatus = statusCfm;
   ...
   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Duplicate Pdcp Cfg Cfm Rcvd \n"));
      RETVALUE(ROK);
   }
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Duplicate Phy Cfg Cfm Rcvd \n"));
      RETVALUE(ROK);
   }
   if (ueMapTbl->status == NH_STATUS_USED) 
   {
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlUeIdChgCfm \n"));
#ifndef ALIGN_64BIT    
   NH_DBG_DEBUG((NH_PRNT_BUF, " transId(%ld) ueId(%d) cellId      \
                (%d)\n", transId, nhIdentity ->ueId, nhIdentity ->cellId));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, " transId(%d) ueId(%d) cellId      \
                (%d)\n", transId, nhIdentity ->ueId, nhIdentity ->cellId));
#endif /* ALIGN_64BIT */

   if (NULLP == (cellCb = nhDBMGetCellCb(nhIdentity ->cellId)))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlUeIdChgCfm:NULLP Cell Cb\n"));
      RETVALUE(RFAILED);
   }

   if ((NhActvUeCb*)NULLP == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity ->ueId)))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlUeIdChgCfm:NULL UE CB \n"));
      RETVALUE(RFAILED); 
   }
    /* Find trans node in list with specified transId value */
   if ((TransCb*)NULLP == 
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  " nhLIMHndlUeIdChgCfm: transId doesnt exist \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm \n"));

#ifndef ALIGN_64BIT   
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm: transId(%ld) ueId(%d) \
         cellId (%d)\n", cfmInfo->transId, cfmInfo->ueId, cfmInfo->cellId));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm: transId(%d) ueId(%d) \
         cellId (%d)\n", cfmInfo->transId, cfmInfo->ueId, cfmInfo->cellId));
#endif /* ALIGN_64BIT */
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:1 \n"));
      /* Raise a critical alarm to layer manager */
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:2 \n"));
            nhSendLmAlarm(LCM_CATEGORY_INTERFACE, LNH_EVENT_RLC_RVRT_FAIL,
                       LCM_CAUSE_UNKNOWN, &dgn);
         }
      }
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:3 \n"));
      RETVALUE(ROK);
   }

   if (NULLP == (cellCb = nhDBMGetCellCb(cfmInfo->cellId)))
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:4 \n"));
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:NULLP Cell Cb\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (cfmInfo->ueId > 0)
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:5 \n"));
      if (ROK != (nhLIMHndlRlcUECfgCfm(cfmInfo, cellCb)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, " nhLIMHndlRlcUECfgCfm: failed \n"));
         RETVALUE(RFAILED);
      }
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRlcCfgCfm:6 \n"));
      if (ROK != (nhLIMHndlRlcCellCfgCfm(cfmInfo, cellCb)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, " nhLIMHndlRlcCellCfgCfm: failed \n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm \n"));
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:1 \n"));
      /* Raise a critical alarm to layer manager */
      for (entIndex = 0; entIndex < pdcpCfgCfm->numEnt && 
                      (entIndex < CPJ_MAX_CFM ); entIndex++)
      {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:2 \n"));
         if (pdcpCfgCfm->cfmEnt[entIndex].status == CPJ_CFG_CFM_NOK)
         {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:3 \n"));
            NH_SET_ZERO (&dgn, sizeof(dgn));
            dgn.type = LNH_USTA_DGNVAL_CELLUEID;
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:4 \n"));
      RETVALUE(ROK);
   }
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:5 \n"));
      NH_DBG_ERROR((NH_PRNT_BUF, "NULL CELL CB \n"));
      RETVALUE(RFAILED); 
   }
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:6 \n"));
      if ((NhActvUeCb*)NULLP == (ueCb = nhDBMGetNewUeCb(cellCb, 
                                        pdcpCfgCfm->ueId)))
      {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:7 \n"));
         NH_DBG_ERROR((NH_PRNT_BUF, "NULL UE CB \n"));
         RETVALUE(RFAILED); 
      }
   }
   ...
   if ((TransCb*)NULLP == (transCtrlBlk = nhUTLGetTransCbFrmNxTransId(ueCb,
                                          pdcpCfgCfm->transId)))
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:8 \n"));
      NH_DBG_ERROR((NH_PRNT_BUF, "transId doesnt exist \n"));
      RETVALUE(RFAILED);
   }
   ...
   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:9 \n"));
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Pdcp Cfg Cfm Rcvd \n"));
      RETVALUE(ROK);
   }
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:10 \n"));
      NH_DBG_DEBUG((NH_PRNT_BUF, "Confirm received for Discard Required\
                    modification\n"));
      transCtrlBlk->numOfPdcpCfms--;
   ...
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPdcpCfgCfm:11 \n"));
      NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm failed\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMRptCfgCfm: cfmStatus(%d), layerId(%d)\n", 
                              cfmStatus, layerId));
      ...
      */
      ...
      else
      {
            ...
            transCtrlBlk->ordrdCfg.ueInfo.crntiCfgType == NH_UE_INFO_MOD_CRNTI)
         {
            ...
            if (ROK != ret)
            {
                NH_DBG_ERROR((NH_PRNT_BUF, "Could not delete from  list.\n"));
            }
         }
      ...
      { 
         NH_DBG_ERROR((NH_PRNT_BUF, "nhUIMSndCfgCfm failed\n"));
         RETVALUE(ret);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, 
               "nhLIMRptCellCfgCfm: cfmStatus(%d) layerId(%d) \n", 
               cfmStatus, layerId));
   ...
   if (0x00 == cellTransCb->cfmMask)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "stop timer failed \n"));
         RETVALUE(RFAILED);
      }
      ...
      if (cellTransCb->cfmStatus == cellTransCb->cfgMask)
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF, 
                   "nhLIMRptCellCfgCfm: nhUDMCommitCellOrdrdCfg failed\n"));
            RETVALUE(RFAILED);
         }
      } 
      else
      ...
      { 
         NH_DBG_ERROR((NH_PRNT_BUF, 
                   "nhLIMRptCellCfgCfm: nhUIMSndCellCfgCfm failed"));
         ret = RFAILED;
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlRvrtCellCfg \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMSendCellCfg: sending cell Cfg ... \n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "nhLIMSendCellCfg: Invalid Cell to Configure\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "start timer failed  \n"));
      RETVALUE(RFAILED);
   }
   ...
   if (cellTransCb->cfgMask & NH_MAC_CONFIGURED)
   {
      if (ROK != nhPIMSendMacCellCfg(cellCb))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, 
                     "nhLIMSendCellCfg: nhPIMSendMacCellCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if (cellTransCb->cfgMask & NH_RLC_CONFIGURED)
   {
      if (ROK != nhPIMSendRlcCellCfg(cellCb))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, 
                     "nhLIMSendCellCfg: nhPIMSendRlcCellCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if (cellTransCb->cfgMask & NH_PHY_CONFIGURED)
   {
      if (ROK != nhPIMSendPhyCellCfg(cellCb))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, 
                     "nhLIMSendCellCfg: nhPIMSendPhyCellCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
#endif /* RRC_PHY_CFG_SUPP */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "In nhLIMHndlRvrtMacCfg \n"));
   ...
   * do not care for cfg confirms for these requests.
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, " nhPIMRvrtMacAddCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
   rvrtCfgType = 0;
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMRvrtMacModCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMRvrtMacRlsCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCellCfgCfm"));
   ...
   {
      NH_DBG_DEBUG((NH_PRNT_BUF, "duplicate Mac Cfg Cfm RCVD \n"));
      RETVALUE(ROK);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMMacUlMapTblUpd"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Duplicate MAC Cfg Cfm RCVD for \
                  logical channel ID (%d)\n", lclId));
      RETVALUE(ROK);
   }
   else
   {
   ...
   if (FALSE == found)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid MAC Cfg Cfm RCVD \n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMMacDlMapTblUpd"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacUECfgCfm"));
   ...
   else if (cfgElms == NH_CRG_LOGCH_MAP)
   {
      ...
      if (dir & NH_LCID_DIR_TX)
      {
         ...
         {
            NH_DBG_DEBUG((NH_PRNT_BUF, " nhLIMMacDlMapTblUpd failed \n"));
            RETVALUE(ROK);
         }
      }
      if (dir & NH_LCID_DIR_RX)
      {
         ...
         {
            NH_DBG_DEBUG((NH_PRNT_BUF, " nhLIMMacUlMapTblUpd failed \n"));
            RETVALUE(ROK);
         }
      }
      if(status == CRG_CFG_CFM_NOK)
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPhyCellCfgCfm"));
   ...
   {
      NH_DBG_DEBUG((NH_PRNT_BUF, "duplicate Phy Cfg Cfm RCVD \n"));
      RETVALUE(ROK);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPhyUECfgCfm"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPhyCfgCfm"));
...
#ifndef ALIGN_64BIT   
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPhyCfgCfm: transId (%ld) status(%d)\
            bitmask(%d) \n", nxTransId, status, phyActionBitMask));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlPhyCfgCfm: transId (%d) status(%d)\
            bitmask(%d) \n", nxTransId, status, phyActionBitMask));
#endif /* ALIGN_64BIT */
   ...
   if (NULLP == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "NULLP Cell Cb\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (nhIdentity.ueId > 0)
   {
      if (ROK != (nhLIMHndlPhyUECfgCfm(&nhIdentity, cellCb, nxTransId, cfgElms,
                                       status)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlPhyUECfgCfm failed \n"));
         RETVALUE(RFAILED); 
      }
   }
   else
   {
      if (ROK != (nhLIMHndlPhyCellCfgCfm(&nhIdentity, cellCb, cfgElms, 
                                         status)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlPhyCellCfgCfm failed \n"));
         RETVALUE(RFAILED); 
      }
   }
   RETVALUE(ROK);
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm"));
...
#ifndef ALIGN_64BIT   
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm: transId (%ld) status(%d)\
            bitmask(%d) \n", nxTransId, status, macActionBitMask));
#else /* ALIGN_64BIT */
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm: transId (%d) status(%d)\
            bitmask(%d) \n", nxTransId, status, macActionBitMask));
#endif /* ALIGN_64BIT */
   ...
   if (nxTransId == NH_RESERVED_TRANS_ID)
   {
       ...
       * status must not be equal to 0. 
       */
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm:1"));
      if (status != CRG_CFG_CFM_OK)
      {
         /* Raise a critical alarm to the layer manager */
         NH_SET_ZERO (&dgn, sizeof(dgn));
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm:2"));
      /* Return from here */
      RETVALUE(ROK);
   }
   if (NULLP == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
   ...
   if (nhIdentity.ueId > 0)
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm:3"));
      if (ROK != (nhLIMHndlMacUECfgCfm(&nhIdentity, cellCb, nxTransId, lclId, 
                                       cfgElms, status, dir)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlMacUECfgCfm failed \n"));
         RETVALUE(RFAILED); 
      }
   }
   else
   {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlMacCfgCfm:4"));
      if (ROK != (nhLIMHndlMacCellCfgCfm(&nhIdentity, cellCb, cfgElms, 
                                         status, lclId)))
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlMacCellCfgCfm failed \n"));
         RETVALUE(RFAILED); 
      }
   }
   RETVALUE(ROK);
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMSendCfgMsg: sending Cfg msg \n"));
   ...
   if (transCtrlBlk->cfgMask & NH_PHY_CONFIGURED)
   {
      if (ROK != nhLIMSendPhyCfg(cellAndUeCb->ueCb, transCtrlBlk))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendPhyCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
#endif /* RRC_PHY_CFG_SUPP */
   ...
   if (transCtrlBlk->cfgMask & NH_MAC_CONFIGURED)
   {
      if (ROK != nhLIMSendMacCfg(cellAndUeCb->ueCb, transCtrlBlk))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendMacCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
         ...
         transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_RESTB_SRB1))
   {
       ...
       if (transCtrlBlk->cfgMask & NH_RLC_CONFIGURED)
       {
           ...
           if (ROK != nhPIMSendRlcCfg(cellAndUeCb->ueCb, transCtrlBlk))
           {
              /* Stop lower Layer Cfg Timer */
              nhStopLLyrCfgTmr(cellAndUeCb, transCtrlBlk->nxTransId);
              NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendRlcCfg failed \n"));
              RETVALUE(RFAILED);
           }
       }
   }
       ...
       (transCtrlBlk->cause != NH_TRANSCB_SDU_STA_RESTAB))
   {
      ...
      if (ROK != nhPIMSendPdcpCfg(cellAndUeCb->ueCb, transCtrlBlk))
      {
         ...
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendPdcpCfg failed \n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   TRC2(nhLIMSendMacCfg)
      ...
      NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMSendMacCfg: sending Cfg to MAC \n"));
   ...
   if(rstCfgType > 0)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF, 
                     "nhLIMSendMacCfg: nhPIMSendMacResetCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if(addCfgType > 0)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMSendMacAddCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if(modCfgType > 0)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendMacModCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMSendPhyCfg: sending Cfg to PHY \n"));
           ...
           transCtrlBlk->ueMapTbl[NH_UE_MAPTBL_IDX_PHY].action))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMSendPhyUeIdChgReq failed.\n"));
         RETVALUE(RFAILED);
      }
   }
         ...
         (NH_PHY_CFG_ADD == transCtrlBlk->phyMapInfo.action))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMSendPhyAddCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
         ...
         (NH_PHY_CFG_MODIFY  == transCtrlBlk->phyMapInfo.action))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendPhyModCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "In nhLIMHndlRvrtPhyCfg \n"));
   ...
   * do not care for cfg confirms for these requests.
          ...
          transCtrlBlk->ueMapTbl[NH_UE_MAPTBL_IDX_PHY].cfmStatus))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, " nhPIMRvrtPhyUeIdChgReq failed.\n"));
         RETVALUE(RFAILED);
      }
   }
         ...
         (CTF_CFG_CFM_OK == transCtrlBlk->phyMapInfo.cfmStatus))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, " nhPIMRvrtPhyAddCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
         ...
         (CTF_CFG_CFM_OK == transCtrlBlk->phyMapInfo.cfmStatus))
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMRvrtPhyModCfg failed.\n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, 
                 "nhLIMRlsLowerLyrCfg: complete release of lower lyr cfg \n"));

   ret = nhPIMRlcPdcpMacRlsCfg(ueCb);
   if (ROK != ret)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "nhPIMRlcPdcpMacRlsCfg failed.\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (ueCb->crntCfg.bitMskPres & NH_CRNT_CFG_PHYCH)
   {
      ...
      if (ROK != ret)
      {
        NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMSendPhyRlsCfg failed.\n"));
        RETVALUE(RFAILED);
      }  
   }
#endif /* RRC_PHY_CFG_SUPP */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMSendUbndReq\n"));
   ...
   switch(sapElmId)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF, " Invalid sapElmId\n"));
         RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, " nhLIMSendBndReq ElmId(%d)\n",sapElmId));
...
#endif /* ANSI */
{
   ...
   if (NH_CHK_INPUT_PARAM(staInd))
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL staInd\n"));
      if (!(NH_CHK_INPUT_PARAM(buf)))
      {
         NH_FREEMBUF(buf);
      }
      NH_FREE(NH_REG, NH_POOL, pdcpId, sizeof(CmLtePdcpId));
      RETVALUE(RFAILED);
   }

   NH_DBG_INFO((NH_PRNT_BUF, "NhLiPjuStaInd: suId(%d), status(%d)\n", 
                         suId, staInd->cause));
   ...
   if (pjuSapCb == NULLP)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, " INVALID SAP ID in NhLiPjuStaInd\n"));
      NH_FREE(NH_REG, NH_POOL, pdcpId, sizeof(CmLtePdcpId));
      NH_FREE(NH_REG, NH_POOL, staInd, sizeof(PjuStaIndInfo));
      RETVALUE(RFAILED);
   }
  ...
  NH_DIAG_LVL0(0x060d0015, NH_DIAG_UEID, nhIdentity.ueId,
               "Received StaInd from PDCP suId = %d, rbId = %d",
               suId, nhIdentity.rbId,0,0); 
   ...
   if (nhIdentity.isPdcpId == TRUE)
   {
      ...
      if (ROK != ret)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMHndlStatusInd failed.\n"));
      }
   }
   else
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "NhLiPjuStaInd: not a pdcp ID\n"));
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlCountCfm"));

#ifndef ALIGN_64BIT    
   NH_DBG_INFO((NH_PRNT_BUF, "nhLIMHndlCountCfm:  transId(%ld) ueId(%d) cellId \
                (%d)\n", countCfmInfo->transId, countCfmInfo->ueId, 
                countCfmInfo->cellId));
#else /* ALIGN_64BIT */
   NH_DBG_INFO((NH_PRNT_BUF, "nhLIMHndlCountCfm:  transId(%d) ueId(%d) cellId  \
                (%d)\n", countCfmInfo->transId, countCfmInfo->ueId, 
                countCfmInfo->cellId));
#endif /* ALIGN_64BIT */
  ...
  sMem.pool = nhCb.init.pool;
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF,
        "Memory Allocation failed for storing DRB Count info List.\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMHndlCountCfm: Count Confirm Failed \n"));
   ...
   else
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "nhLIMHndlCountCfm: Number of DRBs out of rang\ne"));
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
           "Memory Allocation failed for storing DRB Count info.\n"));
         NH_FREEEVNT(nhuDRBCountInfoLst);
         RETVALUE(RFAILED);
      }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "nhLIMHndlCountCfm: nhUIMSndCountCfm failed \n"));
      NH_FREEEVNT(nhuDRBCountInfoLst);
      RETVALUE(RFAILED);
   }
   RETVALUE(ROK);
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF, 
              " nhLIMHndlPdcpSecCfgCfm: transId(%ld) ueId(%d) cellId   \
              (%d)\n", secCfgCfm->transId, secCfgCfm->ueId, secCfgCfm->cellId));
#else /* ALIGN_64BIT */
   NH_DBG_INFO((NH_PRNT_BUF, 
              "nhLIMHndlPdcpSecCfgCfm:  transId(%d) ueId(%d) cellId      \
              (%d)\n", secCfgCfm->transId, secCfgCfm->ueId, secCfgCfm->cellId));
#endif /* ALIGN_64BIT */
...
#endif /* ANSI */
{
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
               "nhLIMCountReqHndlr: Cound not send Count request\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF," nhLIMFillPdcpSduStaInfo called.\n"));
   ...
   if (numRbId == 0)
   {
      NH_DBG_ERROR((NH_PRNT_BUF,"NhuPdcpSduStaInfo Number of RbId is Zero \
                    failed.\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF,"NhuPdcpSduStaInfo memory allocation \
                    failed.\n"));
      RETVALUE(RFAILED);
   }
   ...
   for(rbCnt = 0; rbCnt < numRbId; rbCnt++)
   {
      ...
      if(ulBitMapSize)
      {
         ...
         NH_GET_MEM(nhuSduStaCfm, (ulBitMapSize * sizeof(U8)),
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF," nhLIMFreeCpjSduStaCfm called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm\n"));
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:1\n"));
   }
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:2\n"));
   }
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:3\n"));
      if(rbType == CM_LTE_SRB)
      {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:4\n"));
         if ((ROK != nhLIMSrchAndSetSRBBitMsk(&rcvdSrbIds,
                  mapTbl->srbToCfg, mapTbl->srbCfmSta, rbId, 
                  cfmEnt[entIndex].status, okValue)))
         {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:5\n"));
            RETVALUE(RFAILED);
         }
      }
      else
      {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:6\n"));
         if (ROK != nhLIMSrchAndSetDRBBitMsk(&rcvdDrbIds, mapTbl->drbToCfg,
                  mapTbl->drbCfmSta, rbId, cfmEnt[entIndex].status, okValue))
         {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:7\n"));
            RETVALUE(RFAILED);
         }
      }
   }
      ...
      )
   {
      ...
      nhSendLmAlarm(LCM_CATEGORY_INTERFACE, LNH_EVENT_OPERATION_FAIL,
                 LCM_CAUSE_UNKNOWN, &dgn);
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid number of RLC Cfg Cfm RCVD \n"));
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:8\n"));
      RETVALUE(RFAILED);
   }
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:9\n"));
      /* Now call function to send Cfm Report to CGM */
      if (ROK != nhLIMRptCfgCfm(nhIdentity, cellAndUeCb, transCtrlBlk, 
                                cgmCfmStatus, layerId))
      {
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhLIMVldtAndRptCfgCfm:10\n"));
         NH_DBG_ERROR((NH_PRNT_BUF, "nhLIMRptCfgCfm failed\n"));
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   if (FALSE == found)
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Cfg Cfm RCVD \n"));
      RETVALUE(RFAILED);
   }
