...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "In Downlink Info transfer Handler \n"));
...
#endif /* ANSI */
{
   ...
   if (length > NhBcchPcchTbSzTbl[NH_BCCH_PCCH_TBSZTBL_SIZE - 1])
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
         "encoded length is more than max TB size\n"));
   ...
   if (ROK != ret)
   {
      ...
      NH_DBG_ERROR((NH_PRNT_BUF,
         "Send to lower layer failed(ret:%d) \n", ret));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_MSG_SEND_FAILED, \
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCPMUeCapEnqHndlr called.\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                  "nhCPMUeCapEnqHndlr: nhUTLEncAndSnd Failed.\n"));
      RETVALUE(retVal);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCPMMobFrmEutraCmd called.\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF, 
                  "Security procedure is not initiated.\n"));
      NH_FILL_NHU_ERROR(*errInfo, NHU_ERRTYPE_UE_CFG, 
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCPMHoFrmEutraPrepTfr called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhCPMCDMACsfbParamRsp called.\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "UE Information Request Handler \n"));
