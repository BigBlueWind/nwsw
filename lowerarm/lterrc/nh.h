...
#define NHDBGP(_msgClass, _args)                                  \
do                                                                \
{                                                                 \
   if(nhCb.fileLogEnb)                                            \
   {                                                              \
      if((nhCb.dbgFp == NULLP) ||                                 \
            (nhCb.nmbDbgLines == nhCb.cfgDbgLines))               \
      {                                                           \
         nhUTLCreateDbgFile();                                    \
      }                                                           \
      if((nhCb.dbgFp) && ((nhCb.init.dbgMask) & (_msgClass)))     \
      {                                                           \
         sprintf(NH_PRNT_BUF, "[%s 0x%x:%x] %s:%d ",              \
                  NHLAYERNAME, nhCb.init.ent, nhCb.init.inst,     \
                  __FILE__, __LINE__);                            \
         fwrite((NH_PRNT_BUF),                                    \
               cmStrlen((U8*)NH_PRNT_BUF), 1,                     \
                  (nhCb.dbgFp));                                  \
         sprintf _args;                                         \
         fwrite((NH_PRNT_BUF),                                    \
               cmStrlen((U8*)NH_PRNT_BUF), 1,                     \
   ...
   {                                                              \
      DBGP(&(nhCb.init), NHLAYERNAME, _msgClass, _args);      \
   }                                                              \
}                                                                 \
while(0)
...
/**  
 * @def dbgInfo For logging important events in the source code
 */
#define NH_DBG_INFO(_args) \
{ \
   if (nhCb.init.dbgMask & LNH_DBGMASK_INFO) \
   { \
      Txt prntBuf[10]; \
      sprintf(prntBuf, "[info]"); \
      SPrint(prntBuf); \
   } \
   NHDBGP(LNH_DBGMASK_INFO, _args); \
}

/**  
 * @def dbgDbg For logging additional debug information in the source code.
 */
#define NH_DBG_DEBUG(_args) \
{ \
   if (nhCb.init.dbgMask & LNH_DBGMASK_DBG) \
   { \
      Txt prntBuf[10]; \
      sprintf(prntBuf, "[debug]"); \
      SPrint(prntBuf); \
   } \
   NHDBGP(LNH_DBGMASK_DBG, _args); \
}

/**  
 * @def dbgParm For logging important parameters in the source code.
 */
#define NH_DBG_PARAM(_args) \
{ \
   if (nhCb.init.dbgMask & LNH_DBGMASK_PARAM) \
   { \
      Txt prntBuf[10]; \
      sprintf(prntBuf, "[param]"); \
      SPrint(prntBuf); \
   } \
   NHDBGP(LNH_DBGMASK_PARAM, _args); \
}
/**  
 * @def dbgErr For logging error cases in the source code.
 */
#define NH_DBG_ERROR(_args) \
{ \
   if (nhCb.init.dbgMask & LNH_DBGMASK_ERROR) \
   { \
      Txt prntBuf[10]; \
      sprintf(prntBuf, "[error]"); \
      SPrint(prntBuf); \
   } \
   NHDBGP(LNH_DBGMASK_ERROR, _args); \
}

/**  
 * @def dbgFtl For logging fatal errors in the source code, the system halts here. In such cases, cleanup and go ahead.
 */
#define NH_DBG_FATAL(_args) \
{ \
   if (nhCb.init.dbgMask & LNH_DBGMASK_FATAL) \
   { \
      Txt prntBuf[10]; \
      sprintf(prntBuf, "[fatal]"); \
      SPrint(prntBuf); \
   } \
   NHDBGP(LNH_DBGMASK_FATAL, _args); \
}
...
#ifdef SS_DIAG 
#define NH_DIAG_LVL0(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                                             \
   if(nhCb.init.logMask & SS_DIAG_LVL0)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL0, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   } \
}

/** @details Macro definition for RRC error logs
 *  
*/
#define NH_DIAG_LVL1(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)            \
{                                                                 \
   if(nhCb.init.logMask & SS_DIAG_LVL1)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL1, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for RRC critical logs
 *  
*/
#define NH_DIAG_LVL2(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                              \
   if(nhCb.init.logMask & SS_DIAG_LVL2)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL2, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for RRC logs 
 *  
*/
#define NH_DIAG_LVL3(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(nhCb.init.logMask & SS_DIAG_LVL3)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL3, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}
 ...
 *  
*/
#define NH_DIAG_LVL4(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(nhCb.init.logMask & SS_DIAG_LVL4)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL4, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
} 

/** 
 * @details Macro definition for RRC Hex-Dump
 */
#define NH_DIAG_MSG(_mBuf, _str)                                                                  \
{                                                                                                \
   if(nhCb.init.logMask & SS_DIAG_MSG)                                                      \
   {                                                                                             \
      Buffer  *_newBuf;                                                                          \
      SAddMsgRef(_mBuf, nhCb.init.region, nhCb.init.pool, &_newBuf);                             \
      ssDiagFix(0, NH_DIAG_NA, ENTNH, nhCb.init.inst, SS_DIAG_MSG, SS_DIAG_MSG_TYPE_HEXDUMP, _newBuf, 0, 0, 0,0, _str);\
   }                                                                                             \
}
 ...
 *  
 */
#define NH_DIAG_VAR(_tknId, _splArgEnum, _splArg, _string, _stringPtr)                                   \
{                                                                                                        \
   if(nhCb.init.logMask & SS_DIAG_VAR)                                                              \
   {                                                                                                     \
      ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_VAR, SS_DIAG_MSG_TYPE_FIXED, _splArg, 0,0,0,0, _string);\
      ssDiagVarible(ENTNH, nhCb.init.inst, SS_DIAG_VAR, SS_DIAG_MSG_TYPE_VAR, _tknId, _splArg, _stringPtr);\
   }                                                                                                     \
}
