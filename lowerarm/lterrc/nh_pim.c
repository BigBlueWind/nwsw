...
PRIVATE S16 nhPIMFillRlcRlsCfg ARGS((TransCb        *transCtrlBlk,
                                      CkwCfgInfo     *ckwCfgParam));
...
#endif /* ANSI */
{
   ...
   CmLtePdcpId    *pdcpId = NULLP;            /* PDCP ID */
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendUeMsg\n"));
   ...
   if (nhDataReqParam->isPdcpDatReq)
   {
      ...
      if ((NULLP == lSap) || (lSap->state != LNH_SAP_BOUND))
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                    "Failed to find lower layer or SAP not bound\n"));
         NH_FREEMBUF(mBuf);
         RETVALUE(RFAILED);
      }
      ...
      NH_ALLOC(NH_REG, NH_POOL, &pdcpId, sizeof(CmLtePdcpId));
      if (NULLP == pdcpId)
      {
         NH_DBG_FATAL((NH_PRNT_BUF,"pdcpId memory allocation failed.\n"));
         RETVALUE(RFAILED);
      }
   ...
   NH_GEN_TRC(suId, STNHPJUSAP, LNH_MSG_SENT, mBuf)
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"Failed to send data to lower layer\n"));
         RETVALUE(RFAILED);
      }
   }
   else
   {
      ...
      NH_ALLOC(NH_REG, NH_POOL, &kwuDatReqParam, sizeof(KwuDatReqInfo));
      if (kwuDatReqParam == NULLP)
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for kwuDatReqParam"));
         NH_FREEMBUF(mBuf);
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendRlcCfg\n"));
                 ...
                 transCtrlBlk->ueMapTbl[NH_UE_MAPTBL_IDX_RLC].action))
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for Ordrd config"));
         RETVALUE(RFAILED);
      }
      ...
      {
         transCtrlBlk->numOfRlcCfms--;
         NH_DBG_ERROR((NH_PRNT_BUF,"Failed to send UEID Change Req to RLC\n"));
         RETVALUE(RFAILED);
      }
   }
   else
       ...
       transCtrlBlk->cause == NH_TRANSCB_SDU_STA_RESTAB)
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL ckwCfgParam\n"));
         RETVALUE(RFAILED);
      }
   ...
   {
      NH_DBG_DEBUG((NH_PRNT_BUF,"No Rbs to Configure \n"));
      RETVALUE(ROK);
   }
   ...
   if (transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_RB_ADD)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
               "Failed to fill data to lower interface structure\n"));
   ...
   switch(cellCb->cellTransCb.cfgAction)
   {
      case NH_CFG_ACTN_ADD: 
         if (cellCfgInfo->cellCfgInfo.cellCfgBitMsk & NHU_CELL_CFG_RB)
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF,"Failed to fill data for Cell\n"));
               NH_FREE(NH_REG, NH_POOL, ckwCfgParam, sizeof(CkwCfgInfo));
               RETVALUE(RFAILED);
            }
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"No Action in Ordered config for cell\n"));
         NH_FREE(NH_REG, NH_POOL, ckwCfgParam, sizeof(CkwCfgInfo));
         RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtRlcCellCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendMacModCfg\n"));
   ...
   if (cfgType & NH_CRG_UE_CFG)
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL crgCfgParam\n"));
         RETVALUE(RFAILED);
      }
      /* Set zero */
      ...
      for (i = 0; i < rbCfgList->numOfRbCfgInfo; i++)
      {
         ...
         if (dir)
         {
                                     ...
                                     crgCfgParam))
            {
               transCtrlBlk->numOfMacCfms--;
               NH_DBG_ERROR((NH_PRNT_BUF,
                           "Failed to send data to lower layer\n"));
               RETVALUE(RFAILED);
            }
         }
         else
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF,"nhPIMSendMacRlsCfg: Rlsing Cfg at MAC  \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendMacCellCfg\n"));
...
#endif /* ANSI */
{
   ...
   switch(cellCb->cellTransCb.cfgAction)
   {
      ...
      default:
         NH_DBG_ERROR((NH_PRNT_BUF,"Invalid Cfg Action \n"));
         RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacCellCfg\n"));
       ...
       (cellTransCb->macMapTbl[0].cfmStatus != NH_CFG_FAILED))
   {
      ...
      switch(rvrtAction)
      {
         case NH_CFG_ACTN_RLS:
            ...
            {
                NH_DBG_ERROR((NH_PRNT_BUF,
                         "Failed to configuration to MAC layer\n"));
                RETVALUE(RFAILED);
            }
            RETVALUE(ROK);
         ...
         default:
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "Invalid case of revert config for MAC layer\n"));
            NH_FREE(NH_REG, NH_POOL, crgCfgParam, sizeof(CrgCfgReqInfo));
            RETVALUE(RFAILED);
...
#endif  /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendMacAddCfg\n"));
   ...
   if (cfgType & NH_CRG_UE_CFG)
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                    "Mandatory TRCH config not found\n"));
         RETVALUE(RFAILED);
      }
      else 
      {
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillPdcpRlsCfg\n"));
...
#endif /* ANSI */
{
                   ...
                   transCtrlBlk->ueMapTbl[NH_UE_MAPTBL_IDX_PDCP].action))
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for Ue Info"));
         RETVALUE(RFAILED);
      }
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for New Ue Info"));
           ...
           transCtrlBlk->restbTbl[NH_RESTB_MAPTBL_IDX_PDCP_SRB1].action))
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for Re-Estb Req"));
         RETVALUE(RFAILED);
      }
      ...
      if (ROK != NhLiCpjReEstReq(&tmpPst, lSap->spId, restbReq))
      {
         transCtrlBlk->numOfPdcpCfms--;
         NH_DBG_ERROR((NH_PRNT_BUF,
                     "Failed to send Re-Establish Req to PDCP\n"));
         RETVALUE(RFAILED);
      }
   }
   ...
   if (transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_SEC)
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cpjSecCfgInfo\n"));
         RETVALUE(RFAILED);
      }
      ...
      if (ROK != NhLiCpjSecCfgReq(&tmpPst, lSap->spId, cpjSecCfgInfo))
      {
         transCtrlBlk->numOfPdcpCfms--;
         NH_DBG_ERROR((NH_PRNT_BUF,
                  "Failed to send sec cfg data to lower layer\n"));
         RETVALUE(RFAILED);
      }
   }
       ...
       transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_RB_RESTB)
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL cpjCfgReq\n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillPdcpRvrtCfgParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillPdcpRvrtAddCfgParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillPdcpRvrtModCfgParam\n"));
   ...
   if (rbCfgInfo->rbCfgBitMsk & NHU_RB_CFG_PDCP_INFO)
   {
      ...
      if (rbCfgInfo->pdcpCfg.pdcpCfgBitMsk & NHU_PDCP_SDU_STA_INFO)
      {
         ...
         if(rbCfgInfo->pdcpCfg.hoPdcpSduStaInfo.isHoPres & NHU_HO_UL)
         {
            ...
            {
               NH_DBG_FATAL((NH_PRNT_BUF,
                  "Memory Allocation failed for ulBitMap.\n")); 
               RETVALUE(RFAILED);
            }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillAddPdcpCfgParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillModPdcpCfgParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRlcPdcpMacRlsCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillPdcpSecCfg\n"));
   ...
   switch(secCfg->intgCfgInfo.intgAlgo)
   {
      ...
      case NHU_INTG_ALGO_EIA2:
         ...
         break;
      default:
         NH_DBG_DEBUG((NH_PRNT_BUF, "Invalid intg Algo\n"));
         RETVALUE(RFAILED);
         break;
   }
   ...
   switch(secCfg->cipherCfgInfo.ciphAlgo)
   {
      ...
      case NHU_CIPHER_ALGO_EEA2:
         ...
         break;
      default:
         NH_DBG_DEBUG((NH_PRNT_BUF, "Invalid ciph Algo\n"));
         RETVALUE(RFAILED);
         break;
   }
   ...
   if (rbCfgInfo->rbCfgBitMsk & NHU_RB_CFG_UL)
   {
      ...
      if (rbCfgInfo->ulRbCfg.ulRbBitMsk & NHU_RB_CFG_RLC_MODE_UL)
      {
         ...
         switch(rbCfgInfo->ulRbCfg.ulRlcMode.mode)
         {
            ...
            default:
                 NH_DBG_ERROR((NH_PRNT_BUF,"No default mode of RLC\n"));
                 RETVALUE(RFAILED);
        }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRlcDlCfgParam\n"));
   switch(rbCfgInfo->dlRbCfg.dlRlcMode.mode)
   {
      case NHU_RLC_MODE_AM:
           ckwCfgParam->entCfg[rbCount].m.amInfo.dl.pollRetxTmr =
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRlcUlCfgParam\n"));
...
#else /* ANSI */
PRIVATE S16 nhPIMFillRlcRlsCfg(transCtrlBlk, ckwCfgParam) 
TransCb        *transCtrlBlk;
CkwCfgInfo     *ckwCfgParam;
#endif /* ANSI */
{
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRlcCfgParam\n"));
  ...
  /* get number of entity */ 
   ...
   for ( i=0; i < rbCfgListInfo->numOfRbCfgInfo; i++, rbCount++)
   {
      ...
      if (NULLP == rbCfgInfo)
      {
         NH_DBG_ERROR((NH_PRNT_BUF,"NULLP for rbCfgInfo\n"));
         RETVALUE(RFAILED);
      }
      ckwCfgParam->entCfg[rbCount].cfgType = cfgType;
      ckwCfgParam->entCfg[rbCount].rbId = rbCfgInfo->rbId.rbId;
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRlcCellCfgParam\n"));
   ...
   for (rbCount = 0; rbCount < rbCfgListInfo->numOfRbCfgInfo; rbCount++)
   {
          ...
          rbCfgInfo->ulRbCfg.ulRbBitMsk & NHU_RB_CFG_RLC_MODE_UL)
      {
         ...
         {
             NH_DBG_ERROR((NH_PRNT_BUF, "Invalid Rlc Mode for Cell (%d) \n", 
                                    rbCfgInfo->ulRbCfg.ulRlcMode.mode));
         }
      } /* if (rbCfgInfo->rbCfgBitMsk & NHU_RB_CFG_UL && ... */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRvrtPhyModUeCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPhyUeIdChgReq\n"));
     ...
     (CTF_CFG_CFM_OK == transCtrlBlk->ueMapTbl[NH_UE_MAPTBL_IDX_PHY].cfmStatus))
   {
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,
                         "Could not allocate mem for Old Ue Info"));
         RETVALUE(RFAILED);
      }
      ...
      if (ROK != NhLiCtfUeIdChgReq(&tmpPst, lSap->spId, ctfCfgTransId,
                                             oldUeInfo,  newUeInfo))
      {
          NH_DBG_ERROR((NH_PRNT_BUF,"Failed to send Rvrt UeId \
                    Change Req to PDCP \n"));
          RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPdcpRlsCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPdcpUeIdChgReq\n"));
...
#endif /* ANSI */
{
  ...
  /* unused var deleted*/
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPdcpCfg\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "nhPIMRvrtPdcpUeIdChgReq Failed \n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                "Config parameter is not avaliable to send lower to layer\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacModCfg\n"));
         ...
         (transCtrlBlk->macLchMapTbl[NH_UL_LC].modCfmStatus))
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                      "Failed to Revert modify logical channel\n"));
         RETVALUE(RFAILED);
      }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacModLchCfg\n"));
   ...
   for (i = 0; i < NH_MAX_CFG_REQ; i++)
   {
      ...
      if (NH_LCID_DIR_RX == dir )
      {
         ...
         {
            NH_DBG_ERROR((NH_PRNT_BUF,
                        "Failed to Revert modify RX logical channel\n"));
            RETVALUE(RFAILED);
         }
      }
   } /* for (i = 0; i < NH_MAX_CFG_REQ; i++) */
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacRlsLchCfg\n"));
   ...
   for (i = 0; i < NH_MAX_CFG_REQ; i++)
   {
      ...
      tmpUl = (transCtrlBlk->macLchMapTbl[NH_UL_LC].rlsCfmStatus >> i);
         ...
         if (NH_LCID_DIR_TXRX == dir)
         {
            ...
            {
               NH_DBG_ERROR((NH_PRNT_BUF,
                        "Failed to Revert of release RXTX logical channel\n"));
               RETVALUE(RFAILED);    
            }
         ...
         else
         {
            if (NH_LCID_DIR_RX == dir )
            {
               ...
               {
                  NH_DBG_ERROR((NH_PRNT_BUF,
                        "Failed to Revert of release RX logical channel\n"));
                  RETVALUE(RFAILED);
               }
            ...
            if (NH_LCID_DIR_TX == dir )
            {
               ...
               {
                  NH_DBG_ERROR((NH_PRNT_BUF,
                        "Failed to Revert of release TX logical channel\n"));
                  RETVALUE(RFAILED);
               }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacRlsTXRXLch\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacRlsTXLch\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacRlsRXLch\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendRvrtMacRlsLchCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendRvrtMacModLchCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMGetRvrtMacLch\n"));
   /* Find logical channel id */
   CM_LLIST_FIRST_NODE(&(ueCb->crntCfg.logChCbLst), tmpNode);
   while(NULLP != tmpNode)
   {
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMGetRvrtRlcRb\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtMacRlsCfg\n"));
          ...
          ||transCtrlBlk->macLchMapTbl[NH_UL_LC].rlsCfmStatus ))
   {
      ...
      {
         NH_DBG_ERROR((NH_PRNT_BUF,
                     "Failed to Revert of released logical channel\n"));
         RETVALUE(RFAILED);
      }
   }
   RETVALUE(ROK);
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRlcDatReqParams\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRvrtMacModUeCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRvrtRlcAddParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRvrtRlcModParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMFillRvrtRlcRlsParam\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSndCountReq\n"));

   lSap = nhUTLFindLlyrSapFrmSuId(STNHCPJSAP,suId);
   if (NULLP == lSap || (lSap->state != LNH_SAP_BOUND))
   {
      NH_DBG_ERROR((NH_PRNT_BUF, "Failed to find lower layer\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF,
         "nhPIMSndCountReq: Could not alocate mem for cpjCountReqInfo\n"));
      RETVALUE(RFAILED);
   }
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
               "nhPIMSndCountReq: Could not send count req to PDCP\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_INFO((NH_PRNT_BUF,"nhPIMSendMacResetCfg: Rlsing Cfg at MAC  \n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSndSduStaReq  called  \n"));
   ...
   if(status)
   {
      ...
      if (NULLP == lSap)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "Failed to find PDCP lower layer\n"));
         RETVALUE(RFAILED);
      }
      ...
      {
         NH_DBG_FATAL((NH_PRNT_BUF,"cpjSduStaReq memory allocation failed.\n"));
         RETVALUE(RFAILED);
      }
      ...
      if (ROK != retVal)
      {
         NH_DBG_ERROR((NH_PRNT_BUF, "NhLiCpjSduStaReq failed\n"));
         transCtrlBlk->numOfPdcpCfms--;
         RETVALUE(RFAILED);
      }
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendPhyCellCfg\n"));
   ...
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "GOT NULL ctfCfgParam\n"));
      RETVALUE(RFAILED);
   }
   ...
   if (ROK != NhLiCtfCfgReq(&tmpPst, lSap->spId, ctfCfgTransId, ctfCfgParam))
   {
      ...
      NH_DBG_ERROR((NH_PRNT_BUF,"Failed to send data to PHY layer\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPhyCellCfg\n"));
       ...
       (cellTransCb->phyMapInfo.cfmStatus != NH_CFG_FAILED))
   {
      ...
      switch(rvrtAction)
      {
         case NH_CFG_ACTN_RLS:
            ...
            {
                NH_DBG_ERROR((NH_PRNT_BUF,
                         "Failed to configuration to PHY layer\n"));
                RETVALUE(RFAILED);
            }
            RETVALUE(ROK);
         ...
         default:
            NH_DBG_ERROR((NH_PRNT_BUF,
                     "Invalid case of revert config for PHY layer\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMRvrtPhyModCfg\n"));
...
#endif  /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendPhyAddCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendPhyModCfg\n"));
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendPhyRlsCfg\n"));
   ...
   {
      NH_DBG_ERROR((NH_PRNT_BUF,
                    "Failed to find PHY layer or SAP not bound\n"));
      RETVALUE(RFAILED);
   }
...
#endif /* ANSI */
{
   ...
   NH_DBG_DEBUG((NH_PRNT_BUF, "nhPIMSendPhyUeIdChgReq\n"));
   ...
   if(NULLP == oldUeInfo)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for Ue Info"));
      RETVALUE(RFAILED);
   }
   ...
   if(NULLP == newUeInfo)
   {
      NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for New Ue Info"));
      NH_FREE(NH_REG, NH_POOL, oldUeInfo, sizeof(CtfUeInfo));
      RETVALUE(RFAILED);
   }
   ...
   {
      transCtrlBlk->numOfPhyCfms--;
      NH_DBG_ERROR((NH_PRNT_BUF,"Failed to send UEID Change Req to PHY\n"));
      RETVALUE(RFAILED);
   }
