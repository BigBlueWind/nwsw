...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 121 "../..//ltemac/rg_mux.c" 2
# 1 "../..//cm/cm5.x" 1
# 122 "../..//ltemac/rg_mux.c" 2
# 1 "../..//cm/cm_tkns.x" 1
# 91 "../..//cm/cm_tkns.x"
#define __CMTKNSX 
# 121 "../..//cm/cm_tkns.x"
...
# 136 "../..//ltemac/rg_mux.c" 2

static U8 padArr[128];
...
#define RG_PACK_CE_SHDR(_subHdr,_lcId,_eBit,_mBuf,_ret) { _subHdr.shLen = RG_CE_SHDR_LEN; _subHdr.shData[0] = (0x3F & ((_eBit << 5) | _lcId)); _ret = SAddPstMsgMult(&_subHdr.shData[0], _subHdr.shLen, _mBuf);}





#define RG_PACK_LAST_CE_SHDR(_subHdr,_lcId,_mBuf,_ret) { _subHdr.shLen = RG_CE_SHDR_LEN; _subHdr.shData[0] = (0x1F & _lcId); _ret = SAddPstMsgMult(&_subHdr.shData[0], _subHdr.shLen, _mBuf);}





#define RG_PACK_CE(_ce,_len,_ceBuf,_ret) { _ret = SAddPstMsgMult((U8 *)(&(_ce)), _len, _ceBuf);}



#define RG_MUX_CALC_LEN(_len,_lenBytes,_elmTotLen) { U8 _hdrLen; _lenBytes = (_len <= 127) ? 1 : 2; _hdrLen = _lenBytes + RG_SDU_SHDR_LEN; _elmTotLen = _hdrLen + _len;}
...
#define RG_PACK_SDU_SHDR(_subHdr,_lcId,_len,_eBit,_mBuf,_ret) { _ret = ROK; _subHdr.shData[0] = (0x3F & ((_eBit << 5) | _lcId)); if(_len <= 127) { _subHdr.shLen = 2; _subHdr.shData[1] = (0x7F & _len); _subHdr.shData[2] = 0; } else { _subHdr.shLen = 3; _subHdr.shData[1] = (0x80 | (0x7F & (_len >> 8))); _subHdr.shData[2] = (0xFF & _len); } _ret = SAddPstMsgMult(&_subHdr.shData[0], _subHdr.shLen, _mBuf);}
# 197 "../..//ltemac/rg_mux.c"
#define RG_PACK_LAST_SDU_SHDR(_subHdr,_lcId,_mBuf,_ret) { _ret = ROK; _subHdr.shLen = 1; _subHdr.shData[0] = (0x1F & _lcId); _ret = SAddPstMsgMult(&_subHdr.shData[0], _subHdr.shLen, _mBuf);}
...
#define RG_PACK_SDU(_sdusBuf,_sduBuf,_ret) { _ret = SCatMsg(_sdusBuf, _sduBuf, M1M2);}



#define RG_PACK_PAD_SHDR(_eBit,_mBuf,_ret) { if(_eBit) { _ret = SAddPreMsg(0x3F, _mBuf); } else { _ret = SAddPstMsg(0x1F, _mBuf); }}
# 219 "../..//ltemac/rg_mux.c"
#define RG_PACK_PAD(_padArr,_len,_sdusBuf,_ret) { _ret = SAddPstMsgMult(&_padArr[0], _len, _sdusBuf);}



#define RG_PACK_RAR_SHDR(_byte,_mBuf,_ret) { _ret = SAddPstMsg(_byte, _mBuf);}
...
# 398 "../..//ltemac/rg_mux.c"
   ...
   else
   {
      ...
      { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 0))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_mux.c", 486); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "TB Size is not enough to accomodate" " the element\n"); SPrint((&rgCb.rgInit)->prntBuf); } };
...
# 527 "../..//ltemac/rg_mux.c"
{
   ...
   if(*schdTbSz)
   {
      ...
      if (*schdTbSz)
      {
         if (*schdTbSz <= 128)
         {
            { ret = SAddPstMsgMult(&padArr[0], *schdTbSz, sduBuf);};
         ...
         else
         {
            while (*schdTbSz)
            {
               if (*schdTbSz > 128)
               {
                  { ret = SAddPstMsgMult(&padArr[0], 128, sduBuf);};
...

{
   ...
   if(rgMUXAddPadd(&pdu->schdTbSz, sHdrBuf, sduBuf, 0, err) != 0)
   {
...

{
   ...
   if(rgMUXAddCes(pdu, mBuf1, mBuf2, err) != 0)
   {
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
   ...
   if(rgMUXAddSdus(pdu, mBuf1, mBuf2, err) != 0)
   {
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
      { if (0L != (mBuf2)) { SPutMsg((mBuf2)); mBuf2 = 0L; } };
      err->errType = (0 + 1);
...
# 911 "../..//ltemac/rg_mux.c"
   ...
   if ((ret = rgGetMsg(&hdrBuf)) != 0)
   {

      err->errCause = 0 + 1;
      return(1);
   }
   if ((ret = rgGetMsg(&datBuf)) != 0)
   {
   ...
   if(rgMUXAddPadd(&schdTbSz, hdrBuf, datBuf, 1, err) != 0)
   {
      { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
      { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
      return(1);
