...
# 98 "../..//enbapp/lterrm/rm_cac_adm.c" 2
...
static S16 rmCacAdmProcEmergencyCall
(
   RmCellCb *cellCbPtr,
   U16 crnti,
   TransId transId
...
static S16 rmCacAdmLclUeRel
(
   RmUeCb *ueCbPtr,
   TransId transId
);

static S16 rmCacAdmProcUeToAdmitLst
(
   RmCellCb *cellCbPtr
);
# 158 "../..//enbapp/lterrm/rm_cac_adm.c"
 S16 rmCacAdmProcUeAdmission
(
   RmCellCb *cellCbPtr,
   U16 crnti,
   TransId transId,
...
{
   S16 ret = 0;
   RmWrStatusInfo staInfo;

   { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 170); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmProcUeAdmission : Received                        UE admission request.\n"); SPrint((&(rmCb.init))->prntBuf); } }; };


   switch(estabCause)
   {
      case RRM_WR_EMERGENCY:

         ret = rmCacAdmProcEmergencyCall(cellCbPtr, crnti, transId);
         break;
      case RRM_WR_HIGHPRIORACCESS:
      case RRM_WR_MTACCESS:
      case RRM_WR_MOSIGNALLING:
      case RRM_WR_MODATA:

         if((cellCbPtr->ueCbLst.count + 1) > cellCbPtr->cellCfg.maxNumUes)
         {
            { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 186); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmProcUeAdm:" "Failed to admit the UE as maximum capacity is already exhausted."); SPrint((&(rmCb.init))->prntBuf); } }; };


            staInfo.status = RRM_WR_CFG_FAILURE;
            staInfo.cause = RRM_WR_CAUSE_MAX_CAPACITY_REACHED;

            rmCcmSendUeAdmitResp(cellCbPtr, crnti, &staInfo, transId);
         ...
         else
         {
            ...
            ret = rmCacAdmAddUe(cellCbPtr, crnti, transId, estabCause);
         }
...
# 233 "../..//enbapp/lterrm/rm_cac_adm.c"
static S16 rmCacAdmProcEmergencyCall
(
   RmCellCb *cellCbPtr,
   U16 crnti,
...
{
   S16 ret = 0;
   RmUeCb *relUeCbPtr = 0L;
   RmUeToAdmitCb *ueToAdmitPtr = 0L;
   RmWrStatusInfo staInfo;
   ...
   if((cellCbPtr->ueCbLst.count + 1) > cellCbPtr->cellCfg.maxNumUes)
   {
      ...
      relUeCbPtr = rmCacGetLeastPriorUeToRel(cellCbPtr);

      if(0L != relUeCbPtr)
      {


         { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&ueToAdmitPtr), (sizeof(RmUeToAdmitCb))) == 0) { { cmMemset((U8 *)((*(&ueToAdmitPtr))), 0, ((sizeof(RmUeToAdmitCb)))); }; } else { (*(&ueToAdmitPtr)) = 0L; } };
         if (ueToAdmitPtr == 0L)
         {
            { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 259); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmProcUeAdm:" "Failed to allocate memory for RmUeToAdmitCb"); SPrint((&(rmCb.init))->prntBuf); } }; };


            staInfo.status = RRM_WR_CFG_FAILURE;
            staInfo.cause = RRM_WR_CAUSE_OUT_OF_RESOURCE;
         ...
         rmCcmSendUeRel(relUeCbPtr, RRM_WR_CAUSE_OTHER);


         relUeCbPtr->cacUeCb.isRlsTriggered = 1;


         ueToAdmitPtr->ueId = crnti;
         ueToAdmitPtr->transId = transId;
         ueToAdmitPtr->cacUeCb.conEstabCause = RRM_WR_EMERGENCY;

         ueToAdmitPtr->ueToAdmitLstLnk.node = (U32)ueToAdmitPtr;

         cmLListAdd2Tail(&(cellCbPtr->ueToAdmitCbLst),
                         &(ueToAdmitPtr->ueToAdmitLstLnk));
      }
      else
      {
   ...
   else
   {
      ...
      ret = rmCacAdmAddUe(cellCbPtr, crnti, transId, RRM_WR_EMERGENCY);
   }
   return(ret);
}
# 325 "../..//enbapp/lterrm/rm_cac_adm.c"
static S16 rmCacAdmAddUe
(
   RmCellCb *cellCbPtr,
   U16 crnti,
...
)
{
   ...
   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&ueCbPtr), (sizeof(RmUeCb))) == 0) { { cmMemset((U8 *)((*(&ueCbPtr))), 0, ((sizeof(RmUeCb)))); }; } else { (*(&ueCbPtr)) = 0L; } };
   if (ueCbPtr == 0L)
   {
       { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 341); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmProcUeAdm:" "Failed to allocate memory for RmUeCb"); SPrint((&(rmCb.init))->prntBuf); } }; };
   ...
   ueCbPtr->ueId = crnti;
   ueCbPtr->cellCbPtr = cellCbPtr;
   ueCbPtr->cacUeCb.conEstabCause = estabCause;
   ueCbPtr->cacUeCb.isRlsTriggered = 0;

   ueCbPtr->ueCbLnk.node = (U32)ueCbPtr;

   cmLListAdd2Tail(&(cellCbPtr->ueCbLst), &(ueCbPtr->ueCbLnk));


   staInfo.status = RRM_WR_CFG_SUCCESS;
   staInfo.cause = RRM_WR_CAUSE_NOT_APP;

   rmCcmSendUeAdmitResp(cellCbPtr, crnti, &staInfo, transId);

   { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 368); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmAddUe : UE                        with UE id = %d admitted successfully.\n", crnti); SPrint((&(rmCb.init))->prntBuf); } }; };


   return(0);
}
# 396 "../..//enbapp/lterrm/rm_cac_adm.c"
 S16 rmCacAdmProcUeRel
(
   RmUeCb *ueCbPtr,
   TransId transId
)
{
   ...
   rmCacAdmLclUeRel(ueCbPtr, transId);


   rmCacAdmProcUeToAdmitLst(cellCbPtr);

   return(0);
}
# 429 "../..//enbapp/lterrm/rm_cac_adm.c"
static S16 rmCacAdmLclUeRel
(
   RmUeCb *ueCbPtr,
   TransId transId
)
{
   ...
   relStatusInfo.status = RRM_WR_CFG_SUCCESS;
   relStatusInfo.cause = RRM_WR_CAUSE_NOT_APP;


   rmRbcUeDelAllERabs(ueCbPtr);
   ...
   cmLListDelFrm(&(ueCbPtr->cellCbPtr->ueCbLst), &(ueCbPtr->ueCbLnk));

   { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 452); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmLclUeRel UE with UE ID = %d                         released successfully.\n", ueCbPtr->ueId); SPrint((&(rmCb.init))->prntBuf); } }; };


   rmCcmSendUeRelResp(ueCbPtr->cellCbPtr->cellId, ueCbPtr->ueId,
                         &relStatusInfo, transId);

   { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (ueCbPtr), (Size) (sizeof(RmUeCb))); (ueCbPtr) = 0L; };

   return(0);
}
# 484 "../..//enbapp/lterrm/rm_cac_adm.c"
static S16 rmCacAdmProcUeToAdmitLst
(
   RmCellCb *cellCbPtr
)
{
   U16 ueToAdmitId;
   TransId transId;
   CmLList *ueToAdmitLstLnk = 0L;
   RmUeCb *newUeCbPtr = 0L;
   RmUeToAdmitCb *ueToAdmitInfoPtr = 0L;
   RmWrStatusInfo staInfo;



   ((((ueToAdmitLstLnk) = ((&(cellCbPtr->ueToAdmitCbLst))->crnt = (&(cellCbPtr->ueToAdmitCbLst))->first)) != 0L) ? (ueToAdmitLstLnk)->node : 0L);

   if(0L != ueToAdmitLstLnk)
   {
      ueToAdmitInfoPtr = (RmUeToAdmitCb *)(ueToAdmitLstLnk->node);


      { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&newUeCbPtr), (sizeof(RmUeCb))) == 0) { { cmMemset((U8 *)((*(&newUeCbPtr))), 0, ((sizeof(RmUeCb)))); }; } else { (*(&newUeCbPtr)) = 0L; } };
      if (newUeCbPtr == 0L)
      {
         { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 509); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "WrRmUeAdmitReq:" "Failed to allocate memory for RmUeCb"); SPrint((&(rmCb.init))->prntBuf); } }; };


         staInfo.status = RRM_WR_CFG_FAILURE;
         staInfo.cause = RRM_WR_CAUSE_OUT_OF_RESOURCE;

         rmCcmSendUeAdmitResp(cellCbPtr, ueToAdmitId,
                                               &staInfo, transId);
         return(1);
      }


      newUeCbPtr->ueId = ueToAdmitInfoPtr->ueId;
      newUeCbPtr->cacUeCb.conEstabCause = RRM_WR_EMERGENCY;
      newUeCbPtr->cellCbPtr = cellCbPtr;

      newUeCbPtr->ueCbLnk.node = (U32)newUeCbPtr;

      cmLListAdd2Tail(&(cellCbPtr->ueCbLst), &(newUeCbPtr->ueCbLnk));


      ueToAdmitId = ueToAdmitInfoPtr->ueId;
      transId = ueToAdmitInfoPtr->transId;


      cmLListDelFrm(&(cellCbPtr->ueToAdmitCbLst), ueToAdmitLstLnk);
      { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (ueToAdmitInfoPtr), (Size) (sizeof(RmUeToAdmitCb))); (ueToAdmitInfoPtr) = 0L; };


      staInfo.status = RRM_WR_CFG_SUCCESS;
      staInfo.cause = RRM_WR_CAUSE_NOT_APP;

      { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac_adm.c", 541); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAdmProcUeToAdmitLst : Admited one UE                        with UE Id = %d from the ueToAdmitLis.\n", ueToAdmitId); SPrint((&(rmCb.init))->prntBuf); } }; };



      rmCcmSendUeAdmitResp(cellCbPtr, ueToAdmitId, &staInfo, transId);

   }

   return(0);
}
# 572 "../..//enbapp/lterrm/rm_cac_adm.c"
 S16 rmCacAdmProcUeRelFromRrm
(
   RmUeCb *ueCbPtr,
   RrmWrCause cause
)
{
   ...
   rmCcmSendUeRel(ueCbPtr, cause);


   ueCbPtr->cacUeCb.isRlsTriggered = 1;

   return(0);

}
# 606 "../..//enbapp/lterrm/rm_cac_adm.c"
 S16 rmCacAdmDelAllUes
(
   RmCellCb *cellCbPtr
)
{
   CmLList *ueCbLnk = 0L;
   CmLList *waitUeCbLnk = 0L;
   RmUeCb *ueCbToDelPtr = 0L;
   RmUeToAdmitCb *waitUeToDelPtr = 0L;
   RmWrStatusInfo staInfo;

   ((((ueCbLnk) = ((&(cellCbPtr->ueCbLst))->crnt = (&(cellCbPtr->ueCbLst))->first)) != 0L) ? (ueCbLnk)->node : 0L);

   while (0L != ueCbLnk)
   {


      ueCbToDelPtr = (RmUeCb *)(ueCbLnk->node);


      rmRbcUeDelAllERabs( ueCbToDelPtr);

      cmLListDelFrm(&(cellCbPtr->ueCbLst), ueCbLnk);
      { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (ueCbToDelPtr), (Size) (sizeof(RmUeCb))); (ueCbToDelPtr) = 0L; };
      ueCbLnk = ueCbLnk->next;
   }

   ((((waitUeCbLnk) = ((&(cellCbPtr->ueToAdmitCbLst))->crnt = (&(cellCbPtr->ueToAdmitCbLst))->first)) != 0L) ? (waitUeCbLnk)->node : 0L);

   while (0L != waitUeCbLnk)
   {


      waitUeToDelPtr = (RmUeToAdmitCb *)(waitUeCbLnk->node);


      staInfo.status = RRM_WR_CFG_FAILURE;
      staInfo.cause = RRM_WR_CAUSE_OTHER;

      rmCcmSendUeAdmitResp(cellCbPtr, waitUeToDelPtr->ueId,
                               &staInfo, waitUeToDelPtr->transId);


      cmLListDelFrm(&(cellCbPtr->ueToAdmitCbLst), waitUeCbLnk);
      { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (waitUeToDelPtr), (Size) (sizeof(RmUeToAdmitCb))); (waitUeToDelPtr) = 0L; };

      waitUeCbLnk = waitUeCbLnk->next;
   }
   return(0);
}
# 670 "../..//enbapp/lterrm/rm_cac_adm.c"
 U8 rmCacAdmGetEstabCause
(
   RmUeCb *ueCbPtr
)
{
   return(ueCbPtr->cacUeCb.conEstabCause);
}
