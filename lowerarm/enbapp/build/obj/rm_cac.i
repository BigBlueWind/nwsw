...
static S16 rmCacFillSib2CfgInfo ( RrmWrCellRecfgIndInfo *cellRecfgInd,
                                   RrmWrSib2Cfg *sib2CfgToFill);
# 130 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacSendCellRecfgInd
(
   CmLteCellId cellId,
   U16 cacRecfgBtMsk,
   Void *recfgParamsPtr
...
{
   RrmWrCellRecfgIndInfo *cellRecfgInd;

   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&cellRecfgInd), (sizeof (RrmWrCellRecfgIndInfo))) == 0) { { cmMemset((U8 *)((*(&cellRecfgInd))), 0, ((sizeof (RrmWrCellRecfgIndInfo)))); }; } else { (*(&cellRecfgInd)) = 0L; } };

   { cmMemset((U8 *)(cellRecfgInd), 0, (sizeof (RrmWrCellRecfgIndInfo))); };

   if (cellRecfgInd == 0L)
   {
      { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 145); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "Could not allocate memory\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
      return(1);
   }

   cellRecfgInd->cellId = cellId;


   switch (cacRecfgBtMsk)
   {
      case 0x01:
         rmCacFillSib2CfgInfo(cellRecfgInd, (RrmWrSib2Cfg *)recfgParamsPtr);
         { { if ((&(rmCb.init))->dbgMask & (3)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 157); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCacAcbProcRrcConReqNum: Sending                  changed ACB configuration\n"); SPrint((&(rmCb.init))->prntBuf); } }; };

         break;

      default:
         { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 161); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "Imporper CellREcfg Bit mask\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
         break;
   }


   rmCcmSendCellRecfg(cellRecfgInd);
   return(0);

}
# 188 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacInit(Void)
{
   ...
   retVal = rmCacAcbEnbInit();

   return(retVal);
}
# 219 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacAddCellCfg
(
   RmCellCb *cellCb,
   RrmWrCause *cause
)
{
   ...
   retVal = rmCacAcbCellInit(cellCb->cellId, &(cellCb->cacCellCb));

   if(1 == retVal)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 232); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "ACB Cell Init failed\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
      return(1);
   }

   cmLListInit(&cellCb->ueToAdmitCbLst);

   return(retVal);
}
# 262 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacModCellCfg
(
   RmCellCb *cellCb,
   U32 cacMask
)
{
   ...
   if (cacMask & (0x0002 | 0x0004))
   {
      rmCacAcbModCell(&(cellCb->cellCfg), cacMask);
   }

   return(0);
}
# 296 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacDelCellCfg
(
   RmCellCb *cellCbPtr
)
{
   S16 ret;
   rmCacAcbCellDeinit(cellCbPtr->cellId, &(cellCbPtr->cacCellCb));
   ret = rmCacAdmDelAllUes(cellCbPtr);
   return(ret);
}
# 327 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacEnbStaUpd
(
...
)
{
   ...
   switch(enbStaIndInfo->staType)
   {
      case RRM_WR_ENBSTATYPE_MMESTA:
         {
            rmCacAcbMmeOvldInd(&(enbStaIndInfo->mmeStatus));
         }
         break;
      case RRM_WR_ENBSTATYPE_CPULOAD:
         {
            rmCacAcbCpuLdInd(enbStaIndInfo->cpuLoad);
         }
         break;
      default:
         {
            { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 349); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "Invalide Cac enodeb status update\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
            return(1);
         }
         break;
   }
...
static S16 rmCacFillSib2CfgInfo
(
   RrmWrCellRecfgIndInfo *cellRecfgInd,
   RrmWrSib2Cfg *sib2CfgToFill
)
{
   ...
   cellRecfgInd->recfgType |= 0x0001;

   { cmMemcpy((U8*) (&(cellRecfgInd->sib2Cfg)), (U8*) (sib2CfgToFill), (sizeof(RrmWrSib2Cfg))); };
   return(0);
}
# 392 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacUeAdm
(
   RmCellCb *cellCbPtr,
   U16 crnti,
...
)
{
   ...
   ret = rmCacAcbRrcConReqInd(crnti, &(cellCbPtr->cacCellCb));
   if (ret != 0)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 405); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "CAC ACB RRC Connectiopn Req Count Hyst Mgmt failed\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
      return(ret);
   }

   ret = rmCacAdmProcUeAdmission(cellCbPtr, crnti, transId, estabCause);
   if (ret != 0)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_cac.c", 412); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "CAC  Admit Req processing failed\n"); SPrint((&(rmCb.init))->prntBuf); } }; };
      return(ret);
   }

   return(0);
}
# 436 "../..//enbapp/lterrm/rm_cac.c"
 RmUeCb* rmCacGetLeastPriorUeToRel
(
   RmCellCb *cellCbPtr
)
{
   Bool leastPriorUeFound;
   RmUeCb *ueCbPtr;
   CmLList *node;

   leastPriorUeFound = 1;

   ueCbPtr = rmRbcGetLeastPriorUeToRel(cellCbPtr);

   if (0L == ueCbPtr)
   {
      leastPriorUeFound = 0;

      node = cellCbPtr->ueCbLst.first;

      for (; node; node = node->next)
      {
         ueCbPtr = (RmUeCb *)node->node;

         if (ueCbPtr->cacUeCb.conEstabCause != RRM_WR_EMERGENCY)
         {
            leastPriorUeFound = 1;
            break;
         }
...
)
{
   S16 ret;
    ...
    ret = rmCacAdmProcUeRel(ueCbPtr, transId);

    return(ret);
}
# 528 "../..//enbapp/lterrm/rm_cac.c"
 S16 rmCacRelUeFromRrm
(
   RmUeCb *ueCbPtr,
   RrmWrCause cause
)
{
    ...
    ret = rmCacAdmProcUeRelFromRrm(ueCbPtr, cause);
    return(ret);
}
