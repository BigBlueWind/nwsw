...
# 115 "../..//lterrc/nh_brm.c" 2
# 1 "../..//cm/cm5.x" 1
# 116 "../..//lterrc/nh_brm.c" 2
# 1 "../..//cm/cm_lte.x" 1
# 91 "../..//cm/cm_lte.x"
#define __CM_LTE_X__ 
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_brm.c", 185); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhBRMSysInfoTx"); SPrint((&nhCb.init)->prntBuf); } }; };
                    ...
                    cmnMsg->cmn.bcchDlSchMsg.message.val.c1.choice.val)
   {
      ...
      if ((((sib1->cellAccessRelatedInfo.csg_Identity).pres) != 0))
      {
         cellAndUeCb->cellCb->crntCellCfgInfo.isCSGCell = 1;
      }
      else
      {
         cellAndUeCb->cellCb->crntCellCfgInfo.isCSGCell = 0;
      }
   }
# 228 "../..//lterrc/nh_brm.c"
   if (0x06 == cmnMsg->cmnMsgCategory)
   {
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_brm.c", 255); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "encoded length is more than max TB size\n"); SPrint((&nhCb.init)->prntBuf); } }; };


      { (*errInfo).errPres = 1; (*errInfo).errType = (0x0e); (*errInfo).errCause = (0x18); };

      do{ if ((encBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_brm.c", 259, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((encBuf)); (encBuf) = 0L; } }while(0);
      return(1);
   }
   ...
   datReqParam.u.rlcParam.lcType = 1;
   ...
   if (0 != ret)
   {
      ...
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_brm.c", 285); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Send to lower layer failed(ret:%d) \n", ret); SPrint((&nhCb.init)->prntBuf); } }; };
