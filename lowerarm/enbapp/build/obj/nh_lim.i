...
# 2207 "../..//cm/ssi.h"
#define PID_STK(h) (h)
# 111 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/cm5.h" 1
# 112 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/cm_tkns.h" 1
# 91 "../..//cm/cm_tkns.h"
#define __CMTKNSH 
# 115 "../..//cm/cm_tkns.h"
...
#define CM_PASN_DROP_ELMNTS 1
#define CM_PASN_PASS_ELMNTS 2
#define CM_PASN_GEN_ERR 3



#define CM_PASN_UNEXP_VAL 0x01
#define CM_PASN_RES_ERR 0x02
#define CM_PASN_OUT_RANGE 0x03
#define CM_PASN_EXTRA_PARAM 0x04
#define CM_PASN_BAD_IDX 0x05
#define CM_PASN_UNDEF_PARAM 0x06
#define CM_PASN_MAND_MIS 0x07
#define CM_PASN_DB_ERR 0x08
#define CM_PASN_DEF_MIS 0x09
#define CM_PASN_ESC_FUNC_FLD 0x0A
...
#define CM_PASN_ELM_INV 0x0
#define CM_PASN_ELM_OPT 0x1
#define CM_PASN_ELM_MAND 0x2
#define CM_PASN_ELM_MANDIGN 0x3
#define CM_PASN_ELM_DEF 0x4
...
#define CM_PASN_H245_3 0
#define CM_PASN_H225_2 4
#define CM_PASN_H225RAS_2 8
# 223 "../..//cm/cm_pasn.h"
...
# 278 "../..//cm/cm_pasn.h"
#define CM_PASN_GET_INITBYTE(_msgCp) (CM_PASN_BYTE_ALLONE << (_msgCp->bhldr.nxtBitPos) );


#define CM_PASN_ENC_BYTE(_msgCp,_byte) { if( _msgCp->mMgmt == CM_PASN_USE_MBUF) { if( (SAddPstMsg((Data)_byte, _msgCp->mBuf)) != ROK) RETVALUE(RFAILED); } else if( _msgCp->mMgmt == CM_PASN_USE_SBUF) _msgCp->fBuf.bufP[_msgCp->fBuf.crntIdx++] = _byte; }
# 293 "../..//cm/cm_pasn.h"
#define CM_PASN_OCTET_ALIGN(_msgCp) { if( _msgCp->bhldr.nxtBitPos != CM_PASN_NXTBITPOS_EIGHT) { _msgCp->bhldr.byte &= CM_PASN_GET_INITBYTE(_msgCp); CM_PASN_ENC_BYTE(_msgCp, _msgCp->bhldr.byte); _msgCp->bhldr.nxtBitPos = CM_PASN_NXTBITPOS_EIGHT; _msgCp->bhldr.byte = CM_PASN_VAL_NULL; } }
# 304 "../..//cm/cm_pasn.h"
#define CM_PASN_REMOVE_NULLBYTES(_val,_len) { U8 i, zeroCnt = 0; if (_len > 1) { for (i = 0; i < _len; i++) { if (_val[i] == 0) zeroCnt++; else break; } if (zeroCnt > 0) { for (i = zeroCnt; i < _len; i++) { _val[i - zeroCnt] = _val[i]; } } _len = _len - zeroCnt; } }
# 330 "../..//cm/cm_pasn.h"
#define CM_PASN_COMPARE_UINTXL(_val1,_len1,_val2,_len2,_result) { U8 indx; _result = CM_PASN_UINTXL_VAL_EQUAL; if (_len1 != _len2) { _result = CM_PASN_UINTXL_VAL_LARGE; if (_len1 < _len2) _result = CM_PASN_UINTXL_VAL_SMALL; } else { for (indx = 0; indx < _len1; indx++) { if (_val1[indx] < _val2[indx]) { _result = CM_PASN_UINTXL_VAL_SMALL; break; } if (_val1[indx] > _val2[indx]) { _result = CM_PASN_UINTXL_VAL_LARGE; break; } } } }
# 118 "../..//lterrc/nh_lim.c" 2
...
#define EVTLNHCFGREQ 0
#define EVTLNHCFGCFM 1
#define EVTLNHCNTRLREQ 2
#define EVTLNHCNTRLCFM 3
#define EVTLNHSTAREQ 4
#define EVTLNHSTACFM 5
#define EVTLNHSTSREQ 6
#define EVTLNHSTSCFM 7
#define EVTLNHSTAIND 8
#define EVTLNHTRCIND 9
...
#define LNH_DATAMODE_RLC 0
#define LNH_DATAMODE_MAC 1





#define LNH_RLC_UM_MODE 0
#define LNH_RLC_AM_MODE 1
#define LNH_RLC_TM_MODE 2
...
#define LNH_SEL_LC 0
#define LNH_SEL_TC 1
#define LNH_SEL_LWLC 2




#define LNH_SEL_TC_RU 1
...
#define ELNH001 (ERRLNH + 1)
#define ELNH002 (ERRLNH + 2)
#define ELNH003 (ERRLNH + 3)
#define ELNH004 (ERRLNH + 4)
#define ELNH005 (ERRLNH + 5)
#define ELNH006 (ERRLNH + 6)
#define ELNH007 (ERRLNH + 7)
#define ELNH008 (ERRLNH + 8)
#define ELNH009 (ERRLNH + 9)
#define ELNH010 (ERRLNH + 10)
#define ELNH011 (ERRLNH + 11)
#define ELNH012 (ERRLNH + 12)
#define ELNH013 (ERRLNH + 13)
#define ELNH014 (ERRLNH + 14)
#define ELNH015 (ERRLNH + 15)
#define ELNH016 (ERRLNH + 16)
#define ELNH017 (ERRLNH + 17)
#define ELNH018 (ERRLNH + 18)
#define ELNH019 (ERRLNH + 19)
#define ELNH020 (ERRLNH + 20)
#define ELNH021 (ERRLNH + 21)
#define ELNH022 (ERRLNH + 22)
#define ELNH023 (ERRLNH + 23)
#define ELNH024 (ERRLNH + 24)
#define ELNH025 (ERRLNH + 25)
#define ELNH026 (ERRLNH + 26)
#define ELNH027 (ERRLNH + 27)
#define ELNH028 (ERRLNH + 28)
#define ELNH029 (ERRLNH + 29)
#define ELNH030 (ERRLNH + 30)
#define ELNH031 (ERRLNH + 31)
#define ELNH032 (ERRLNH + 32)
# 120 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/kwu.h" 1
# 92 "../..//cm/kwu.h"
#define __KWU_H__ 
# 105 "../..//cm/kwu.h"
...
#define KWU_EVT_BND_REQ 0x50
#define KWU_EVT_BND_CFM 0x51
#define KWU_EVT_UBND_REQ 0x52
#define KWU_EVT_DAT_REQ 0x53
#define KWU_EVT_DAT_IND 0x54
#define KWU_EVT_DAT_CFM 0x55
#define KWU_EVT_DISC_SDU_REQ 0x56
#define KWU_EVT_STA_IND 0x57
#define KWU_EVT_REEST_CMP_IND 0x58


#define KWU_EVT_DISC_SDU_CFM 0x59
#define KWU_EVT_DAT_ACK_IND 0x5a

#define KWU_MAX_STA_IND_SDU 10



#define ERRKWU 0
#define EKWUXXX 0
#define EKWU001 (ERRKWU + 1)
#define EKWU002 (ERRKWU + 2)
#define EKWU003 (ERRKWU + 3)
#define EKWU004 (ERRKWU + 4)
#define EKWU005 (ERRKWU + 5)
#define EKWU006 (ERRKWU + 6)
#define EKWU007 (ERRKWU + 7)
#define EKWU008 (ERRKWU + 8)
#define EKWU009 (ERRKWU + 9)
#define EKWU010 (ERRKWU + 10)
#define EKWU011 (ERRKWU + 11)
#define EKWU012 (ERRKWU + 12)
#define EKWU013 (ERRKWU + 13)
#define EKWU014 (ERRKWU + 14)
#define EKWU015 (ERRKWU + 15)
#define EKWU016 (ERRKWU + 16)
#define EKWU017 (ERRKWU + 17)
#define EKWU018 (ERRKWU + 18)
#define EKWU019 (ERRKWU + 19)
#define EKWU020 (ERRKWU + 20)
#define EKWU021 (ERRKWU + 21)
#define EKWU022 (ERRKWU + 22)
#define EKWU023 (ERRKWU + 23)
#define EKWU024 (ERRKWU + 24)
#define EKWU025 (ERRKWU + 25)
#define EKWU026 (ERRKWU + 26)
#define EKWU027 (ERRKWU + 27)
#define EKWU028 (ERRKWU + 28)
#define EKWU029 (ERRKWU + 29)
#define EKWU030 (ERRKWU + 30)
#define EKWU031 (ERRKWU + 31)
#define EKWU032 (ERRKWU + 32)
#define EKWU033 (ERRKWU + 33)
#define EKWU034 (ERRKWU + 34)
#define EKWU035 (ERRKWU + 35)
...
#define CKW_MAX_ENT_CFG 24





#define CKW_CFG_ADD 1
#define CKW_CFG_MODIFY 2
#define CKW_CFG_DELETE 3
#define CKW_CFG_REESTABLISH 4
#define CKW_CFG_DELETE_UE 5
#define CKW_CFG_DELETE_CELL 6


#define CKW_CFG_DIR_UL 1
#define CKW_CFG_DIR_DL 2
#define CKW_CFG_DIR_BOTH 3


#define CKW_CFG_CFM_OK 1
#define CKW_CFG_CFM_NOK 2


#define CKW_CFG_REAS_NONE 0
#define CKW_CFG_REAS_RB_CREAT_FAIL 1
#define CKW_CFG_REAS_UE_CREAT_FAIL 2
#define CKW_CFG_REAS_CELL_CREAT_FAIL 3
#define CKW_CFG_REAS_RB_PRSNT 4
#define CKW_CFG_REAS_LCHTYPE_MIS 5

#define CKW_CFG_REAS_RB_UNKWN 6
#define CKW_CFG_REAS_UE_UNKWN 7
#define CKW_CFG_REAS_CELL_UNKWN 8
#define CKW_CFG_REAS_RB_DEL_FAIL 9
#define CKW_CFG_REAS_UE_DEL_FAIL 10
#define CKW_CFG_REAS_RB_REEST_FAIL 11
#define CKW_CFG_REAS_RB_MODE_MIS 12
#define CKW_CFG_REAS_REEST_FAIL 13
#define CKW_CFG_REAS_RECFG_FAIL 14
#define CKW_CFG_REAS_INVALID_CFG 15
#define CKW_CFG_REAS_SAME_UEID 16
#define CKW_CFG_REAS_CELL_DEL_FAIL 17
#define CKW_CFG_REAS_INVALID_LCHID 18
#define CKW_CFG_REAS_INVALID_DIR 19
#define CKW_CFG_REAS_UE_EXISTS 20




#define CKW_EVT_BND_REQ 0x01
#define CKW_EVT_BND_CFM 0x02
#define CKW_EVT_UBND_REQ 0x03
#define CKW_EVT_CFG_REQ 0x04
#define CKW_EVT_CFG_CFM 0x05
#define CKW_EVT_UEIDCHG_REQ 0x06
#define CKW_EVT_UEIDCHG_CFM 0x07


#define ERRCKW 0
#define ECKWXXX 0
#define ECKW001 (ERRCKW + 1)
#define ECKW002 (ERRCKW + 2)
#define ECKW003 (ERRCKW + 3)
#define ECKW004 (ERRCKW + 4)
#define ECKW005 (ERRCKW + 5)
#define ECKW006 (ERRCKW + 6)
#define ECKW007 (ERRCKW + 7)
#define ECKW008 (ERRCKW + 8)
#define ECKW009 (ERRCKW + 9)
#define ECKW010 (ERRCKW + 10)
#define ECKW011 (ERRCKW + 11)
#define ECKW012 (ERRCKW + 12)
#define ECKW013 (ERRCKW + 13)
#define ECKW014 (ERRCKW + 14)
#define ECKW015 (ERRCKW + 15)
#define ECKW016 (ERRCKW + 16)
#define ECKW017 (ERRCKW + 17)
#define ECKW018 (ERRCKW + 18)
#define ECKW019 (ERRCKW + 19)
#define ECKW020 (ERRCKW + 20)
#define ECKW021 (ERRCKW + 21)
#define ECKW022 (ERRCKW + 22)
#define ECKW023 (ERRCKW + 23)
#define ECKW024 (ERRCKW + 24)
#define ECKW025 (ERRCKW + 25)
#define ECKW026 (ERRCKW + 26)
#define ECKW027 (ERRCKW + 27)
#define ECKW028 (ERRCKW + 28)
#define ECKW029 (ERRCKW + 29)
#define ECKW030 (ERRCKW + 30)
#define ECKW031 (ERRCKW + 31)
#define ECKW032 (ERRCKW + 32)
#define ECKW033 (ERRCKW + 33)
#define ECKW034 (ERRCKW + 34)
#define ECKW035 (ERRCKW + 35)
#define ECKW036 (ERRCKW + 36)
#define ECKW037 (ERRCKW + 37)
#define ECKW038 (ERRCKW + 38)
#define ECKW039 (ERRCKW + 39)
#define ECKW040 (ERRCKW + 40)
#define ECKW041 (ERRCKW + 41)
#define ECKW042 (ERRCKW + 42)
#define ECKW043 (ERRCKW + 43)
#define ECKW044 (ERRCKW + 44)
# 122 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/crg.h" 1
# 89 "../..//cm/crg.h"
#define __CRG_H__ 
...
#define CRG_CELL_CFG 1
#define CRG_UE_CFG 2
#define CRG_LCH_CFG 3



#define CRG_CFG_TRANSID_SIZE 12


#define CRG_DIR_TX_RX CRG_DIR_TX | CRG_DIR_RX

#define CRG_DIR_TX (1<<0)

#define CRG_DIR_RX (1<<1)



#define CRG_CFG_CFM_OK 0x00
#define CRG_CFG_CFM_NOK 0x0F


#define EVTCRGBNDREQ 1
#define EVTCRGBNDCFM 2
#define EVTCRGUBNDREQ 3
#define EVTCRGCFGREQ 4
#define EVTCRGCFGCFM 5


#define CRG_ACTV_WIN_SIZE 20
...
#define ECRG001 (ERRCRG + 1)
#define ECRG002 (ERRCRG + 2)
#define ECRG003 (ERRCRG + 3)
#define ECRG004 (ERRCRG + 4)
#define ECRG005 (ERRCRG + 5)
#define ECRG006 (ERRCRG + 6)
#define ECRG007 (ERRCRG + 7)
#define ECRG008 (ERRCRG + 8)
#define ECRG009 (ERRCRG + 9)
#define ECRG010 (ERRCRG + 10)
#define ECRG011 (ERRCRG + 11)
#define ECRG012 (ERRCRG + 12)
#define ECRG013 (ERRCRG + 13)
#define ECRG014 (ERRCRG + 14)
#define ECRG015 (ERRCRG + 15)
#define ECRG016 (ERRCRG + 16)
#define ECRG017 (ERRCRG + 17)
#define ECRG018 (ERRCRG + 18)
#define ECRG019 (ERRCRG + 19)
#define ECRG020 (ERRCRG + 20)
#define ECRG021 (ERRCRG + 21)
#define ECRG022 (ERRCRG + 22)
#define ECRG023 (ERRCRG + 23)
#define ECRG024 (ERRCRG + 24)
#define ECRG025 (ERRCRG + 25)
#define ECRG026 (ERRCRG + 26)
#define ECRG027 (ERRCRG + 27)
#define ECRG028 (ERRCRG + 28)
#define ECRG029 (ERRCRG + 29)
#define ECRG030 (ERRCRG + 30)
#define ECRG031 (ERRCRG + 31)
# 123 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/cpj.h" 1
# 88 "../..//cm/cpj.h"
#define __CPJ_H__ 
# 99 "../..//cm/cpj.h"
...
#define NH_EVNT_LI_INVALID 0
#define NH_EVNT_LI_RRC_CON_REQ 1
#define NH_EVNT_LI_RRC_REESTAB_REQ 2
#define NH_EVNT_LI_UL_DT 3
#define NH_EVNT_LI_RRC_CON_SETUP_CMP 4
#define NH_EVNT_LI_RRC_CON_RECFG_CMP 5
#define NH_EVNT_LI_SEC_MOD_CMP 6
#define NH_EVNT_LI_SEC_MOD_FAIL 7
#define NH_EVNT_LI_RRC_CON_RLS_CMP 8
#define NH_EVNT_LI_UL_HO_PREP_TFR 9
#define NH_EVNT_LI_CSFB_PARAM_REQ 10
#define NH_EVNT_LI_UE_CAP_INFO 11
#define NH_EVNT_LI_COUNT_CHK_RSP 12
#define NH_EVNT_LI_RRC_MES_RPT 13
#define NH_EVNT_LI_RRC_CON_RESTB_CMP 14

#define NH_EVNT_LI_UE_INFO_RSP 15
#define NH_EVNT_LI_PRXMTY_IND 16
...
#define NH_EVNT_LI_MAX_EVNTS 17
# 147 "../..//lterrc/nh.h"
#define NH_EVNT_UI_INVALID (NH_EVNT_LI_MAX_EVNTS + 0)
#define NH_EVNT_UI_DL_DT (NH_EVNT_LI_MAX_EVNTS + 1)
#define NH_EVNT_UI_SEC_MOD_CMD (NH_EVNT_LI_MAX_EVNTS + 2)
#define NH_EVNT_UI_RRC_CON_RECFG (NH_EVNT_LI_MAX_EVNTS + 3)
#define NH_EVNT_UI_RRC_CON_RLS (NH_EVNT_LI_MAX_EVNTS + 4)
#define NH_EVNT_UI_SYS_INFO_TX (NH_EVNT_LI_MAX_EVNTS + 5)
#define NH_EVNT_UI_PAGING_MSG (NH_EVNT_LI_MAX_EVNTS + 6)
#define NH_EVNT_UI_CONN_REESTB (NH_EVNT_LI_MAX_EVNTS + 7)
#define NH_EVNT_UI_CONN_REESTB_REJ (NH_EVNT_LI_MAX_EVNTS + 8)
#define NH_EVNT_UI_CONN_SETUP (NH_EVNT_LI_MAX_EVNTS + 9)
#define NH_EVNT_UI_CONN_REJ (NH_EVNT_LI_MAX_EVNTS + 10)
#define NH_EVNT_UI_MOB_FRM_EUTRA_CMD (NH_EVNT_LI_MAX_EVNTS + 11)
#define NH_EVNT_UI_HO_FRM_EUTRA_PRP_TFR (NH_EVNT_LI_MAX_EVNTS + 12)
#define NH_EVNT_UI_CSFB_PARAM_RSP (NH_EVNT_LI_MAX_EVNTS + 13)
#define NH_EVNT_UI_UE_CAP_ENQ (NH_EVNT_LI_MAX_EVNTS + 14)
#define NH_EVNT_UI_COUNT_CHK (NH_EVNT_LI_MAX_EVNTS + 15)

#define NH_EVNT_UI_UE_INFO_REQ (NH_EVNT_LI_MAX_EVNTS + 16)


#define NH_INVALID_RRC_TRANS_ID 0xff
...
#define NH_EVNT_UI_MAX_EVNTS 17
# 186 "../..//lterrc/nh.h"
#define NH_MAX_RB_PER_CELL_CFG 8
#define NH_MAX_CFGD_RB_CELL 6
#define NH_MAX_MAC_CELL_CFG_ELMNT 2
#define NH_MAX_MAC_UE_CFG_ELMNT 2




#define NH_LCID_DIR_TXRX (NH_LCID_DIR_TX | NH_LCID_DIR_RX)
#define NH_LCID_DIR_TX 0x01
#define NH_LCID_DIR_RX 0x02




#define NH_MAX_LC_PER_RB 2
#define NH_DL_LC 0
#define NH_UL_LC 1

#define NH_MAX_SEC_CFG 2
#define NH_INTG_CFG 0
#define NH_CIPH_CFG 1

#define NH_STATUS_UNUSED 0
#define NH_STATUS_USED 1





#define NH_CFG_ACTN_ADD 0x01
#define NH_CFG_ACTN_MODIFY 0x02
#define NH_CFG_ACTN_RLS 0x04




#define NH_NEW_UECB_BINS 32

#define NH_UECB_BINS 256
#define NH_UECB_OFFSET 0

#define NH_CELLCB_BINS 256
#define NH_CELLCB_OFFSET 0




#define NH_TRANSCB_BINS 256
#define NH_TRANSCB_OFFSET 0





#define NH_RESERVED_TRANS_ID 0xffffffff
...
#define NH_DAT_REQ_SDU 0x00
#define NH_DAT_RSP_SDU 0x01





#define NH_AWAIT_NO_RSP 0x00
#define NH_AWAIT_UE_RSP 0x01
#define NH_AWAIT_DAT_CFM 0x02
...
#define NH_SRB1_CFGD 0x01
#define NH_SEC_INIT 0x02
#define NH_SEC_CFG_DONE 0x04
#define NH_SEC_CMPLTD 0x08




#define NH_CON_RESTB_CMP 0x01





#define NH_RB_ID_SRB0 0
#define NH_RB_ID_SRB1 1
#define NH_RB_ID_SRB2 2




#define NH_IGNORE 0
#define NH_MSGTYPE_SIB1 1
#define NH_MSGTYPE_SI 2
#define NH_MSGTYPE_PAG 3
#define NH_MSGTYPE_CONNREQ 4
#define NH_MSGTYPE_CONNSETUP 5
#define NH_MSGTYPE_CONNSETUPCOMP 6
#define NH_MSGTYPE_CONNREJ 7
#define NH_MSGTYPE_CONNRECFG 8
#define NH_MSGTYPE_CONNRECFGCOMP 9
#define NH_MSGTYPE_CONNRLS 10
#define NH_MSGTYPE_DLTFR 11
#define NH_MSGTYPE_ULTFR 12
#define NH_MSGTYPE_CONNREESTABREQ 13
#define NH_MSGTYPE_CONNREESTAB 14
#define NH_MSGTYPE_CONNREESTABCOMP 15
#define NH_MSGTYPE_CONNREESTABREJ 16
#define NH_MSGTYPE_SECMODCMD 17
#define NH_MSGTYPE_SECMODCOMP 18
#define NH_MSGTYPE_SECMODFAIL 19
#define NH_MSGTYPE_UECAPENQ 20
#define NH_MSGTYPE_UECAPINFO 21
#define NH_MSGTYPE_COUNTCHK 22
#define NH_MSGTYPE_COUNTCHKRSP 23
#define NH_MSGTYPE_MEASUREMENTRPT 24
#define NH_MSGTYPE_MOBFRMEUTRAN 25
#define NH_MSGTYPE_HOFRMEUTRAPREPREQ 26
#define NH_MSGTYPE_ULHOPREPTFR 27
#define NH_MSGTYPE_CSFBPARAMREQCDMA 28
#define NH_MSGTYPE_CSFBPARAMRSPCDMA 29
#define NH_MSGTYPE_CONNRECFGCOMP_HO 30

#define NH_MSGTYPE_HOPREPINFO 31

#define NH_MSGTYPE_UEINFOREQ 32
#define NH_MSGTYPE_UEINFORSP 33
#define NH_MSGTYPE_PRXMTYIND 34




#define NH_STS_CAUSE_CON_SUCC 1
#define NH_STS_CAUSE_CON_FAIL 2
#define NH_STS_CAUSE_RECFG_SUCC 3

#define NH_STS_CAUSE_RECFG_FAIL 4
#define NH_STS_CAUSE_LOWER_LYR_FAIL 5

#define NH_STS_CAUSE_DEC_ERR 6
#define NH_STS_CAUSE_ENC_ERR 7
#define NH_STS_CAUSE_PDCP_ERR 8



#define NH_STS_CAUSE_RLC_ERR 9


#define NH_STS_CAUSE_CON_RESTB_SUCC 10
#define NH_STS_CAUSE_CON_RESTB_FAIL 11
...
#define NH_TRANSCB_CFG_REQ 0xEE
#define NH_TRANSCB_SDU_STA_RESTAB 0xEF





#define NH_STS_CAUSE_SUCCESS 0
#define NH_STS_CAUSE_MOB_FRM_EUTRA_FAIL 1
#define NH_STS_CAUSE_HO_TO_EUTRA_SUCC 2




#define NH_PASN_MAX_BLOCK_SIZE 4096
#define NH_PASN_DBG_FLAG TRUE




#define NH_TMR_WAIT_CPJ_BNDCFM 1
#define NH_TMR_WAIT_CKW_BNDCFM 2
#define NH_TMR_WAIT_CRG_BNDCFM 3
#define NH_TMR_WAIT_PJU_BNDCFM 4
#define NH_TMR_WAIT_KWU_BNDCFM 5
#define NH_TMR_CELL_CFGCFM 6
#define NH_TMR_UE_LLYR_CFGCFM 7
...
#define NH_TMR_STOP 0
#define NH_TMR_START 1





#define NH_MAX_TMRS 255
#define NH_MAX_CFG_TMRS 64
#define NH_MAX_LSAP_TMRS 8
#define NH_BND_INTRETRY 3
#define NH_GEN_TMR_TIMEOUT 8000
#define NH_TQNUMENT 64


#define NH_ORD_CFG_RB_ADD 0x0001
#define NH_ORD_CFG_RB_MOD 0x0002
#define NH_ORD_CFG_RB_RLS 0x0004
#define NH_ORD_CFG_TRCH 0x0008
#define NH_ORD_CFG_SCHED 0x0010
#define NH_ORD_CFG_SEC 0x0020
#define NH_ORD_CFG_UEID 0x0040
#define NH_ORD_CFG_RB_RESTB 0x0080



#define NH_ORD_CFG_RESTB_SRB1 0x0200
#define NH_ORD_CFG_RESET_MAC 0x0400


#define NH_CRNT_CFG_RB 0x0001
#define NH_CRNT_CFG_LOGCHCB 0x0002
#define NH_CRNT_CFG_TRCH 0x0004
#define NH_CRNT_CFG_SEC 0x0008
#define NH_CRNT_CFG_SCHED 0x0010





#define NH_CRNT_CFG_SRB2 0x0040
#define NH_CRNT_CFG_DRBS 0x0080



#define NH_LAYER_NONE 0x00
#define NH_RLC_CONFIGURED 0x01
#define NH_MAC_CONFIGURED 0x02
#define NH_PDCP_CONFIGURED 0x04
...
#define NH_MAC_CFG_ADD 0x01
#define NH_MAC_CFG_MODIFY 0x02
#define NH_MAC_CFG_RLS 0x04
#define NH_MAC_CFG_RESET 0x08



#define NH_CRG_UE_CFG 0x01
#define NH_CRG_LOGCH_MAP 0x02
#define NH_CRG_UE_MAC_RST 0x03


#define NH_CRG_CELL_LOGCH_MAP 0x01
#define NH_CRG_CELL_CFG 0x02


#define NH_RLC_CFG_ESTABLISH 0x01
#define NH_RLC_CFG_RLS 0x02
#define NH_RLC_CFG_RECONFIGURE 0x03
#define NH_RLC_CFG_STOP 0x04
#define NH_RLC_CFG_CONTINUE 0x05
#define NH_RLC_CFG_REESTABLISH 0x06
#define NH_RLC_CFG_UEID_CHNG 0x07



#define NH_PDCP_CFG_ESTABLISH 0x01
#define NH_PDCP_CFG_RLS 0x02
#define NH_PDCP_CFG_RECONFIGURE 0x03
#define NH_PDCP_CFG_UEID_CHNG 0x04
#define NH_PDCP_CFG_RESTB_SRB1 0x05

#define NH_PDCP_DAT_CFM_SUCCESS 0x01
#define NH_PDCP_DAT_CFM_DISCARD 0x00
# 487 "../..//lterrc/nh.h"
#define NH_CFG_RES_UNKNOWN 0x00
#define NH_CFG_SUCCESS 0x01
#define NH_CFG_FAILED 0x02


#define NH_NOT_USED 0x00
#define NH_USED 0x01


#define NH_UE_INFO_NEW_CRNTI 0x01
#define NH_UE_INFO_MOD_CRNTI 0x02
#define NH_UE_INFO_RLS_CRNTI 0x03



#define NH_ZERO_VAL 0
#define NH_OCTET_VAL 8


#define NH_UE_HASH_LIST_SIZE 67
#define NH_CELL_HASH_LIST_SIZE 67

#define NH_MAX_CFG_REQ 32
#define NH_MAC_MAP_RB_MAP_IDX 1
#define NH_CRG_RB_MAP 0x02
#define NH_UEID 0x01
#define NH_CELLID 0x02
#define NH_MAC_MAP_CELL_MAP_IDX 1

#define NH_UE_MAPTBL_IDX_MAC 0
#define NH_UE_MAPTBL_IDX_RLC 1
#define NH_UE_MAPTBL_IDX_PDCP 2
#define NH_RESTB_MAPTBL_IDX_PDCP_SRB1 0
#define NH_RESET_MAPTBL_IDX_MAC 0

#define NH_UE_MAPTBL_MAX_IDX 3




#define NH_RESTB_SRB1_TBL_MAX_IDX 1
#define NH_MAC_RST_MAX_IDX 1


#define NH_LOGCH_CFG_LCH 0x01
#define NH_LOGCH_CFG_UL_TRCHTYP 0x02
#define NH_LOGCH_CFG_DL_TRCHTYP 0x04



#define NH_SEL_SEC_MOD_CFG_DISABLE 0x00
#define NH_SEL_SEC_MOD_CFG_ENABLE 0x01

#define NH_RB_TO_ADD 0
#define NH_RB_TO_MOD 1
#define NH_RB_TO_RLS 2
#define NH_RB_TO_REST 3
#define NH_MAX_RB_ACTION 4
...
#define NH_SET_ZERO(_memPtr,_size) do { (Void)cmMemset((U8*)(_memPtr), 0, (Size)(_size)); } while(0)
# 570 "../..//lterrc/nh.h"
#define NH_ALLOC(_region,_pool,_buf,_size) do { if (SGetSBuf((_region), (_pool), (Data **)(_buf), (_size)) == ROK) { NH_SET_ZERO((*(_buf)), (_size)); } else { (*(_buf)) = NULLP; } } while(0)
# 588 "../..//lterrc/nh.h"
#define NH_FREE(_region,_pool,_buf,_size) { if (NH_CHK_INPUT_PARAM(_buf)) { NHLOGERROR(ERRCLS_INT_PAR, 0,(ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((_region), (_pool), (Data *)(_buf), (Size)(_size)); (_buf) = NULLP; } }
# 606 "../..//lterrc/nh.h"
#define NH_ALLOCMBUF(_region,_pool,_buf) do { if (SGetMsg((_region), (_pool), (Buffer **)(_buf)) != ROK) { (*(_buf)) = NULLP; } } while(0)
# 620 "../..//lterrc/nh.h"
#define NH_FREEMBUF(_mBuf) do{ if (NH_CHK_INPUT_PARAM(_mBuf)) { NHLOGERROR(ERRCLS_INT_PAR, 0,(ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((_mBuf)); (_mBuf) = NULLP; } }while(0)
# 638 "../..//lterrc/nh.h"
#define NH_ALLOCEVNT(_evntPtr,_size,_mem) do { (*(_evntPtr)) = NULLP; (Void)cmAllocEvnt((Size)(_size), NH_MEM_PDU_SIZE, (_mem), (Ptr *)(_evntPtr)); } while(0)
# 651 "../..//lterrc/nh.h"
#define NH_GET_MEM(_memPtr,_size,_allocPtr) do { (*(_allocPtr)) = NULLP; (Void)cmGetMem((_memPtr), (_size), (Ptr *)(_allocPtr)); } while(0)
# 663 "../..//lterrc/nh.h"
#define NH_FREEEVNT(_evnt) do { cmFreeMem((_evnt)); (_evnt) = NULLP; } while(0)
# 675 "../..//lterrc/nh.h"
#define NH_MEM_COPY(_tgtPtr,_srcPtr,_size) do { (Void)cmMemcpy((U8*)(_tgtPtr),(CONSTANT U8*)(_srcPtr), (Size)(_size));} while(0)
# 685 "../..//lterrc/nh.h"
#define NH_ALLOC_TRANS_ID(_transCb,_pTransId) { if((_transCb)->transId >= nhCb.genCfg.minTransLstLen) { SGetSysTime((_pTransId)); } }
# 699 "../..//lterrc/nh.h"
#define NH_PRNT_BUF nhCb.init.prntBuf
#define NH_INIT_ENT nhCb.init.ent
#define NH_INIT_INST nhCb.init.inst
# 740 "../..//lterrc/nh.h"
#define NHDBGP(_msgClass,_args) DBGP(&nhCb.init, NHLAYERNAME, _msgClass, _args)
...
#define NH_DBG_INFO(_args) { if (nhCb.init.dbgMask & LNH_DBGMASK_INFO) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } NHDBGP(LNH_DBGMASK_INFO, _args); }
# 761 "../..//lterrc/nh.h"
#define NH_DBG_DEBUG(_args) { if (nhCb.init.dbgMask & LNH_DBGMASK_DBG) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } NHDBGP(LNH_DBGMASK_DBG, _args); }
# 775 "../..//lterrc/nh.h"
#define NH_DBG_PARAM(_args) { if (nhCb.init.dbgMask & LNH_DBGMASK_PARAM) { Txt prntBuf[10]; sprintf(prntBuf, "[param]"); SPrint(prntBuf); } NHDBGP(LNH_DBGMASK_PARAM, _args); }
# 788 "../..//lterrc/nh.h"
#define NH_DBG_ERROR(_args) { if (nhCb.init.dbgMask & LNH_DBGMASK_ERROR) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } NHDBGP(LNH_DBGMASK_ERROR, _args); }
# 802 "../..//lterrc/nh.h"
#define NH_DBG_FATAL(_args) { if (nhCb.init.dbgMask & LNH_DBGMASK_FATAL) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } NHDBGP(LNH_DBGMASK_FATAL, _args); }
# 832 "../..//lterrc/nh.h"
#define NHLOGERROR(errCls,errCode,errVal,errDesc) SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, __FILE__, __LINE__, errCls, errCode, errVal, errDesc)
...
#define NH_VALIDATE_CNTR(_cntr) { if (*(_cntr) == 0xFFFFFFFF) *(_cntr) = 0; }
# 855 "../..//lterrc/nh.h"
#define NH_FORM_NHU_HDR(_hdr,_nhId) { (_hdr).cellId = (_nhId).cellId; (_hdr).ueId = (_nhId).ueId; }
# 865 "../..//lterrc/nh.h"
#define NH_FILL_MSG_PDU_AND_TYPE(_msgPduType,_pdu,_type) { (_msgPduType).pdu = (_pdu); (_msgPduType).type = (_type); }
# 875 "../..//lterrc/nh.h"
#define NH_FILL_NHU_ERROR(_err,_errType,_errCause) { (_err).errPres = TRUE; (_err).errType = (_errType); (_err).errCause = (_errCause); }
# 885 "../..//lterrc/nh.h"
#define NH_FORM_NH_ID(_nhId,_datRspSdu,_crnti) { (_nhId).cellId = (_datRspSdu)->hdr.cellId; (_nhId).ueId = (_crnti); if((_datRspSdu)->sdu.msgCategory == NHU_MSG_DLDCCH) { (_nhId).isPdcpId = TRUE; (_nhId).rbId = (_datRspSdu)->sdu.m.dlDcchMsg.pdcpId; } else { (_nhId).isPdcpId = FALSE; (_nhId).rbId = (_datRspSdu)->sdu.m.dlCcchMsg.rbId; } }
# 904 "../..//lterrc/nh.h"
#define NH_FORM_NH_ID_FRM_DATRSP(_nhId,_datRspSdu) { (_nhId).cellId = (_datRspSdu)->hdr.cellId; (_nhId).ueId = 0; (_nhId).isPdcpId = FALSE; (_nhId).rbId = (_datRspSdu)->sdu.m.dlCcchMsg.rbId; }
# 915 "../..//lterrc/nh.h"
#define NH_FORM_NH_ID_FRM_DCCH_DATRSP(_nhId,_datRspSdu) { (_nhId).cellId = (_datRspSdu)->hdr.cellId; (_nhId).ueId = (_datRspSdu)->hdr.ueId; (_nhId).isPdcpId = TRUE; (_nhId).rbId = (_datRspSdu)->sdu.m.dlDcchMsg.pdcpId; }
# 926 "../..//lterrc/nh.h"
#define NH_FORM_NHID_FRM_DATREQ(_nhId,_datReqSdu) { (_nhId).cellId = (_datReqSdu)->hdr.cellId; (_nhId).ueId = (_datReqSdu)->hdr.ueId; if ((_datReqSdu)->sdu.ueMsg.isMsgDed) { (_nhId).isPdcpId = TRUE; (_nhId).rbId = (_datReqSdu)->sdu.ueMsg.msg.dedMsg.dlDcchMsg.pdcpId; } else { (_nhId).isPdcpId = FALSE; (_nhId).rbId = (_datReqSdu)->sdu.ueMsg.msg.cmnMsg.cmnMsgInfo.rbId; } }
# 947 "../..//lterrc/nh.h"
#define NH_GET_TRANSCB_KEY(_key,_msgType,_transId) (_key) = (((_msgType) << 8) | (_transId));





#define NH_ALLOC_MUI_ID(_muiCb,_pMui) { if ((_muiCb)->mui >= nhCb.genCfg.minMuiLstLen) { SGetSysTime((_pMui)); } }
# 964 "../..//lterrc/nh.h"
#define NH_SEND_DAT_CFM(_nhId,_nxTransId,_pdcpCause) { NhuDatCfmSdus *(_datCfmSdus)=NULLP; NH_ALLOC(NH_REG, NH_POOL, &(_datCfmSdus), sizeof(NhuDatCfmSdus)); if(NULLP == (_datCfmSdus)) { NH_DBG_FATAL((NH_PRNT_BUF,"NhuDatCfmSdus memory allocation failed.\n")); } else { (_datCfmSdus)->hdr.ueId = (_nhId).ueId; (_datCfmSdus)->hdr.cellId = (_nhId).cellId; (_datCfmSdus)->hdr.transId = (_nxTransId); (_datCfmSdus)->pdcpId = (_nhId).rbId; (_datCfmSdus)->datCfmStatus = (_pdcpCause); nhUIMSndDatCfm((_datCfmSdus)); } }
# 986 "../..//lterrc/nh.h"
#define NH_SEND_STATUS_IND(_nhId,_cause) { NhuStaIndSdus *_staIndSdus=NULLP; NH_ALLOC(NH_REG, NH_POOL, &_staIndSdus, sizeof(NhuStaIndSdus)); if(NULLP == _staIndSdus) { NH_DBG_FATAL((NH_PRNT_BUF, "NhuStaIndSdus memory allocation failed.\n")); } else { _staIndSdus->hdr.ueId = (_nhId).ueId; _staIndSdus->hdr.cellId = (_nhId).cellId; _staIndSdus->hdr.transId = NH_ZERO_VAL; _staIndSdus->pdcpId = (_nhId).rbId; _staIndSdus->cause = (_cause); nhUIMSndStaInd(_staIndSdus); } }
# 1009 "../..//lterrc/nh.h"
#define NH_MAP_PDCP_DAT_STA_IND_CAUSE(_pdcpCause,_nhuCause) { switch((_pdcpCause)) { case PJU_INTVER_FAILED: (_nhuCause) = NHU_ERRCAUSE_INTEGRITY_FAILED; break; case PJU_DCIP_FAILED: (_nhuCause) = NHU_ERRCAUSE_DECIPHERING_FAILED; break; case PJU_DCOMP_FAILED: (_nhuCause) = NHU_ERRCAUSE_DECOMPRESSION_FAILED; break; case PJU_COMP_FAILED: (_nhuCause) = NHU_ERRCAUSE_COMPRESSION_FAILED; break; case PJU_INTPROT_FAILED: (_nhuCause) = NHU_ERRCAUSE_INTEG_PROT_FAILED; break; case PJU_CIPHER_FAILED: (_nhuCause) = NHU_ERRCAUSE_CIPHERING_FAILED; break; default: (_nhuCause) = NHU_ERRCAUSE_NONE; break; } }
# 1040 "../..//lterrc/nh.h"
#define NH_MAP_PDCP_DAT_CFM_CAUSE(_pdcpCause,_nhuCause) { if(PJU_RLC_TRANS_FAIL == (_pdcpCause)) (_nhuCause) = NHU_ERRCAUSE_MAX_RLC_TRANS_REACH; else if(PJU_DISCARD_TMR_EXP == (_pdcpCause)) (_nhuCause) = NHU_ERRCAUSE_PDCP_SDU_DIS_TMR_EXP; else if(PJU_DAT_CFM_SUCCESS == (_pdcpCause)) (_nhuCause) = NHU_ERRCAUSE_NONE; }
# 1053 "../..//lterrc/nh.h"
#define NH_SEND_ERROR_IND(_errSdu,_hdr) { NhuErrIndSdus *_errIndSdus=NULLP; if ((_errSdu).sdu.errPres) { NH_ALLOC(NH_REG, NH_POOL, &_errIndSdus, sizeof(NhuErrIndSdus)); if(NULLP == _errIndSdus) { NH_DBG_FATAL((NH_PRNT_BUF, "NhuErrIndSdus memory allocation failed.\n")); } else { NH_MEM_COPY(&(_errIndSdus->hdr), &(_hdr), sizeof(NhuHdr)); NH_MEM_COPY(&(_errIndSdus->sdu), &((_errSdu).sdu), sizeof(NhuErrorInfo)); nhUIMSndErrInd(_errIndSdus); } } }
# 1075 "../..//lterrc/nh.h"
#define NH_CHK_LYR_CFG(_fn,_ret) { (_ret) = ROK; if (nhCb.init.cfgDone != TRUE) { NH_DBG_ERROR((nhCb.init.prntBuf, "_fn: General Configuration not done")); (_ret) = LCM_REASON_GENCFG_NOT_DONE; } if(((_ret) == ROK) && (nhCb.eNodeBCb.cfgDone != TRUE)) { NH_DBG_ERROR((nhCb.init.prntBuf, "_fn: Protocol Configuration not done")); (_ret) = LNH_REASON_PROTCFG_NOT_DONE; } }
# 1095 "../..//lterrc/nh.h"
#define NH_GET_MAC_CFG_REQ_TRANSID(_cfgTransId,_cellId,_crnti,_transId,_cfgElem,_action,_lcId,_dir) { U8 *_tmp = (_cfgTransId); cmMemcpy((U8*)_tmp, (CONSTANT U8*)&(_cellId), sizeof(U16)); _tmp = _tmp + sizeof(U16); cmMemcpy((U8*)_tmp, (CONSTANT U8*)&(_crnti), sizeof(U16)); _tmp = _tmp + sizeof(U16); cmMemcpy((U8*)_tmp, (CONSTANT U8*)&(_transId), sizeof(U32)); _tmp = _tmp + sizeof(U32); *_tmp = (_cfgElem); _tmp++; *_tmp = (_action); _tmp++; *_tmp = (_lcId); _tmp++; *_tmp = (_dir); }
# 1136 "../..//lterrc/nh.h"
#define NH_GET_PJU_SAP(_lSapCb,_spId) { (_lSapCb) = *(nhCb.pjuSapCbLst + (_spId)); }
# 1145 "../..//lterrc/nh.h"
#define NH_GET_KWU_SAP(_lSapCb,_spId) { (_lSapCb) = nhCb.kwuSapCbLst[(_spId)]; }
...
#define NH_GET_CPJ_SAP(_lSapCb,_spId) { (_lSapCb) = nhCb.cpjSapCbLst[(_spId)]; }
...
#define NH_GET_CKW_SAP(_lSapCb,_spId) { (_lSapCb) = nhCb.ckwSapCbLst[(_spId)]; }
...
#define NH_GET_CRG_SAP(_lSapCb,_spId) { (_lSapCb) = nhCb.crgSapCbLst[(_spId)]; }
# 1223 "../..//lterrc/nh.h"
#define NH_FORM_CFG_CFM_RESULT(_cfgMsk,_cfgCfm,_cfmStatus,_errCause) { if ((_cfgMsk) & NH_RLC_CONFIGURED) { if (!((_cfmStatus) & NH_RLC_CONFIGURED )) { (_cfgCfm).sdu.cfgError.errPres = TRUE; (_cfgCfm).sdu.cfgError.errType |= NHU_CFGERRTYPE_RLC; } } if ((_cfgMsk) & NH_MAC_CONFIGURED) { if (!((_cfmStatus) & NH_MAC_CONFIGURED )) { (_cfgCfm).sdu.cfgError.errPres = TRUE; (_cfgCfm).sdu.cfgError.errType |= NHU_CFGERRTYPE_MAC; } } if ((_cfgMsk) & NH_PDCP_CONFIGURED) { if (!((_cfmStatus) & NH_PDCP_CONFIGURED )) { (_cfgCfm).sdu.cfgError.errPres = TRUE; (_cfgCfm).sdu.cfgError.errType |= NHU_CFGERRTYPE_PDCP; } } if ((_cfgCfm).sdu.cfgError.errPres) { (_cfgCfm).sdu.isCfgSuccess = FALSE; (_cfgCfm).sdu.cfgError.errCause = _errCause; } else { (_cfgCfm).sdu.isCfgSuccess = TRUE; } }
# 1296 "../..//lterrc/nh.h"
#define NH_FORM_PDCP_STA_CFM_RESULT(_cfgMsk,_status,_cfmStatus) { if ((_cfgMsk) & NH_RLC_CONFIGURED) { if (!((_cfmStatus) & NH_RLC_CONFIGURED )) { _status |= NHU_CFGERRTYPE_RLC; } } if ((_cfgMsk) & NH_MAC_CONFIGURED) { if (!((_cfmStatus) & NH_MAC_CONFIGURED )) { _status |= NHU_CFGERRTYPE_MAC; } } if ((_cfgMsk) & NH_PDCP_CONFIGURED) { if (!((_cfmStatus) & NH_PDCP_CONFIGURED )) { _status |= NHU_CFGERRTYPE_PDCP; } } }
# 1323 "../..//lterrc/nh.h"
#define NH_PRNT_BUF nhCb.init.prntBuf
...
#define NH_RESET_TRANS_CFM_MASK(_cfmMask,_layerId) { (_cfmMask) &= ~(_layerId); }





#define NH_SET_CFG_CFM_STATUS(_cfmStatus,_status,_layerId) { if ((_status) == NH_CFG_SUCCESS) { (_cfmStatus) |= (_layerId); } else if ((_status) == NH_CFG_FAILED) { (_cfmStatus) &= ~(_layerId); } }
# 1352 "../..//lterrc/nh.h"
#define NH_SET_MAC_CFG_TYPE(_elmId,_cfgType) { (_cfgType) |= (_elmId); }




#define NH_IS_LOWER_LYR_CFG_SUCCESS(_transCb,_status) { if ((_transCb)->cfmStatus == (_transCb)->cfgMask) { (_status) = 1; } else { (_status) = 0; } }
# 1370 "../..//lterrc/nh.h"
#define NH_MAP_RLCID_TO_NHID(_nhId,_rlcId) { (_nhId).cellId = (_rlcId).cellId; (_nhId).ueId = (_rlcId).ueId; (_nhId).isPdcpId = FALSE; (_nhId).rbId = (_rlcId).rbId; }
# 1379 "../..//lterrc/nh.h"
#define NH_MAP_PDCPID_TO_NHID(_nhId,_pdcpId) { (_nhId).cellId = (_pdcpId)->cellId; (_nhId).ueId = (_pdcpId)->ueId; (_nhId).isPdcpId = TRUE; (_nhId).rbId = (_pdcpId)->rbId; }
...
#define NH_VALIDATE_DRBID_TO_LCID_MAP(_rbCfg,_drbMsk,_dlcMsk,_ddirMsk,_ret) { U32 _tmpDrb = 0, _tmpDlc = 0; U8 _drbId, _dlcId; _drbId = _rbCfg->rbId.rbId; _tmpDrb = (1 << (_drbId - 1)); _dlcId = _rbCfg->lcId.lcId; _tmpDlc = (1 << (_dlcId - 1)); if((_drbMsk) & _tmpDrb) (_ret) = RFAILED; else (_drbMsk) |= _tmpDrb; if((_dlcMsk) & _tmpDlc) { if(((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_DL) && (_ddirMsk[0] & _tmpDlc)) (_ret) = RFAILED; else _ddirMsk[0] |= _tmpDlc; if(((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_UL) && (_ddirMsk[1] & _tmpDlc)) (_ret) = RFAILED; else _ddirMsk[1] |= _tmpDlc; } else { (_dlcMsk) |= _tmpDlc; if((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_DL) _ddirMsk[0] |= _tmpDlc; if((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_UL) _ddirMsk[1] |= _tmpDlc; } }
# 1420 "../..//lterrc/nh.h"
#define NH_VALIDATE_SRBID_TO_LCID_MAP(_rbCfg,_srbMsk,_slcMsk,_sdirMsk,_ret) { U32 _tmpSrb = 0, _tmpSlc = 0; U8 _srbId, _slcId; _srbId = _rbCfg->rbId.rbId; _tmpSrb = (1 << (_srbId - 1)); _slcId = _rbCfg->lcId.lcId; _tmpSlc = (1 << (_slcId - 1)); if((_srbMsk) & _tmpSrb) (_ret) = RFAILED; else (_srbMsk) |= _tmpSrb; if((_slcMsk) & _tmpSlc) { if(((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_DL) && (_sdirMsk[0] & _tmpSlc)) (_ret) = RFAILED; else _sdirMsk[0] |= _tmpSlc; if(((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_UL) && (_sdirMsk[1] & _tmpSlc)) (_ret) = RFAILED; else _sdirMsk[1] |= _tmpSlc; } else { (_slcMsk) |= _tmpSlc; if((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_DL) _sdirMsk[0] |= _tmpSlc; if((_rbCfg)->rbCfgBitMsk & NHU_RB_CFG_UL) _sdirMsk[1] |= _tmpSlc; } }
# 1453 "../..//lterrc/nh.h"
#define NH_VALIDATE_RB_ID(_rbId,_rbType,_ret) { if (_rbType == CM_LTE_SRB) { if ((_rbId == NH_RB_ID_SRB1) || (_rbId == NH_RB_ID_SRB2)) _ret = ROK; else _ret = RFAILED; } }
# 1464 "../..//lterrc/nh.h"
#define NH_CHECK_NULLP_CKWCFG(_ckwCfgParam) { if ((CkwCfgInfo *)NULLP == (_ckwCfgParam)) { NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for ckwCfgParam")); RETVALUE(RFAILED); } }
# 1474 "../..//lterrc/nh.h"
#define NH_CHECK_NULLP_CRGCFG(_crgCfgParam) { if ((CrgCfgReqInfo *)NULLP == (_crgCfgParam)) { NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for crgCfgParam")); RETVALUE(RFAILED); } }
# 1496 "../..//lterrc/nh.h"
#define NH_CHECK_NULLP_CPJCFG(_cpjCfgReq) { if (NULLP == (_cpjCfgReq)) { NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for cpjCfgReq")); RETVALUE(RFAILED); } }
# 1506 "../..//lterrc/nh.h"
#define NH_CHECK_NULLP_CPJSECCFG(_cpjSecCfgReq) { if ((CpjSecCfgReqInfo *)NULLP == (_cpjSecCfgReq)) { NH_DBG_FATAL((NH_PRNT_BUF, "Could not allocate mem for CpjSecCfgReqInfo")); RETVALUE(RFAILED); } }
# 1516 "../..//lterrc/nh.h"
#define NH_GEN_TRC(_suId,_sapType,_trcEvnt,_mBuf) { switch(_sapType) { case STNHKWUSAP: if(nhCb.kwuSapCbLst[(_suId)]->trc != FALSE) nhUTLGenTrc(_suId, _sapType, _trcEvnt, _mBuf); break; case STNHPJUSAP: if(nhCb.pjuSapCbLst[(_suId)]->trc != FALSE) nhUTLGenTrc(_suId, _sapType, _trcEvnt, _mBuf); break; default : NH_DBG_ERROR((NH_PRNT_BUF, "Invalid SAP Type received")); break; }}
# 1593 "../..//lterrc/nh.h"
...
#define NH_CONDIES_GET_SDU_TYPE(_msgSdu,_evtType,_reCfgIEs) { switch((_evtType)) { default: (_reCfgIEs) = &(((NhuDatReqSdus *)(_msgSdu))->sdu.ueMsg.msg.dedMsg. dlDcchMsg.m.dcchMsg.message.val.c1.val.rrcConRecfgn. criticalExtns.val.c1.val.rrcConRecfgn_r8); break; case EVTNHUENCODEREQ: (_reCfgIEs) = &(((NhuEncReqSdus *)(_msgSdu))->sdu.u.dcchMsg.message. val.c1.val.rrcConRecfgn.criticalExtns.val.c1.val. rrcConRecfgn_r8); break; }}
# 1624 "../..//lterrc/nh.h"
#define NH_GET_UL_BITMAP_SIZE(_numBits,_size) { _size = _numBits / NH_OCTET_VAL; if(_numBits % NH_OCTET_VAL) { _size++; } }
# 1637 "../..//lterrc/nh.h"
#define NH_CHK_RB_ACTN_CFG_CFM(_tbl,_rbBitMsk,_found,_idx) { for (_idx = 0; _idx < NH_MAX_RB_ACTION; _idx++) { if(_tbl[_idx] & _rbBitMsk) { _found = TRUE; break; } } }
# 1655 "../..//lterrc/nh.h"
#define NH_SET_RCVD_CFG_CFM_MSK(_rcvdRbIds,_rbBitMsk) { if(*_rcvdRbIds & _rbBitMsk) { NH_DBG_ERROR((NH_PRNT_BUF, "Duplicate Cfg Cfm RCVD\n")); } else { *_rcvdRbIds |= _rbBitMsk; } }
# 1669 "../..//lterrc/nh.h"
#define NH_SET_RCVD_STA_CFM_MSK(_tbl,_status,_okVal,_rbBitMsk,_idx) { if ((_status == _okVal) && (_idx < NH_MAX_RB_ACTION)) _tbl[_idx] &= ~_rbBitMsk; }
...
#define NH_COPY_TMG_INFO(_dstTmgInfo,_srcTmgInfo) { _dstTmgInfo.sfn = _srcTmgInfo.sfn; _dstTmgInfo.subframe = _srcTmgInfo.subframe; }
# 1686 "../..//lterrc/nh.h"
#define NH_SET_CELLCFG_RBMAP_ELMNT(_chkForRb,_idx,_ret) { if (TRUE == _chkForRb[(_idx)]) { _ret = RFAILED; } else { _chkForRb[(_idx)] = TRUE; _ret = ROK; } }
# 1703 "../..//lterrc/nh.h"
#define NH_SET_CELLCFG_BCCH_DLSCH_RBMAP(_chkForRb,_idx,_bcchLcId,_lcId,_ret) { if (TRUE == _chkForRb[(_idx)]) { if ((TRUE == _chkForRb[(_idx)+1]) || ((0xFF != _bcchLcId) && (_lcId == _bcchLcId))) { _ret = RFAILED; } else { _chkForRb[(_idx)+1] = TRUE; _ret = ROK; } } else { _chkForRb[(_idx)] = TRUE; _bcchLcId = _lcId; _ret = ROK; } }
# 1731 "../..//lterrc/nh.h"
#define NH_GET_SYS_BW(_cfg) ((_cfg == NHU_SYS_BW_N6) ? CTF_BW_RB_6 : (_cfg == NHU_SYS_BW_N15) ? CTF_BW_RB_15 : (_cfg == NHU_SYS_BW_N25) ? CTF_BW_RB_25 : (_cfg == NHU_SYS_BW_N50) ? CTF_BW_RB_50 : (_cfg == NHU_SYS_BW_N75) ? CTF_BW_RB_75 : CTF_BW_RB_100 )
# 1746 "../..//lterrc/nh.h"
#define NH_GET_DELTA_SHIFT(_cfg) ((_cfg == NHU_PUCCH_DS1) ? CTF_PUCCH_DELTASHIFT1 : (_cfg == NHU_PUCCH_DS2) ? CTF_PUCCH_DELTASHIFT2 : CTF_PUCCH_DELTASHIFT3)
# 1756 "../..//lterrc/nh.h"
#define NH_GET_ANT_PORT_CNT(_cfg) ((_cfg == 1) ? CTF_AP_CNT_1 : (_cfg == 2) ? CTF_AP_CNT_2 : CTF_AP_CNT_4 )
# 1765 "../..//lterrc/nh.h"
#define NH_GET_BCCH_PCCH_TBSZ(_msgOctets,_tbSz) do { S32 _minIdx, _midIdx, _maxIdx; for (_minIdx = 0, _maxIdx = NH_BCCH_PCCH_TBSZTBL_SIZE - 1; _minIdx <= _maxIdx;) { _midIdx = ((_minIdx + _maxIdx) / 2); if (NhBcchPcchTbSzTbl[_midIdx] < _msgOctets) { _minIdx = _midIdx + 1; } else if (NhBcchPcchTbSzTbl[_midIdx] > _msgOctets) { _maxIdx = _midIdx - 1; } else { _tbSz = NhBcchPcchTbSzTbl[_midIdx]; break; } } if (_minIdx > _maxIdx) _tbSz = NhBcchPcchTbSzTbl[_minIdx]; } while(0)
# 1794 "../..//lterrc/nh.h"
typedef enum
{
   NH_DIAG_SPID,
...
#define NH_DIAG_LVL0(_tknId,_splArgEnum,_splArg,_string,_arg1,_arg2,_arg3,_arg4) { if(nhCb.init.logMask & SS_DIAG_LVL0) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL0, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string); } }
# 1815 "../..//lterrc/nh.h"
#define NH_DIAG_LVL1(_tknId,_splArgEnum,_splArg,_string,_arg1,_arg2,_arg3,_arg4) { if(nhCb.init.logMask & SS_DIAG_LVL1) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL1, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string); } }
# 1826 "../..//lterrc/nh.h"
#define NH_DIAG_LVL2(_tknId,_splArgEnum,_splArg,_string,_arg1,_arg2,_arg3,_arg4) { if(nhCb.init.logMask & SS_DIAG_LVL2) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL2, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string); } }
# 1837 "../..//lterrc/nh.h"
#define NH_DIAG_LVL3(_tknId,_splArgEnum,_splArg,_string,_arg1,_arg2,_arg3,_arg4) { if(nhCb.init.logMask & SS_DIAG_LVL3) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL3, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string); } }
# 1848 "../..//lterrc/nh.h"
#define NH_DIAG_LVL4(_tknId,_splArgEnum,_splArg,_string,_arg1,_arg2,_arg3,_arg4) { if(nhCb.init.logMask & SS_DIAG_LVL4) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_LVL4, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string); } }
# 1859 "../..//lterrc/nh.h"
#define NH_DIAG_MSG(_mBuf,_str) { if(nhCb.init.logMask & SS_DIAG_MSG) { Buffer *_newBuf; SAddMsgRef(_mBuf, nhCb.init.region, nhCb.init.pool, &_newBuf); ssDiagFix(0, NH_DIAG_NA, ENTNH, nhCb.init.inst, SS_DIAG_MSG, SS_DIAG_MSG_TYPE_HEXDUMP, _newBuf, 0, 0, 0,0, _str); } }
# 1872 "../..//lterrc/nh.h"
#define NH_DIAG_VAR(_tknId,_splArgEnum,_splArg,_string,_stringPtr) { if(nhCb.init.logMask & SS_DIAG_VAR) { ssDiagFix(_tknId, _splArgEnum, ENTNH, nhCb.init.inst, SS_DIAG_VAR, SS_DIAG_MSG_TYPE_FIXED, _splArg, 0,0,0,0, _string); ssDiagVarible(ENTNH, nhCb.init.inst, SS_DIAG_VAR, SS_DIAG_MSG_TYPE_VAR, _tknId, _splArg, _stringPtr); } }
# 1937 "../..//lterrc/nh.h"
#define NH_GET_SC_SPACING(_cfg) ((_cfg == NHU_TX_SCHM_SC_SPACING_15KHZ) ? CTF_SC_SPACING_15KHZ : CTF_SC_SPACING_7DOT5KHZ)
# 131 "../..//lterrc/nh_lim.c" 2
...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 135 "../..//lterrc/nh_lim.c" 2

# 1 "../..//cm/cm5.x" 1
# 137 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/cm_tkns.x" 1
# 91 "../..//cm/cm_tkns.x"
#define __CMTKNSX 
# 121 "../..//cm/cm_tkns.x"
...
# 147 "../..//lterrc/nh_lim.c" 2
# 1 "../..//cm/lnh.x" 1
# 100 "../..//cm/lnh.x"
#define __LNHX__ 
# 121 "../..//cm/lnh.x"
typedef struct nhGenCfg
{
   Pst lmPst;
...
# 158 "../..//lterrc/nh_lim.c" 2




static S16 nhLIMHndlStatusInd (NhId *nhIdentity, PjuStaIndInfo *staInd, Buffer *buf);


static S16 nhLIMMacDlMapTblUpd (TransCb *transCtrlBlk, U8 lclId, U8 status);


static S16 nhLIMMacUlMapTblUpd (TransCb *transCtrlBlk, U8 lclId, U8 status);


static S16 nhLIMHndlRlcCfgCfm (CkwCfgCfmInfo *cfmInfo);
static S16 nhLIMHndlRlcUECfgCfm (CkwCfgCfmInfo *cfmInfo, NhCellCb *cellCb);

static S16 nhLIMHndlRlcCellCfgCfm (CkwCfgCfmInfo *cfmInfo, NhCellCb *cellCb);


static S16 nhLIMHndlPdcpCfgCfm (CpjCfgCfmInfo *pdcpCfgCfm);

static S16 nhLIMHndlMacCfgCfm (CrgCfgTransId transId, U8 status);

static S16 nhLIMHndlMacCellCfgCfm (NhId *nhIdentity, NhCellCb *cellCb, U8 cfgElms, U8 status, U8 lclId);


static S16 nhLIMHndlMacUECfgCfm (NhId *nhIdentity, NhCellCb *cellCb, NhTransId nxTransId, U8 lclId, U8 cfgElms, U8 status, U8 dir);
...
static S16 nhLIMHndlUeIdChgCfm ( NhId *nhIdentity , U32 transId, CmStatus status, U8 lyrCfgd);




static S16 nhLIMHndlCountCfm (CpjCountCfmInfo *countCfmInfo);
static S16 nhLIMHndlPdcpSecCfgCfm (CpjSecCfgCfmInfo *secCfgCfm);
static S16 nhLIMHndlRestbCfgCfm (NhId *nhIdentity, U32 transId, Status statusCfm);


static S16 nhLIMFreeCpjSduStaCfm (Pst *pst, CpjSduStaCfmInfo *cpjSduStaCfm);

static S16 nhLIMFillPdcpSduStaInfo (CpjSduStaCfmInfo *cpjSduStaCfm, NhuPdcpSduStaCfm *nhuSduStaCfm);


static S16 nhLIMVldtAndRptCfgCfm ( Void *vCfmInfo, TransCb *transCtrlBlk, NhId *nhIdentity, NhCellAndUeCb *cellAndUeCb, U8 layerId);





static S16 nhLIMSrchAndSetSRBBitMsk ( U8 *rcvdRbIds, U8 *rbCfgTbl, U8 *rbStaTbl, U8 rbId, U16 status, U8 okValue );
...
static S16 nhLIMSrchAndSetDRBBitMsk ( U32 *rcvdRbIds, U32 *rbCfgTbl, U32 *rbStaTbl, U8 rbId, U16 status, U8 okValue );
...
static S16 nhLIMHndlUeIdChgCfmRlcCfg (TransCb *transCtrlBlk, NhCellCb *cellCb, NhActvUeCb *ueCb, U16 cfmStatus, NhId *nhIdentity );
...
static S16 nhLIMHndlUeIdChgCfmPdcpCfg (TransCb *transCtrlBlk, NhCellCb *cellCb, NhActvUeCb *ueCb, U16 cfmStatus, NhId *nhIdentity );
# 283 "../..//lterrc/nh_lim.c"
 PCDFLIS16 nhLISMTbl[17] =
{
   0L,
   nhCCMUlCmnHndlr,
   nhCCMUlCmnHndlr,
...

{
   ...
   ;
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 341); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlLSapBndCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   switch(sapId)
   {
      ...
      default:
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 367); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid sapId suId (%d) \n", sapId); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
   }

   if( 0 != (ret = nhSchedGenTmr(lSapCb, event, 0)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 373); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "stop timer failed for event (%d) \n", event); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if (2 == bndCfmStatus)
   {
      lSapCb->bndRetryCnt = 0;
      lSapCb->state = 1;
      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 444); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCkwBndCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };


   ckwSapCb = nhUTLFindLlyrSapFrmSuId(4, suId);
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0001, NH_DIAG_SPID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, ckwSapCb->spId, suId, ckwSapCb->state, 0, 0, "Received BndCfm from RLC for control SAP suId = %d sap state = %d"); } };

# 460 "../..//lterrc/nh_lim.c"
   if (3 != ckwSapCb->state)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 462); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Sap state (%d) \n",ckwSapCb->state); SPrint((&nhCb.init)->prntBuf); } }; };

      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
      dgn.u.sapInfo.sapId = ckwSapCb->spId;
      dgn.u.sapInfo.sapElmId = 4;
      nhSendLmAlarm(2, 1,
                    3, &dgn);
      return(1);
   }

   ret = nhLIMHndlLSapBndCfm(4, ckwSapCb, status);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 516); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiKwuBndCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };


   kwuSapCb = nhUTLFindLlyrSapFrmSuId(3,suId);
# 529 "../..//lterrc/nh_lim.c"
   if (3 != kwuSapCb->state)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 531); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Sap state \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
      dgn.u.sapInfo.sapId = kwuSapCb->spId;
      dgn.u.sapInfo.sapElmId = 3;
      nhSendLmAlarm(2, 1,
                    3, &dgn);
      return(1);
   }

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0002, NH_DIAG_SPID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, kwuSapCb->spId, suId, kwuSapCb->state, 0, 0, "Received BndCfm from RLC for data SAP suId = %d sap status = %d"); } };




   ret = nhLIMHndlLSapBndCfm(3, kwuSapCb, status);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 588); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCrgBndCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };


   crgSapCb = nhUTLFindLlyrSapFrmSuId(5,suId);

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0003, NH_DIAG_SUID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, suId, suId, status, 0, 0, "Received BndCfm from MAC suId = %d sap status = %d"); } };

# 605 "../..//lterrc/nh_lim.c"
   if (3 != crgSapCb->state)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 607); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Sap state \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
      dgn.u.sapInfo.sapId = crgSapCb->spId;
      dgn.u.sapInfo.sapElmId = 5;
      nhSendLmAlarm(2, 1,
                    3, &dgn);
      return(1);
   }

   ret = nhLIMHndlLSapBndCfm(5, crgSapCb, status);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 660); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjBndCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };


   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6,suId);
# 673 "../..//lterrc/nh_lim.c"
   if (3 != cpjSapCb->state)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 675); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Sap state \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
      dgn.u.sapInfo.sapId = cpjSapCb->spId;
      dgn.u.sapInfo.sapElmId = 6;
      nhSendLmAlarm(2, 1,
                    3, &dgn);
      return(1);
   }

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0004, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0xffffffff, suId, status, 0, 0, "RRC Recived Sap Bind Cfm from PDCP suId = %d, status = %d"); } };




   ret = nhLIMHndlLSapBndCfm(6, cpjSapCb, status);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 731); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiPjuBndCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0005, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0xffffffff, suId, status, 0, 0, "Received BndCfm from PDCP suId = %d, sap status = %d"); } };



   pjuSapCb = nhUTLFindLlyrSapFrmSuId(7,suId);
# 746 "../..//lterrc/nh_lim.c"
   if (3 != pjuSapCb->state)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 748); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Sap state \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
      dgn.type = 3;
      dgn.u.sapInfo.sapId = pjuSapCb->spId;
      dgn.u.sapInfo.sapElmId = 7;
      nhSendLmAlarm(2, 1,
                    3, &dgn);
      return(1);
   }

   ret = nhLIMHndlLSapBndCfm(7, pjuSapCb, status);
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 876); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiKwuDatInd: suId(%d) \n", suId); SPrint((&nhCb.init)->prntBuf); } }; };


   if ((mBuf) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 881); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL mBuf\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      if (!((diParam) == 0L))
      {
         { if ((diParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 884, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(diParam), (Size)(sizeof(KwuDatIndInfo))); (diParam) = 0L; } };
      }
      return(1);
   }
   if ((diParam) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 890); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL diParam\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 891, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
      return(1);
   }
   ...
   kwuSapCb = nhUTLFindLlyrSapFrmSuId(3, suId);
# 913 "../..//lterrc/nh_lim.c"
   { (nhIdentity).cellId = ((diParam->rlcId)).cellId; (nhIdentity).ueId = ((diParam->rlcId)).ueId; (nhIdentity).isPdcpId = 0; (nhIdentity).rbId = ((diParam->rlcId)).rbId; };


   tCrnti = diParam->tCrnti;

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0006, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, nhIdentity.rbId, 0, 0, "Received DatInd from RLC suId = %d, rbId = %d"); } };
   ...
   if (0 != nhLIMHndlDatInd(&nhIdentity, mBuf, tCrnti))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 926); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 927, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
      { if ((diParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 928, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(diParam), (Size)(sizeof(KwuDatIndInfo))); (diParam) = 0L; } };
      return(1);
   }


   do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 933, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
   { if ((diParam) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 934, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(diParam), (Size)(sizeof(KwuDatIndInfo))); (diParam) = 0L; } };
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 979); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " NhLiPjuDatInd: suId(%d), pdcpId(%d) \n", suId, pdcpId->rbId); SPrint((&nhCb.init)->prntBuf); } }; };



   if ((mBuf) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 984); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL mBuf\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      if (!((pdcpId) == 0L))
      {
         { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 987, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      }
      return(1);
   }
   if ((pdcpId) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 993); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL pdcpId\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 994, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
      return(1);
   }

   pjuSapCb = nhUTLFindLlyrSapFrmSuId(7, suId);
# 1016 "../..//lterrc/nh_lim.c"
   { (nhIdentity).cellId = (pdcpId)->cellId; (nhIdentity).ueId = (pdcpId)->ueId; (nhIdentity).isPdcpId = 1; (nhIdentity).rbId = (pdcpId)->rbId; };

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0007, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, pdcpId->ueId, 0, 0, "Received DatInd from PDCP suId = %d, pdcp UeId = %d"); } };
   ...
   if (0 != nhLIMHndlDatInd(&nhIdentity, mBuf, tCrnti))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1024); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1025, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1026, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      return(1);
   }


   do{ if ((mBuf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1031, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((mBuf)); (mBuf) = 0L; } }while(0);
   { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1032, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };

   return(0);
}
# 1054 "../..//lterrc/nh_lim.c"
 S16 NhLiPjuDatCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1085); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " NhLiPjuDatCfm: suId(%d) status(%d)\n", suId, datCfm->cfmSta[0].status); SPrint((&nhCb.init)->prntBuf); } }; };



   if ((datCfm) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1090); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL datCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      if (!((pdcpId) == 0L))
      {
         { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1093, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      }
      return(1);
   }
   if ((pdcpId) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1099); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL pdcpId\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1100, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
      return(1);
   }
   ...
   if (datCfm->numSdus == 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1107); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid interface param\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1108, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1109, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
      return(1);
   }

   nhIdentity.cellId = pdcpId->cellId;
   nhIdentity.ueId = pdcpId->ueId;
   nhIdentity.isPdcpId = 1;
   nhIdentity.rbId = pdcpId->rbId;

   if (0L == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1120); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL CELL CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1121, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1122, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
      return(1);
   }


   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity.ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1129); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1130, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1131, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
      return(1);
   }


   if ((MuiInfo*)0L ==
             (muiInfo = nhDBMGetMuiCb(ueCb, datCfm->cfmSta[0].sduId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1139); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "mui doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1140, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1141, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
      return(1);
   }
  ...
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0008, NH_DIAG_CRNTI, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, ueCb->ueCbKey.crnti, suId, datCfm->cfmSta[0].status, 0, 0, "Received DatCfm from PDCP suId = %d, status = %d"); } };
   ...
   if (0 == datCfm->cfmSta[0].status)

   {



      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1163); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiPjuDatCfm: status is SUCCESS, deleting mui\n"); SPrint((&nhCb.init)->prntBuf); } }; };



      { NhuDatCfmSdus *(_datCfmSdus)=0L; do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&(_datCfmSdus)), (sizeof(NhuDatCfmSdus))) == 0) { do { (Void)cmMemset((U8*)((*(&(_datCfmSdus)))), 0, (Size)((sizeof(NhuDatCfmSdus)))); } while(0); } else { (*(&(_datCfmSdus))) = 0L; } } while(0); if(0L == (_datCfmSdus)) { { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1166); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NhuDatCfmSdus memory allocation failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { (_datCfmSdus)->hdr.ueId = (nhIdentity).ueId; (_datCfmSdus)->hdr.cellId = (nhIdentity).cellId; (_datCfmSdus)->hdr.transId = (nxTransId); (_datCfmSdus)->pdcpId = (nhIdentity).rbId; (_datCfmSdus)->datCfmStatus = (0x00); nhUIMSndDatCfm((_datCfmSdus)); } };

      transCtrlBlk = nhUTLGetTransCbFrmNxTransId(ueCb, nxTransId);
      if (transCtrlBlk != 0L)
      {
         if (transCtrlBlk->cause == 10)
         {
            ...
            if (0 != nhLISMTbl[8](0L,
                                &cellandUeCb, msgTransId, 0))
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1185); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Lower state Machine Function Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };

               { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1186, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
               { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1187, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
               return(1);
            }
         }
      }
   ...
   else
   {
      ...
      { if(1 == (datCfm->cfmSta[0].status)) (nhuCause) = 0x23; else if(2 == (datCfm->cfmSta[0].status)) (nhuCause) = 0x24; else if(0 == (datCfm->cfmSta[0].status)) (nhuCause) = 0x00; };
      ...
      { NhuDatCfmSdus *(_datCfmSdus)=0L; do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&(_datCfmSdus)), (sizeof(NhuDatCfmSdus))) == 0) { do { (Void)cmMemset((U8*)((*(&(_datCfmSdus)))), 0, (Size)((sizeof(NhuDatCfmSdus)))); } while(0); } else { (*(&(_datCfmSdus))) = 0L; } } while(0); if(0L == (_datCfmSdus)) { { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1229); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NhuDatCfmSdus memory allocation failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { (_datCfmSdus)->hdr.ueId = (nhIdentity).ueId; (_datCfmSdus)->hdr.cellId = (nhIdentity).cellId; (_datCfmSdus)->hdr.transId = (nxTransId); (_datCfmSdus)->pdcpId = (nhIdentity).rbId; (_datCfmSdus)->datCfmStatus = (nhuCause); nhUIMSndDatCfm((_datCfmSdus)); } };
   }

   if(0 == released)
   {
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1234, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      { if ((datCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1235, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(datCfm), (Size)(sizeof(PjuDatCfmInfo))); (datCfm) = 0L; } };
   }

   return(0);
}
# 1260 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjCfgCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1279); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjCfgCfm: suId(%d), \n", suId); SPrint((&nhCb.init)->prntBuf); } }; };

   if ((pdcpCfgCfm) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1283); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL pdcpCfgCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6,suId);
# 1298 "../..//lterrc/nh_lim.c"
   if (0 != nhLIMHndlPdcpCfgCfm(pdcpCfgCfm))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1300); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlPdcpCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((pdcpCfgCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1301, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpCfgCfm), (Size)(sizeof(CpjCfgCfmInfo))); (pdcpCfgCfm) = 0L; } };
      return(1);
   }

   { if ((pdcpCfgCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1305, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(pdcpCfgCfm), (Size)(sizeof(CpjCfgCfmInfo))); (pdcpCfgCfm) = 0L; } };

   return(0);
}
# 1329 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjUeIdChgCfm
(
Pst *pst,
SuId suId,
U32 transId,
...
# 1345 "../..//lterrc/nh_lim.c"
{
   ...
   if ((ueInfo) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1356); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL ueInfo\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1361); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjUeIdChgCfm:                 for UeId (%d) and status %d \n", ueInfo->ueId, status.status); SPrint((&nhCb.init)->prntBuf); } }; };


   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6, suId);
# 1374 "../..//lterrc/nh_lim.c"
   if (transId == 0xffffffff)
   {
      ...
      if (status.status == 2)
      {
         ...
         nhSendLmAlarm(2, (256 + 1),
                      0, &dgn);
      }
      { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1386, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CpjUeInfo))); (ueInfo) = 0L; } };
      return(0);
   }
   nhIdentity.cellId = ueInfo->cellId;
   nhIdentity.ueId = ueInfo->ueId;

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0009, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, status.status, 0, 0, "Received UeIdChgCfm from PDCP suId = %d, status = %d"); } };
   ...
   if (0 != nhLIMHndlUeIdChgCfm(&nhIdentity, transId, status,
                                  0x04))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1401); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlUeIdChgCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1402, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CpjUeInfo))); (ueInfo) = 0L; } };
      return(1);
   }

   { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1406, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CpjUeInfo))); (ueInfo) = 0L; } };

   return(0);
}
# 1623 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjReEstCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   if ((restbCfm) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1645); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL restbCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1650); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjReEstCfm:         for UeId (%d) and status %d \n", restbCfm->ueId, restbCfm->status); SPrint((&nhCb.init)->prntBuf); } }; };


   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6, suId);
# 1663 "../..//lterrc/nh_lim.c"
   nhIdentity.cellId = restbCfm->cellId;
   nhIdentity.ueId = restbCfm->ueId;

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000a, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, restbCfm->status, 0, 0, "Received ReEstCfm from PDCP, re-establish Request only for SRB1 suId = %d status = %d"); } };




   if (0 != nhLIMHndlRestbCfgCfm(&nhIdentity, restbCfm->transId,
                                   restbCfm->status ))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1673); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRestbCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((restbCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1674, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(restbCfm), (Size)(sizeof(CpjReEstCfmInfo))); (restbCfm) = 0L; } };
      return(1);
   }

   { if ((restbCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1678, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(restbCfm), (Size)(sizeof(CpjReEstCfmInfo))); (restbCfm) = 0L; } };

   return(0);
}
# 1700 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjSecCfgCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1720); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjSecCfgCfm: suId(%d), \n", suId); SPrint((&nhCb.init)->prntBuf); } }; };

   if ((secCfgCfm) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1724); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL secCfgCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6,suId);
# 1739 "../..//lterrc/nh_lim.c"
   if (0 != nhLIMHndlPdcpSecCfgCfm(secCfgCfm))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1741); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlPdcpSecCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((secCfgCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1742, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(secCfgCfm), (Size)(sizeof(CpjSecCfgCfmInfo))); (secCfgCfm) = 0L; } };
      return(1);
   }

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000b, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0xffffffff, suId, 0, 0, 0, "Received SecCfgCfm from PDCP suId = %d, status = %d"); } };





   { if ((secCfgCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1751, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(secCfgCfm), (Size)(sizeof(CpjSecCfgCfmInfo))); (secCfgCfm) = 0L; } };

   return(0);

}
# 1775 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjCountCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   if ((countCfmInfo) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1799); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL countCfmInfo\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1805); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjCountCfm:                 for UeId (%d) and status %d \n", countCfmInfo->ueId, countCfmInfo->status); SPrint((&nhCb.init)->prntBuf); } }; };



   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6, suId);
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000c, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, countCfmInfo->ueId, suId, 0, 0, 0, "Received CountCfm, COUNT values of all established DRB from PDCP layer suId = %d"); } };

# 1820 "../..//lterrc/nh_lim.c"
   if (0L == (cellCb = nhDBMGetCellCb(countCfmInfo->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1822); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL CELL CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((countCfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1823, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(countCfmInfo), (Size)(sizeof(CpjCountCfmInfo))); (countCfmInfo) = 0L; } };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, countCfmInfo->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1829); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((countCfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1830, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(countCfmInfo), (Size)(sizeof(CpjCountCfmInfo))); (countCfmInfo) = 0L; } };
      return(1);
   }


   if (0 != nhLIMHndlCountCfm(countCfmInfo))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1837); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjCountCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((countCfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1838, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(countCfmInfo), (Size)(sizeof(CpjCountCfmInfo))); (countCfmInfo) = 0L; } };
      return(1);
   }

   { if ((countCfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 1842, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(countCfmInfo), (Size)(sizeof(CpjCountCfmInfo))); (countCfmInfo) = 0L; } };

   return(0);

}
# 1866 "../..//lterrc/nh_lim.c"
 S16 NhLiCpjSduStaCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1894); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCpjSduStaCfm: suId(%d), \n", suId); SPrint((&nhCb.init)->prntBuf); } }; };

   if ((cpjSduStaCfm) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1898); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL cpjSduStaCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   cpjSapCb = nhUTLFindLlyrSapFrmSuId(6,suId);
# 1915 "../..//lterrc/nh_lim.c"
   if (0L == (cellCb = nhDBMGetCellCb(cpjSduStaCfm->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1917); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL CELL CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, cpjSduStaCfm->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1924); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
      return(1);
   }
   ...
   if ( ((TransCb*)0L == transCtrlBlk) ||
        (transCtrlBlk->cause != 0xEF) )
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1935); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
      return(1);
   }
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1943); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Pdcp Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }
   ...
   do { (*((Ptr *)&nhuSduStaCfm)) = 0L; (Void)cmAllocEvnt((Size)(sizeof(NhuPdcpSduStaCfm)), 3584, (&sMem), (Ptr *)((Ptr *)&nhuSduStaCfm)); } while(0);
   if(0L == nhuSduStaCfm)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 1962); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NhuPdcpSduStaCfm memory allocation                    failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if(0 == cpjSduStaCfm->status)
   {
      ...
      if(0 != retVal)
      {
         do { cmFreeMem((nhuSduStaCfm)); (nhuSduStaCfm) = 0L; } while(0);
         nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);
         return(1);
      }
   }
   else if(1 == cpjSduStaCfm->status)
   {
      cfmStatus = 0x02;
   }
   ...
   if (0 != nhLIMRptCfgCfm(&nhIdentity, &cellAndUeCb, transCtrlBlk,
            cfmStatus, 0x04))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2012); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
# 2023 "../..//lterrc/nh_lim.c"
   nhLIMFreeCpjSduStaCfm(pst,cpjSduStaCfm);

   return(0);
}
# 2044 "../..//lterrc/nh_lim.c"
 S16 NhLiCkwCfgCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2063); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCkwCfgCfm: \n"); SPrint((&nhCb.init)->prntBuf); } }; };
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000d, NH_DIAG_SUID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, suId, 0, 0, 0, 0, "Received CfgCfm from RLC"); } };
   ...
   if ((cfmInfo) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2070); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL cfmInfo\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   ckwSapCb = nhUTLFindLlyrSapFrmSuId(4, suId);
# 2086 "../..//lterrc/nh_lim.c"
   if (0 != nhLIMHndlRlcCfgCfm(cfmInfo))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2088); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((cfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 2089, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(cfmInfo), (Size)(sizeof(CkwCfgCfmInfo))); (cfmInfo) = 0L; } };
      return(1);
   }

   { if ((cfmInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 2093, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(cfmInfo), (Size)(sizeof(CkwCfgCfmInfo))); (cfmInfo) = 0L; } };

   return(0);
}
# 2117 "../..//lterrc/nh_lim.c"
 S16 NhLiCkwUeIdChgCfm
(
Pst *pst,
SuId suId,
U32 transId,
...
# 2133 "../..//lterrc/nh_lim.c"
{
   ...
   if ((ueInfo) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2144); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL ueInfo\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2149); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCkwUeIdChgCfm:                 for UeId (%d) and status %d \n", ueInfo->ueId, status.status); SPrint((&nhCb.init)->prntBuf); } }; };


   ckwSapCb = nhUTLFindLlyrSapFrmSuId(4, suId);
# 2162 "../..//lterrc/nh_lim.c"
   if (transId == 0xffffffff)
   {
      ...
      if (status.status == 2)
      {
         ...
         dgn.u.cellUeId.ueId = ueInfo->ueId;
         dgn.u.cellUeId.cellId = ueInfo->cellId;
         nhSendLmAlarm(2, (256 + 2),
                      0, &dgn);
      }
      { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 2174, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CkwUeInfo))); (ueInfo) = 0L; } };
      return(0);
   }

   nhIdentity.ueId = ueInfo->ueId;
   nhIdentity.cellId = ueInfo->cellId;
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000e, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, 0, 0, 0, "Received UeIdChgCfm from RLC suId = %d status = %d"); } };
   ...
   if (0 != nhLIMHndlUeIdChgCfm(&nhIdentity, transId, status,
                                  0x01))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2188); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlUeIdChgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 2189, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CkwUeInfo))); (ueInfo) = 0L; } };
      return(1);
   }
   { if ((ueInfo) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 2192, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(ueInfo), (Size)(sizeof(CkwUeInfo))); (ueInfo) = 0L; } };

   return(0);
}
# 2217 "../..//lterrc/nh_lim.c"
 S16 NhLiCrgCfgCfm
(
Pst *pst,
SuId suId,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2239); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiCrgCfgCfm: suId(%d), status(%d)\n", suId, status); SPrint((&nhCb.init)->prntBuf); } }; };


   crgSapCb = nhUTLFindLlyrSapFrmSuId(5, suId);

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f000f, NH_DIAG_SUID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, suId, status, 0, 0, 0, "Received CfgCfm from MAC status = %d"); } };

# 2256 "../..//lterrc/nh_lim.c"
   if (0 != nhLIMHndlMacCfgCfm(transId, status))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2258); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   return(0);
}
# 2286 "../..//lterrc/nh_lim.c"
 S16 nhLIMHndlRvrtCfg
(
NhId *nhIdentity,
NhActvUeCb *ueCb,
TransCb *transCtrlBlk
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2309); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRvrtCfg \n"); SPrint((&nhCb.init)->prntBuf); } }; };
# 2318 "../..//lterrc/nh_lim.c"
   if (transCtrlBlk->cfgMask & 0x02)
   {
      macRet = nhLIMHndlRvrtMacCfg(nhIdentity, ueCb, transCtrlBlk);
   }

   if (transCtrlBlk->cfgMask & 0x01)
   {
      rlcRet = nhPIMRvrtRlcCfg(ueCb, transCtrlBlk);
   }

   if (transCtrlBlk->cfgMask & 0x04)
   {
   ...
   if((0 != macRet) || (0 != rlcRet) || (0 != pdcpRet))
...
# 2366 "../..//lterrc/nh_lim.c"
static S16 nhLIMDecFailNotify
(
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2385); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Decoding failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   do { (Void)cmMemset((U8*)((U8*)&sduHdr), 0, (Size)(sizeof(NhuHdr))); } while(0);
   { (sduHdr).cellId = ((*nhIdentity)).cellId; (sduHdr).ueId = ((*nhIdentity)).ueId; };
   { (errIndSdus.sdu).errPres = 1; (errIndSdus.sdu).errType = (0x08); (errIndSdus.sdu).errCause = (0x00); };

   { NhuErrIndSdus *_errIndSdus=0L; if ((errIndSdus).sdu.errPres) { do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&_errIndSdus), (sizeof(NhuErrIndSdus))) == 0) { do { (Void)cmMemset((U8*)((*(&_errIndSdus))), 0, (Size)((sizeof(NhuErrIndSdus)))); } while(0); } else { (*(&_errIndSdus)) = 0L; } } while(0); if(0L == _errIndSdus) { { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2401); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhuErrIndSdus memory allocation failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { do { (Void)cmMemcpy((U8*)(&(_errIndSdus->hdr)),(const U8*)(&(sduHdr)), (Size)(sizeof(NhuHdr)));} while(0); do { (Void)cmMemcpy((U8*)(&(_errIndSdus->sdu)),(const U8*)(&((errIndSdus).sdu)), (Size)(sizeof(NhuErrorInfo)));} while(0); nhUIMSndErrInd(_errIndSdus); } } };

   do { cmFreeMem((datIndSdu)); (datIndSdu) = 0L; } while(0);
   return(1);

}
# 2425 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlStatusInd
(
NhId *nhIdentity,
...

{
   ...
   NhCellCb *cellCb = 0L;
   U8 nhuCause = 0x00;
   NhuDatIndSdus *datIndSdu = 0L;


   Void *pdu = 0L;
   U8 nhMsgType = 0;
   U8 nhEvnt = 0;
   NhuUL_DCCH_Msg *ulDcchPdu = 0L;
   U8 rrcTransId = 0;
   Mem sMem;
   NhCellAndUeCb cellandUeCb;
   ...
   ;
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2454); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0L == cellCb)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2462); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " NULL cellCb \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2468); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }


   { switch((staInd->cause)) { case 1: (nhuCause) = 0x21; break; case 2: (nhuCause) = 0x22; break; case 3: (nhuCause) = 0x2F; break; case 4: (nhuCause) = 0x30; break; case 5: (nhuCause) = 0x31; break; case 6: (nhuCause) = 0x32; break; default: (nhuCause) = 0x00; break; } };
   ...
   if (staInd->cause == 1)
   {
      do { (*((Ptr *)&datIndSdu)) = 0L; (Void)cmAllocEvnt((Size)(sizeof(NhuDatIndSdus)), 3584, (&sMem), (Ptr *)((Ptr *)&datIndSdu)); } while(0);
      if (0L == datIndSdu)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2488); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd: cmAllocEvnt failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(1);
      }

      datIndSdu->hdr.transId = 0;
      datIndSdu->err.errPres = 0;

      datIndSdu->hdr.ueId = nhIdentity->ueId;
      datIndSdu->hdr.cellId = nhIdentity->cellId;
      datIndSdu->sdu.msgCategory = 0x02;
      pdu = (Void *)(&(datIndSdu->sdu.m.ulDcchMsg.dcchMsg));


      if (0 != nhEDMDecReq(datIndSdu->sdu.msgCategory, buf, (Void *)datIndSdu,
                     pdu))
      {
         return(nhLIMDecFailNotify(datIndSdu, nhIdentity));
      }
      ...
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2511); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd: UE MSG RCVD: event is (%d)\n ", nhEvnt); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      if(nhMsgType == 19)
      {
         ...
         if (0 != nhUTLValidateSRBforMsgRx(ueCb, nhIdentity->rbId,
                     ulDcchPdu->message.val.c1.choice.val))
         {
            datIndSdu->err.errPres = 1;
            datIndSdu->err.errType = 0x02;
            datIndSdu->err.errCause = 0x01;

            if (0 != nhUIMDatInd(datIndSdu))
            {
               { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2527); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd: Send to UIM Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         ...
         if (0 != nhLISMTbl[nhEvnt](datIndSdu, &cellandUeCb,
                                      rrcTransId, nhMsgType))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2543); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd: nhCCMSecModFailHndlr failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   { NhuStaIndSdus *_staIndSdus=0L; do { if (SGetSBuf((nhCb.init.region), (nhCb.init.pool), (Data **)(&_staIndSdus), (sizeof(NhuStaIndSdus))) == 0) { do { (Void)cmMemset((U8*)((*(&_staIndSdus))), 0, (Size)((sizeof(NhuStaIndSdus)))); } while(0); } else { (*(&_staIndSdus)) = 0L; } } while(0); if(0L == _staIndSdus) { { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2555); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhuStaIndSdus memory allocation failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { _staIndSdus->hdr.ueId = (*nhIdentity).ueId; _staIndSdus->hdr.cellId = (*nhIdentity).cellId; _staIndSdus->hdr.transId = 0; _staIndSdus->pdcpId = (*nhIdentity).rbId; _staIndSdus->cause = (nhuCause); nhUIMSndStaInd(_staIndSdus); } };

   return(0);
}
# 2577 "../..//lterrc/nh_lim.c"
 S16 nhLIMHndlDatInd
(
NhId *nhIdentity,
Buffer *mBuf,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2605); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0L == datIndSdu)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2613); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: cmAllocEvnt failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if (0L == cellCb)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2623); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: NULL cellCb \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      do { cmFreeMem((datIndSdu)); (datIndSdu) = 0L; } while(0);
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 2)) { Txt prntBuf[10]; sprintf(prntBuf, "[param]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 2))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2629); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: cellId(%d)\n", nhIdentity->cellId); SPrint((&nhCb.init)->prntBuf); } }; };


   if (1 == nhIdentity->isPdcpId)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 2)) { Txt prntBuf[10]; sprintf(prntBuf, "[param]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 2))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2634); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: ueId(%d)\n", nhIdentity->ueId); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      if (0L == ueCb)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2640); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: NULL ueCb \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         do { cmFreeMem((datIndSdu)); (datIndSdu) = 0L; } while(0);
         return(1);
      }
      datIndSdu->hdr.ueId = nhIdentity->ueId;
   ...
   {
      datIndSdu->hdr.cellId = nhIdentity->cellId;
      datIndSdu->sdu.msgCategory = 0x04;
   ...
   if (0 != nhEDMDecReq(datIndSdu->sdu.msgCategory,
   ...
   if (datIndSdu->sdu.msgCategory == 0x02)
   {
      ulDcchPdu = (NhuUL_DCCH_Msg*)pdu;
      if (0 != nhUTLValidateSRBforMsgRx(ueCb, nhIdentity->rbId,
                                  ulDcchPdu->message.val.c1.choice.val))
      {
         ...
         if (0 != nhUIMDatInd(datIndSdu))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2682); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: Send to UIM Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      nhUTLGetMsgTypeAndEvntFrmSdu (ulDcchPdu->message.val.c1.choice.val,
                        NH_SDU_UL_DCCH, &nhMsgType, &nhEvnt);
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2691); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlDatInd: UE MSG RCVD: event is (%d)\n ", nhEvnt); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0x04 == datIndSdu->sdu.msgCategory)
   {
   ...
   if (0 != nhLISMTbl[nhEvnt](datIndSdu, &cellandUeCb, rrcTransId, nhMsgType))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2714); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Lower state Machine Function Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 2740 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlRlcCellCfgCfm
(
CkwCfgCfmInfo *cfmInfo,
NhCellCb *cellCb
)
...
{
   NhCellTransCb *cellTransCb = 0L;
   U8 count;
   U8 cgmCfmStatus = 0x01;
   U8 entityIndex;
   U8 rbId;
   U16 status;
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2760); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCellCfgCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   cellTransCb = &(cellCb->cellTransCb);

   if(cellTransCb->cfgAction != 0x04)
   {
      for (entityIndex = 0; entityIndex < cfmInfo->numEnt; entityIndex++)
      {
         ...
         {
            if (1 == (cellTransCb->rlcMapTbl[count].status) &&
                                  (cellTransCb->rlcMapTbl[count].rbId == rbId))
            {
               cellTransCb->numOfRlcCfms--;
               if (1 == status)
                  cellTransCb->rlcMapTbl[count].cfmStatus = 0x01;
               else
               {
                  cellTransCb->rlcMapTbl[count].cfmStatus = 0x02;
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0010, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0xffffffff, rbId, 0, 0, 0, "Received CellCfgCfm from RLC Status = Failed, RAB-ID = %d"); } };
   ...
   else
   {
      ...
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2796); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCellCfgCfm: status=%d numOfRlcCfms=%d\n", status, cellTransCb->numOfRlcCfms); SPrint((&nhCb.init)->prntBuf); } }; };


      if (1 == (cellTransCb->rlcMapTbl[0].status) &&
         (0 == cellTransCb->rlcMapTbl[0].rbId))
      {
         cellTransCb->numOfRlcCfms--;
         if (1 == status)
            cellTransCb->rlcMapTbl[0].cfmStatus = 0x01;
         else
         {
  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0011, NH_DIAG_NA, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, 0xffffffff, cellTransCb->rlcMapTbl[0].rbId, 0, 0, 0, "Received CellCfgCfm from RLC, Releasing of RAB failed  RAB-ID = %d"); } };



            cellTransCb->rlcMapTbl[0].cfmStatus = 0x02;
      }
      }
   ...
   if (cellTransCb->numOfRlcCfms == 0)
   {
      ...
      if (0 != nhLIMRptCellCfgCfm(cellCb, cgmCfmStatus, 0x01))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2829); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCellCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
}
# 2853 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlRlcUECfgCfm
(
CkwCfgCfmInfo *cfmInfo,
NhCellCb *cellCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2871); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcUECfgCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   nhIdentity.ueId = cfmInfo->ueId;
   nhIdentity.cellId = cfmInfo->cellId;

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, cfmInfo->ueId)))
   {


      if ((NhActvUeCb*)0L == (ueCb = nhDBMGetNewUeCb(cellCb, cfmInfo->ueId)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2882); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcUECfgCfm:NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }

   if ((TransCb*)0L == (transCtrlBlk = nhUTLGetTransCbFrmNxTransId (ueCb,
                                     cfmInfo->transId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2891); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMHndlRlcUECfgCfm: transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2898); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Rlc Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }
   ...
   if (0 != nhLIMVldtAndRptCfgCfm ((Void *)cfmInfo, transCtrlBlk,
                        &nhIdentity, &cellAndUeCb, 0x01))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2908); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMVldtAndRptCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   return(0);
}
# 2932 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlRestbCfgCfm
(
NhId *nhIdentity,
U32 transId,
Status statusCfm
...

{
   ...
   U16 cfmStatus = 0;
   U8 cgmCfmStatus = 0x01;
   NhSrb1RestbInfo *restTbl = 0L;
   NhCellAndUeCb cellAndUeCb;

   ;


   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2957); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " transId(%ld) ueId(%d) cellId                      (%d)\n", transId, nhIdentity->ueId, nhIdentity->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0L == (cellCb = nhDBMGetCellCb(nhIdentity->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2965); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRestbCfgCfm:NULLP Cell Cb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity->ueId)))
   {
      if ((NhActvUeCb*)0L == (ueCb = nhDBMGetNewUeCb(cellCb,
                                        nhIdentity->ueId)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2974); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2985); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRestbCfgCfm:transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };

      return(1);
   }
   cfmStatus = statusCfm;

   restTbl = &(transCtrlBlk->restbTbl[0]);


   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 2995); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate Pdcp Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }
   ...
   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
     if(((restTbl->status == 1) &&
       (restTbl->cfmStatus == 2))
      ||
       ((transCtrlBlk->ueMapTbl[2].status ==
                                1) &&
        (transCtrlBlk->ueMapTbl[2].cfmStatus !=
                                1)))
      {
         cgmCfmStatus = 0x02;
      }
      ...
      if (0 != nhLIMRptCfgCfm(nhIdentity, &cellAndUeCb, transCtrlBlk,
                        cgmCfmStatus, 0x04))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3034); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
...
# 3061 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlUeIdChgCfmRlcCfg
(
TransCb *transCtrlBlk,
NhCellCb *cellCb,
NhActvUeCb *ueCb,
...
# 3078 "../..//lterrc/nh_lim.c"
{
   U8 cgmCfmStatus = 0x01;
   NhUeCfgCfmMapInfo *ueMapTbl = 0L;
   NhCellAndUeCb cellAndUeCb;


   ueMapTbl = &(transCtrlBlk->ueMapTbl[1]);


   if (transCtrlBlk->numOfRlcCfms <= 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3089); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Rlc Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }
   if (ueMapTbl->status == 1)
   {
      if (cfmStatus == 2)
      {
         ueMapTbl->cfmStatus = 2;

         nhUTLUpdGenSts(5);
      }
      else
      {
         ueMapTbl->cfmStatus = 1;
      }
   }
   ...
   if (transCtrlBlk->numOfRlcCfms <= 0)
   {
       if (((transCtrlBlk->rlcMapTbl.srbCfmSta[1] !=
            transCtrlBlk->rlcMapTbl.srbToCfg[1]) ||
       (transCtrlBlk->rlcMapTbl.srbCfmSta[2] !=
            transCtrlBlk->rlcMapTbl.srbToCfg[2]) ||
       (transCtrlBlk->rlcMapTbl.srbCfmSta[0] !=
            transCtrlBlk->rlcMapTbl.srbToCfg[0]) ||
       (transCtrlBlk->rlcMapTbl.srbCfmSta[3] !=
            transCtrlBlk->rlcMapTbl.srbToCfg[3]) ||

       (transCtrlBlk->rlcMapTbl.drbCfmSta[1] !=
            transCtrlBlk->rlcMapTbl.drbToCfg[1]) ||
       (transCtrlBlk->rlcMapTbl.drbCfmSta[2] !=
            transCtrlBlk->rlcMapTbl.drbToCfg[2]) ||
       (transCtrlBlk->rlcMapTbl.drbCfmSta[0] !=
            transCtrlBlk->rlcMapTbl.drbToCfg[0]) ||
       (transCtrlBlk->rlcMapTbl.drbCfmSta[3] !=
            transCtrlBlk->rlcMapTbl.drbToCfg[3]))
      ||
      ((ueMapTbl->status == 1) &&
          (ueMapTbl->cfmStatus == 2)))
      {
      ...
      if (0 !=
         nhLIMRptCfgCfm(nhIdentity , &cellAndUeCb, transCtrlBlk,
                        cgmCfmStatus, 0x01))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3142); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
...
# 3169 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlUeIdChgCfmPdcpCfg
(
TransCb *transCtrlBlk,
NhCellCb *cellCb,
NhActvUeCb *ueCb,
...
# 3186 "../..//lterrc/nh_lim.c"
{
   U8 cgmCfmStatus = 0x01;
   NhUeCfgCfmMapInfo *ueMapTbl = 0L;
   NhCellAndUeCb cellAndUeCb;

   ueMapTbl = &(transCtrlBlk->ueMapTbl[2]);


   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3196); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate Pdcp Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }
   if (ueMapTbl->status == 1)
   {
   ...
   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
      ...
      || ((transCtrlBlk->pdcpMapTbl.srbCfmSta[1] !=
            transCtrlBlk->pdcpMapTbl.srbToCfg[1]) ||
       (transCtrlBlk->pdcpMapTbl.srbCfmSta[2] !=
            transCtrlBlk->pdcpMapTbl.srbToCfg[2]) ||
       (transCtrlBlk->pdcpMapTbl.srbCfmSta[0] !=
            transCtrlBlk->pdcpMapTbl.srbToCfg[0]) ||
       (transCtrlBlk->pdcpMapTbl.srbCfmSta[3] !=
            transCtrlBlk->pdcpMapTbl.srbToCfg[3]) ||

       (transCtrlBlk->pdcpMapTbl.drbCfmSta[1] !=
            transCtrlBlk->pdcpMapTbl.drbToCfg[1]) ||
       (transCtrlBlk->pdcpMapTbl.drbCfmSta[2] !=
            transCtrlBlk->pdcpMapTbl.drbToCfg[2]) ||
       (transCtrlBlk->pdcpMapTbl.drbCfmSta[0] !=
            transCtrlBlk->pdcpMapTbl.drbToCfg[0]) ||
       (transCtrlBlk->pdcpMapTbl.drbCfmSta[3] !=
            transCtrlBlk->pdcpMapTbl.drbToCfg[3]))

      ||
      ((transCtrlBlk->restbTbl[0].status ==
                                                1) &&
      transCtrlBlk->restbTbl[0].cfmStatus ==
                                                2))
      {
          cgmCfmStatus = 0x02;
      }
      ...
      if (0 != nhLIMRptCfgCfm(nhIdentity , &cellAndUeCb, transCtrlBlk,
                        cgmCfmStatus, 0x04))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3254); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
...
# 3372 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlUeIdChgCfm
(
NhId *nhIdentity ,
U32 transId,
CmStatus status,
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3395); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlUeIdChgCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3398); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " transId(%ld) ueId(%d) cellId                      (%d)\n", transId, nhIdentity ->ueId, nhIdentity ->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0L == (cellCb = nhDBMGetCellCb(nhIdentity ->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3406); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlUeIdChgCfm:NULLP Cell Cb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity ->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3412); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlUeIdChgCfm:NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if ((TransCb*)0L ==
         (transCtrlBlk = nhUTLGetTransCbFrmNxTransId (ueCb, transId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3420); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMHndlUeIdChgCfm: transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 3455 "../..//lterrc/nh_lim.c"
   return(0);
}
# 3472 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlRlcCfgCfm
(
CkwCfgCfmInfo *cfmInfo
)
...

{
   ...
   ;
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3486); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCfgCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };


   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3490); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCfgCfm: transId(%ld) ueId(%d)          cellId (%d)\n", cfmInfo->transId, cfmInfo->ueId, cfmInfo->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
# 3499 "../..//lterrc/nh_lim.c"
   if (cfmInfo->transId == 0xffffffff)
   {

      for (entIndex = 0; entIndex < cfmInfo->numEnt; entIndex++)
      {
         if (cfmInfo->entCfgCfm[entIndex].status.status == 2)
         {
            do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
            dgn.type = 7;
   ...
   if (0L == (cellCb = nhDBMGetCellCb(cfmInfo->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3519); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRlcCfgCfm:NULLP Cell Cb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if (cfmInfo->ueId > 0)
   {
      if (0 != (nhLIMHndlRlcUECfgCfm(cfmInfo, cellCb)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3528); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMHndlRlcUECfgCfm: failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   ...
   {
      if (0 != (nhLIMHndlRlcCellCfgCfm(cfmInfo, cellCb)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3537); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMHndlRlcCellCfgCfm: failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
...
# 3560 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlPdcpCfgCfm
(
CpjCfgCfmInfo *pdcpCfgCfm
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3579); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlPdcpCfgCfm \n"); SPrint((&nhCb.init)->prntBuf); } }; };





   if (pdcpCfgCfm->transId == 0xffffffff)
   {

      for (entIndex = 0; entIndex < pdcpCfgCfm->numEnt &&
                      (entIndex < 10 ); entIndex++)
      {
         if (pdcpCfgCfm->cfmEnt[entIndex].status == 2)
         {
            do { (Void)cmMemset((U8*)(&dgn), 0, (Size)(sizeof(dgn))); } while(0);
            dgn.type = 7;
   ...
   if (0L == (cellCb = nhDBMGetCellCb(pdcpCfgCfm->cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3609); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL CELL CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, pdcpCfgCfm->ueId)))
   {
      if ((NhActvUeCb*)0L == (ueCb = nhDBMGetNewUeCb(cellCb,
                                        pdcpCfgCfm->ueId)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3618); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   ...
   if ((TransCb*)0L == (transCtrlBlk = nhUTLGetTransCbFrmNxTransId(ueCb,
                                          pdcpCfgCfm->transId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3627); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if (transCtrlBlk->numOfPdcpCfms <= 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3634); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Pdcp Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }


   if( (transCtrlBlk->cause == 17) &&
       (transCtrlBlk->ordrdCfg.actionMask == 0x0002) )
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3643); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Confirm received for Discard Required                    modification\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0 != nhLIMVldtAndRptCfgCfm ((Void *)pdcpCfgCfm, transCtrlBlk,
                        &nhIdentity, &cellAndUeCb, 0x04))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3667); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMVldtAndRptCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   return(0);
}
# 3696 "../..//lterrc/nh_lim.c"
 S16 nhLIMRptCfgCfm
(
NhId *nhIdentity ,
NhCellAndUeCb *cellAndUeCb,
...
# 3713 "../..//lterrc/nh_lim.c"
{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3723); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm: cfmStatus(%d), layerId(%d)\n", cfmStatus, layerId); SPrint((&nhCb.init)->prntBuf); } }; };


   if(layerId != 0x00)
   {

      { (transCtrlBlk->cfmMask) &= ~(layerId); };

      { if ((cfmStatus) == 0x01) { (transCtrlBlk->cfmStatus) |= (layerId); } else if ((cfmStatus) == 0x02) { (transCtrlBlk->cfmStatus) &= ~(layerId); } };
   }
   ...
   if (0x00 == transCtrlBlk->cfmMask)
   {
      ...
      { if ((transCtrlBlk)->cfmStatus == (transCtrlBlk)->cfgMask) { (status) = 1; } else { (status) = 0; } };
# 3763 "../..//lterrc/nh_lim.c"
      if (transCtrlBlk->cause == 0xEF)
      {
         if(transCtrlBlk->nhuSduStaCfm != 0L)
         {
      ...
      if (status)
      {
         if (0 != nhDBMCommitOrdrdCfg(cellAndUeCb->ueCb, transCtrlBlk))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3778); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhDBMCommitOrdrdCfg failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };

            return(1);
         }

         cellAndUeCb->ueCb->rbSecCfgBitMask |= 0x01;
         ...
         if ((transCtrlBlk->cause == 5) ||
             (transCtrlBlk->cause == 14) ||
             (transCtrlBlk->cause == 17) ||
             (transCtrlBlk->cause == 8))
         {
            if (transCtrlBlk->storedMsg)
            {
               if (0 != nhUTLSendStoredMsg(cellAndUeCb, transCtrlBlk))
               {
                  { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3799); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhUTLSendStoredMsg failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      else
      {
         ...
         if(transCtrlBlk->ordrdCfg.actionMask & 0x0040 &&
            transCtrlBlk->ordrdCfg.ueInfo.crntiCfgType == 0x02)
         {
            ret = nhHashListDelete(&(cellAndUeCb->cellCb->newUeHlCp),
                                   (U32)(cellAndUeCb->ueCb));
            if (0 != ret)
            {
                { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3817); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Could not delete from  list.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
            }
         }
      ...
      if (0 != nhUIMSndCfgCfm(*nhIdentity , nxTransId, tmpCfgMask,
                                tmpCfmStatus, 0x04))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3839); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhUIMSndCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(ret);
      }
      ...
      if ((!status) || (transCtrlBlk->cause != 17))
      {
...
# 3880 "../..//lterrc/nh_lim.c"
 S16 nhLIMRptCellCfgCfm
(
NhCellCb *cellCb,
U8 cfmStatus,
U8 layerId
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3901); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCellCfgCfm: cfmStatus(%d) layerId(%d) \n", cfmStatus, layerId); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   { (cellTransCb->cfmMask) &= ~(layerId); };

   { if ((cfmStatus) == 0x01) { (cellTransCb->cfmStatus) |= (layerId); } else if ((cfmStatus) == 0x02) { (cellTransCb->cfmStatus) &= ~(layerId); } };


   if (0x00 == cellTransCb->cfmMask)
   {

      if( 0 != (ret = nhSchedGenTmr(cellCb, 6, 0)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3916); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "stop timer failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
      ...
      {
         if (0 != nhDBMCommitCellOrdrdCfg(cellCb))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3931); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCellCfgCfm: nhUDMCommitCellOrdrdCfg failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      else
      {
         ...
         if(0x04 != cellCb->cellTransCb.cfgAction)
         {
      ...
      if (0 != nhUIMSndCellCfgCfm(cellTransCb->cfmStatus, cellCb,
                                                0x04))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3955); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCellCfgCfm: nhUIMSndCellCfgCfm failed"); SPrint((&nhCb.init)->prntBuf); } }; };
      ...
      if((0x04 == cellCb->cellTransCb.cfgAction) ||
            ((0x01 == cellCb->cellTransCb.cfgAction) &&
            ((0x02 == status) || (1 == ret))))
      {
         if(0 != nhDBMDeleteCellCb(&cellCb))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 3968); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhDBMDeleteCellCb failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         }
         return(ret);
      }
      else
      {
         cellCb->cellTransCb.status = 0x00;
      }
   }

   return(0);
}
# 3999 "../..//lterrc/nh_lim.c"
 S16 nhLIMHndlRvrtCellCfg
(
NhCellCb *cellCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4018); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlRvrtCellCfg \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   cellTransCb = &(cellCb->cellTransCb);



   cellTransCb->nxTransId = 0xffffffff;
# 4034 "../..//lterrc/nh_lim.c"
   if (cellTransCb->cfgMask & 0x02)
   {
      macRet = nhPIMRvrtMacCellCfg(cellCb);
   }

   if (cellTransCb->cfgMask & 0x01)
   {
   ...
   cellTransCb->status = 0;
   cellTransCb->numOfRlcCfms = 0;
   cellTransCb->numOfMacCfms = 0;
   do { (Void)cmMemset((U8*)(cellTransCb->rlcMapTbl), 0, (Size)(8*sizeof(NhRlcCellCfgCfmMap))); } while(0);

   do { (Void)cmMemset((U8*)(cellTransCb->macMapTbl), 0, (Size)(8*sizeof(NhMacCellCfgCfmMap))); } while(0);


   return(0);
}
# 4088 "../..//lterrc/nh_lim.c"
 S16 nhLIMSendCellCfg
(
NhCellCb *cellCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4102); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendCellCfg: sending cell Cfg ... \n"); SPrint((&nhCb.init)->prntBuf); } }; };

   cellTransCb = &(cellCb->cellTransCb);

   if (1 != cellTransCb->status)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4109); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendCellCfg: Invalid Cell to Configure\n"); SPrint((&nhCb.init)->prntBuf); } }; };

      return(1);
   }


   if( 0 != (ret = nhSchedGenTmr(cellCb, 6, 1)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4116); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "start timer failed  \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if (cellTransCb->cfgMask & 0x02)
   {
      if (0 != nhPIMSendMacCellCfg(cellCb))
      {

         if( 0 != (ret = nhSchedGenTmr(cellCb, 6,
                                         0)))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4128); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "stop timer failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(1);
         }

         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4133); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendCellCfg: nhPIMSendMacCellCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (cellTransCb->cfgMask & 0x01)
   {
      if (0 != nhPIMSendRlcCellCfg(cellCb))
      {

         if( 0 != (ret = nhSchedGenTmr(cellCb, 6,
                                         0)))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4146); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "stop timer failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(1);
         }

         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4151); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendCellCfg: nhPIMSendRlcCellCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(1);
      }
   }
# 4175 "../..//lterrc/nh_lim.c"
   return(0);
}
# 4198 "../..//lterrc/nh_lim.c"
 S16 nhLIMHndlRvrtMacCfg
(
NhId *nhIdentity,
NhActvUeCb *ueCb,
TransCb *transCtrlBlk
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4218); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "In nhLIMHndlRvrtMacCfg \n"); SPrint((&nhCb.init)->prntBuf); } }; };





   if ((transCtrlBlk->macLchMapTbl[0].addCfmStatus > 0)
         || (transCtrlBlk->macLchMapTbl[1].addCfmStatus > 0))
   {
      rvrtCfgType |= 0x02;
   }

   if ((1 ==
            transCtrlBlk->ueMapTbl[0].status) &&
       (0x01 ==
            transCtrlBlk->ueMapTbl[0].action) &&
       (0x00 ==
            transCtrlBlk->ueMapTbl[0].cfmStatus))
   {
      rvrtCfgType |= 0x01;
   }
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4245); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhPIMRvrtMacAddCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   rvrtCfgType = 0;

   if (transCtrlBlk->macLchMapTbl[1].modCfmStatus > 0)
   {
      rvrtCfgType |= 0x02;
   }
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4271); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhPIMRvrtMacModCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   ...
   if ((transCtrlBlk->macLchMapTbl[0].rlsCfmStatus > 0)
         || (transCtrlBlk->macLchMapTbl[1].rlsCfmStatus > 0))
   {
      rvrtCfgType |= 0x02;
   }
   ...
   if(rvrtCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4289); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhPIMRvrtMacRlsCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
...
# 4318 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlMacCellCfgCfm
(
NhId *nhIdentity,
NhCellCb *cellCb,
U8 cfgElms,
...
# 4334 "../..//lterrc/nh_lim.c"
{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4341); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacCellCfgCfm"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4349); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "duplicate Mac Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }

   if (cfgElms & 0x02)
   {
      if (cellTransCb->macMapTbl[0].status == 1)
      {
         cellTransCb->numOfMacCfms--;
         if (status != 0x00)
         {
            cellTransCb->macMapTbl[0].cfmStatus = 0x02;
         }
         else
         {
            cellTransCb->macMapTbl[0].cfmStatus = 0x01;
         }
      }
   }
   else if (cfgElms & 0x01)
   {
      cellTransCb->numOfMacCfms--;



      for (rbIndex = 1; rbIndex < 8; rbIndex++)
      {
         if ((cellTransCb->macMapTbl[rbIndex].status == 1) &&
                         (lclId == cellTransCb->macMapTbl[rbIndex].logChId))
         {
            if (status != 0x00)
            {
               cellTransCb->macMapTbl[rbIndex].cfmStatus = 0x02;
            }
            else
            {
               cellTransCb->macMapTbl[rbIndex].cfmStatus = 0x01;
            }
            break;
         }
      }
   ...
   if (cellTransCb->numOfMacCfms <= 0)
   {
      for (rbIndex = 0; rbIndex < 8; rbIndex++)
      {
         if ((cellTransCb->macMapTbl[rbIndex].status == 1) &&
             (0x02 == cellTransCb->macMapTbl[rbIndex].cfmStatus))
         {
               cgmCfmStatus = 0x02;
         }
      }

      if (0 != nhLIMRptCellCfgCfm(cellCb, cgmCfmStatus, 0x02))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4406); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCellCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
}
# 4432 "../..//lterrc/nh_lim.c"
static S16 nhLIMMacUlMapTblUpd
(
TransCb *transCtrlBlk,
U8 lclId,
U8 status
...

{
   ...
   ;
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4450); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMMacUlMapTblUpd"); SPrint((&nhCb.init)->prntBuf); } }; };

   found = 0;

   tmp = (1 << (lclId - 1));

   if(transCtrlBlk->macLchMapTbl[1].logchsRcvd & tmp)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4459); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate MAC Cfg Cfm RCVD for                   logical channel ID (%d)\n", lclId); SPrint((&nhCb.init)->prntBuf); } }; };

      return(0);
   }
   else
   {
      transCtrlBlk->macLchMapTbl[1].logchsRcvd |= tmp;
   }

   if (transCtrlBlk->macLchMapTbl[1].logchsToAdd & tmp)
   {
      found = 1;
      tmpCfmStatus = &(transCtrlBlk->macLchMapTbl[1].addCfmStatus);
   }
   else if (transCtrlBlk->macLchMapTbl[1].logchsToMod & tmp)
   {
      found = 1;
      tmpCfmStatus = &(transCtrlBlk->macLchMapTbl[1].modCfmStatus);
   }
   else if (transCtrlBlk->macLchMapTbl[1].logchsToRls & tmp)
   {
      found = 1;
      tmpCfmStatus = &(transCtrlBlk->macLchMapTbl[1].rlsCfmStatus);
   }

   if (0 == found)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4485); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid MAC Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
...
# 4514 "../..//lterrc/nh_lim.c"
static S16 nhLIMMacDlMapTblUpd
(
TransCb *transCtrlBlk,
U8 lclId,
U8 status
...

{
   ...
   ;
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4532); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMMacDlMapTblUpd"); SPrint((&nhCb.init)->prntBuf); } }; };

   found = 0;

   tmp = (1 << (lclId - 1));
   if(transCtrlBlk->macLchMapTbl[0].logchsRcvd & tmp)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4540); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate MAC Cfg Cfm RCVD for                   logical channel ID (%d)\n", lclId); SPrint((&nhCb.init)->prntBuf); } }; };

      return(0);
   }
   else
   {
      transCtrlBlk->macLchMapTbl[0].logchsRcvd |= tmp;
   }

   if (transCtrlBlk->macLchMapTbl[0].logchsToAdd & tmp)
   {
      found = 1;
      tmpCfmStatus = &(transCtrlBlk->macLchMapTbl[0].addCfmStatus);
   }
   else if (transCtrlBlk->macLchMapTbl[0].logchsToRls & tmp)
   {
      found = 1;
      tmpCfmStatus = &(transCtrlBlk->macLchMapTbl[0].rlsCfmStatus);
   }

   if (0 == found)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4561); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid MAC Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
...
# 4592 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlMacUECfgCfm
(
NhId *nhIdentity,
NhCellCb *cellCb,
NhTransId nxTransId,
...
# 4613 "../..//lterrc/nh_lim.c"
{
   TransCb *transCtrlBlk = 0L;
   NhUeCfgCfmMapInfo *macUeMapTblPtr = 0L;
   U8 cgmCfmStatus = 0x01;
   NhActvUeCb *ueCb = 0L;
   NhCellAndUeCb cellAndUeCb;
   NhMacRstInfo *rstTbl=0L;


   ;

   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4624); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacUECfgCfm"); SPrint((&nhCb.init)->prntBuf); } }; };

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, nhIdentity->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4628); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if (0L == (transCtrlBlk = nhUTLGetTransCbFrmNxTransId(ueCb, nxTransId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4634); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "transId doesnt exist\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4641); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "duplicate Mac Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(0);
   }

   if (cfgElms == 0x01)
   {
      macUeMapTblPtr = &(transCtrlBlk->ueMapTbl[0]);

      if (macUeMapTblPtr->status == 1)
      {
         if (status != 0x00)
         {
            macUeMapTblPtr->cfmStatus = 0x0F;

            nhUTLUpdGenSts(5);
         }
         else
         {
            macUeMapTblPtr->cfmStatus = 0x00;
         }
      }
      transCtrlBlk->numOfMacCfms--;
   }
   else if (cfgElms == 0x03)
   {
      rstTbl = &(transCtrlBlk->rstTbl[0]);

      if (rstTbl->status == 1)
      {
         if (status != 0x00)
         {
            rstTbl->cfmStatus = 0x0F;

            nhUTLUpdGenSts(5);
         }
         else
         {
            rstTbl->cfmStatus = 0x00;
         }
      }
      transCtrlBlk->numOfMacCfms--;
   }
   else if (cfgElms == 0x02)
   {
      ...
      if (dir & 0x01)
      {
         if (0 != (nhLIMMacDlMapTblUpd(transCtrlBlk, lclId, status)))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4694); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMMacDlMapTblUpd failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(0);
         }
      }
      if (dir & 0x02)
      {
         if (0 != (nhLIMMacUlMapTblUpd(transCtrlBlk, lclId, status)))
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4702); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMMacUlMapTblUpd failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(0);
         }
      }
      if(status == 0x0F)
   ...
   if (transCtrlBlk->numOfMacCfms <= 0)
   {
      ...
      if ((transCtrlBlk->macLchMapTbl[0].logchsToAdd !=
                  transCtrlBlk->macLchMapTbl[0].addCfmStatus) ||
          (transCtrlBlk->macLchMapTbl[0].logchsToRls !=
                  transCtrlBlk->macLchMapTbl[0].rlsCfmStatus) ||
          (transCtrlBlk->macLchMapTbl[1].logchsToAdd !=
                  transCtrlBlk->macLchMapTbl[1].addCfmStatus) ||
          (transCtrlBlk->macLchMapTbl[1].logchsToMod !=
                  transCtrlBlk->macLchMapTbl[1].modCfmStatus) ||
          (transCtrlBlk->macLchMapTbl[1].logchsToRls !=
                  transCtrlBlk->macLchMapTbl[1].rlsCfmStatus))
      {
         cgmCfmStatus = 0x02;
      }
      ...
      if (0 != nhLIMRptCfgCfm(nhIdentity, &cellAndUeCb, transCtrlBlk,
                                cgmCfmStatus, 0x02))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 4754); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
}
# 5084 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlMacCfgCfm
(
CrgCfgTransId transId,
U8 status
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5107); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacCfgCfm"); SPrint((&nhCb.init)->prntBuf); } }; };

   transIdPtr = transId.trans;


   do { (Void)cmMemcpy((U8*)((U8*)&nhIdentity.cellId),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U16)));} while(0);
   transIdPtr += sizeof(U16);
   do { (Void)cmMemcpy((U8*)((U8*)&nhIdentity.ueId),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U16)));} while(0);
   transIdPtr += sizeof(U16);

   do { (Void)cmMemcpy((U8*)((U8*)&nxTransId),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(NhTransId)));} while(0);
   transIdPtr += sizeof(NhTransId);
   do { (Void)cmMemcpy((U8*)((U8*)&cfgElms),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U8)));} while(0);
   transIdPtr += sizeof(U8);
   do { (Void)cmMemcpy((U8*)((U8*)&macActionBitMask),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U8)));} while(0);
   transIdPtr += sizeof(U8);
   do { (Void)cmMemcpy((U8*)((U8*)&lclId),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U8)));} while(0);
   transIdPtr += sizeof(U8);
   do { (Void)cmMemcpy((U8*)((U8*)&dir),(const U8*)((const U8*)transIdPtr), (Size)(sizeof(U8)));} while(0);


   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5129); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacCfgCfm: transId (%ld) status(%d)            bitmask(%d) \n", nxTransId, status, macActionBitMask); SPrint((&nhCb.init)->prntBuf); } }; };
# 5138 "../..//lterrc/nh_lim.c"
   if (nxTransId == 0xffffffff)
   {
   ...
   if (0L == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5158); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULLP Cell Cb\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if (nhIdentity.ueId > 0)
   {
      if (0 != (nhLIMHndlMacUECfgCfm(&nhIdentity, cellCb, nxTransId, lclId,
                                       cfgElms, status, dir)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5167); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacUECfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   else
   {
      if (0 != (nhLIMHndlMacCellCfgCfm(&nhIdentity, cellCb, cfgElms,
                                         status, lclId)))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5176); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlMacCellCfgCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
}
# 5201 "../..//lterrc/nh_lim.c"
 S16 nhLIMSendCfgMsg
(
NhCellAndUeCb *cellAndUeCb,
TransCb *transCtrlBlk
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5217); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendCfgMsg: sending Cfg msg \n"); SPrint((&nhCb.init)->prntBuf); } }; };


   nhStartLLyrCfgTmr(cellAndUeCb, transCtrlBlk->nxTransId);
# 5238 "../..//lterrc/nh_lim.c"
   if (transCtrlBlk->cfgMask & 0x02)
   {
      if (0 != nhLIMSendMacCfg(cellAndUeCb->ueCb, transCtrlBlk))
      {

         nhStopLLyrCfgTmr(cellAndUeCb, transCtrlBlk->nxTransId);

         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5245); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendMacCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }

   if (!(transCtrlBlk->ordrdCfg.actionMask & 0x0080 ||
         transCtrlBlk->ordrdCfg.actionMask & 0x0200))
   {
       ...
       if (transCtrlBlk->cause == 0xEF)
       {
          ret = nhPIMSndSduStaReq(nhIdentity , transCtrlBlk, cellAndUeCb, 0x01);
          if (ret != 0)
            return(ret);
       }
       ...
       if (transCtrlBlk->cfgMask & 0x01)
       {
           ...
           if (0 != nhPIMSendRlcCfg(cellAndUeCb->ueCb, transCtrlBlk))
           {

              nhStopLLyrCfgTmr(cellAndUeCb, transCtrlBlk->nxTransId);
              { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5271); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendRlcCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
              return(1);
           }
       }
   }
       ...
       (transCtrlBlk->cause != 0xEF))
   {
      ...
      if (0 != nhPIMSendPdcpCfg(cellAndUeCb->ueCb, transCtrlBlk))
      {

         nhStopLLyrCfgTmr(cellAndUeCb, transCtrlBlk->nxTransId);

         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5288); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendPdcpCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
            ...
            transCtrlBlk->ordrdCfg.actionMask & 0x0200))
   {
      ...
      if (transCtrlBlk->cfgMask & 0x01)
      {
         ...
         if (0 != nhPIMSendRlcCfg(cellAndUeCb->ueCb, transCtrlBlk))
         {
            ...
            { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5315); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendRlcCfg failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
            return(1);
         }
      }
   }
...

{
   ...
   ;
      ...
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5358); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendMacCfg: sending Cfg to MAC \n"); SPrint((&nhCb.init)->prntBuf); } }; };



   if ((1 == transCtrlBlk->rstTbl[0].status)
   &&(0x08 == transCtrlBlk->rstTbl[0].action))
   {
      rstCfgType |= 0x03;
   }
   if(rstCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5373); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendMacCfg: nhPIMSendMacResetCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

         return(1);
      }
   }

   if ((transCtrlBlk->macLchMapTbl[0].logchsToAdd > 0) ||
         (transCtrlBlk->macLchMapTbl[1].logchsToAdd > 0))
   {
      addCfgType |= 0x02;
   }
   ...
   if ((1 ==
           transCtrlBlk->ueMapTbl[0].status) &&
       (0x01 == transCtrlBlk->ueMapTbl[0].action))
   {
      addCfgType |= 0x01;
   }
   ...
   if(addCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5396); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhPIMSendMacAddCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }

   if ((transCtrlBlk->macLchMapTbl[0].logchsToMod > 0) ||
         (transCtrlBlk->macLchMapTbl[1].logchsToMod > 0))
   {
      modCfgType |= 0x02;
   }
   ...
   if(modCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5420); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendMacModCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }

   if ((transCtrlBlk->macLchMapTbl[0].logchsToRls > 0) ||
         (transCtrlBlk->macLchMapTbl[1].logchsToRls > 0))
   {
      rlsCfgType |= 0x02;
   }
   ...
   if(rlsCfgType > 0)
   {
      ...
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5436); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendMacModCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
...
# 5621 "../..//lterrc/nh_lim.c"
 S16 nhLIMRlsLowerLyrCfg
(
NhActvUeCb *ueCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5635); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRlsLowerLyrCfg: complete release of lower lyr cfg \n"); SPrint((&nhCb.init)->prntBuf); } }; };


   ret = nhPIMRlcPdcpMacRlsCfg(ueCb);
   if (0 != ret)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5640); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhPIMRlcPdcpMacRlsCfg failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
# 5658 "../..//lterrc/nh_lim.c"
   return(0);
}
# 5677 "../..//lterrc/nh_lim.c"
 S16 nhLIMSendUbndReq
(
U8 sapElmId,
NhLSapCb *lSapCb
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5692); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMSendUbndReq\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   switch(sapElmId)
   {
      ...
      default:
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5723); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " Invalid sapElmId\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
   }

   return(ret);
}
# 5746 "../..//lterrc/nh_lim.c"
 S16 nhLIMSendBndReq
(
U16 sapElmId,
NhLSapCb *lSapCb
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5761); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMSendBndReq ElmId(%d)\n",sapElmId); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   switch(sapElmId)
   {
      ...
      default:
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5792); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " Invalid sapElmId\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
   }

   return(ret);
}
# 5818 "../..//lterrc/nh_lim.c"
 S16 NhLiPjuStaInd
(
Pst *pst,
SuId suId,
...
# 5834 "../..//lterrc/nh_lim.c"
{
   ...
   if ((pdcpId) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5845); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL pdcpId\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      if (!((staInd) == 0L))
      {
         { if ((staInd) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5848, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(staInd), (Size)(sizeof(PjuStaIndInfo))); (staInd) = 0L; } };
      }
      if (!((buf) == 0L))
      {
         do{ if ((buf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5852, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((buf)); (buf) = 0L; } }while(0);
      }
      return(1);
   }
   if ((staInd) == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5858); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "GOT NULL staInd\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      if (!((buf) == 0L))
      {
         do{ if ((buf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5861, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((buf)); (buf) = 0L; } }while(0);
      }
      { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5863, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
      return(1);
   }

   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5868); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiPjuStaInd: suId(%d), status(%d)\n", suId, staInd->cause); SPrint((&nhCb.init)->prntBuf); } }; };


   pjuSapCb = nhUTLFindLlyrSapFrmSuId(7, suId);
# 5882 "../..//lterrc/nh_lim.c"
   { (nhIdentity).cellId = (pdcpId)->cellId; (nhIdentity).ueId = (pdcpId)->ueId; (nhIdentity).isPdcpId = 1; (nhIdentity).rbId = (pdcpId)->rbId; };

  { if(nhCb.init.logMask & 0x1) { ssDiagFix(0x004f0012, NH_DIAG_UEID, 0xd9, nhCb.init.inst, 0x1, SS_DIAG_MSG_TYPE_FIXED, nhIdentity.ueId, suId, nhIdentity.rbId, 0, 0, "Received StaInd from PDCP suId = %d, rbId = %d"); } };




   if (nhIdentity.isPdcpId == 1)
   {

      ret = nhLIMHndlStatusInd(&nhIdentity, staInd, buf);
      if (0 != ret)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5894); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlStatusInd failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
      }
   }
   else
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5899); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NhLiPjuStaInd: not a pdcp ID\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   }

   { if ((pdcpId) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5902, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(pdcpId), (Size)(sizeof(CmLtePdcpId))); (pdcpId) = 0L; } };
   { if ((staInd) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5903, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((nhCb.init.region), (nhCb.init.pool), (Data *)(staInd), (Size)(sizeof(PjuStaIndInfo))); (staInd) = 0L; } };
   if (buf != 0L)
   {
      do{ if ((buf) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 5906, 0x2, 0, (ErrVal) 0, "Can't free the mBuf: GOT NULL PTR\n"); } else { (Void)SPutMsg((buf)); (buf) = 0L; } }while(0);
   }

   return(ret);
}
# 5928 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlCountCfm
(
CpjCountCfmInfo *countCfmInfo
)
...
{
   NhuDRBCountInfoLst *nhuDRBCountInfoLst = 0L;
   NhuDrbCountInfo *nhuDrbCountInfo = 0L;
   NhuErrIndSdus errIndSdus;
   Mem sMem;
   U8 rbIndex;
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5945); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlCountCfm"); SPrint((&nhCb.init)->prntBuf); } }; };


   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5950); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlCountCfm:  transId(%ld) ueId(%d) cellId                 (%d)\n", countCfmInfo->transId, countCfmInfo->ueId, countCfmInfo->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
# 5959 "../..//lterrc/nh_lim.c"
  sMem.region = nhCb.init.region;
  sMem.pool = nhCb.init.pool;

   do { (*((Ptr *)&nhuDRBCountInfoLst)) = 0L; (Void)cmAllocEvnt((Size)(sizeof(NhuDRBCountInfoLst)), 3584, (&sMem), (Ptr *)((Ptr *)&nhuDRBCountInfoLst)); } while(0);
   if (nhuDRBCountInfoLst == 0L)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5966); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for storing DRB Count info List.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (countCfmInfo->status != 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5980); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlCountCfm: Count Confirm Failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };

      nhuDRBCountInfoLst->status = 0;
   }
   else
   {
      nhuDRBCountInfoLst->status = 1;
      if (countCfmInfo->numRb >= 11)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 5990); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlCountCfm: Number of DRBs out of rang\ne"); SPrint((&nhCb.init)->prntBuf); } }; };

         { (errIndSdus.sdu).errPres = 1; (errIndSdus.sdu).errType = (0x06); (errIndSdus.sdu).errCause = (0x2E); };

         do { cmFreeMem((nhuDRBCountInfoLst)); (nhuDRBCountInfoLst) = 0L; } while(0);
         return(1);
      }

      do { (*(&nhuDrbCountInfo)) = 0L; (Void)cmGetMem((nhuDRBCountInfoLst), (countCfmInfo->numRb * sizeof(NhuDrbCountInfo)), (Ptr *)(&nhuDrbCountInfo)); } while(0);



      if (nhuDrbCountInfo == 0L)
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6004); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for storing DRB Count info.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (0 != nhUIMSndCountCfm(nhuDRBCountInfoLst))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6030); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMHndlCountCfm: nhUIMSndCountCfm failed \n"); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 6051 "../..//lterrc/nh_lim.c"
static S16 nhLIMHndlPdcpSecCfgCfm
(
CpjSecCfgCfmInfo *secCfgCfm
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 0)) { Txt prntBuf[10]; sprintf(prntBuf, "[info]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 0))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6072); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, " nhLIMHndlPdcpSecCfgCfm: transId(%ld) ueId(%d) cellId                 (%d)\n", secCfgCfm->transId, secCfgCfm->ueId, secCfgCfm->cellId); SPrint((&nhCb.init)->prntBuf); } }; };
# 6079 "../..//lterrc/nh_lim.c"
   nhIdentity.cellId = secCfgCfm->cellId;
   nhIdentity.ueId = secCfgCfm->ueId;

   if (0L == (cellCb = nhDBMGetCellCb(nhIdentity.cellId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6084); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL CELL CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   if ((NhActvUeCb*)0L == (ueCb = nhDBMGetUeCb(cellCb, secCfgCfm->ueId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6090); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "NULL UE CB \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   cellAndUeCb.ueCb = ueCb;
   cellAndUeCb.cellCb = cellCb;
   ...
   if ((TransCb*)0L == (transCtrlBlk = nhUTLGetTransCbFrmNxTransId(ueCb,
                                          secCfgCfm->transId)))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6101); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "transId doesnt exist \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
    ...
    if (transCtrlBlk->numOfPdcpCfms <= 0)
    {
       { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6108); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Pdcp Cfg Cfm Rcvd \n"); SPrint((&nhCb.init)->prntBuf); } }; };
       return(0);
    }
    transCtrlBlk->numOfPdcpCfms--;
   ...
   if ((1 == transCtrlBlk->secCfgMapTbl[0].status) ||
       (1 == transCtrlBlk->secCfgMapTbl[1].status))
   {
         if (secCfgCfm->status == 2)
         {
            cgmCfmStatus = 0x02;
         }
   }
   ...
   if(transCtrlBlk->numOfPdcpCfms <= 0)
   {
      if (cgmCfmStatus == 0x01)
      {
         ueCb->rbSecCfgBitMask |= 0x04;
      }

      if (0 != nhLIMRptCfgCfm(&nhIdentity, &cellAndUeCb, transCtrlBlk,
                                 cgmCfmStatus, 0x04))
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6140); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
...
# 6163 "../..//lterrc/nh_lim.c"
 S16 nhLIMCountReqHndlr
(
NhuHdr *cntHdr,
NhActvUeCb *ueCb
)
...

{
   ...
   ;
   (ueCb=ueCb);
   if (0 != nhPIMSndCountReq(cntHdr, ueCb))
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6180); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMCountReqHndlr: Cound not send Count request\n"); SPrint((&nhCb.init)->prntBuf); } }; };
...
# 6206 "../..//lterrc/nh_lim.c"
 S16 nhLIMFillPdcpSduStaInfo
(
CpjSduStaCfmInfo *cpjSduStaCfm,
NhuPdcpSduStaCfm *nhuSduStaCfm
)
...
{
   NhuPdcpSduStaInfo *pdcpSduStaInfo = 0L;
   U8 numRbId;
   Size ulBitMapSize;
   U8 rbCnt;
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6224); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf," nhLIMFillPdcpSduStaInfo called.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   if (numRbId == 0)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6232); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NhuPdcpSduStaInfo Number of RbId is Zero                     failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };

      return(1);
   }

   do { (*(&(nhuSduStaCfm->pdcpSduStaInfo))) = 0L; (Void)cmGetMem((nhuSduStaCfm), ((sizeof(NhuPdcpSduStaInfo) * numRbId)), (Ptr *)(&(nhuSduStaCfm->pdcpSduStaInfo))); } while(0);

   if(0L == nhuSduStaCfm->pdcpSduStaInfo)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6241); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf,"NhuPdcpSduStaInfo memory allocation                     failed.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   for(rbCnt = 0; rbCnt < numRbId; rbCnt++)
   {
      ...
      { ulBitMapSize = cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.numBits / 8; if(cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.numBits % 8) { ulBitMapSize++; } };
      ...
      if(ulBitMapSize)
      {
         ...
         do { (*(&pdcpSduStaInfo->rcvStaOfUlPdcpSdus.ulBitMap)) = 0L; (Void)cmGetMem((nhuSduStaCfm), ((ulBitMapSize * sizeof(U8))), (Ptr *)(&pdcpSduStaInfo->rcvStaOfUlPdcpSdus.ulBitMap)); } while(0);

         if(0L == pdcpSduStaInfo->rcvStaOfUlPdcpSdus.ulBitMap)
         {
            { if (nhCb.init.dbgMask & (0x00000100 << 4)) { Txt prntBuf[10]; sprintf(prntBuf, "[fatal]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 4))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6273); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Memory Allocation failed for storing ulBitMap.\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         ...
         else
         {
            ...
            do { (Void)cmMemcpy((U8*)((U8*)pdcpSduStaInfo->rcvStaOfUlPdcpSdus.ulBitMap),(const U8*)((const U8*)cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.ulBitMap), (Size)(ulBitMapSize));} while(0);
...
# 6304 "../..//lterrc/nh_lim.c"
 S16 nhLIMFreeCpjSduStaCfm
(
Pst *pst,
CpjSduStaCfmInfo *cpjSduStaCfm
)
...

{
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6320); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf," nhLIMFreeCpjSduStaCfm called.\n"); SPrint((&nhCb.init)->prntBuf); } }; };


   for(rbCnt = 0; rbCnt < cpjSduStaCfm->numRb; rbCnt++)
   {
      if(0 != cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.numBits)
      {
         { ulBitMapSize = cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.numBits / 8; if(cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.numBits % 8) { ulBitMapSize++; } };
         ...
         { if ((cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.ulBitMap) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 6334, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.ulBitMap), (Size)(sizeof(U8) * ulBitMapSize)); (cpjSduStaCfm->sduStaInfo[rbCnt].ulSduStaInfo.ulBitMap) = 0L; } };


      }
   }
   { if ((cpjSduStaCfm) == 0L) { SLogError(nhCb.init.ent, nhCb.init.inst, nhCb.init.procId, "../..//lterrc/nh_lim.c", 6337, 0x2, 0, (ErrVal) 0, "Can't free the memory: GOT NULL PTR\n"); } else { (Void) SPutSBuf((pst->region), (pst->pool), (Data *)(cpjSduStaCfm), (Size)(sizeof(CpjSduStaCfmInfo))); (cpjSduStaCfm) = 0L; } };
   return(0);
}
# 6360 "../..//lterrc/nh_lim.c"
static S16 nhLIMVldtAndRptCfgCfm
(
Void *vCfmInfo,
TransCb *transCtrlBlk,
NhId *nhIdentity,
...
# 6377 "../..//lterrc/nh_lim.c"
{
   NhRlcPdcpCfgCfmMapInfo *mapTbl;
   NhuHdr hdr;
   NhRlcPdcpCfmEnt cfmEnt[24];
   U8 cgmCfmStatus = 0x01;
   U8 numEnt;
   U8 rbId;
   Bool rbType;
   ...
   { if (nhCb.init.dbgMask & (0x00000100 << 1)) { Txt prntBuf[10]; sprintf(prntBuf, "[debug]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 1))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6396); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMVldtAndRptCfgCfm\n"); SPrint((&nhCb.init)->prntBuf); } }; };
   ...
   for (entIndex = 0; entIndex < numEnt; entIndex++)
   {
      ...
      if(rbType == 0)
      {
         if ((0 != nhLIMSrchAndSetSRBBitMsk(&rcvdSrbIds,
      ...
      {
         if (0 != nhLIMSrchAndSetDRBBitMsk(&rcvdDrbIds, mapTbl->drbToCfg,
   ...
   if (
           (rcvdSrbIds != ((mapTbl->srbToCfg[0]) |
                          (mapTbl->srbToCfg[1]) |
                          (mapTbl->srbToCfg[2]) |
                          (mapTbl->srbToCfg[3])))
           ||
           (rcvdDrbIds != ((mapTbl->drbToCfg[0]) |
                          (mapTbl->drbToCfg[1]) |
                          (mapTbl->drbToCfg[2]) |
                          (mapTbl->drbToCfg[3])))
      )
   {
      ...
      nhSendLmAlarm(2, (256 + 0),
                 0, &dgn);
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6491); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid number of RLC Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
   ...
   if(numOfCfms <= 0)
   {
      if( (mapTbl->srbCfmSta[1] !=
          ...
          (mapTbl->drbCfmSta[3] !=
               mapTbl->drbToCfg[3]) ||
          ((transCtrlBlk->ueMapTbl[maxTblIdx].status == 1) &&
      ...
      {
         { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6538); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "nhLIMRptCfgCfm failed\n"); SPrint((&nhCb.init)->prntBuf); } }; };
         return(1);
      }
   }
   return(0);
}
# 6564 "../..//lterrc/nh_lim.c"
static S16 nhLIMSrchAndSetSRBBitMsk
(
U8 *rcvdRbIds,
U8 *rbCfgTbl,
U8 *rbStaTbl,
...
# 6583 "../..//lterrc/nh_lim.c"
{
   ...
   { if(*rcvdRbIds & tmp) { { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6600); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate Cfg Cfm RCVD\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { *rcvdRbIds |= tmp; } };

   { for (j = 0; j < 4; j++) { if(rbCfgTbl[j] & tmp) { found = 1; break; } } };

   if (0 == found)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6606); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }

   { if ((status == okValue) && (j < 4)) rbStaTbl[j] &= ~tmp; };

   return(0);
}
# 6634 "../..//lterrc/nh_lim.c"
static S16 nhLIMSrchAndSetDRBBitMsk
(
U32 *rcvdRbIds,
U32 *rbCfgTbl,
U32 *rbStaTbl,
...
# 6653 "../..//lterrc/nh_lim.c"
{
   ...
   { if(*rcvdRbIds & tmp) { { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6668); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Duplicate Cfg Cfm RCVD\n"); SPrint((&nhCb.init)->prntBuf); } }; }; } else { *rcvdRbIds |= tmp; } };

   { for (j = 0; j < 4; j++) { if(rbCfgTbl[j] & tmp) { found = 1; break; } } };

   if (0 == found)
   {
      { if (nhCb.init.dbgMask & (0x00000100 << 3)) { Txt prntBuf[10]; sprintf(prntBuf, "[error]"); SPrint(prntBuf); } { if ((&nhCb.init)->dbgMask & ((0x00000100 << 3))) { sprintf((&nhCb.init)->prntBuf, "[%s 0x%x:%x] %s:%d ", "LTE-RRC", (&nhCb.init)->ent, (&nhCb.init)->inst, "../..//lterrc/nh_lim.c", 6674); SPrint((&nhCb.init)->prntBuf); sprintf (nhCb.init.prntBuf, "Invalid Cfg Cfm RCVD \n"); SPrint((&nhCb.init)->prntBuf); } }; };
      return(1);
   }
