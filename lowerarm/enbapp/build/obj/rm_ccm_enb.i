...
# 97 "../..//enbapp/lterrm/rm_ccm_enb.c" 2
# 119 "../..//enbapp/lterrm/rm_ccm_enb.c"
static S16 rmCcmAddMme
(
 RrmWrMmeCfg *mmeInfo,
 RrmWrCause *cause
)
{
   ...
   mmeCbPtr = rmUtlGetMmeCb(mmeInfo->mmeId);
   if(mmeCbPtr != 0L)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 131); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " MME already exists with mmeId:" "(%u)", mmeInfo->mmeId); SPrint((&(rmCb.init))->prntBuf); } }; };

      *cause = RRM_WR_CAUSE_MME_ALREADY_EXISTS;
      return(1);
   }

   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&(mmeCbPtr)), (sizeof(RmMmeCb))) == 0) { { cmMemset((U8 *)((*(&(mmeCbPtr)))), 0, ((sizeof(RmMmeCb)))); }; } else { (*(&(mmeCbPtr))) = 0L; } };
   if (0L == mmeCbPtr)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 140); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " Failed to allocate memory for" "MME Control Block (%u)", mmeInfo->mmeId); SPrint((&(rmCb.init))->prntBuf); } }; };
   ...
   mmeCbPtr->mmeId = mmeInfo->mmeId;
   mmeCbPtr->mmeCbLnk.node = (U32)mmeCbPtr;


   cmLListAdd2Tail(&rmCb.enbCb.mmeCbLst, &mmeCbPtr->mmeCbLnk);

   return(0);
}
# 174 "../..//enbapp/lterrm/rm_ccm_enb.c"
static S16 rmCcmDelMme
(
 RrmWrMmeCfg *mmeInfo,
 RrmWrCause *cause
)
{
   ...
   if(0L == mmeCbPtr)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 186); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " Failed to Find MMECb with mmeId:" "(%u)", mmeInfo->mmeId); SPrint((&(rmCb.init))->prntBuf); } }; };

      *cause = RRM_WR_CAUSE_MMEID_UNKNOWN;
      return(1);
   }

   cmLListDelFrm(&rmCb.enbCb.mmeCbLst, &mmeCbPtr->mmeCbLnk);


   { (Void) SPutSBuf(rmCb.init.region, rmCb.init.pool, (Data *) (mmeCbPtr), (Size) (sizeof(RmMmeCb))); (mmeCbPtr) = 0L; };

   return(0);
}
# 222 "../..//enbapp/lterrm/rm_ccm_enb.c"
static S16 rmCcmProcMmeCfg
(
 RrmWrMmeCfg *mmeInfo,
 RrmWrCause *cause
)
{
   switch(mmeInfo->addModOrDel)
   {
      case RRM_WR_CFG_ADD:
      {
         if(0 != rmCcmAddMme(mmeInfo, cause))
         {
            return(1);
         }
         break;
      ...
      case RRM_WR_CFG_DEL:
      {
         if(0 != rmCcmDelMme(mmeInfo, cause))
         {
            return(1);
         }
         break;
      ...
      default:
      {
         { { if ((&(rmCb.init))->dbgMask & (2)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 257); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcMmeCfg: " "Received %u invalid action request received", mmeInfo->addModOrDel); SPrint((&(rmCb.init))->prntBuf); } }; };
...
# 285 "../..//enbapp/lterrm/rm_ccm_enb.c"
 S16 rmCcmProcEnodebCfg
(
RrmWrEnbCfgReqInfo *enbInfo
)
{
   static Bool enbBoot = 0;
   RrmWrEnbCfgRespInfo *enbCfgRspPtr = 0L;


   { if (SGetSBuf(rmCb.init.region, rmCb.init.pool, (Data **)(&(enbCfgRspPtr)), (sizeof(RrmWrEnbCfgRespInfo))) == 0) { { cmMemset((U8 *)((*(&(enbCfgRspPtr)))), 0, ((sizeof(RrmWrEnbCfgRespInfo)))); }; } else { (*(&(enbCfgRspPtr))) = 0L; } };
   if (0L == enbCfgRspPtr)
   {
      { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 298); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " Failed to allocate memory for" "Enode Config Response "); SPrint((&(rmCb.init))->prntBuf); } }; };

      return(1);
   }
   enbCfgRspPtr->statusInfo.cause = RRM_WR_CAUSE_NOT_APP;

   if(enbInfo->cfgType == 0x01)
   {
      if (0 == enbBoot)
      {
         rmCb.init.dbgMask = 0xffffffff;

         enbBoot = 1;



         cmLListInit(&rmCb.enbCb.cellCbLst);
         cmLListInit(&rmCb.enbCb.mmeCbLst);
      ...
      {
     { { if ((&(rmCb.init))->dbgMask & (2)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 331); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcEnodebCfg: " "RRM Initialization is already happend"); SPrint((&(rmCb.init))->prntBuf); } }; };

         enbCfgRspPtr->statusInfo.cause = RRM_WR_CAUSE_ENB_ALREADY_UP;
      }
   }
   else if(enbInfo->cfgType == 0x02 )
   {

    if(0 != rmCcmProcMmeCfg(&enbInfo->mmeCfg, &enbCfgRspPtr->statusInfo.cause))
    {
     { { if ((&(rmCb.init))->dbgMask & (2)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 341); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "rmCcmProcEnodebCfg: " "MME Configuration Failed"); SPrint((&(rmCb.init))->prntBuf); } }; };

    }
   }


   if(RRM_WR_CAUSE_NOT_APP != enbCfgRspPtr->statusInfo.cause)
   {
      enbCfgRspPtr->statusInfo.status = RRM_WR_CFG_FAILURE;
   }
   else
   {
      enbCfgRspPtr->statusInfo.status = RRM_WR_CFG_SUCCESS;
   }


   RmWrEnodebCfgResp(enbCfgRspPtr);

   return(0);
}
# 379 "../..//enbapp/lterrm/rm_ccm_enb.c"
 S16 rmCcmEnodebStaInd
(
 RrmWrEnbStaIndInfo *enbStaIndInfo
)
{
   U8 staType = enbStaIndInfo->staType;
   ...
   switch(staType)
   {
      case RRM_WR_ENBSTATYPE_CPULOAD:
      case RRM_WR_ENBSTATYPE_MMESTA:
         {
            if(0 != rmCacEnbStaUpd(enbStaIndInfo))
            {
               { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 397); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, " rmCcmEnodebStaInd :Failed at rmCacEnbStaUpd" "with staType:(%u)", staType); SPrint((&(rmCb.init))->prntBuf); } }; };
       ...
       default:
         {
            { { if ((&(rmCb.init))->dbgMask & (1)) { sprintf((&(rmCb.init))->prntBuf, "[%s 0x%x:%x] %s:%d ", (Txt *)"ENB RRM", (&(rmCb.init))->ent, (&(rmCb.init))->inst, "../..//enbapp/lterrm/rm_ccm_enb.c", 404); SPrint((&(rmCb.init))->prntBuf); sprintf (rmCb.init.prntBuf, "Unknown Enb Status : %d", staType); SPrint((&(rmCb.init))->prntBuf); } }; };
            return(1);
         }
   }
