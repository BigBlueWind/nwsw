...
#define SS_MT_TMR 1
# 1 "../..//enbapp/lterrm/rm_cac_util.c"
# 96 "../..//enbapp/lterrm/rm_cac_util.c"
...
# 97 "../..//enbapp/lterrm/rm_cac_util.c" 2
# 115 "../..//enbapp/lterrm/rm_cac_util.c"
 RmCacRrcConEnum rmFindRrcConReqCat
(
   U8 count
)
{
   RmCacRrcConEnum crntState;
   RmCacAcbParams *acbParamsPtr = 0L;

   ;


   acbParamsPtr = &(rmCb.enbCb.cacEnbCb.enbAcbInfo.rrcConAcbParams[0]);

   if (count < acbParamsPtr[RM_CAC_RRCCON_HIGH].startThres)
   {
      crntState = RM_CAC_RRCCON_NORMAL;
   }
   else if (count < acbParamsPtr[RM_CAC_RRCCON_VERYHIGH].startThres)
   {
      crntState = RM_CAC_RRCCON_HIGH;
   }
   else
   {
      crntState = RM_CAC_RRCCON_VERYHIGH;
   }

   return(crntState);
}
# 160 "../..//enbapp/lterrm/rm_cac_util.c"
 RmCacCpuLdEnum rmFindCpuLdCat
(
   U8 cpuLoad
)
{
   RmCacCpuLdEnum crntState;
   RmCacAcbParams *acbParamsPtr = 0L;

   ;

   acbParamsPtr = &(rmCb.enbCb.cacEnbCb.enbAcbInfo.cpuLdAcbParams[0]);

   if (cpuLoad < acbParamsPtr[RM_CAC_CPULD_MINOR].startThres)
   {
      crntState = RM_CAC_CPULD_NORMAL;
   }
   else if (cpuLoad < acbParamsPtr[RM_CAC_CPULD_MAJOR].startThres)
   {
      crntState = RM_CAC_CPULD_MINOR;
   }
   else if (cpuLoad < acbParamsPtr[RM_CAC_CPULD_CRITICAL].startThres)
   {
      crntState = RM_CAC_CPULD_MAJOR;
   }
   else
   {
      crntState = RM_CAC_CPULD_CRITICAL;
   }

   return(crntState);

}
# 211 "../..//enbapp/lterrm/rm_cac_util.c"
 Bool rmCacChkCpuldInActvableLvl
(
   RmCacCpuLdEnum cpuLd
)
{
   U8 idx ;
   Bool inLvl = 0;
   RmCacEnbAcbInfo *tmpEnbAcbInfo = 0L;

   tmpEnbAcbInfo = &(rmCb.enbCb.cacEnbCb.enbAcbInfo);

   for (idx = 0; idx < RM_CAC_MAX_CPULD_LVL; idx ++)
   {
      if (tmpEnbAcbInfo->cpuLdAcbParams[idx].brrFact < RRM_WR_ACBFACTOR_NA)
      {
         break;
      }
   }
   if (idx == (U8) RM_CAC_MAX_CPULD_LVL ||
      cpuLd < (RmCacCpuLdEnum) idx)
   {
      inLvl = 0;
   }
   else
...
# 260 "../..//enbapp/lterrm/rm_cac_util.c"
 RmCacCellAcbInfo* rmGetCellAcbInfoFrmLstNod
(
   CmLList *nodePtr,
   CmLteCellId *cellIdPtr
)
{
   RmCacCellAcbInfo *cellAcbInfoPtr = 0L;
   cellAcbInfoPtr = &(((RmCellCb *)(nodePtr -> node))->cacCellCb.cellAcbInfo);
   if (cellIdPtr != 0L)
   {
      *cellIdPtr = ((RmCellCb *)(nodePtr -> node))->cellId;
   }
   return(cellAcbInfoPtr);
}
