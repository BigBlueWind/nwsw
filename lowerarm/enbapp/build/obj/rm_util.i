...
# 97 "../..//enbapp/lterrm/rm_util.c" 2



 RmCb rmCb;
# 119 "../..//enbapp/lterrm/rm_util.c"
 RmMmeCb* rmUtlGetMmeCb
(
  U16 mmeId
)
{
   CmLListCp *mmeCbLst = 0L;
   CmLList *lnk = 0L;
   RmMmeCb *mmeCbPtr = 0L;

   mmeCbLst = &rmCb.enbCb.mmeCbLst;

   lnk = mmeCbLst->first;

   while(lnk != 0L)
   {
      mmeCbPtr = (RmMmeCb*)(lnk->node);

      if(mmeCbPtr->mmeId == mmeId)
      {
         return(mmeCbPtr);
      }
      lnk = lnk->next;
...
)
{
   ...
   while(lnk != 0L)
   {
      ueCbPtr = (RmUeCb*)(lnk->node);

      if(ueCbPtr->ueId == ueId)
      {
         return(ueCbPtr);
      }
      lnk = lnk->next;
...
# 215 "../..//enbapp/lterrm/rm_util.c"
 RmCellCb* rmUtlGetCellCb
(
   CmLteCellId cellId
)
{
   CmLList *nodePtr = 0L;
   RmCellCb *tmpCellCbPtr = 0L;
   ...
   while(nodePtr)
   {
      tmpCellCbPtr = (RmCellCb *)(nodePtr -> node);
      nodePtr = nodePtr -> next;
      if (tmpCellCbPtr->cellId == cellId)
      {
         return(tmpCellCbPtr);
      }
   }
