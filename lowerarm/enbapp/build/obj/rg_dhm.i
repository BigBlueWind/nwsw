...
# 2207 "../..//cm/ssi.h"
#define PID_STK(h) (h)
# 100 "../..//ltemac/rg_dhm.c" 2
# 1 "../..//cm/cm5.h" 1
# 101 "../..//ltemac/rg_dhm.c" 2
# 1 "../..//cm/cm_hash.h" 1
# 91 "../..//cm/cm_hash.h"
#define __CMHASHH__ 
...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 118 "../..//ltemac/rg_dhm.c" 2
# 1 "../..//cm/cm5.x" 1
# 119 "../..//ltemac/rg_dhm.c" 2
# 1 "../..//cm/cm_lib.x" 1
# 94 "../..//cm/cm_lib.x"
#define __CMLIBX__ 
...
static Void rgDHMBldTfuDatReq (RgDlSf *dlSf, RgDlHqProcCb *hqP, RgTfuDatReqPduInfo *datReq);


static Void rgDHMPrntDciInfo ( TfuDatReqPduInfo *datReq );
# 166 "../..//ltemac/rg_dhm.c"
 S16 rgDHMHqEntInit
(
RgDlHqEnt *hqE,
...

{
   ...
   if (rgAllocSBuf((Data **)&(hqE->procs),
            sizeof(RgDlHqProcCb)*maxHqProcs) != 0)
   {
      return(1);
   }
   ...
   { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 0))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 198); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "Harq Entity Initialized\n"); SPrint((&rgCb.rgInit)->prntBuf); } };
...

{
   ...
   { if (0L != (hqP->tbInfo[tbIndex-1].tb)) { SPutMsg((hqP->tbInfo[tbIndex-1].tb)); hqP->tbInfo[tbIndex-1].tb = 0L; } };
   ...
   { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 0))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 302); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "Harq Proc (%d) released.\n", hqP->procId); SPrint((&rgCb.rgInit)->prntBuf); } };
...

{
   ...
   { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 0))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 340); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "Fetching the harq proc for index(%d)\n", idx); SPrint((&rgCb.rgInit)->prntBuf); } };
         ...
         if (hqP->tbInfo[i].sf == dlSf)
         {
...

            {
                     ...
                     hqP->tbInfo[i].contResCe == 1)
               {
                  ...
                  if (0 != rgMUXBldPdu(&bldPdu, &(hqP->tbInfo[i].tb), err))
                  {
                     return(1);
                  }
...

{
   ...
   for(i=0;i<datReq->nmbOfTbs;i++)
   {
      ...
      bldPdu.contResId = 0L;
# 621 "../..//ltemac/rg_dhm.c"
      if(rgMUXBldPdu(&bldPdu, &(hqProc->tbInfo[i].tb), err) != 0)
      {
...

{
   ...
   if(rgMUXBldPdu(&bldPdu, &(hqProc->tbInfo[0].tb), err) != 0)
   {
...
# 750 "../..//ltemac/rg_dhm.c"
{
   ...
   for(idx=allocInfo->tbStrtIdx;((idx-allocInfo->tbStrtIdx) < allocInfo->nmbOfTBs); idx++)
    ...
    if (allocInfo->tbInfo[idx].schdDat[0].lcId == 0x00)
      {
         ...
         if ((ret = rgAllocSBuf ((Data**)&cStaInd, sizeof(RguCStaIndInfo))) != 0)
         {
   ...
   hqP->tbInfo[idx].schdTa.pres = allocInfo->tbInfo[idx].ta.pres;
         ...
         hqP->tbInfo[idx].timingInfo = timingInfo;
         hqP->tbInfo[idx].tbSz = allocInfo->tbInfo[idx].schdTbSz;
         if((0L == dStaInd) && (allocInfo->tbInfo[idx].numSchLch))
         {
            if ((ret = rgAllocSBuf ((Data**)&dStaInd, sizeof(RguDStaIndInfo))) != 0)
            {
               err->errType = (((0 + 11) + 1) + 2);
               err->errCause = 0 + 3 + 1;
               return(1);
...

{
   ...
   for(i=0;i<2;i++)
   {
      if ((hqP->tbInfo[i].sf == dlSf) && (hqP->tbInfo[i].tb != 0L))
      {
         ...
         SCpyMsgMsg(hqP->tbInfo[i].tb, (rgCb.rgInit.region),
               (rgCb.rgInit.pool), &datReq->mBuf[i]);
         {
            MsgLen dbgBufLen;
...
static Void rgDHMPrntDciInfo(TfuDatReqPduInfo *datReq)
{
   TfuPdschDciInfo dciInfo;
      ...
      TfuDciFormat2AAllocInfo *format2AllocInfo;


  dciInfo = datReq->dciInfo;
  if (datReq->rnti == 65535)
     {
            return;
     }
  switch (dciInfo.format)
   {

       case TFU_DCI_FORMAT_1A:
# 1055 "../..//ltemac/rg_dhm.c"
    break;

       case TFU_DCI_FORMAT_2A:
        format2AllocInfo = &dciInfo.u.format2AAllocInfo;
...
# 1184 "../..//ltemac/rg_dhm.c"
   ...
   if((cell = rgDBMGetCellCb(rlshqUeInfo->cellId)) == 0L)
   {
      { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 1194); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "RgSchMacRlsHqReq(): " "No cellCb found with cellId = (%d)\n", rlshqUeInfo->cellId); SPrint((&rgCb.rgInit)->prntBuf); } };
   ...
   for(idx1 = 0; idx1 < rlshqUeInfo->numUes; idx1++)
   {
      if((ue=rgDBMGetUeCb (cell, rlshqUeInfo->ueHqInfo[idx1].rnti)) == 0L)
      {
         ...
         {
            { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 1216); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "RgSchMacRlsHqReq(): " "No ueCb found with rnti = (%d)\n", rlshqUeInfo->ueHqInfo[idx1].rnti); SPrint((&rgCb.rgInit)->prntBuf); } };
...
# 1240 "../..//ltemac/rg_dhm.c"
      for(idx2=0; idx2 < rlshqUeInfo->ueHqInfo[idx1].numOfTBs; idx2++)
      {
         if(rgDHMRlsHqProcTB(hqP,
               rlshqUeInfo->ueHqInfo[idx1].tbId[idx2]) != 0)
         {
            { if ((&rgCb.rgInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgCb.rgInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC", (&rgCb.rgInit)->ent, (&rgCb.rgInit)->inst, "../..//ltemac/rg_dhm.c", 1247); SPrint((&rgCb.rgInit)->prntBuf); sprintf (rgCb.rgInit.prntBuf, "RgSchMacRlsHqReq():" "Failure in releasing hq TB for UE = (%d)\n", rlshqUeInfo->ueHqInfo[idx1].rnti); SPrint((&rgCb.rgInit)->prntBuf); } };
