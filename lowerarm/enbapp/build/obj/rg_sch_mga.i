...
#define MSPD_LOG(...) 
#define MSPD_ERR(...) 
# 115 "../..//ltemac/rg_sch_mga.c" 2
# 1 "../..//cm/cm5.x" 1
# 116 "../..//ltemac/rg_sch_mga.c" 2
# 1 "../..//cm/cm_hash.x" 1
# 92 "../..//cm/cm_hash.x"
#define __CMHASHX__ 
# 102 "../..//cm/cm_hash.x"
...

{
   ...
   switch (cfg->gapPrd)
   {
      ...
      case 80:
         ...
         break;
      default:
         { if ((&rgSchCb[cell->instIdx].rgSchInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC Scheduler", (&rgSchCb[cell->instIdx].rgSchInit)->ent, (&rgSchCb[cell->instIdx].rgSchInit)->inst, "../..//ltemac/rg_sch_mga.c", 410); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); sprintf (rgSchCb[cell->instIdx].rgSchInit.prntBuf, "rgSCHMeasGapANRepUeRecfg() Incorrect GAP Period\n"); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); } };

         return(1);
   }
   return(0);
}
# 432 "../..//ltemac/rg_sch_mga.c"
   ...
   for (hqIdx = 0; hqIdx < ue->dl.hqEnt->numHqPrcs; hqIdx++)
   {
      ...
      for (repIdx = 1; repIdx < ue->ackNakRepCb.cfgRepCnt; repIdx++)
      {
         for (tbCnt = 0; tbCnt < 2; tbCnt++)
         {
            ...
            {
               cmLListDelFrm(&((hqP->tbInfo[tbCnt].crntSubfrm[repIdx])-> ackNakRepQ), &hqP->tbInfo[tbCnt].anRepLnk[repIdx]);
...
# 709 "../..//ltemac/rg_sch_mga.c"
   ...
   rgSCHMeasGapANRepUtlHdlTti (cell, queue);
# 737 "../..//ltemac/rg_sch_mga.c"
   { if (cell->crntTime.sfn == 0) { if ((cell->crntTime.subframe - ((4 - 2))) < 0) { repTime.sfn = 1024 - 1; } else { repTime.sfn = cell->crntTime.sfn; } } else { if ((cell->crntTime.subframe - ((4 - 2))) < 0) { repTime.sfn = cell->crntTime.sfn - 1; } else repTime.sfn = cell->crntTime.sfn; } repTime.subframe = (10 + cell->crntTime.subframe - ((4 - 2))) % (10); };
...

{
   ...
   schedTime = cell->crntTime.sfn * 10 + cell->crntTime.subframe + (2 + 1);
# 943 "../..//ltemac/rg_sch_mga.c"
   harqFdbkOffset = 4;
...
# 997 "../..//ltemac/rg_sch_mga.c"
   if ((cell->crntTime.subframe + (((2 + 1) - 1))) >= 10) ackNakTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + (((2 + 1) - 1)))/10) % 1024; else ackNakTime.sfn = cell->crntTime.sfn; ackNakTime.subframe = (cell->crntTime.subframe + (((2 + 1) - 1))) % 10;;
...

{
   ...
   schedTime = cell->crntTime.sfn * 10 + cell->crntTime.subframe + 2;
# 1128 "../..//ltemac/rg_sch_mga.c"
   harqFdbkOffset = 4;
...

{
   ...
   if (!ue->dl.dlInactvMask)
   {
      ...
      if (0 != rgSCHUtlDlActvtUe (cell, ue))
      {
         { if ((&rgSchCb[cell->instIdx].rgSchInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC Scheduler", (&rgSchCb[cell->instIdx].rgSchInit)->ent, (&rgSchCb[cell->instIdx].rgSchInit)->inst, "../..//ltemac/rg_sch_mga.c", 1288); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); sprintf (rgSchCb[cell->instIdx].rgSchInit.prntBuf, "rgSCHMeasGapANRepDlInactvTmrExpry() Unable to add UE \n"); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); } };
...

{
   ...
   if (!ue->ul.ulInactvMask)
   {
      ...
      if (0 != rgSCHUtlUlActvtUe (cell, ue))
      {
         { if ((&rgSchCb[cell->instIdx].rgSchInit)->dbgMask & ((0x00000100 << 1))) { sprintf((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf, "[%s 0x%x:%x] %s:%d ", "MAC Scheduler", (&rgSchCb[cell->instIdx].rgSchInit)->ent, (&rgSchCb[cell->instIdx].rgSchInit)->inst, "../..//ltemac/rg_sch_mga.c", 1354); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); sprintf (rgSchCb[cell->instIdx].rgSchInit.prntBuf, "rgSCHMeasGapANRepUlInactvTmrExpry() Unable to add UE \n"); SPrint((&rgSchCb[cell->instIdx].rgSchInit)->prntBuf); } };
