...
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_CL_MAC_TTI_IND, RESOURCE_LARM, t, GetTIMETICK());
#endif
...
#ifdef MSPD_MLOG_NEW
   if(ttiInd->isDummyTti)
   MLogTask(PID_SCH_DUMMY_TTI_IND, RESOURCE_LARM, t, GetTIMETICK());
   else
...
#else
PUBLIC S16 YsUiCtfKdfCfm(pst, suId, transId, status)
Pst* pst;
SuId suId;
CtfCfgTransId transId;
U8 status;
