...
PRIVATE S16 ysMsLimProcessMsg ARGS((U8 msgType, U16 entityId, Ptr msg, Bool isFree, Bool isLstEnd));
...
#ifndef YS_MS_NO_TA
EXTERN Void ysMsUtlTmrProc ARGS ((YsCellCb    *cellCb));
#endif
...
{
    S16            ret = ROK;
    ret = ysReceivePHYMsg(isList, l1Msg);
    RETVALUE(ret);
...
*/
   ...
   if (!isList)
   {
      ...
      ret = ysMsLimProcessMsg(pMsgDesc->msgType, ysCb.cellId, pMsgDesc, FALSE, FALSE);
      if(ret != ROK)
      {
         YS_DBG_ERR((_ysp, "ysReceivePHYMsg(): Failed to process message\n"));
         MSPD_ERR("Failed to process message\n");
         RETVALUE(FAILURE);
      }
      RETVALUE(SUCCESS);
   }
   ...
   /* Process the incoming list - free all nodes/msgs except the first node (the incoming pointer) */
   {
      ...
      while (elem)
      {
         pMsgDesc = (PGENMSGDESC)elem->MessagePtr;
		 {
		    ...
		    syscoreinfo[MxGetCpuID()].mBuf	= (unsigned int)pMsgDesc;
		    syscoreinfo[MxGetCpuID()].event	= (unsigned int)pMsgDesc->msgType; /* This will tell the rcvd PHY msg  */
			syscoreinfo[MxGetCpuID()].dstEnt	= (unsigned int)0xd5;
			syscoreinfo[MxGetCpuID()].dstInst = (unsigned int)0;
	        syscoreinfo[MxGetCpuID()].srcEnt	= (unsigned int)0xff;/* ENT for PHY is 0xff */
		    syscoreinfo[MxGetCpuID()].timestamp = *(volatile U32 *) 0xFE050004;
		    syscoreinfo[MxGetCpuID()].res[0] = 0; /* 0 is for API msg */
		    syscoreinfo[MxGetCpuID()].res[1] = 1; /* 1 is for start of the task */

	     }
		 if (!(elem->Next) || ((PGENMSGDESC)(elem->Next->MessagePtr))->msgType == PHY_RXEND_IND)
		 {
		    isLstEnd = TRUE;
		 }
         ret = ysMsLimProcessMsg(pMsgDesc->msgType, ysCb.cellId, pMsgDesc, TRUE, isLstEnd);
         {
	        extern SysStayInfo syscoreinfo [4];
            syscoreinfo[MxGetCpuID()].res[1] = 255; /* 1 is for start of the task */
         }
         next = elem->Next;
         SvsrFreeIMem(elem);
         elem = next;
...
PRIVATE U32 ysMonitorCore2()
{
   extern U32 macGT;
   S32  core2diff = ysGT - macGT;
   if (core2diff < 0)
      core2diff += 10240;

   if (core2diff > 15)
   {
      MSPD_DBG("Warning: Core 2 hasn't processed TTI"
            " since %d ms, macGT %u\n", core2diff, macGT);
      rbCallstackShowForCore(2);
      stop_printf("PHY is stopped since core 2 is stuck for more than 15ms\n");
   }
   RETVALUE((U32)core2diff);
}
...
#else
PRIVATE S16 ysMsLimProcessMsg (msgType, entityId, msg, isfree, isLstEnd)
U8    msgType;
U16   entityId;
Ptr   msg;
Bool  isfree;
...
#endif
{
   ...
   switch(msgType)
   {
	 ...
	 if (0 != startTti)
	 {
	    ...
	    invaldl1Tti++;
         ...
         {
            MSPD_DBG("Marking as dummy TTI as it is picked late by (%f)ms."
                  " PrevSf cellTime(%d,%d)\n", driftDiff,
         ...
         noTti++;
	 /* Sending the missed CRC indication for the previus TTI if it */
	 /* was expected before proceeding with this TTI*/
	 if(ysMsSndMissedCrcInd(cellCb) != ROK)
	 {
        /*Review Comment: Correct Indentation*/
        YS_DBG_ERR((_ysp, "ysMsLimProcessMsg: Sending Missed CRC indication\
	       failed\n"));
	    /*API memory should be released in failure cases*/
        ret = RFAILED;
	    break;
	 }
         ...
         if (lsf != 30 && lsf != lastsf.subframe)
         {
            ...
            if (++errCnt == 20)
            {
               //stop_printf("Missed sending vectors for 20 times\n");
            }
         }
	 ...
	 rbTmrHdlr();
      ...
      case PHY_RXSDU_IND:
      {
         ...
         CmLteTimingInfo rxtime0;
         CmLteTimingInfo rxtime1;
         CmLteTimingInfo rxtime2;

#ifdef MSPD_MLOG_NEW
         t1 = GetTIMETICK();
#endif
         rxtime2.sfn = rxtime1.sfn = rxtime0.sfn = pRxSduInd->frameNum;
         rxtime2.subframe = rxtime1.subframe = rxtime0.subframe \
               = pRxSduInd->subFrameNum;
         YS_INCR_TIMING_INFO(rxtime1, 1);
         YS_INCR_TIMING_INFO(rxtime2, 2);
         if (!isFree)
         {
         ...
         {
            if ((ret = ysMsUlmPrcPucch(pRxSduInd, cellCb)) != ROK)
            {
               /*Review Comment: Correct Indentation*/
			...
			if (isLstEnd && pRxSduInd->listType == UL_LIST_TYPE_PUCCH)
			{
			   isUlCntrlLstEnd = TRUE;
			   ulCntrlTiming.sfn = rxtime0.sfn;
			   ulCntrlTiming.subframe = rxtime0.subframe;
			}
         }
#endif
         if (YS_TIMING_INFO_SAME(cellCb->timingInfo, rxtime2))
         {
         ...
         else if (YS_TIMING_INFO_SAME(cellCb->timingInfo, rxtime1)
                  || YS_TIMING_INFO_SAME(cellCb->timingInfo, rxtime0))
         {
            ulRxSduInds = &cellCb->ulRxSduInds[rxtime2.subframe];
            ulRxSduInds->sduIndCnt++;
            ulRxSduInds->pRxSduInd[(ulRxSduInds->sduIndCnt - 1)] = pRxSduInd;
            isFree = FALSE;
            if (YS_TIMING_INFO_SAME(cellCb->timingInfo, rxtime0))
            {
               if (pRxSduInd->status != TIMEOUT_RXSTART_REQ)
               {
                  MSPD_DBG("Warning: SDU time same as cell time, but status is"
                     " *not* TIMEOUT\n");
               }
            }
         }
         ...
         {
            MSPD_DBG("Warning, SDUIND (%u,%u) too late, stop at late rxend\n",
      ...
      case PHY_RXEND_IND:
      {
         ...
         t2 = GetTIMETICK();
#endif
         rxtime2.sfn = rxtime1.sfn = rxtime0.sfn = pRxEndInd->frameNumber;
         rxtime2.subframe = rxtime1.subframe = rxtime0.subframe \
               = pRxEndInd->subFrameNum;
         YS_INCR_TIMING_INFO(rxtime1, 1);
         YS_INCR_TIMING_INFO(rxtime2, 2);
         if (!isFree)
         {
         ...
         {
            MSPD_DBG("RXENDIND (%d,%d) rcvd late, stopping\n",
                  pRxEndInd->frameNumber,pRxEndInd->subFrameNum);
			#if 0
            stop_printf("RXENDIND rcvd late (%d,%d), cell(%u,%u)\n",
                  pRxEndInd->frameNumber,pRxEndInd->subFrameNum,
                  cellCb->timingInfo.sfn, cellCb->timingInfo.subframe);
			#endif
         }
      ...
      case PHY_STOP_CONF:
      {
         ...
         MSPD_DBG("PST: Received Phy Stop Confirm\n");
            /* fix for stop_rsp - value is 6 however we can accomodate only 5 */
         ret = ysMsCfgSm(cellCb, (YS_MS_EVENT_STOP_RSP ), pInitInd); /* PST-CellReset removed -1 (YS_MS_EVENT_STOP_RSP-1)  */
         break;
      }
...
#ifdef DLHQ_RTT_OPT
		 if (isLstEnd && pRxStatusInd->listType == UL_LIST_TYPE_PUCCH)
         {
            isUlCntrlLstEnd = TRUE;
			ulCntrlTiming.sfn = pRxStatusInd->frameNum;
			ulCntrlTiming.subframe = pRxStatusInd->subFrameNum;
	     }
#endif
      }
      break;
   ...
   if (isUlCntrlLstEnd)
   {
      ysMsUlmPrcUlCntrl(&ulCntrlTiming,cellCb);
   }
#endif
...
//            uart_printf("    pPwrCtl->paprThreshold=%i\n", pPwrCtl->paprThreshold);
      ...
      case PHY_TXSDU_REQ:
         ...
         for (i = 0; i < pTxSduReq->msgLen;)
         {
            ...
            /* Commenting because isprint is showing warning as "implicit declaration", 
             * And if convert this to mapping function osIsprint, this requires cm_os.c to 
             * be compiled, but this file has some header file inclusion, which is not supported
             * by lower arm compiler */
#if 0
            for (j = 0; j < 16; j++)
            {
               if (isprint(pDat[j]))
