...
/*MSPD_DBG("*****rbType = %d, rbId = %d\n",pSecInfo->rbType, pSecInfo->rbId);*/
	...
	/*MSPD_DBG("Actual User Plane Ciphering key:");
		...
		MSPD_DBG("******flatBuf:\n");	   
		for(index=0;index<pPt->ddt.len;index++)
		MSPD_DBG("%x ", flatBuf[index]);
		MSPD_DBG("\n");                

		MSPD_DBG("******pPt->ddt.ptr:\n");	   
		for(index=0;index<pPt->ddt.len;index++)
		MSPD_DBG("%x ", pPt->ddt.ptr[index]);
		MSPD_DBG("\n");                      
#endif
...
#if 0 
  MSPD_DBG("Actual Integrity key:");
	       for(index=0;index<16;index++)
                   MSPD_DBG("%x ", pSecInfo->integ_key[index]);
		   MSPD_DBG("\n");
#endif
	       ...
	       MSPD_DBG("0x%x ", pVec->hmac_key[index]);
	       MSPD_DBG("\n");
#endif
...
#if 0
for(index = 0; index < YS_MS_KENB_LEN; index++)
MSPD_DBG(" %x",pSecInfo->hash_key[index]);
MSPD_DBG("\n ");
#endif
...
MSPD_DBG("\n ***********************hmac_key**************************** \n");
for(index = 0; index < YS_MS_KENB_LEN; index++)
MSPD_DBG(" %x",pVec->hmac_key[index]);
MSPD_DBG("\n ");
#endif
...
#if 0
	 MSPD_DBG("************pSecInfo->strS*****\n");
	 for(index=0; index<pSecInfo->strSlen; index++)
	 MSPD_DBG("%x ",pSecInfo->strS[index]);
	 MSPD_DBG("\n");
...
#if 0
/* sidhartha */
MSPD_DBG("\n After Key is derived Key is: \n"); 
for(idx =0; idx < currDDT->ddt.len; idx++)
MSPD_DBG(" %x",currDDT->ddt.ptr[idx]);
MSPD_DBG("\n");
#endif
...
#if 0
for(idx =0; idx < YS_MS_KDF_OP_LEN; idx++)
