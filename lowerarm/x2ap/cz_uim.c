...
#endif
{
   ...
   if(((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr))
                !=ROK) ||( czCbPtr == NULLP))
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF, "Bind Request Failed, can not derive\
...
#endif
{
   ...
   if(((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr))
                !=ROK)||( czCbPtr == NULLP))
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF, "Unbind Request Failed, can not derive\
               czCb with DstEnt (%d), dstInst (%d), dstProcId (%d) \n",
               pst->dstEnt, pst->dstInst, pst->dstProcId));
      CZ_RETVALUE(RFAILED);
   }
...
{  
   ...
   if(((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr))
                !=ROK)||( czCbPtr == NULLP))
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF, "Failed, can not derive\
               czCb with DstEnt (%d), dstInst (%d), dstProcId (%d) \n",
               pst->dstEnt, pst->dstInst, pst->dstProcId));
      if(gpReq)
      {
...
#endif
{
   ...
   if(((SGetXxCb(pst->dstProcId,pst->dstEnt,pst->dstInst,(Void **)&czCbPtr))
                !=ROK))/*x2ap kwork fix*/
   {
      CZ_DBG_FATAL((CZ_PRNT_BUF, "Failed can not"
               "derive czCb with DstEnt (%d), dstInst (%d), dstProcId (%d) \n",
               pst->dstEnt, pst->dstInst, pst->dstProcId));
      if(peerAssocRsp)
      {
