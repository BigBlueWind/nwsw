...
#endif /* ANSI */
{
   ...
   /*x2ap build warning fix---we are never using variable "alarmSta"*/
   /*CzUsta      alarmSta;*/
   ...
   /*CZ_SET_ZERO(&alarmSta, sizeof(CzUsta));*/
   ...
   {
      /* print DEBUG_ERROR */
      CZ_DBG_ERROR((CZ_PRNT_BUF, "Failed, can not derive \
               czCbPtr with DstEnt (%d), dstInst (%d), dstProcId (%d) ", 
               pst->dstEnt, pst->dstInst, pst->dstProcId));
      CZ_RETVALUE(RFAILED);
   }
   ...
   else
   {
      ...
      CzLiSctEndpOpenReq(&sctSapCb->pst, sctSapCb->spId, CZ_LCL_SU_ENDPID,
                         ...
                         sctSapCb->endpCb.srcAddrLst.nAddr);
...
#endif /* ANSI */
{
   ...
   {
      CZ_DBG_ERROR((CZ_PRNT_BUF, "Failed, can not derive \
               czCb: with DstEnt (%d), dstInst (%d), dstProcId (%d) \n",
               pst->dstEnt, pst->dstInst, pst->dstProcId));
      CZ_RETVALUE(RFAILED);
   }
#endif /* end of SS_MULTIPLE_PROCS */
...
#endif /* ANSI */
{
   ...
   /*x2ap build warning fixes--here variable "sta" is never used*/
   /*CztStaInd   sta;*/
