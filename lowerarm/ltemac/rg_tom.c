...
#endif
{
 ...
 /* Moving node declaration to limited scope for optimization */
   ...
   if (dlSf->bcch.tb != NULLP)
   {
      ...
      /* rg002.201: ccpu00108810: Setting the pointer to NULL post 
       * transmission */
      dlSf->bcch.tb = NULLP;
   }
   /* Fill PCCH data */
   ...
   if (dlSf->tbs.count != 0)
   {
      ...
      while (dlSf->tbs.first)
      {
         ...
         if ((ret = rgDHMSndDatReq (dlSf, datInfo, hqCb, err)) != ROK)
         {
            RGDBGERR ((rgPBuf,"rgTOMUtlFillDatReqPdus() DHM unable \
                     to fill DATA request for cellId (%d)\n", cellCb->cellId));
            MSPD_ERR ("rgTOMUtlFillDatReqPdus() DHM unable \
                     to fill DATA request for cellId (%d)\n", cellCb->cellId);
...
#endif
{
   ...
   {
      RGDBGERR ((rgPBuf,"rgTOMUtlProcDlSf() Unable to Allocate \
                 TfuDatReqInfo for cellId=%d \n", cellCb->cellId));
      MSPD_ERR ("rgTOMUtlProcDlSf() Unable to Allocate \
                 TfuDatReqInfo for cellId=%d \n", cellCb->cellId);
      RETVALUE(ret);
   }
   else
   {
      ...
      {
         RGDBGERR ((rgPBuf,"rgTOMUtlProcDlSf() Unable to send \
                            data for cellId (%d)\n", cellCb->cellId));
         MSPD_ERR ("rgTOMUtlProcDlSf() Unable to send \
                            data for cellId (%d)\n", cellCb->cellId);
      ...
      if((datInfo->pdus.count) || (datInfo->bchDat.pres == TRUE))
      {
         ...
         {
            RGDBGERR ((rgPBuf,"rgTOMUtlProcDlSf() Unable to send data info \
                     for cellId (%d)\n", cellCb->cellId));
            MSPD_ERR ("rgTOMUtlProcDlSf() Unable to send data info \
                     for cellId (%d)\n", cellCb->cellId);
         }
      }
      else
...
/* ADD Changes for Downlink UE Timing Optimization */
   ...
   if (cellCb == NULLP)
   {
      RGDBGERR ((rgPBuf,"rgTOMTtiInd() Unable to get the cellCb for cellId (%d))\n", 
               ttiInd->cellId));
      MSPD_ERR ("rgTOMTtiInd() Unable to get the cellCb for cellId (%d))\n", 
               ttiInd->cellId);
      err.errType = RGERR_TOM_TTIIND;
...
#ifdef MSPD
   macGT = ttiInd->timingInfo.sfn * 10 + ttiInd->timingInfo.subframe;
#endif
#ifdef LTE_L2_MEAS
   rgL2Meas(cellCb);
...
/* ADD Changes for Downlink UE Timing Optimization */
   ...
   {
	  MSPD_DBG("CRFIX: Data Req not sent for [%d:%d]"
	           "sending it in [%d:%d] remDatReqCnt [%d]\n",
			   prevTmInfo.sfn,prevTmInfo.subframe,ttiInd->timingInfo.sfn,ttiInd->timingInfo.subframe,prevDlSf->remDatReqCnt);  
      if ((ret = rgTOMUtlProcDlSf (prevDlSf, cellCb, &err)) != ROK)
      {
         RGDBGERR ((rgPBuf,"rgTOMTtiInd() Unable to process previous "
                  "downlink subframe for cellId (%d))\n", ttiInd->cellId));
         MSPD_ERR ("rgTOMTtiInd() Unable to process previous "
                  "downlink subframe for cellId (%d))\n", ttiInd->cellId);
         err.errType = RGERR_TOM_TTIIND;
      }
...
#ifdef MSPD
            MSPD_DBG("Dummy Data req at BCH time (%u,%u) at celltime (%u,%u)\n",
...
/* ADD Changes for Downlink UE Timing Optimization */
   ...
   {
      RGDBGERR ((rgPBuf,"rgTOMTtiInd() Unable to process downlink subframe\
      for cellId (%d))\n", ttiInd->cellId));
      MSPD_ERR ("rgTOMTtiInd() Unable to process downlink subframe\
      for cellId (%d))\n", ttiInd->cellId);
      err.errType = RGERR_TOM_TTIIND;
   }
...
#endif
{
   ...
   if (cmAllocEvnt (sizeof (RgMacPdu), RG_BLKSZ, &evntMem, (Ptr*)pdu) != ROK)
   {
      ...
      RGDBGERR ((rgPBuf,"rgTOMUtlAllocPduEvnt() Allocation of DUX event failed\n"));
      MSPD_ERR ("rgTOMUtlAllocPduEvnt() Allocation of DUX event failed\n");
      RETVALUE(RFAILED);
   }
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_TOMUTL_CMALLCEVT, RESOURCE_LARM, t, t2=GetTIMETICK());
#endif
...
#endif
{
   ...
   if (cmAllocEvnt (sizeof (RgInfSfDatInd), RG_BLKSZ, &evntMem, (Ptr*)sfInfo) != ROK)
   {
      ...
      rgLMMStaInd(LCM_CATEGORY_RESOURCE, LCM_EVENT_DMEM_ALLOC_FAIL,
                                       LCM_CAUSE_MEM_ALLOC_FAIL, &dgn);
      RGDBGERR ((rgPBuf,"rgTOMInfAllocPduEvnt() Allocation failed\n"));
      MSPD_ERR ("rgTOMInfAllocPduEvnt() Allocation failed\n");
      RETVALUE(RFAILED);
   }
   

#ifdef MSPD_MLOG_NEW
   MLogTask(PID_TOMINF_CMALLCEVT, RESOURCE_LARM, t, t2=GetTIMETICK());
#endif
...
#else
PRIVATE S16 rgTomUtlPrepareL2MUlThrpInfo(ttiInd)
   RgCellCb *cellCb;
   RgUeCb *ueCb;
   RgRguDedDatInd    *dDatInd;
#endif
{
   ...
   for(loop=0;loop<dDatInd->numLch;loop++)
   {
      ...
      if(ueCb->ul.lcCb[lcId].measOn)
      {
         lcgId = ueCb->ul.lcCb[lcId].lcgId;
         if(ueCb->ul.lcgArr[lcgId].lcgBsInfo.outStndngBs > 0)
         {
            if(ueCb->ul.lcgArr[lcgId].lcgBsInfo.firstDatSegRcvd == FALSE)
            {
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.firstDatSegRcvd = TRUE;
               /*Send First Seg Indication for the corresponding LC*/
               dDatInd->lchData[loop].timeInfo.firstLastSegInd
                  = RGU_L2M_ULTHRP_FIRST;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.segInd = RGU_L2M_ULTHRP_FIRST;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.frstSegTimInfo =
                  cellCb->crntTime;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.fSegTtiCycNum = cellCb->ttiCycle;
            }
            else
            {
               /*Implies an Intermediate segment*/
               dDatInd->lchData[loop].timeInfo.firstLastSegInd
                  = RGU_L2M_ULTHRP_INTERIM;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.segInd = RGU_L2M_ULTHRP_INTERIM;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.nextSegTimInfo =
                  cellCb->crntTime;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.nSegTtiCycNum = cellCb->ttiCycle;
            }
         }
         else
         {
            if(ueCb->ul.lcgArr[lcgId].lcgBsInfo.firstDatSegRcvd == TRUE)
            {
               /* Send the Last Segment Indication */
               if(ueCb->ul.lcgArr[lcgId].lcgBsInfo.segInd  ==
                     RGU_L2M_ULTHRP_INTERIM)
               {
                  cycDiff = ueCb->ul.lcgArr[lcgId].lcgBsInfo.nSegTtiCycNum -
                     ueCb->ul.lcgArr[lcgId].lcgBsInfo.fSegTtiCycNum;
                  if(cycDiff > 0) 
                  {
                     idx3 = ((ueCb->ul.lcgArr[lcgId].lcgBsInfo.nextSegTimInfo.sfn
                     *RG_NUM_SUB_FRAMES)+
                           ueCb->ul.lcgArr[lcgId].lcgBsInfo.nextSegTimInfo.subframe);
                     idx4 = ((ueCb->ul.lcgArr[lcgId].lcgBsInfo.frstSegTimInfo.sfn
                     *RG_NUM_SUB_FRAMES)+
                           ueCb->ul.lcgArr[lcgId].lcgBsInfo.frstSegTimInfo.subframe);
                     dDatInd->lchData[loop].timeInfo.thrpTimeUl = 
                        (cycDiff*(RG_MAX_SFN*RG_NUM_SUB_FRAMES)+idx3)-idx4;
                  }
                  else
                  {
                     dDatInd->lchData[loop].timeInfo.thrpTimeUl = 
                        RG_CALC_SF_DIFF(ueCb->ul.lcgArr[lcgId].lcgBsInfo.nextSegTimInfo, 
                              ueCb->ul.lcgArr[lcgId].lcgBsInfo.frstSegTimInfo);
                  }
               }
               else if (ueCb->ul.lcgArr[lcgId].lcgBsInfo.segInd ==
                     RGU_L2M_ULTHRP_FIRST)
               {
                  dDatInd->lchData[loop].timeInfo.thrpTimeUl = 0; 
               }
               dDatInd->lchData[loop].timeInfo.firstLastSegInd = 
                  RGU_L2M_ULTHRP_LAST;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.segInd = RGU_L2M_ULTHRP_LAST;
               ueCb->ul.lcgArr[lcgId].lcgBsInfo.firstDatSegRcvd = FALSE;
            }
            else
            {
               dDatInd->lchData[loop].timeInfo.firstLastSegInd =
                  RGU_L2M_ULTHRP_NOTDEF;
            }
            ueCb->ul.lcgArr[lcgId].lcgBsInfo.outStndngBs = 0;
            ueCb->ul.lcgArr[lcgId].lcgBsInfo.fSegTtiCycNum= 0;
            ueCb->ul.lcgArr[lcgId].lcgBsInfo.nSegTtiCycNum= 0;
         }
      }
      else
      {
...
#else
PRIVATE S16 rgTOMUtlProcMsg(cellCb, ueCb, pdu, isSpsRnti)
   RgCellCb      *cellCb; 
   RgUeCb        *ueCb;
   RgMacPdu      *pdu;
   Bool          isSpsRnti;
...
#else
PRIVATE S16 rgTOMUtlProcMsg(cellCb, ueCb, pdu)
   RgCellCb      *cellCb; 
   RgUeCb        *ueCb;
   RgMacPdu      *pdu;
#endif 
#endif
{
   ...
   CmLList           *node;
   RgMacSdu          *sdu;
   /* rg004.201 fixed contention resolution id. */
   MsgLen            ccchSz;
   MsgLen            cpySz;
#ifdef LTEMAC_SPS
   Pst               schPst1;  
...
#endif
#endif
  /*rg002.301:ccpu00120383-MOD-moved outside of LTE_L2_MEAS
   ...
   */
   ...
   if(pdu->sduLst.first)
   {
      ...
      if ((sdu->lcId == RG_CCCH_LCID))
      {
         ...
         if ((ret = rgAllocSBuf ((Data**)&cDatInd, sizeof(RgRguCmnDatInd))) != ROK)
         {
            RETVALUE(RFAILED);
         }
	 ...
	 /* MS_FIX : syed Contention resolution ID copy should consider only
...
#ifdef MSPD
	 MSPD_DBG("CCCH SDU of size(%d) received for UE(%d)\n", ccchSz, ueCb->ueId);
#endif
         sdu->mBuf = NULLP;
         rgUIMSndCmnDatInd(cDatInd);
         RETVALUE(ROK);
      } /* end of common channel processing */
      if ((ret = rgAllocSBuf ((Data**)&dDatInd, sizeof(RgRguDedDatInd))) != ROK)
      {
         RETVALUE(RFAILED);
      }
...
#ifdef LTE_L2_MEAS
   ulSf = &cellCb->ulSf[cellCb->crntTime.subframe];
   ...
   while (node)
   {
      ...
      /*rg002.301:ccpu00120383-ADD-check if the lcId is valid 
       *          and configured */
      ulLcCb = rgDBMGetUlDedLcCb (ueCb, sdu->lcId);
      
      if(ulLcCb == NULLP)
      {
         RGDBGERR ((rgPBuf,"rgTOMUtlProcMsg() Unconfigured LCID (%d)\n", 
                  sdu->lcId));
         MSPD_ERR ("rgTOMUtlProcMsg() Unconfigured LCID (%d)\n", 
                  sdu->lcId);
         node = node->next;
         continue;
      }

      /* rg006.201:[ccpu00110797] Removed check for not configured logical 
        * channel */
      ...
      {
				MSPD_DBG1("PWAV: ( rgTOMUtlProcMsg ) In the WHILE for processing SDUs. \n");
         if ((ret = rgUIMSndDedDatInd(dDatInd)) != ROK)
         {
            RGDBGERR ((rgPBuf,"rgTOMUtlProcMsg() Failed to send datIndication to RGU \n"));
            MSPD_ERR ("rgTOMUtlProcMsg() Failed to send datIndication to RGU \n");
            RETVALUE(ret);
         }
         if ((ret = rgAllocSBuf ((Data**)&dDatInd, sizeof(RgRguDedDatInd))) != ROK)
         {
...
/*Added for explicit release - start*/
   ...
   /*Added for explicit release - end */
	 if (dDatInd){
		 MSPD_DBG1("PWAV: ( rgTOMUtlProcMsg ) SENDING MAC SDUs TO RLC, numLch=%d\n", dDatInd->numLch);
	 }
...
#endif
#endif
			MSPD_DBG1("PWAV: ( rgTOMUtlProcMsg ) SENDING MAC SDUs TO RLC\n");
   ...
   {
      /*rg002.301-ADD-[ccpu00120383]Free the memory allocated for dDatInd if we 
...
#else
PRIVATE S16 rgTOMUtlInsSchInfo (pdu, sfInfo, ceInfo, rnti)
RgMacPdu *pdu;
RgInfSfDatInd *sfInfo;
RgInfCeInfo *ceInfo;
CmLteRnti   rnti;
...
#endif
{
   ...
   if (cellCb == NULLP)
   {
      RGDBGERR ((rgPBuf,"rgTOMDatInd() Unable to get the cellCb for cellId (%d))\n", 
               datInd->cellId));
      MSPD_ERR ("rgTOMDatInd() Unable to get the cellCb for cellId (%d))\n", 
               datInd->cellId);
      RETVALUE(RFAILED);
   }
   /* rg005.201 Avoiding memset as all the fields are getting initialized 
      further */

   if (rgTOMInfAllocPduEvnt (&sfInfo) != ROK)
   {
      err.errType = RGERR_TOM_DATIND;
      RGDBGERR ((rgPBuf,"rgTOMDatInd() Unable to Allocate PDU for DUX cellId (%d))\n", 
               datInd->cellId));
      MSPD_ERR ("rgTOMDatInd() Unable to Allocate PDU for DUX cellId (%d))\n", 
               datInd->cellId);
   ...
   for (;node; node=node->next)
   {
      ...
      {
         /* MS_FIX: sriky memory corruption precautions */
         RG_FREE_MSG(datInfo->mBuf);	      
         rgTOMUtlFreePduEvnt (pdu, TRUE);
         err.errType = RGERR_TOM_DATIND;
         RGDBGERR ((rgPBuf,"rgTOMDatInd() DUX processing failed. \n"));
         MSPD_ERR ("rgTOMDatInd() DUX processing failed. \n");
      ...
      else
      {
         ...
         if (ueCb == NULLP)
         {
...
#endif 
            {
               ...
               rgTOMUtlFreePduEvnt (pdu, TRUE);
               err.errType = RGERR_TOM_DATIND;
               RGDBGERR ((rgPBuf,"rgTOMDatInd() Unable to get the UE CB for RNTI (%d) \n", 
                        datInfo->rnti));
               MSPD_ERR ("rgTOMDatInd() Unable to get the UE CB for RNTI (%d) \n", 
                        datInfo->rnti);
               tfuSap->sapSts.numPduDrop++;
               continue;
            }
...
#ifdef LTEMAC_SPS
         if ((ret = rgTOMUtlProcMsg(cellCb, ueCb, pdu, isSpsRnti)) != ROK)
#else
         if ((ret = rgTOMUtlProcMsg (cellCb, ueCb, pdu)) != ROK)
#endif /* LTEMAC_SPS */
         {
            rgTOMUtlFreePduEvnt (pdu, TRUE);
            err.errType = RGERR_TOM_DATIND;
            RGDBGERR ((rgPBuf,"rgTOMDatInd() Unable to handle Data Indication\n"));
            MSPD_ERR ("rgTOMDatInd() Unable to handle Data Indication\n");
            tfuSap->sapSts.numPduDrop++;
            continue;
         }
      }

      if(rgTOMUtlInsSchInfo(pdu, sfInfo, &ceInfo, datInfo->rnti) != ROK)
      {
         rgTOMInfFreePduEvnt (sfInfo);
...
#ifdef LTE_L2_MEAS
   if(cellCb->ulSf[cellCb->crntTime.subframe].ueUlAllocInfo != NULLP)
   {
      /*ccpu00117052 - MOD - Passing double for proper NULLP
                             assignment */
      rgFreeSBuf((Data **)&(cellCb->ulSf[cellCb->crntTime.subframe].ueUlAllocInfo), 
      ((cellCb->ulSf[cellCb->crntTime.subframe].numUe) * sizeof(RgUeUlAlloc)));
   }
#endif
...
/*rg002.201: Changed the fucntions description */
/**
...
#endif
{
   ...
   if(cmnLcInfo->bitMask & RGINF_BCH_INFO)
   {
      ...
      if (rgAllocSBuf ((Data**)&staInd, sizeof(RguCStaIndInfo)) != ROK)
      {
         err->errCause = RGERR_TOM_MEM_EXHAUST;
         RETVALUE(RFAILED);
      }
...
#endif
{
   ...
   {
      if((ue=rgDBMGetUeCb (cell, ueInfo->allocInfo[idx].rnti)) == NULLP)
      {
#ifdef MSPD
         MSPD_ERR("Failed to get UE(%d) from cell->ueLst\n", ueInfo->allocInfo[idx].rnti);
#endif
         /* Check in RachLst */
         if((ue=rgDBMGetUeCbFromRachLst (cell, 
                     ueInfo->allocInfo[idx].rnti)) == NULLP)
         {
            ...
            MSPD_ERR("rgHndlSchdUe(): " 
                     "No ueCb found with rnti = (%d)\n", 
                        ueInfo->allocInfo[idx].rnti);
#ifdef MSPD
            MSPD_ERR("Failed to get UE(%d) from cell->rachLst\n", ueInfo->allocInfo[idx].rnti);
#endif
            /*MS_FIX: If one UE is not present dont return, look for the next.*/
            continue;
         }
      }
      ...
      {
         RGDBGERR((rgPBuf, "rgHndlSchdUe(): " 
                  "Failure in sending StaInd for UE = (%d)\n",
                     ueInfo->allocInfo[idx].rnti));
         /*RETVALUE(RFAILED); */
#ifdef PWAV // Fix for CPU0 dataabort when msglen 0 comes from MAC
	 continue;
#endif
      }
   } /* end of ues loop */
...
#endif
{
   ...
   ulSf->totalUlBwAvail = ueInfo->totalUlAvailBw;
...
#endif
{
   ...
   {
      RGDBGERR((rgPBuf, "rgHndlSchdUe(): SUBFRAME Not pushed to the PHY"));
#ifdef MSPD
      MSPD_ERR("ERROR: subframe not pushed to cl, releasing TBs\n");
#endif
...
#endif
{
   ...
   {
      RGDBGERR((rgPBuf, "RgSchMacSfAllocReq(): " 
               "No cellCb found with cellId = (%d)\n", sfInfo->cellId));
      MSPD_ERR("RgSchMacSfAllocReq(): " 
               "No cellCb found with cellId = (%d)\n", sfInfo->cellId);
      RETVALUE(RFAILED);
   }
...
/*ccpu00127620*/
   ...
   /*rg002.301 ccpu00119012-Add-Added the handling for pushing down
   ...
   if(!(dlSf->txDone) && (0 == dlSf->remDatReqCnt) && (dlSf->statIndDone))
   {
       ...
       * it right away.*/
      if ((ret = rgTOMUtlProcDlSf (dlSf, cell, &err)) != ROK)
      {
         RGDBGERR ((rgPBuf,"RgSchMacSfAllocReq() Unable to process "
                "downlink subframe for cellId (%d))\n", cell->cellId));
         MSPD_ERR ("RgSchMacSfAllocReq() Unable to process "
                "downlink subframe for cellId (%d))\n", cell->cellId);
...
#endif
#ifdef MSPD_MLOG_NEW
 MLogTask(PID_MAC_SF_ALLOC_REQ, RESOURCE_LARM, t, MacGetTick());
#endif
   RETVALUE(ROK);
...
#else
PRIVATE S16 rgTOMProcCrntiCEInDatInd( pdu, prevUeCb, cellCb, datInfo, ceInfo)
RgMacPdu          *pdu;
RgUeCb            *prevUeCb;
RgCellCb          *cellCb;
TfuDatInfo        *datInfo;
...
#endif
{
   ...
   {
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       RGDBGERR ((rgPBuf,"\nrgTOMProcCrntiCEInDatInd() Received MSG3 with CRNTI, \
                   unable to find ueCb for rnti(%d)\n", 
                     ceInfo->ces.cRnti));
       MSPD_ERR ("\nrgTOMProcCrntiCEInDatInd() Received MSG3 with CRNTI, \
                   unable to find ueCb for rnti(%d)\n", 
                     ceInfo->ces.cRnti);
   ...
   MSPD_DBG("CRNTI CE(%d) received through tmpCrnti(%d)\n",  ceInfo->ces.cRnti, datInfo->rnti);
   rgDBMDelUeCbFromRachLst(cellCb, ueCb);
   rgRAMFreeUeCb(ueCb);
   ueCb = prevUeCb;
#ifdef LTEMAC_SPS
   if ((rgTOMUtlProcMsg(cellCb, ueCb, pdu, FALSE)) != ROK)
#else
   if ((rgTOMUtlProcMsg (cellCb, ueCb, pdu)) != ROK)
#endif /* LTEMAC_SPS */
   {
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       RGDBGERR ((rgPBuf,"\nrgTOMProcCrntiCEInDatInd() Processing for MSG3 \
                   failed for rnti(%d)\n", 
                     datInfo->rnti));
       MSPD_ERR ("\nrgTOMProcCrntiCEInDatInd() Processing for MSG3 \
                   failed for rnti(%d)\n", 
...
#else
PRIVATE S16 rgTOMProcCCCHSduInDatInd( pdu, prevUeCb, cellCb, datInfo, ceInfo)
RgMacPdu          *pdu;
RgUeCb            *prevUeCb;
RgCellCb          *cellCb;
TfuDatInfo        *datInfo;
...
#endif
{
   ...
   {
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       RGDBGERR ((rgPBuf,"\n rgTOMProcCCCHSduInDatInd() Received MSG3 \
                   with CRNTI and also CCCH (%d)\n", 
                     ceInfo->ces.cRnti));
       MSPD_ERR ("\n rgTOMProcCCCHSduInDatInd() Received MSG3 \
                   with CRNTI and also CCCH (%d)\n", 
                     ceInfo->ces.cRnti);
   ...
   {
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       RGDBGERR ((rgPBuf,"\n rgTOMProcCCCHSduInDatInd() Processing for MSG3 \
                   failed for rnti(%d)\n", 
                     datInfo->rnti));
       MSPD_ERR ("\n rgTOMProcCCCHSduInDatInd() Processing for MSG3 \
                   failed for rnti(%d)\n", 
   ...
   /* MS_FIX: syed Drop any duplicate Msg3(CCCH Sdu) */
   if (ueCb->dl.hqEnt.numHqProcs)
   {
      /* HqE is already initialized by a previuos Msg3 */ 
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       MSPD_DBG("Duplicate MSG3 received. Dropping\n");
       RGDBGERR ((rgPBuf,"\n rgTOMProcCCCHSduInDatInd() Processing for MSG3 \
                   failed for rnti(%d)\n", 
                     datInfo->rnti));
       MSPD_ERR ("\n rgTOMProcCCCHSduInDatInd() Processing for MSG3 \
   ...
   {
       rgTOMUtlFreePduEvnt (pdu, TRUE);
       RGDBGERR ((rgPBuf,"\n rgTOMProcCCCHSduInDatInd() Harq Initialization \
                   failed for rnti(%d)\n", 
                     datInfo->rnti));
       MSPD_ERR ("\n rgTOMProcCCCHSduInDatInd() Harq Initialization \
                   failed for rnti(%d)\n", 
...
#ifdef MSPD
   MSPD_DBG("CCCH SDU received through tmpCrnti(%d)\n",  datInfo->rnti);
#endif
#ifdef LTEMAC_SPS
   if ((rgTOMUtlProcMsg(cellCb, ueCb, pdu, FALSE)) != ROK)
#else
...
#endif
{
   ...
   if(ceInfo->bitMask & RG_TRUNC_BSR_CE_PRSNT)
   {
      ...
      ueCb->ul.lcgArr[lcgId].lcgBsInfo.outStndngBs += rgBsrTbl[bsr];
   }
   ...
   {
      ueCb->ul.lcgArr[0].lcgBsInfo.outStndngBs += rgBsrTbl[ceInfo->ces.bsr.longBsr.bs1];
      ueCb->ul.lcgArr[1].lcgBsInfo.outStndngBs += rgBsrTbl[ceInfo->ces.bsr.longBsr.bs2];
      ueCb->ul.lcgArr[2].lcgBsInfo.outStndngBs += rgBsrTbl[ceInfo->ces.bsr.longBsr.bs3];
      ueCb->ul.lcgArr[3].lcgBsInfo.outStndngBs += rgBsrTbl[ceInfo->ces.bsr.longBsr.bs4];
   }
   RETVALUE(ROK);
