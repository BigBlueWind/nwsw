...
#endif
{
         ...
         || rgDBMGetInactvCellCb(cellCfg->cellId) != NULLP)
   {
      ...
      MSPD_ERR("Cell already exists %d \n",cellCfg->cellId);
      RETVALUE(RFAILED);
   }
            ...
            || cellCfg->bwCfg.ulTotalBw > RG_MAX_UL_BW))
   {
      ...
      MSPD_ERR("Invalid Bandwidth configuration: ul %d dl %d",
            cellCfg->bwCfg.ulTotalBw, cellCfg->bwCfg.dlTotalBw);
      RETVALUE(RFAILED);
   }
   if (cellCfg->rachCfg.maxMsg3Tx < RG_MIN_HQ_TX)
   {
      ...
      MSPD_ERR("Invalid RACH configuration: maxMsg3Tx %d",
            cellCfg->rachCfg.maxMsg3Tx);
      RETVALUE(RFAILED);
   }
...
#endif
{
       ...
       (ueCfg->txMode.tm == CRG_UE_TM_5))
   {
      ...
      MSPD_ERR("Transmission Mode=%d not supported\n", ueCfg->txMode.tm);
      RETVALUE(RFAILED);
   }
#endif
   ...
   if ((*cell = rgDBMGetCellCb(ueCfg->cellId)) == NULLP)
   {
      ...
      MSPD_ERR("Active Cell does not exist %d\n", ueCfg->cellId);
      RETVALUE(RFAILED);
   }
   ...
   if (ueCfg->ueUlHqCfg.maxUlHqTx < RG_MIN_HQ_TX)
   {
      ...
      MSPD_ERR("Invalid Uplink HARQ config for UE %d\n",
            ueCfg->ueUlHqCfg.maxUlHqTx);
      RETVALUE(RFAILED);
   }
   errInfo->errCause = RGERR_NONE;
...
#endif
{
   ...
   if (lcCfg->lcType == CM_LTE_LCH_DTCH || lcCfg->lcType == CM_LTE_LCH_DCCH)
   {
      ...
      if ((rgCFGVldtCrgDedLcCfg(lcCfg, cell, ue, errInfo)) != ROK)
      {
         ...
         MSPD_ERR("Validation for dedicated LC failed\n");
         RETVALUE(RFAILED);
      }
   }
         ...
         || lcCfg->lcType == CM_LTE_LCH_CCCH)
   {
      if ((rgCFGVldtCrgCmnLcCfg(lcCfg, cell, errInfo)) != ROK)
      {
         ...
         MSPD_ERR("Validation for common logical channels failed\n");
         RETVALUE(RFAILED);
      }
   }
   else
...
#endif
{
   ...
   if ((*ue = rgDBMGetUeCb(*cell, lcRecfg->crnti)) == NULLP)
   {
      RGDBGERR((rgPBuf, "Ue does not exist for dedicated logical channel %d\n",
               lcRecfg->crnti));
      MSPD_ERR("Ue does not exist for dedicated logical channel %d\n",
               lcRecfg->crnti);
      RETVALUE(RFAILED);
   }
   ...
   if ((*ulLc = rgDBMGetUlDedLcCb((*ue), lcRecfg->lcId)) == NULLP)
   {
      RGDBGERR((rgPBuf, "Dedicated UL LC does not exist %d\n",
               lcRecfg->lcId));
      MSPD_ERR("Dedicated UL LC does not exist %d\n",
               lcRecfg->lcId);
      RETVALUE(RFAILED);
   }
   ...
   if (lcRecfg->ulRecfg.lcgId > (RG_MAX_LCG_PER_UE - 1))
   {
      ...
      MSPD_ERR("Invalid lcgId for uplink logical channel: lcg %d\
               lc %d\n", lcRecfg->ulRecfg.lcgId, lcRecfg->lcId);
...
#endif
{
   ...
   if (ret != ROK)
   {
      RGDBGERR((rgPBuf, "DBM initialization for cell %d failed\n",
               cellCfg->cellId));
      MSPD_ERR("DBM initialization for cell %d failed\n",
               cellCfg->cellId);
...
/* Start: LTEMAC_2.1_DEV_CFG */
   if ((ue = rgDBMGetUeCbFromRachLst(cell, ueCfg->crnti)) == NULLP)
   {
      ...
      if((ue=rgRAMCreateUeCb(cell, ueCfg->crnti, FALSE, errInfo)) == NULLP)
      {
         ...
         MSPD_ERR("UeCb creation failed for UE %d\n", ueCfg->crnti);
         RETVALUE(RFAILED);
      }
	  ...
	  MSPD_ERR("UeCb Harq Entity Initialization failed for HO UE %d \n", ueCfg->crnti);
         RETVALUE(RFAILED);
      }
   ...
   if (lcCfg->lcType == CM_LTE_LCH_DTCH || lcCfg->lcType == CM_LTE_LCH_DCCH)
   {
      if ((rgCFGCrgDedLcCfg(cell, ue, lcCfg, errInfo)) != ROK)
      {
         RGDBGERR((rgPBuf, "Dedicated logical channel configuration\
                  failed %d\n", lcCfg->lcId));
         MSPD_ERR( "Dedicated logical channel configuration\
                  failed %d\n", lcCfg->lcId);
         RETVALUE(RFAILED);
      }
   }
   else
   {
      if ((rgCFGCrgCmnLcCfg(cell, lcCfg, errInfo)) != ROK)
      {
         RGDBGERR((rgPBuf, "Common logical channel configuration failed %d\n",
               lcCfg->lcId));
         MSPD_ERR("Common logical channel configuration failed %d\n",
               lcCfg->lcId);
         RETVALUE(RFAILED);
      }
   }
...
#endif
{
   ...
   if (ueRecfg->oldCrnti != ueRecfg->newCrnti)
   {
      ...
      {
#ifdef MSPD
          MSPD_ERR("rgDBMGetUeCbFromRachLst Failed\n");
#endif
          RGDBGERR((rgPBuf, "rgDBMGetUeCbFromRachLst Failed. OldUeId %d"
                            " newUeId %d, cell %d\n",
                    ueRecfg->oldCrnti, ueRecfg->newCrnti, ueRecfg->cellId));
          MSPD_ERR("rgDBMGetUeCbFromRachLst Failed. OldUeId %d"
                            " newUeId %d, cell %d\n",
                    ueRecfg->oldCrnti, ueRecfg->newCrnti, ueRecfg->cellId);
          RETVALUE(RFAILED);
      }
   ...
   /*RE EST MAC FIX End */
      ...
      rgDBMInsUeCb(cell, ue);
      /* MS_FIX: syed Removing stale UE entry from RachLst */
      rgDBMDelUeCbFromRachLst(cell, newCrntiUeCb);
      rgRAMFreeUeCb(newCrntiUeCb);/*MS_FIX:vrr*/
   }
...
#endif
{
   RgCellCb      *cell;
   RgUeCb        *ue;
   /*MS_FIX:start: Send UE delete indication to scheduler*/
   Pst           schPst;
   RgInfUeDelInd ueDelInd;
   /*MS_FIX:end Send UE delete indication to scheduler*/
   ...
   /*MS_FIX: start: Inform UE delete to scheduler*/
   ueDelInd.cellSapId  = cell->schInstMap.cellSapId;
   ueDelInd.cellId  = ueDelInfo->u.ueDel.cellId;
   ueDelInd.rnti    = ueDelInfo->u.ueDel.crnti; 
   rgGetPstToInst(&schPst, cell->schInstMap.schInst);
   RgMacSchUeDel(&schPst, &ueDelInd);
   /*MS_FIX: end: Inform UE delete to scheduler*/
   RETVALUE(ROK);
}  /* rgCFGCrgUeDel */
...
#endif
{
   ...
   if ((ue = rgDBMGetUeCb(cell, lcDelInfo->u.lchDel.crnti)) == NULLP)
   {
      ...
      MSPD_ERR("Ue %d does not exist for dedicated logical channel\n",
            lcDelInfo->u.lchDel.crnti);
      RETVALUE(RFAILED);
   }
   ...
   if (lcDelInfo->u.lchDel.dir & CRG_DIR_TX)
   {
            ...
            == NULLP)
      {
         ...
         MSPD_ERR("DL LC %d does not exist for UE %d\n",
               lcDelInfo->u.lchDel.lcId, lcDelInfo->u.lchDel.crnti);
         RETVALUE(RFAILED);
      }
   ...
   if (lcDelInfo->u.lchDel.dir & CRG_DIR_RX)
   {
            ...
            == NULLP)
      {
         RGDBGERR((rgPBuf, "UL LC %d does not exist for UE %d\n",
               lcDelInfo->u.lchDel.lcId, lcDelInfo->u.lchDel.crnti));
         MSPD_ERR("UL LC %d does not exist for UE %d\n",
               lcDelInfo->u.lchDel.lcId, lcDelInfo->u.lchDel.crnti);
         RETVALUE(RFAILED);
      }
   ...
   if (!dirVld)
   {
      RGDBGERR((rgPBuf, "Invalid direction %d for LC Delete\n",
            lcDelInfo->u.lchDel.dir));
     MSPD_ERR("Invalid direction %d for LC Delete\n",
            lcDelInfo->u.lchDel.dir);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if ((*cell = rgDBMGetCellCb(lcCfg->cellId)) == NULLP)
   {
      RGDBGERR((rgPBuf, "Active Cell does not exist: Cell %d\n",
               lcCfg->cellId));
      MSPD_ERR("Active Cell does not exist: Cell %d\n",
               lcCfg->cellId);
      RETVALUE(RFAILED);
   }
   ...
   if ((*ue = rgDBMGetUeCb(*cell, lcCfg->crnti)) == NULLP)
   {
      RGDBGERR((rgPBuf, "Ue %d does not exist for dedicated logical\
       ...
       MSPD_ERR("Ue %d does not exist for dedicated logical\
               channel %d\n", lcCfg->crnti, lcCfg->lcId);
      RETVALUE(RFAILED);
   }
   ...
   if (lcCfg->dir & CRG_DIR_TX)
   {
      if (rgDBMGetDlDedLcCb((*ue), lcCfg->lcId) != NULLP)
      {
         ...
         MSPD_ERR("Dedicated DL LC %d already configured for UE %d\n",
                  lcCfg->lcId, lcCfg->crnti);
         RETVALUE(RFAILED);
      }
      dirVld = TRUE;
   ...
   if (lcCfg->dir & CRG_DIR_RX)
   {
      if (lcCfg->ulInfo.lcgId > (RG_MAX_LCG_PER_UE - 1))
      {
         RGDBGERR((rgPBuf, "Invalid lcgId for uplink logical channel %d\n",
                  lcCfg->ulInfo.lcgId));
         MSPD_ERR("Invalid lcgId for uplink logical channel %d\n",
                  lcCfg->ulInfo.lcgId);
         RETVALUE(RFAILED);
      }
      if (rgDBMGetUlDedLcCb((*ue), lcCfg->lcId) != NULLP)
      {
         RGDBGERR((rgPBuf, "Dedicated UL LC %d already configured for UE %d\n",
                  lcCfg->lcId, lcCfg->crnti));
         MSPD_ERR("Dedicated UL LC %d already configured for UE %d\n",
                  lcCfg->lcId, lcCfg->crnti);
         RETVALUE(RFAILED);
      }
      dirVld = TRUE;
...
#endif
{
   ...
   if ((*cell = rgDBMGetCellCb(lcCfg->cellId)) != NULLP)
   {
      RGDBGERR((rgPBuf, "Active Cell %d exists for common channels\n",
            lcCfg->cellId));
      MSPD_ERR("Active Cell %d exists for common channels\n",
            lcCfg->cellId);
      RETVALUE(RFAILED);
   }
   ...
   if ((*cell = rgDBMGetInactvCellCb(lcCfg->cellId)) == NULLP)
   {
      RGDBGERR((rgPBuf, "Inactive Cell %d does not exist for common channels\n",
            lcCfg->cellId));
      MSPD_ERR("Inactive Cell %d does not exist for common channels\n",
            lcCfg->cellId);
      RETVALUE(RFAILED);
   }
   ...
   if (lcCfg->dir & CRG_DIR_TX)
   {
      ...
      /* rg001.201: Removed the CRG Transport channel types */
         if (lcCfg->dlInfo.dlTrchType == CM_LTE_TRCH_DL_SCH)
         {
            if (rgDBMGetBcchOnDlsch(*cell,lcCfg->lcId) != NULLP)
            {
               RGDBGERR((rgPBuf, "BCCH on DLSCH already configured for\
                        cell %d\n", (*cell)->cellId));
               MSPD_ERR("BCCH on DLSCH already configured for\
                        cell %d\n", (*cell)->cellId);
         ...
         else if (lcCfg->dlInfo.dlTrchType == CM_LTE_TRCH_BCH)
         {
            if (rgDBMGetBcchOnBch(*cell) != NULLP)
            {
               RGDBGERR((rgPBuf, "BCCH on BCH already configured for cell %d\n",
                     (*cell)->cellId));
               MSPD_ERR("BCCH on BCH already configured for cell %d\n",
                     (*cell)->cellId);
               RETVALUE(RFAILED);
            }
         }
	     ...
	     MSPD_ERR("Invalid transport channel %d for cell %d\n",
                  lcCfg->dlInfo.dlTrchType, (*cell)->cellId);
            RETVALUE(RFAILED);
         }
      }
      ...
      else if (lcCfg->lcType == CM_LTE_LCH_PCCH)
      {
         if (rgDBMGetPcch(*cell) != NULLP)
         {
            RGDBGERR((rgPBuf, "PCCH already configured for cell %d\n",
                     (*cell)->cellId));
            MSPD_ERR("PCCH already configured for cell %d\n",
                     (*cell)->cellId);
            RETVALUE(RFAILED);
         }
      }
      else if (RG_DLCCCH_ISCFGD(*cell))
      {
         RGDBGERR((rgPBuf, "DL CCCH already configured for cell %d\n",
               (*cell)->cellId));
         MSPD_ERR("DL CCCH already configured for cell %d\n",
               (*cell)->cellId);
         RETVALUE(RFAILED);
      }
      dirVld = TRUE;
   ...
   if (lcCfg->dir & CRG_DIR_RX)
   {
      ...
      if (lcCfg->lcType != CM_LTE_LCH_CCCH)
      {
         ...
         MSPD_ERR("Invalid UL common lcType %d for cell %d\n",
                  lcCfg->lcType, (*cell)->cellId);
         RETVALUE(RFAILED);
      }
      if (RG_ULCCCH_ISCFGD(*cell))
      {
         RGDBGERR((rgPBuf, "UL CCCH already configured for cell %d\n",
               (*cell)->cellId));
         MSPD_ERR("UL CCCH already configured for cell %d\n",
               (*cell)->cellId);
         RETVALUE(RFAILED);
      }
      dirVld = TRUE;
