...
#endif /* ANSI */
{
   ...
   RGSCHDBGPRM(cellCb->instIdx,(rgSchPBuf(cellCb->instIdx),
               "\n rgSCHHdFddUeCfg(): CellID=%d  UeId =%d hdFddEnbl=%d",
               cellCb->cellId, ueCb->ueId, hdFddEnbl));
...
#endif /* ANSI */
{
 TRC3(rgSCHHdFddUeDel)
    ...
    RGSCHDBGPRM(cellCb->instIdx,(rgSchPBuf(cellCb->instIdx),
             "\n rgSCHHdFddUeDel(): CellID=%d  UeId =%d hdFdd=%x",
             cellCb->cellId, ueCb->ueId, ueCb->hdFddEnbld));
...
#endif /* ANSI */
{
   ...
   *allow = FALSE;
   ...
   RGSCHADDTOCRNTTIME(cellCb->crntTime, timeInfo, ulOffset);
   ...
   *allow = FALSE;
   ...
   RGSCHADDTOCRNTTIME(cellCb->crntTime, timeInfo, 
...
#endif /* ANSI */
{
   ...
   *allow = FALSE;
   ...
   RGSCHADDTOCRNTTIME(timeInfo, tempTimeInfo, RG_SCH_CMN_HARQ_INTERVAL)
   /* Also get subframe pointer to fetch Common Ch allocation */
   ...
   RGSCHADDTOCRNTTIME(timeInfo, tempTimeInfo, (RG_SCH_CMN_HARQ_INTERVAL-1))
   /* Also get subframe pointer to fetch Common Ch allocation */
   sf = rgSCHUtlSubFrmGet(cellCb, tempTimeInfo);
   if (RG_SCH_HDFDD_ISCMN_SCHED(sf))
   {
