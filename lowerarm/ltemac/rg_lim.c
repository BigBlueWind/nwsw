...
#endif
{
   ...
   if((ret = RgLiTfuBndReq (&pst, suId, spId)) != ROK)
   {
      ...
      MSPD_ERR("rgLIMTfuBndReq() Call to RgLiTfuBndReq() failed\n");
   }
   RETVALUE(ret);
}  /* rgLIMTfuBndReq */
...
#endif
{
   ...
   if((ret = RgLiTfuUbndReq (&pst, tfuSap->sapCfg.spId, reason)) != ROK)
   {
      ...
      MSPD_ERR("rgLIMTfuUbndReq() Call to RgLiTfuUbndReq() failed\n");
   }
   RETVALUE(ret);
...
#endif
{
   ...
   if (suId != tfuSap->sapCfg.suId)
   {
      ...
      MSPD_ERR("Incorrect SuId. Configured (%d) Recieved (%d)\n",
            tfuSap->sapCfg.suId, suId);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if (tfuSap->sapSta.sapState != LRG_BND)
   {
      RGDBGERR((rgPBuf,"Lower SAP not enabled SuId (%d)\n",
            tfuSap->sapCfg.suId));
      MSPD_ERR("Lower SAP not enabled SuId (%d)\n",
            tfuSap->sapCfg.suId);
      RETVALUE(RFAILED);
   }
   RETVALUE(ROK);
...
#endif
{
   ...
   if ((ret = rgLIMValidateSap (suId)) != ROK)
   {
      ...
      MSPD_ERR("RgLiTfuDatInd() SAP Validation failed.\n");
...
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_MAC_TFU_DATIND, RESOURCE_LARM, t, GetTIMETICK());
#endif
   RETVALUE(ret);
}  /* RgLiTfuDatInd*/
...
#endif
{
   ...
   if (tfuSap->sapSta.sapState != LRG_BND)
   {
      ...
      MSPD_ERR("rgLIMTfuDatReq() Lower SAP not bound (%d) \n",
            tfuSap->sapSta.sapState);
#ifdef RG_UNUSED
                            ...
                            datReq)) != ROK)
   {
      ...
      MSPD_ERR("rgLIMTfuDatReq() Call to RgLiTfuDatReq() failed\n");
   }
   RETVALUE(ret);
}  /* rgLIMTfuDatReq*/
...
/* Combine compilation fixes */
   ...
   if ((ret = rgLIMValidateSap (suId)) != ROK)
   {
      ...
      MSPD_ERR("RgLiTfuTtiInd() SAP Validation failed.\n");
