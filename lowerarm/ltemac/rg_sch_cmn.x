...
{
   U8              newScaledCqi;/*!< Latest assumed value of CQI for
                                      transmission */
...
typedef struct rgSchCmnDlUeSpsInfo
{
  ...
  /*rg002.301 ccpu00119494-ADD-added handling to retrnasmit RelPDCCH in case no
   ...
   Bool           isSpsActv;     /*!< Indicates if UE's DL SPS is
                                      active currently */
...
typedef struct rgSchCmnDlCellSpsInfo
{
  ...
  CmLListCp       toBeRelUes;    /*!< List of DL SPS enabled UEs with release
                                      pending */
 /*rg002.301 ccpu00119494-ADD-added handling when no feedback is received
...
typedef struct rgSchCmnUlUeSpsInfo
{
   ...
   RgSchCmnSpsUlUeSchdInfo ulSpsSchdInfo; /*!< Scheduled info for UL SPS 
                                               active UE */
#ifdef LTE_TDD
   U8                      nMod2;         /*!< N%2 value for nxt sps occasion */
#endif
} RgSchCmnUlUeSpsInfo;
...
typedef struct rgSchCmnSpsUlSf
{
   ...
   U8             numSpsAllocs;    /*!< Number of SPS allocations */
...
/* rg007.201 - Changes for MIMO feature addition */
/* rg008.201 - Removed dependency on MIMO compile-time flag */
   Void                *cqiToTbsTbl[RGSCH_MAX_NUM_LYR_PERCW]; /*!< CQI to TBS translation */
   /* cqi to Tbs tables for each 1 and 2 layer TbSz table */
   Void                *cqiToEffTbl[RGSCH_MAX_NUM_LYR_PERCW]; /*!< CQI to efficiency translation */
   U16                 noResPerRb;   /*!< Number of REs per RB */
...
#endif
   /* rg009.201: ccpu00115595- Member to store no. of Bits per RB */
...
typedef struct rgSchCmnUlCell
{
   ...
   /*rg002.301 ccpu00118423 ADD added new variable maxUlBwPerUe */
   ...
   U8               puschBw;         /*!< PUSCH BW in RBs */
   U8               numSb;           /*!< PUSCH BW in subbands */
   U8               sbStartRb;       /*!< Starting RB for first configured subband */
   U8               sbStartIdx;      /*!< Starting subband to be used for alloc */
   ...
   U8               hqFdbkIdx;       /*!< Subframe to send HARQ feedback (DL) for */
   U8               reTxIdx;         /*!< Retransmission Index */
#ifdef LTEMAC_SPS
   ...
   /*rg003.201:Added support for non-adaptive retransmission in uplink */
   U8               maxAllocPerUlSf; /*!< Max Allocations in a given SF */
#ifdef RGR_V1
/*rg003.301 [ccpu00121085]-ADD-added a param to limit msg3 allocations */
...
/* rg009.201. Added changes of TFU_UPGRADE */
   ...
   U32               nonLcg0Bs;       /*!< Total BSR as reported by UE (in bytes) */
   ...
   U32               cfgdUeBR;        /*!< Byte rate of UE per Refresh Cycle */
   U32               effUeBR;         /*!< Remaining Bytes of Byte rate available in a refresh Cycle */
...
typedef struct rgSchCmnUeRbAlloc
{
   RgSchDlSf     *dedDlSf;      /*!< DL sub-frame for which dedicated
                                     ...
                                     by RR/MAX C/I/PFS */ 
   CmLListCp     txUeLst;       /*!< List of UEs to be scheduled for Tx per 
                                     TTI: RgSchUeCb list  */
   CmLListCp     retxUeLst;     /*!< List of UEs scheduled for ReTx per 
                                     TTI: RgSchUeCb list */
#ifdef LTEMAC_SPS
   CmLListCp     retxSpsUeLst;   /*!< List of SPS UEs scheduled for ReTx per 
                                     TTI: RgSchUeCb list */
   CmLListCp     txSpsUeLst;     /*!< List of SPS UEs scheduled for Tx per 
                                     TTI: RgSchUeCb list */
#endif
   CmLListCp     schdTxUeLst;     /*!< List of transmitting UEs scheduled per TTI */
   CmLListCp     schdRetxUeLst;   /*!< List of re-transmitting UEs scheduled per TTI */
   CmLListCp     nonSchdTxUeLst;  /*!< List of transmitting UEs not scheduled in the TTI */
   CmLListCp     nonSchdRetxUeLst;/*!< List of re-transmitting UEs not scheduled in the TTI */
/* rg007.201 - Changes for MIMO feature addition */
/* rg008.201 - Removed dependency on MIMO compile-time flag */
   /* MIMO Tx+Retx hqProc scheduling handling */
   CmLListCp     txRetxUeLst;   /*!< List of UEs scheduled for tx and retx per 
                                     TTI(MIMO case): RgSchUeCb list */
   CmLListCp     schdTxRetxUeLst; /*!< List of TX&RETXing(MIMO case) UEs scheduled per TTI */
   CmLListCp     nonSchdTxRetxUeLst; /*!< List of TX&RETXing(MIMO case) UEs not scheduled in the TTI */
#ifdef LTEMAC_SPS
   CmLListCp     schdRetxSpsUeLst;   /*!< List of re-transmitting SPS UEs scheduled per TTI */
   CmLListCp     nonSchdRetxSpsUeLst;/*!< List of re-transmitting SPS UEs 
                                          not scheduled in the TTI */
   CmLListCp     schdTxSpsUeLst;     /*!< List of transmitting SPS UEs scheduled per TTI */
   CmLListCp     nonSchdTxSpsUeLst; /*!< List of transmitting SPS UEs not scheduled per TTI */
#endif
} RgSchCmnUeRbAlloc;
...
{
   RgSchCmnDlUeCwInfo   cwInfo[2];/*!< Codeword related feddback Information */ 
...
/*rg003.301[ccpu00119901]-ADD- Added new variable for Ul eff */
...
EXTERN Void rgSCHCmnDlProcDtx ARGS((
RgSchCellCb        *cell,
RgSchDlHqProcCb    *hqP
));
...
/* rg001.201: Removed declaration for function "rgSCHCmnDlSfFinalize" */
/* rg004.201: ccpu00110457: isDtx added for DTX Change */
/* rg007.201 - Changes for MIMO feature addition */
/* rg008.201 - Removed dependency on MIMO compile-time flag */
...
/* rg001.201:106531: fixing incorrect Imcs derivation */
/* rg001.301 -ADD - [ccpu00117270] adding ueCtg to argument list */
...
/* rg004.201 - ccpu00110457: proc is added for DTX support */
...
EXTERN Void rgSCHCmnSpsDlProcDtx ARGS((
RgSchCellCb           *cell,
RgSchDlHqProcCb       *hqP
));
