...
#endif
{
   ...
   /* MS_FIX: syed It is better to compute offset dynamically
...
#endif
{
   ...
   for(idx = 0; idx < RGSCH_MAX_RA_RNTI; idx++)
   {
...
#endif
{
   ...
   MSPD_DBG("<RNTI> STARTRNTI(%d) MAXRNTI(%d)\n", rntiStart, maxRntis);	   
   ...
   {
      /*rg009.201 Added the changes for changing the data types from U8 to      U16 */
      U16 idx;
      rntiPool[0].rnti = rnti++;
      rntiPool[0].prv = NULLP;
...
#endif
{
   ...
   {
      MSPD_DBG("<RNTI> RNTI exhausted count(%d)\n", cellCb->rntiDb.count);	   
      RETVALUE(NULLP);
   }
   ...
   cellCb->rntiDb.count--;
   MSPD_DBG("<RNTI> RNTI(%d) assigned count(%d)\n", rntiLnk->rnti, cellCb->rntiDb.count);	   
...
#endif
{
   ...
   cellCb->rntiDb.count++;
   MSPD_DBG("<RNTI> RNTI(%d) released count(%d)\n", rntiLnk->rnti, cellCb->rntiDb.count);	   
