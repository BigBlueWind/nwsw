...
#ifdef LTEMAC_SPS
/**
 * @brief This function returns the Y value of UE for SPS-C-RNTI
 *         for a sub frame
 *
 * @details
 *
 *     Function: rgSCHUtlGetYsps
 *     Purpose:  This function updates the value of Y stored in the
 *               UE control block for the SPS-C-RNTI. It uses the previously
 *               computed value for computing for this subframe.
 *     
 *     Invoked by: Scheduler
 *     
 *  @param[in]  RgSchUeCb*   ue
 ...
 **/
#ifdef ANSI
PUBLIC U32 rgSCHUtlGetYsps
(
RgSchUeCb         *ue,
U8                subFrm
)
#else
PUBLIC U32 rgSCHUtlGetYsps(ue, subFrm)
RgSchUeCb         *ue;
U8                subFrm;
#endif
{
   TRC2(rgSCHUtlGetYsps);
#if (ERRCLASS & ERRCLS_DEBUG)
   /* Must be covered under error flags */
   if (subFrm > 9)
   {
...
#endif
{
   ...
   if (!alloc->forMsg3)
   {
      ulUe = RG_SCH_CMN_GET_UL_UE(alloc->ue);
      if (alloc->grnt.isRtx)
      {
	 ulUe->schedRetxOccns++;
      }
      ...
      ulUe->prbAlloc += alloc->grnt.numRb;
   }
#endif
...
/* rg009.201. Added changes of TFU_UPGRADE */
   ...
   MSPD_DBG("Configuring ueCatEnum=%d\n", cfg->ueCatEnum);
...
/* rg009.201. Added changes of TFU_UPGRADE */
      ...
      MSPD_DBG("Reconfiguring ueCatEnum=%d\n", ueRecfg->ueCatEnum);
      /* MS_FIX: ccpu00124012 */
...
#endif
{
...
#endif
   {
                      ...
                      (sizeof(RgInfUeHqInfo)*RGSCH_MAX_UE_PER_DL_SF))) != ROK)
      {
         ...
         MSPD_ERR("Memory allocation FAILED for UE Alloc \n");
         RETVALUE(RFAILED);
      }
   }
...
#endif
{
...
#endif
   {
                      ...
                      (sizeof(RgInfRaRntiInfo)*RGSCH_MAX_RARNTI_PER_DL_SF))) != ROK)
      {
         ...
         MSPD_ERR("Memory allocation FAILED for RARNTI");
         RETVALUE(RFAILED);
      }
      for(indx = 0; indx < RGSCH_MAX_RARNTI_PER_DL_SF; indx++)
      {
                   ...
                   (sizeof(RgInfCrntiInfo)* (cellUl->maxMsg3PerUlSf)))) != ROK)
         {
            ...
            MSPD_ERR("Memory allocation FAILED for RNTI");
            RETVALUE(RFAILED);
         }
      }
...
#endif
{
   ...
   *pData = NULLP;
                      ...
                      pData, size) != ROK)
   {
     ...
     MSPD_ERR("Unable to Allocate the Buffer \n");
     RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgSCHUtlFreeSBuf failed");
      RETVOID;
   }
...
#endif
{
   ...
   /*rg009.201 - CR timer implementation changes*/
      ...
      MSPD_ERR("Invalid RNTI to fetch raCb");
      RETVALUE(RFAILED);
#endif
   }
   ...
   /* MSG4 MS_FIX Start */
      /*MS_FIX: If RaCb exists, then MSG4 is not completed yet*/
      ...
      {
         /*MS_FIX:Guard timer has expired */
 	  ...
 	  *Should be added to the list after MSG4 completion*/
         ...
         {
            /* Handle Ue fetch failure */
            RGSCHLOGERROR(inst, ERRCLS_INT_PAR,ERG017,(ErrVal)boRpt->u.rnti,
            "rgSCHUtlUpdtBo(): Invalid ue Id");
            MSPD_ERR("Invalid UE ID \n ");
            RETVALUE(RFAILED);
         }
...
#endif
{
   ...
   if (dlLc == NULLP)
   {
      ...
      MSPD_ERR("No Logical Channel. dlLc is NULLP \n");
      RETVALUE(RFAILED);
   }
   if (boUpdt->lcId != dlLc->lcId)
   {
      if (rgSCHUtlGetAllwdCchTbSz(boUpdt->bo*8, &nPrb, &mcs) != (boUpdt->bo*8))
      {
         RGSCHLOGERROR(inst,ERRCLS_INT_PAR,ERG020,(ErrVal)boUpdt->bo,
               ...
               any valid TB Size.");
        MSPD_ERR("bo = %d does not match any valid TB Size.\n ", boUpdt->bo);
        RETVALUE(RFAILED);
      }
   }
       ...
       (!boRpt))
   {
      ...
      MSPD_ERR("Allocation of common BO report failed \n");
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if (raCb == NULLP)
   {
      RGSCHDBGERR (inst, (rgSchPBuf(inst),"\n rgSCHUtlProcMsg3() Received MSG3, unable to find raCb for rnti(%d)\n",
               rnti));
      MSPD_ERR ("Received MSG3, unable to find raCb for rnti(%d)\n", rnti);
      RETVALUE(RFAILED);
   }
   ...
   if (ret != ROK)
   {
      RGSCHDBGERR (inst, (rgSchPBuf(inst),"rgSCHUtlProcMsg3() Processing failed in the RAM cellId (%d) \n",
               cellCb->cellId));
      MSPD_ERR ("Processing failed in the RAM cellId (%d) \n",
               cellCb->cellId);
      RETVALUE(ret);
   }
      ...
      for ( idx = 0; idx < RGINF_MAX_TB_PER_UE; idx++ )
      {
	     ...
	     * accessed to fetch the harq Entity */
            ...
            else  /* Fetch raCb from rnti */	     
            {
               ...
               {
                  RGSCHDBGERR(inst, (rgSchPBuf(inst),"No RaCb exists for UE%d\n ", 
                           rnti));
                  MSPD_ERR("DRXUE. No RaCb exists for UE %d\n", rnti);

                  continue;
               }
               dlHq = raCb->dlHqE->msg4Proc;     

               MSPD_DBG("DRXUE. Fetch dlHqE from raCb. UE [%d], procId = %d\n", 
			       ueCb->ueId, dlHq->procId);
            } 		    
	       ...
	       /* This condition should never occur */ 	    
               ...
               {
                  MSPD_ERR("DRXUE: UE RNTI [%d], RETX IDX [%d] is out of bound," 
                        "dlInactvMask = %d, procId %d\n", 
                        ueCb->ueId, drxHq->reTxIndx, ueCb->dl.dlInactvMask, dlHq->procId);		  
               }
...
#endif
{
   ...
   if(rgSCHUtlRgrStaInd(cell, staInfo) != ROK)
   {
      ...
      MSPD_ERR ("Could not send CQI reports\n");
      RETVALUE(RFAILED);
   } else {
		 /*MSPD_DBG1("PWAV: Sending of CQI report to RRM successful, crnti=%d, numreports=%d\n",ue->ueId, numCqiRept);
	 */
   }
...
#endif
{
   ...
   {
      RGSCHDBGERR (cell->instIdx,(rgSchPBuf(cell->instIdx),\
            "Could not send UE Sta reports\n"));
      MSPD_ERR ("Could not send UeSta  reports\n");
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   RGSCHDBGPRM(cell->instIdx,
         (rgSchPBuf(cell->instIdx),"rgSCHUtlRgrUeStaInd: cellId = %d \n",
          cell->cellId));
