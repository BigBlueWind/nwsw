...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmSapCfg(): SAP Config Requested"
          " for %dth sap of sapType=%d", sapIdx, sapType));
...
#endif
{
   ...
   {
      RGSCHDBGERR(inst,(rgSchPBuf(inst), "\nrgSCHLmmInstCfg: SGetSBuf for TFU "
                 "saps failed"));
      RETVALUE(RFAILED);
   }
   ...
   {
      RGSCHDBGERR(inst,(rgSchPBuf(inst), "\nrgSCHLmmInstCfg: SGetSBuf for bind"
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmShutdown(): Layer Shutdown "
                 "Requested"));
#ifdef LTE_L2_MEAS
...
#ifdef LTE_TDD
        for(ulAllocIdx = 0; ulAllocIdx < 2*RGSCH_NUM_SUB_FRAMES; ulAllocIdx++)
#else
...
#ifdef MSPD
                  MSPD_DBG("\nPhyErrorLog: Mcs[%d],NumOfRb[%d],RbStart[%d], testStart=[%d], enaLog=[%d], logTime[%d]\n",
                        rgSchUlAllocCntr.mcs,rgSchUlAllocCntr.numOfRb,rgSchUlAllocCntr.rbStart, 
                        rgSchUlAllocCntr.testStart, rgSchUlAllocCntr.enaLog, rgSchUlAllocCntr.logTime);
#endif
#ifdef MAC_SCH_STATS
                  /* Reset
                   * L2
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmSapCntrl(): Control Req for"
             " SAP (%d)", cntrl->hdr.elmId.elmnt));
         ...
         switch(cntrl->t.cntrl.action)
         {
            case ABND:
               ...
               else
               {
                  ...
                  /* rg003.201: Sending Control Confirm before sending Bind
                   * Request to TFU  */
                  RgMiLrgSchCntrlCfm(cfmPst, cfm);
...
/*   Inst        dInst = inst + RGSCH_INST_START; */
   ...
   UNUSED(tmrEvnt);
#ifndef ALIGN_64BIT
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmStartTmr(): Timer start "
         "request for tmrEvnt=%d, tmrVal=%ld", tmrEvnt, tmrVal));
#else
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "rgSCHLmmStartTmr(): Timer start"
       " request for tmrEvnt=%d, tmrVal=%d", tmrEvnt, tmrVal));
#endif
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmStopTmr(): Timer stop request"
           " for tmrEvnt=%d",tmrEvnt));
   ret = RFAILED;
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmStopTmr(): tmrEvnt=%d"
         " search result is ret=%d",tmrEvnt,ret));
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmTmrExpiry(): Timer Expired"
              " for tmrEvnt=%d",tmrEvnt));
   ...
   switch(tmrEvnt)
   {
      case RGSCH_BNDREQ_TMR:
         ...
         if(tfuSap->numBndRetries > RGSCH_MAX_BNDRETRY)
         {
            ...
            RGSCHDBGINFO(inst,(rgSchPBuf(inst), "\nrgSCHLmmTmrExpiry(): Max"
                    " Bind Retries Exceeded."));
         }
         else
         {
            RGSCHDBGINFO(inst,(rgSchPBuf(inst), "\nrgSCHLmmTmrExpiry(): Retry"
                  " Bind"));
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmBndCfm(): suId=%d, status=%d,"
     " sapState=%d", suId, status, rgSchCb[inst].tfuSap[suId].sapSta.sapState));
...
#endif
{
   ...
   RGSCHDBGPRM(inst,(rgSchPBuf(inst), "\nrgSCHLmmStaInd(): category=%d,"
          " event=%d, cause=%d", category, event, cause));
   if(rgSchCb[inst].rgSchInit.usta == FALSE)
   {
      RGSCHDBGINFO(inst,(rgSchPBuf(inst), "\nrgSCHLmmStaInd(): USTA Ind "
               "Disabled."));
      RETVALUE(ROK);
   }
