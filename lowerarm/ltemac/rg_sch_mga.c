...
#else
PUBLIC Void rgSCHMeasGapANRepUeDel(cell, ue)
RgSchCellCb *cell;
RgSchUeCb   *ue;
#endif
{
...
#else
   RGSCHSUBFRMCRNTTIME(cell->crntTime, repTime, (RG_SCH_CMN_HARQ_INTERVAL - TFU_RECPREQ_DLDELTA)); 
#endif
...
#endif
{
...
#ifdef LTE_TDD
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, RG_DL_DELTA);
   ...
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, RG_DL_DELTA);
#else
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, 
         (RG_DL_DELTA - 1));
   dlSf = rgSCHUtlSubFrmGet (cell, ackNakTime);
#endif
   queue = &(dlSf->tbs);
...
#endif
{
   ...
   schedTime = cell->crntTime.sfn * 10 + cell->crntTime.subframe + \
               RG_UL_SCHED_DELTA;
#ifdef LTE_TDD
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, RG_UL_SCHED_DELTA);
   if((rgSchTddUlDlSubfrmTbl[cell->ulDlCfgIdx][ackNakTime.subframe] !=
            RG_SCH_TDD_DL_SUBFRAME) ||
         (rgSchTddPuschTxKTbl[cell->ulDlCfgIdx][ackNakTime.subframe] == 0))
   {
      isUl = FALSE;
   }
   harqFdbkOffset = rgSchTddPuschTxKTbl[cell->ulDlCfgIdx][ackNakTime.subframe];
#else
   harqFdbkOffset = RG_SCH_CMN_HARQ_INTERVAL;
#endif
   if(isUl)
   ...
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, TFU_DELTA);
   if(rgSchTddUlDlSubfrmTbl[cell->ulDlCfgIdx][ackNakTime.subframe] !=
         RG_SCH_TDD_DL_SUBFRAME)
   {
      RETVALUE(ROK);
...
#else
   /* rg002.301 -MOD-[ccpu00118351] Introduced timing delta for DL control in FDD */
   RGSCHADDTOCRNTTIME(cell->crntTime, ackNakTime, TFU_DLCNTRL_DLDELTA);
#endif
   dlSf = rgSCHUtlSubFrmGet (cell, ackNakTime);
   queue = &(dlSf->tbs);
...
#endif
{
   ...
   if (!ue->dl.dlInactvMask)
   {
      /* inform the Dl Scheduler, if it needs to */
      if (ROK != rgSCHUtlDlActvtUe (cell, ue))
      {
         RGSCHDBGERR(cell->instIdx, (rgSchPBuf(cell->instIdx), 
                  "rgSCHMeasGapANRepDlInactvTmrExpry() Unable to add UE \n"));
      }
      cmInitTimers (&ue->measGapCb.measGapDlInactvTmr, 1);
      cmInitTimers (&ue->ackNakRepCb.ackNakRepDlInactvTmr, 1);
   }
...
#endif
{
   ...
   if (!ue->ul.ulInactvMask)
   {
      /* inform the UL Scheduler, if it needs to */
      if (ROK != rgSCHUtlUlActvtUe (cell, ue))
      {
         RGSCHDBGERR(cell->instIdx, 
               (rgSchPBuf(cell->instIdx), 
                "rgSCHMeasGapANRepUlInactvTmrExpry() Unable to add UE \n"));
      }
      cmInitTimers (&ue->measGapCb.measGapUlInactvTmr, 1);
      cmInitTimers (&ue->ackNakRepCb.ackNakRepUlInactvTmr, 1);
   }
...
#endif
{
   ...
   RGSCHADDTOCRNTTIME(repTime, ulfrm, noSubfrms);
   diff = repCnt-1;
   idx = ulfrm.subframe;
   while(diff)
      {
