...
#endif
{
   ...
   if (datReq == NULLP)
   {
      ...
      MSPD_ERR("Input Message Buffer is NULL\n");
      RETVALUE(RFAILED);
   }
   ...
   if (ret == RFAILED)
   {
      ...
      MSPD_ERR("RgUiRguDDatReq:processing Of Dat Req Failed\n");
      RG_DROP_RGUDDATREQ_MBUF(datReq);
   }
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("RgUiRguDStaRsp : Processing Of Status Response Failed\n");
   }

   SPutSBuf (pst->region, pst->pool, (Data *)staRsp,
         sizeof(RguDStaRspInfo));
#ifdef MSPD_MLOG_NEW
   MLogTask(PID_MAC_STA_RSP, RESOURCE_LARM, t, MacGetTick());
#endif
   RETVALUE(ret);
}  /* RgUiRguDStaRsp */
...
#endif
{
   ...
   RGDBGPRM((rgPBuf,"RgUiRguCStaRsp(): spId = %d; staRsp = %p;",spId, (void *)staRsp));
   ...
   {
      RGDBGERR((rgPBuf,"Input Response Buffer is NULL\n"));
      MSPD_ERR("Input Response Buffer is NULL\n");
      RETVALUE(RFAILED);
   }
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("RgUiRguCStaRsp : Processing Of Status Response Failed\n");
      RETVALUE(ret);
   }
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("RgUiRguL2MUlThrpMeasReq : Processing Of Meas Request Failed\n");
   }
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgUIMSndDedStaInd: RgUiRguDStaInd Failed\n");
      RETVALUE(ret);
   }
   RETVALUE(ret);
}  /* rgUIMSndDedStaInd */
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgUIMSndCmnStaInd: RgUiRguCStaInd Failed\n");
      RETVALUE(ret);
   }
   RETVALUE(ret);
}  /* rgUIMSndCmnStaInd */
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgUIMSndDedDatInd: RgUiRguDdatInd Failed\n");
      RETVALUE(ret);
   }
   RETVALUE(ret);
}  /* rgUIMSndDedDatInd */
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgUIMSndCmnDatInd: RgUiRguCDatInd Failed\n");
      RETVALUE(ret);
   }
   RETVALUE(ret);
}  /* rgUIMSndCmnDatInd */
...
#endif
{
   ...
   if (cfgReqInfo == NULLP)
   {
      ...
      MSPD_ERR("Input Param crgReqInfo is NULL \n");
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("RgUiCrgCfgReq:Configuration Request Handling Failed \n");
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if (ret != ROK)
   {
      ...
      MSPD_ERR("rgUIMCrgCfgCfm: RgUiCrgCfgCfm Failed \n");
      RETVALUE(ret);
   }
