...
/* rg008.201: Added to support SPS feature */
#ifdef LTEMAC_SPS
...
#endif
{
   ...
   if(cmPkPtr((PTR)measInfo, mBuf) != ROK)
   {
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if(cmUnpkPtr((PTR *)&measInfo, mBuf) != ROK)
   {
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if(cmPkPtr((PTR)measCfm, mBuf) != ROK)
   {
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if(cmUnpkPtr((PTR *)&measCfm, mBuf) != ROK)
   {
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
