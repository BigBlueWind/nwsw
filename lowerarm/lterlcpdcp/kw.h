...
/********************************************************************20**
        ...
        Sid:      kw.h@@/main/2 - Fri Nov 13 14:14:08 2009
...
#define KW_MAX_RGUSAPS        1     /*!< Maximum number of CKW Saps. */
...
#define KW_RMV_SDU(_sduQ,_sdu)\
{\
   if(_sdu->mBuf)\
      KW_FREE_BUF(_sdu->mBuf);\
   _sdu->mBuf  = NULLP;\
   cmLListDelFrm(_sduQ,&_sdu->lstEnt);\
   KW_FREE(_sdu, sizeof(KwSdu));\
}

#ifdef MSPD
#define KW_RMV_SDU_UM(_sduQ,_sdu)\
{\
   KW_RMV_SDU(_sduQ,_sdu); \
   if (kwCb.numDlUmSdus == 0) \
   { \
      MSPD_DBG("ERROR: kwCb.numDlUmSdus is zero!\n"); \
   } \
   --kwCb.numDlUmSdus; \
}
#else
...
{\
   ...
   if (kwCb.numDlUmSdus == 0) \
   { \
      printf("ERROR: kwCb.numDlUmSdus is zero!\n"); \
   } \
   --kwCb.numDlUmSdus; \
}
#endif
...
#ifdef LTE_L2_MEAS
#define KW_SEND_SAPID_ALARM(_sapId, _evnt, _cause) \
{ \
   kwLmmSendAlarm(LCM_CATEGORY_INTERFACE, _evnt, _cause, _sapId, 0, 0); \
}
#define KW_SEND_UEID_ALARM(_ueId, _qci, _evnt, _cause) \
{ \
   kwLmmSendAlarm(LCM_CATEGORY_INTERFACE, _evnt, _cause, 0, _ueId, _qci); \
}
#else /* LTE_L2_MEAS */
#define KW_SEND_SAPID_ALARM(_sapId, _evnt, _cause) \
{ \
   kwLmmSendAlarm(LCM_CATEGORY_INTERFACE, _evnt, _cause, _sapId, 0); \
}
#define KW_SEND_UEID_ALARM(_ueId, _evnt, _cause) \
{ \
   kwLmmSendAlarm(LCM_CATEGORY_INTERFACE, _evnt, _cause, 0, _ueId); \
}
#endif /* LTE_L2_MEAS */
/* RLC Configuration parameters */
#define KW_MAX_LI                   KW_MAXIMUM_LI
#define KW_MAX_DL_LI                6
...
#define KW_VALDATE_SAP(_chkSpId, _sap, _ret)                         \
{                                                                    \
   ...
   {                                                                 \
      KWLOGERROR(ERRCLS_INT_PAR, EKWXXX, (ErrVal) RFAILED,           \
            "Sap State Invalid.");                                   \
      KW_SEND_SAPID_ALARM(0, LCM_EVENT_UI_INV_EVT, LCM_CAUSE_INV_STATE); \
      _ret = RFAILED;                                                \
   }                                                                 \
}
...
#endif /* KW_BG_UL_PROC */
#define EVTKWSELFMSGREEST              3
#define EVTKWSELFMSGDELETE             4

#define EVTPJSELFMSGDL                 1
#define EVTKWSELFMSGPJDELRB              5
/* UMM defines */
...
{\
   KwSn winSz = _rbCb->m.um.umUl.umWinSz << 1; \
   KwSn _umReOrdWinSz = _rbCb->m.um.umUl.umWinSz;\
   KwSn _vrUh = _rbCb->m.um.umUl.vrUh;\
   KwSn _vrUr = _rbCb->m.um.umUl.vrUr;\
   ...
   if ( ((ur < sn) && (sn < uh) && \
         (_rbCb->m.um.umUl.recBuf[_curSn] != NULLP)) || \
         (sn < ur)) \
   {\
       _ret = TRUE;\
   }\
...
{\
   KwSn _umWinSz      = _rbCb->m.um.umUl.umWinSz << 1;\
   KwSn _vrUh         = _rbCb->m.um.umUl.vrUh;\
   KwSn _umReOrdWinSz = _rbCb->m.um.umUl.umWinSz;\
...
#define KW_ALL_BYTES_MISSING           0x7fff /*Fix due to reestablishment*/

#define KW_LLIST_FIRST_SDU(_lstCp, _nod)                   \
{                                                          \
   CmLList *_tmpNode;                                      \
   _tmpNode = (_lstCp).first;                              \
   if (_tmpNode != NULLP)                                  \
      _nod = (KwSdu *)(_tmpNode->node);                    \
   else                                                    \
      _nod = NULLP;                                        \
}                                                          
                                                           
#define KW_LLIST_FIRST_SEG(_lstCp, _nod)                   \
{                                                          \
   CmLList *_tmpNode;                                      \
   _tmpNode = (_lstCp).first;                              \
   if (_tmpNode != NULLP)                                  \
      (_nod) = (KwSeg *)_tmpNode->node;                    \
   else                                                    \
      (_nod) = NULLP;                                      \
}                                                          
                                                           
#define KW_LLIST_FIRST_RETX(_lstCp, _nod)                  \
{                                                          \
   CmLList *_tmpNode;                                      \
   _tmpNode = _lstCp.first;                                \
   if (_tmpNode != NULLP)                                  \
      _nod = (KwRetx *)_tmpNode->node;                     \
   else                                                    \
      _nod = NULLP;                                        \
}

#define KW_LLIST_NEXT_SDU(_lstCp, _nod)                    \
{                                                          \
   CmLList *_tmpNode = NULLP;                              \
   _tmpNode = _nod->lstEnt.next;                           \
   if (_tmpNode != NULLP)                                  \
      _nod = (KwSdu *)_tmpNode->node;                      \
   else                                                    \
      _nod = NULLP;                                        \
...
#define KW_LLIST_NEXT_SEG(_lstCp, _nod)                    \
{                                                          \
   CmLList *_tmpNode;                                      \
   _tmpNode = _nod->lstEnt.next;                           \
   if (_tmpNode != NULLP)                                  \
...
#define KW_LLIST_NEXT_RETX(_lstCp, _nod)                   \
{                                                          \
   CmLList *_tmpNode;                                      \
   _tmpNode = _nod.next;                                   \
   if (_tmpNode != NULLP)                                  \
...
#define KW_LLIST_LAST_RETX(_lstCp, _nod)                   \
{                                                          \
   CmLList *_tempNode = NULLP;                             \
   _tempNode = _lstCp.last;                                \
   if (_tempNode != NULLP)                                 \
      _nod = (KwRetx *)_tempNode->node;                    \
   else                                                    \
      _nod = NULLP;                                        \
}

#define KW_LLIST_LAST_SEG(_lstCp, _nod)                    \
{                                                          \
   CmLList *_tempNode = NULLP;                             \
   _tempNode = _lstCp.last;                                \
   if (_tempNode != NULLP)                                 \
      _nod = (KwSeg *)_tempNode->node;                     \
   else                                                    \
      _nod = NULLP;                                        \
}

#define KW_LLIST_LAST_SDU(_lstCp, _nod)                    \
{                                                          \
   CmLList *_tempNode = NULLP;                             \
   _tempNode = _lstCp.last;                                \
   if (_tempNode != NULLP)                                 \
      _nod = (KwSdu *)_tempNode->node;                     \
   else                                                    \
      _nod = NULLP;                                        \
}

#define CM_LLIST_INS_AFT_CRNT(_lstCp, _nod)                \
{                                                          \
   CmLList *_nodeToIns = &_nod->lstEnt;                    \
   _nodeToIns->node = (PTR)_nod;                           \
   cmLListInsAfterCrnt(&_lstCp, _nodeToIns);               \
}

#define CM_LLIST_INS_BEF_CRNT(_lstCp, _nod)                \
{                                                          \
   CmLList *_nodeToIns = &_nod->lstEnt;                    \
   _nodeToIns->node = (PTR)_nod;                           \
   cmLListInsCrnt(&_lstCp, _nodeToIns);                    \
}
...
#define MODAMT(_x, _y, _z)                                 \
{                                                          \
   _y = (((_x) - (_z)) + 1024) % 1024;                     \
}                                                         

#define MODAMR(_x, _y, _z)                                 \
{                                                          \
   _y = (((_x) - (_z)) + 1024) % 1024;                     \
}

#define MODUMR(_x, _y, _z,_snLen)                          \
{                                                          \
   U16 modVal = KW_POWER(2, (5 * _snLen));	               \
   _y = (((_x) - (_z)) + modVal) % modVal;                 \
}
/* Accumulate bo */
...
#define KW_FILL_HDR_ARGS(_hdrInfo, _val, _len)             \
{                                                          \
   _hdrInfo.val = _val;                                    \
   _hdrInfo.len = _len;                                    \
}
...
{                                                          \
  U16 _tmpPdu;                                             \
  _tmpPdu = staPdu[_i];                                    \
  cntrlPdu[_j++] = (U8)(_tmpPdu >> KW_BYTE_LEN);           \
...
#define KW_PRNT_HLINE(_pMsg)                                        \
{                                                                   \
   sprintf(_kwPBuf, "[RLC_LAYER: %s:%d]::", __FILE__, __LINE__);  \
   SPrint(_kwPBuf);                                                 \
   KW_PRNT_TSTAMP;                                                  \
   sprintf(_kwPBuf, _pMsg);                                         \
   SPrint(_kwPBuf);                                                 \
}


#define KW_PRNT(_prntbuf)      \
{                              \
   sprintf _prntbuf;           \
   SPrint(_kwPBuf);            \
}
...
{                                           \
   ...
   KW_PRNT((_kwPBuf, "[%s]", _buf));       \
}

#define KW_PRNT_MBUF(_mBufPtr)                              \
do                                                          \
{                                                           \
   if(kwCb.init.dbgMask & (KW_DBGMASK_MBUF_PRNT))           \
   {                                                        \
...
#define KW_PRNT_MEMINFO                                       \
do                                                            \
{                                                             \
   U32  _memInfo;                                             \
   if(kwCb.init.dbgMask & (KW_DBGMASK_MEM_INFO))              \
   {                                                          \
     KW_PRNT_HLINE(("\nMemory Information:\n"));              \
...
do                                                            \
{                                                             \
   if (!((kwCb.init.dbgMask & _mask) ^ _mask))                \
   {                                                          \
      KW_PRNT_BORDER;                                         \
      KW_PRNT_HLINE(("\nDebug Log:\n"));                      \
      KW_PRNT(_arg);                                          \
...
#ifdef KW_PDCP
#ifdef DEBUGP
#define KW_PJ_PRNT_HLINE(_pMsg)                                        \
{                                                                   \
   sprintf(_kwPBuf, "[PDCP_LAYER: %s:%d]::", __FILE__, __LINE__);  \
   SPrint(_kwPBuf);                                                 \
   KW_PRNT_TSTAMP;                                                  \
   sprintf(_kwPBuf, _pMsg);                                         \
   SPrint(_kwPBuf);                                                 \
...
{                                                                    \
   ...
   MODAMT(_rbCb->m.am.amDl.vtA, _mVtA, _rbCb->m.am.amDl.vtA);         \
                                                                     \
...
#define kwCpyMsg(x, y) \
      (SAddMsgRef((x), KW_GET_MEM_REGION(kwCb), KW_GET_MEM_POOL(kwCb), (y)))
...
/* LMM Module Macros */
#define KW_LMM_RB_STS_INC()                                    \
{                                                              \
   kwCb.genSts.numOfRb++;                                      \
}

#define KW_LMM_RB_STS_DEC()                                    \
{                                                              \
   kwCb.genSts.numOfRb--;                                      \
}
...
{                                                                    \
   if(SLock(_lockPtr) != ROK)                                        \
   {                                                                 \
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0, "Lock failed");  \
   }                                                                 \
}

#define KW_RDWR_UNLOCK(_lockPtr)                                     \
{                                                                    \
   if(SUnlock(_lockPtr) != ROK)                                      \
   {                                                                 \
      KWLOGERROR(ERRCLS_ADD_RES, EKWXXX, (ErrVal)0, "UnLock failed");\
   }                                                                 \
}
#else
#define KW_RDWR_LOCK(_lockPtr)
...
/* Sumanth : Added macros required for security */
...
#ifdef SS_DIAG 
#define PJ_DIAG_LVL0(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                                             \
   if(kwCb.pjInit.logMask & SS_DIAG_LVL0)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTPJ, kwCb.pjInit.inst, SS_DIAG_LVL0, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   } \
}

/** @details Macro definition for PDCP error logs
 *  
*/
#define PJ_DIAG_LVL1(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)            \
{                                                                 \
   if(kwCb.pjInit.logMask & SS_DIAG_LVL1)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTPJ, kwCb.pjInit.inst, SS_DIAG_LVL1, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for PDCP critical logs
 *  
*/
#define PJ_DIAG_LVL2(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                              \
   if(kwCb.pjInit.logMask & SS_DIAG_LVL2)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTPJ, kwCb.pjInit.inst, SS_DIAG_LVL2, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for PDCP logs 
 *  
*/
#define PJ_DIAG_LVL3(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(kwCb.pjInit.logMask & SS_DIAG_LVL3)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTPJ, kwCb.pjInit.inst, SS_DIAG_LVL3, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}
 ...
 *  
*/
#define PJ_DIAG_LVL4(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(kwCb.pjInit.logMask & SS_DIAG_LVL4)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTPJ, kwCb.pjInit.inst, SS_DIAG_LVL4, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}
...
#ifdef SS_DIAG                                        
#define KW_DIAG_LVL0(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                                             \
   if(kwCb.init.logMask & SS_DIAG_LVL0)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTKW, kwCb.init.inst, SS_DIAG_LVL0, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }\
}

/** @details Macro definition for RLC error logs
 *  
*/
#define KW_DIAG_LVL1(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)            \
{                                                                 \
   if(kwCb.init.logMask & SS_DIAG_LVL1)                             \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTKW, kwCb.init.inst, SS_DIAG_LVL1, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for RLC critical logs
 *  
*/
#define KW_DIAG_LVL2(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)       \
{                                                              \
   if(kwCb.init.logMask & SS_DIAG_LVL2)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTKW, kwCb.init.inst, SS_DIAG_LVL2, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}

/** @details Macro definition for RLC logs 
 *  
*/
#define KW_DIAG_LVL3(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(kwCb.init.logMask & SS_DIAG_LVL3)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTKW, kwCb.init.inst, SS_DIAG_LVL3, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}
 ...
 *  
*/
#define KW_DIAG_LVL4(_tknId, _splArgEnum, _splArg, _string, _arg1, _arg2, _arg3, _arg4)        \
{                                                              \
   if(kwCb.init.logMask & SS_DIAG_LVL4)                            \
   {                                                              \
      ssDiagFix(_tknId, _splArgEnum, ENTKW, kwCb.init.inst, SS_DIAG_LVL4, SS_DIAG_MSG_TYPE_FIXED, _splArg, _arg1, _arg2, _arg3, _arg4, _string);\
   }                                                              \
}
...
/********************************************************************30**
         ...
         End of file:     kw.h@@/main/2 - Fri Nov 13 14:14:08 2009
