...
#endif
{
   ...
   if (rbCb->lch[0].lChType == CM_LTE_LCH_BCCH)
   {
      if ( rbCb->m.tm.bcchSduQ[sdu->mode.tm.sfn][sdu->mode.tm.subframe] != NULL)
      {
         KW_FREE_BUF(((KwSdu *)rbCb->m.tm.bcchSduQ[sdu->mode.tm.sfn][sdu->mode.tm.subframe])->mBuf);
         KW_FREE((KwSdu *)rbCb->m.tm.bcchSduQ[sdu->mode.tm.sfn][sdu->mode.tm.subframe], sizeof(KwSdu));
      }
      rbCb->m.tm.bcchSduQ[sdu->mode.tm.sfn][sdu->mode.tm.subframe] = (PTR) sdu;
   }
   else
#endif
   {
...
#endif
{
   ...
   if ( rbCb->lch[0].lChType == CM_LTE_LCH_BCCH)
   {
      if ( rbCb->m.tm.bcchSduQ[sfn][(subframe + TFU_DELTA)%10] != NULLP)
      {
        sdu = (KwSdu *) rbCb->m.tm.bcchSduQ[sfn][(subframe + TFU_DELTA)%10]; 
        rbCb->m.tm.bcchSduQ[sfn][(subframe + TFU_DELTA)%10] = NULLP;
      }
      else
      {
        RETVALUE(RFAILED);
...
#endif   
   {
     ...
     while (node != NULLP)
     {
        sdu = (KwSdu *)(node->node);
#ifdef PWAV
        if(rbCb->lch[0].lChType == CM_LTE_LCH_PCCH )
#else
        if ( rbCb->lch[0].lChType == CM_LTE_LCH_BCCH ||
        rbCb->lch[0].lChType == CM_LTE_LCH_PCCH )
#endif
