/*
 *  $Id: globals.c,v 1.4 2010/03/02 17:15:31 pavleny Exp $
 *
        ...
        for (i = 0; i < 50; i++) {
            ...
            if (strcmp(tracedModules[i], "LOGFILE") == 0) {
                ...
                */
                fileTrace = TRUE;
#ifdef _MSC_VER
                sprintf(filename, "tmp%d.log",(int)_getpid());
#else
...
int debug_vwrite(FILE * fd, char *format, va_list ap)
{
    ...
    // There were cases when returned value was NULL....
    if (the_time)
    {
...
void event_log1(short event_log_level, char *module_name, char *log_info, ...)
{
    ...
    if ((globalTrace && event_log_level <= Current_event_log_) ||
        ...
        adl_gettime(&tv);
#if _MSC_VER > 1200
        the_time = _localtime32((__time32_t *) & (tv.tv_sec));
#else
        the_time = localtime((time_t *) & (tv.tv_sec));
#endif
        if (the_time) {
