...
RESULTCODE PhiInit(U32 cpuid)
{
    ...
    if (OsalCreateRingBuf(&PhiRxQueue, PhiRxQData,
                          PHI_RXQ_SIZE * sizeof(OSAL_PADDRESS)) != SUCCESS)
        return FAILURE;
    if (OsalCreateRingBuf(&PhiTxQueue, PhiTxQData,
 ...
 * @return
 */
RESULTCODE PhiWrite(U32 size, PUNKNOWN data)
{
    PHI_MESSAGE *msg;
    ...
    if (OsalAllocMemArrayBlock(&PhiTxArray, (OSAL_PADDRESS*)&msg) != SUCCESS)
        return FAILURE;
    ...
    if (OsalPutRingBuf(&PhiTxQueue, msg) != SUCCESS)
    {
        OsalFreeMemArrayBlock(&PhiTxArray, msg);
        return FAILURE;
    }
...
void PhiIntHandler(void)
{
    ...
    {
        if (OsalAllocMemArrayBlockIsr(&PhiRxArray, (OSAL_PADDRESS*)&msg) == SUCCESS)
        {
    ...
    {
        if (OsalGetRingBuf(&PhiTxQueue, (OSAL_PADDRESS*) &msg) == SUCCESS)
        {
            PhiSend(msg);
