...
//Modified to support TDD ACK/NACK bunding and multiplexing by ArrayComm 2011-12-5

#ifdef AC_MSPD_TDD
    BIT_FIELD  Harq_loc:2; //0: ack/nack is detected on PUSCH, 1: ack/nack is deteted on PUCCH.
    BIT_FIELD  harq_pt:2;
#else
    BIT_FIELD  harq_pt:4;
#endif
//Modified to support TDD ACK/NACK bunding and multiplexing 
	
    BIT_FIELD  ri_pt:2;
    BIT_FIELD  cqi_pmi_pt:2;
...
#ifdef AC_MSPD_TDD
//Modified to support TDD ACK/NACK bunding and multiplexing by ArrayComm 2011-12-5
typedef struct PhyCqiRiHiIndAckDescriptor
{ BIT_FIELD ackNackPayload:4;
  BIT_FIELD pucchResouceIdx:2;
  BIT_FIELD feedbackMode:2;
} RX_CQIRIHI_STATUS_ACKNACK_BYTE, *PRX_CQIRIHI_STATUS_ACKNACK_BYTE;
#endif
//Modified to support TDD ACK/NACK bunding and multiplexing 
...
typedef struct DlDedicatedPrecoderDescriptor
{
    // Cyclic Diversity Delay
    BIT_FIELD cddType :1;
    //Added PMI selection for precoding in DCI format 1B by ArrayComm 2010-12-15
    BIT_FIELD PMISelection :1;
    // Code Book Index
    BIT_FIELD codeBookIdx :4;
    BIT_FIELD reserved :26;
    U8 subBandCodeBookIdx[16]; //Use first 100 bits, the remaining 28 bits are padding. (100bit = 4bit x 25sub-band)
    //End added PMI selection for precoding in DCI format 1B,2
    U8 codebookSubsetRestriction[8];
} DLDEDICATEDPRECODERINFO, *PDLDEDICATEDPRECODERINFO;
#endif
...
#else
    //Added distributed indication and UE category by ArrayComm 2011-1-14
    BIT_FIELD distributedRbInd :1;
    BIT_FIELD ueCategory :3;
    //End Added distributed indication and UE category
    BIT_FIELD transmissionMode :4;
#endif
