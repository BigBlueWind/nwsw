/* $Id: recvctrl.c,v 1.4 2012/05/30 13:28:44 nedosis Exp $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
boolean rxc_update_fragments(rxc_buffer * rbuf, unsigned int ch_tsn)
{
    ...
    while (current != NULL) {
        ...
        if (between(lo, ch_tsn, hi)) {
            gapsize = hi - lo + 1;
            if (gapsize > 1) {
                event_logi(INTERNAL_EVENT_0, "Value of Gapsize (%d) (should be > 1 :) ", gapsize);
                if (ch_tsn == hi) {
...
boolean rxc_create_sack(unsigned int *destination_address, boolean force_sack)
{
    ...
    event_logi(INTERNAL_EVENT_0, "num_of_frags = %d", num_of_frags);
...
void rxc_all_chunks_processed(boolean new_data_received)
{
    ...
    memset(&chunk_frag, 0, sizeof(fragment));
    ...
    while ((temp != NULL) && (count < num_of_frags)) {
        ...
        _memcpy(&sack->fragments_and_dups[pos], &chunk_frag, sizeof(fragment));
        pos += sizeof(fragment);
    ...
    while ((temp != NULL) && (count < num_of_dups)) {
        ...
        _memcpy(&sack->fragments_and_dups[pos], &d, sizeof(duplicate));
