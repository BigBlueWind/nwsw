/* $Id: reltransfer.c,v 1.4 2012/05/30 13:28:44 nedosis Exp $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
*/
    ...
    event_logi(5, "rtx_dequeue_up_to...%u ", ctsna);
    ...
    if (rtx->chunk_list == NULL) {
        event_log(0, "List is NULL in rtx_dequeue_up_to()");
        return -1;
    }
    ...
    while (tmp) {
        ...
        if (before(dat->chunk_tsn, ctsna) || (dat->chunk_tsn == ctsna)) {
            ...
            event_logi(5, "Now delete chunk with tsn...%u", chunk_tsn);
            old_dat = dat;
...
int rtx_send_forward_tsn(rtx_buffer *rtx, unsigned int forward_tsn, unsigned int idx, gboolean sendAtOnce){
    ...
    memset(&hton_psd, 0, sizeof(pr_stream_data));
    ...
    for (count = 0; count < rtx->prChunks->len; count++) {
        ...
        _memcpy(&(chk.variableParams[count*sizeof(pr_stream_data)]), &hton_psd, sizeof(pr_stream_data));
    }
...
int rtx_process_sack(unsigned int adr_index, void *sack_chunk, unsigned int totalLen)
{
    ...
    event_logi(5, "rtx_process_sack(address==%u)", adr_index);
    ...
    event_logii(5, "Received ctsna==%u, old_own_ctsna==%u", ctsna, old_own_ctsna);

    adl_gettime(&(rtx->sack_arrival_time));

    event_logii(5, "SACK Arrival Time : %lu secs, %06lu usecs",
    ...
    if (var_len != gap_len+dup_len) {
        event_logiiii(0, "Drop SACK chunk (incorrect length fields) chunk_len=%u, var_len=%u, gap_len=%u, dup_len=%u",
                     chunk_len, var_len, gap_len, dup_len);
        uart_printf("\r\nDrop SACK chunk (incorrect length fields) chunk_len=%u, var_len=%u, gap_len=%u, dup_len=%u\r\n", chunk_len, var_len, gap_len, dup_len);
        return -1;
    }

    event_logiiiii(5, "chunk_len=%u, a_rwnd=%u, var_len=%u, gap_len=%u, du_len=%u",
                    chunk_len, advertised_rwnd, var_len, gap_len, dup_len);

    if (after(ctsna, rtx->lowest_tsn) || (ctsna == rtx->lowest_tsn)) {
        event_logiii(5, "after(%u, %u) == true, call rtx_dequeue_up_to(%u)",
        ...
        event_logi(5, "Updated rtx->lowest_tsn==ctsna==%u", ctsna);
    }

    chunk_list_debug(VVERBOSE, rtx->chunk_list);

    if (ntohs(sack->num_of_fragments) != 0) {
        event_logi(5, "Processing %u fragment reports", ntohs(sack->num_of_fragments));
        ...
        if (max_rtx_arraysize == 0) {
            ...
            if (rtx->chunk_list != NULL && dat != NULL) {
                ...
                frag = (fragment *) & (sack->fragments_and_dups[pos]);
                    ...
                    event_logiii(0, "chunk_tsn==%u, lo==%u, hi==%u", dat->chunk_tsn, low, hi);
                    if (after(dat->chunk_tsn, hi)) {
                        event_logii(0, "after(%u,%u)==true", dat->chunk_tsn, hi);
                        pos += sizeof(fragment);
                        if (pos >= gap_len)
                            break;
                        continue;
                    ...
                    if (before(dat->chunk_tsn, low)) {
                        ...
                        if (dat->gap_reports >= 4) {
                            ...
                            event_logi(5, "Got four gap_reports, ==checking== chunk %u for rtx OR drop", dat->chunk_tsn);
                            ...
                            if(dat->hasBeenDropped == FALSE) {
                                if (timerisset(&dat->expiry_time) && timercmp(&(rtx->sack_arrival_time), &(dat->expiry_time), >)) {
                                    event_logi(5, "Got four gap_reports, dropping chunk %u !!!", dat->chunk_tsn);
                                    dat->hasBeenDropped = TRUE;
                                    /* this is a trick... */
                                    dat->hasBeenFastRetransmitted = TRUE;
                                } else if (dat->hasBeenFastRetransmitted == FALSE) {
                                    event_logi(0, "Got four gap_reports, scheduling %u for RTX", dat->chunk_tsn);
                        ...
                        event_logiii(5, "between(%u,%u,%u)==true", low, dat->chunk_tsn, hi);
                        if (dat->hasBeenAcked == FALSE && dat->hasBeenDropped == FALSE) {
                            rtx->newly_acked_bytes += dat->chunk_len;
                            dat->hasBeenAcked = TRUE;
        ...
        if (rtx->all_chunks_are_unacked == FALSE) {
            ...
            event_log(5, "rtx_process_sack: resetting all *hasBeenAcked* attributes");
    ...
    event_log(5, "Marking of Chunks done in rtx_process_sack()");
    chunk_list_debug(VVERBOSE, rtx->chunk_list);
    ...
    if (mdi_supportsPRSCTP() == TRUE) {
        ...
        if (after(rtx->advancedPeerAckPoint, ctsna)) {
            ...
            event_logi(0, "rtx_process_sack: sent FORWARD_TSN, result : %d", result);
        }
    }
    /*-----------------------------------------------------------------------------*/
                                  ...
                                  unsigned char* flags, gpointer* ctx)
{
    ...
    *len = dat->chunk_len - FIXED_DATA_CHUNK_SIZE;
    _memcpy(buf, dchunk->data, dat->chunk_len - FIXED_DATA_CHUNK_SIZE);
    *tsn = dat->chunk_tsn;
