/* $Id: adaptation.c,v 1.4 2012/05/30 13:28:43 nedosis Exp $
 * --------------------------------------------------------------------------
 *
 *           //=====   //===== ===//=== //===//  //       //   //===//
 *          //        //         //    //    // //       //   //    //
...
int adl_open_udp_socket(union sockunion* me)
{
    ...
    switch (sockunion_family(me)) {
        case AF_INET:
            ...
            ch = bind(sfd, (sockaddr *)me, sizeof(sockaddr_in));
                     ...
                     unsigned char destination[], unsigned short dest_port)
{
    ...
    switch (sockunion_family(&dest_su)) {
        case AF_INET:
            ...
            result = sendto(sfd, buf, length, 0, (sockaddr *) &(dest_su.sin), dest_len);
            break;
#ifdef HAVE_IPV6
        case AF_INET6:
            dest_su.sin6.sin6_port = htons(dest_port);
            dest_len = sizeof(struct sockaddr_in6);
            result = sendto(sfd, buf, length, 0, (sockaddr *) &(dest_su.sin6), dest_len);
            break;
#endif
        default :
...
int adl_send_message(int sfd, void *buf, int len, union sockunion *dest, unsigned char tos)
{
    ...
    case AF_INET:
        ...
        _memcpy(&outBuffer[sizeof(udp_header)], buf, len);
...
#else
        txmt_len = sendto(sfd, buf, len, 0, (sockaddr *) &(dest->sin), sizeof(sockaddr_in));
#endif
    ...
    case AF_INET6:
        ...
        txmt_len = sendto(sfd, (char*)&outBuffer, sizeof(udp_header) + len,
                          0, ( sockaddr *) &(dest->sin6), sizeof(sockaddr_in6));
        if(txmt_len >= (int)sizeof(udp_header)) {
           txmt_len -= (int)sizeof(udp_header);
        }
#else
        txmt_len = sendto(sfd, buf, len, 0, (sockaddr *)&(dest->sin6), sizeof(sockaddr_in6));
#endif
        break;
#endif
    default:
...
//#else
//        to->sin.sin_addr.s_addr = iph->.s_addr;
//#endif
...
//#endif
    ...
    if (sfd == sctpv6_sfd) {
        ...
        _memcpy(&(to->sin6.sin6_addr), &(pkt6info->ipi6_addr), sizeof(struct in6_addr));
...
int adl_get_message(int sfd, void *dest, int maxlen, union sockunion *from, socklen_t *from_len)
{
    ...
    len = recvfrom(sfd, dest, maxlen, 0, (sockaddr *) from, from_len);
    if (len < 0)
    ...
    if (!poll_fds[i].revents)
        ...
        if (poll_fds[i].revents & POLLERR) {
            ...
            if (event_callbacks[i]->eventcb_type == EVENTCB_TYPE_USER) {
                ...
                ((sctp_userCallback)*(event_callbacks[i]->action)) (poll_fds[i].fd, poll_fds[i].revents, (short *)&poll_fds[i].events, event_callbacks[i]->userData);
            } else {
...
int adl_gettime(struct timeval *tv)
{
   ...
   return 0;
#else
   return (_gettimeofday(tv, (struct timezone *) NULL));
#endif
}
...
int adl_extendedGetEvents(void (*lock)(void* data), void (*unlock)(void* data), void* data)
{
   ...
   result = extendedPoll(poll_fds, &num_of_fds, 1, lock, unlock, data);
   if(unlock != NULL) {
     unlock(data);
   }
...

{
	 ...
	 localAddresses = calloc(1,sizeof(union sockunion));
	 localAddresses->sin.sin_family = AF_INET;
	 localAddresses->sin.sin_addr.s_addr = net_get_own_addr();
	//   localAddresses->	
	// TODO 
