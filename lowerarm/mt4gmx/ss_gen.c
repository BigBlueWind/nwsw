...
#include "ss_diag.h"        /* logging header file */
...
/* ss004.301: Cavium changes */
   ...
   /*initialise the circular buffer to store logs*/
   if (ssDiagInitBuf() != ROK)
   {
      sprintf(prntBufLoc,"\n SInit(): ssDiagInitBuf failed to initialize circular buffer\n");
      SDisplay(1,prntBufLoc);
      RETVALUE(RFAILED);
