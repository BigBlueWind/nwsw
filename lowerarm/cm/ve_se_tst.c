...
#ifdef PWAV_HC 
void sfiSendStartQueryToEnt(Ent entity);
#endif
#ifdef __cplusplus
      }
#endif  /*__cplusplus*/
...
#endif
{
   ...
   {
      //MSPD_DBG("<Rahul>Failed to attach RLC-UL Task\n");
      RETVALUE(RFAILED);
   } /* end of if statement */
   ...
   //MSPD_DBG("<Rahul>Exit Function for smKwInit\n");
    ...
    )
{
  ...
  {
    MSPD_DBG("sfiSendStartQueryToEnt(): Allocating Memory for event EVTSTARTQ Failed to entity=%d", entity);
    return;
  }
  ...
  sendPst.pool     = DFLT_POOL;
  sendPst.selector = 0;
  sendPst.region   = DFLT_REGION;
...
------------ -------- ---- ----------------------------------------------
$SID$     ----       ragrawal     1. eNodeb 3.1 Release
*********************************************************************91*/
