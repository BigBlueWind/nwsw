...
#endif
{
   ...
   if (pst->selector != CTF_SEL_LWLC) {
      if (SPutSBuf(pst->region, pst->pool, (Data *)cfgReqInfo, sizeof(CtfCfgReqInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)
      /* ctf_c_001.main_3 Compilation warning fix with g++ */
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
         (Txt*)&__FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
/*ctf_c_001.main_1*/
   ...
   {
     CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
     param->duration = tmpEnum;
     CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
     param->resource = tmpEnum;
   }
   RETVALUE(ROK);
}
...
/* ctf_c_001.main_2: added Unpackning for cqiMask*/
      ...
      {
	 CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
         param->reportMode.aPeriodicRpt.aPeriodicRptMode = tmpEnum;
      }
...
/*ctf_c_001.main_1*/
   if(param->pres != FALSE)
   {
       ...
       */
      CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
      param->codeBookSubsetRes.ctfCdBkRestType = tmpEnum;
      switch(param->codeBookSubsetRes.ctfCdBkRestType)
      {
          case N2TX_ANT_TM3:
      ...
      CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
      param->txMode = tmpEnum;
      CMCHKUNPK(SUnpkU8, &param->ueTxAntSelection.cfgType, mBuf);
      CMCHKUNPK(SUnpkU32, (U32 *)&tmpEnum, mBuf);
      param->ueTxAntSelection.txAntSelect = tmpEnum;
   }
   RETVALUE(ROK);
}
