 ...
 */
typedef struct rgrSpsDlCellCfg
{
   U8        maxSpsDlBw; /*!< Maximum DL bandwidth to be allocated for SPS UEs */
} RgrSpsDlCellCfg;
#endif
/** @} */
...
/** @brief Down Link Scheduler information */  
  union   rgrDlSchInfoS
   {
     RgrCellDlPfs     cellDlPfs;      /*!< Information related to DL PFS 
                                          Scheduler */
   } dlSchInfo;
...
/** @brief Up Link Scheduler information */  
  union               rgrUlSchInfoS
   {
      RgrCellUlPfs     cellUlPfs;      /*!< This structure holds the parameters   
...
/** @{ */
#ifdef LTEMAC_SPS
   RgrSpsDlCellCfg    dlSpsCfg;  /* Cell-specific configuration for DL SPS */
#endif
/** @} */
} RgrCellCfg;
/** 
...
typedef struct rgrUeSrSetupCfg
{
   ...
   RgrUeDsrTransMax dTMax;          /*!< Range: 4, 16, 32, 64  SchedulingRequestConfig */
}RgrUeSrSetupCfg;
...
typedef struct rgrUeSpsUlCfg
{
   ...
   U32               packetSize;       /*!< Packet size for SPS - SPS allocation
                                            is restricted by this */
   U8                lcCnt;            /*!< Number of logical channels */
   U8                lcId[RGR_MAX_SPS_LC];/*!< Array of SPS logical channels -
...
typedef struct rgrLchSpsCfg
{
   Bool isSpsEnabled;        /*!< Bool indicating if SPS is enabled for
                                  the service */
   U32  packetSize;          /*!< Packet size for the SPS service */
   U16  packetPrdcty;        /*!< Periodicity of the packet for this SPS 
                                  service */
...
{
   CmLteCellId         cellId;        /*!< Cell ID */
   U16                 recfgTypes;    /*!< Bitmask indicating reconfiguration types */
...
typedef struct rgrUeRecfg
{
   ...
   /*rgr_x_001.main_11 LTEMAC_DRX : changing RecfgTypes to U32 
    * as all 16 bits are exhausted*/
#ifdef RGR_V2
   U32                ueRecfgTypes;      /*!< Bitmask indicating UE reconfiguration items */
#else
   U16                ueRecfgTypes;      /*!< Bitmask indicating UE reconfiguration items */
#endif
...
typedef struct rgrDel
{
   ...
   union rgrDelU
   {
      ...
      struct rgrUeDelS 
      {
         CmLteCellId cellId; /*!< Cell ID */
         CmLteRnti   crnti;  /*!< UE ID: CRNTI */
				 U8          relType; /* stop-printf only for RLF */
...
/*rgr_x_001.main_9 - Added support for SPS*/
#ifdef LTEMAC_SPS
EXTERN S16 cmPkRgrSpsDlCellCfg ARGS((
...
#endif /* LTE_TDD*/
/** @} */
EXTERN S16 cmPkRgrCellDlPfs ARGS((
   RgrCellDlPfs         *param,
   Buffer               *mBuf
));
EXTERN S16 cmUnpkRgrCellDlPfs ARGS((
   RgrCellDlPfs         *param,
   Buffer               *mBuf
));
EXTERN S16 cmPkRgrCellUlPfs ARGS((
   RgrCellUlPfs         *param,
   Buffer               *mBuf
));
EXTERN S16 cmUnpkRgrCellUlPfs ARGS((
   RgrCellUlPfs         *param,
   Buffer               *mBuf
));
