...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)datReq, sizeof(RguCDatReqInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&datReq, sizeof(RguCDatReqInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)datReq, sizeof(RguDDatReqInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&datReq, sizeof(RguDDatReqInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)datInd, sizeof(RguCDatIndInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&datInd, sizeof(RguCDatIndInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)datInd, sizeof(RguDDatIndInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&datInd, sizeof(RguDDatIndInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)staRsp, sizeof(RguCStaRspInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&staRsp, sizeof(RguCStaRspInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
/***********************************************************
*
*     Func : cmPkRguL2MUlThInfo 
*
*
* Segment Indicator and Timing Info from MAC to RLC as a part 
* Dedicated Data Indication for Uplink
...
**********************************************************/
#ifdef ANSI
PUBLIC S16 cmPkRguL2MUlThInfo 
(
RgL2MeasUlThrpTimInfo* param,
Buffer *mBuf
)
#else
PUBLIC S16 cmPkRguL2MUlThInfo(param, mBuf)
RgL2MeasUlThrpTimInfo* param; 
Buffer *mBuf;
#endif
{
   TRC3(cmPkRguL2MUlThInfo);

   CMCHKPK(SPkU32, param->firstLastSegInd, mBuf);
   CMCHKPK(SPkU32, param->thrpTimeUl, mBuf);
   RETVALUE(ROK);
}
...
**********************************************************/
#ifdef ANSI
PUBLIC S16 cmUnpkRguL2MUlThInfo 
(
RgL2MeasUlThrpTimInfo * param,
Buffer *mBuf
)
#else
PUBLIC S16 cmUnpkRguL2MUlThInfo(param, mBuf)
RgL2MeasUlThrpTimInfo  *param;
Buffer *mBuf;
#endif
{
   TRC3(cmUnpkRguL2MUlThInfo);

   CMCHKUNPK(SUnpkU32, &param->thrpTimeUl, mBuf);
   CMCHKUNPK(SUnpkU32, (U32 *)&param->firstLastSegInd, mBuf);
   RETVALUE(ROK);
}
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)measReq, sizeof(RguL2MUlThrpMeasReqInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&measReq, sizeof(RguL2MUlThrpMeasReqInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)staRsp, sizeof(RguDStaRspInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
          (ErrVal)ERGU059, (ErrVal)0, "Packing failed");
#endif      
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if ((SGetSBuf(pst->region, pst->pool, (Data **)&staRsp, sizeof(RguDStaRspInfo))) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)staInd, sizeof(RguCStaIndInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...

/**
* @brief Packing function for RguSnMapInfo
*
* @details
*
*     Function : cmPkRguSnMapInfo
*  @param[in]   RguSnMapInfo  *param
*  @param[out]  Buffer        *mBuf 
*  @return   S16
*      -# ROK
**/
#ifdef ANSI
PRIVATE S16 cmPkRguSnMapInfo
(
RguSnMapInfo    *param,
Buffer          *mBuf
)
#else
PRIVATE S16 cmPkRguSnMapInfo(param, mBuf)
RguSnMapInfo    *param;
Buffer          *mBuf;
#endif
{
   ...
   for(idx = 0; idx < param->numLch; idx++)
   {
      CMCHKPK(cmPkRguLchMapInfo, &param->lchMap[idx], mBuf);
   }
...
#endif      
      SPutSBuf(pst->region, pst->pool, (Data *)harqStatusInd, sizeof(RguHarqStatusInd));
      RETVALUE(RFAILED);
   }

   for(idx = 0; idx < harqStatusInd->numTbs; idx++)
   {
      CMCHKPK(SPkU16, harqStatusInd->status[idx], mBuf);
   }
   for(idx = 0; idx < harqStatusInd->numTbs; idx++)
   {
      CMCHKPK(cmPkRguSnMapInfo, harqStatusInd->rguSnMapInfo[idx], mBuf);
      SPutSBuf(pst->region, pst->pool, (Data *)harqStatusInd->rguSnMapInfo[idx],
               sizeof(RguSnMapInfo));
   }
   CMCHKPK(SPkU8, harqStatusInd->numTbs, mBuf);
   CMCHKPK(cmPkLteRnti, harqStatusInd->ueId, mBuf);
   CMCHKPK(cmPkLteCellId, harqStatusInd->cellId, mBuf);
   if (SPkS16(suId, mBuf) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif      
      SPutSBuf(pst->region, pst->pool, (Data *)harqStatusInd,
              sizeof(RguHarqStatusInd));
      SPutMsg(mBuf);
      RETVALUE(RFAILED);
   }
   if (SPutSBuf(pst->region, pst->pool, (Data *)harqStatusInd,
...
#endif
{
   ...
   for(idx = hqStaInd->numTbs; idx > 0; idx--)
   {
      if ((SGetSBuf(pst->region, pst->pool, 
           (Data **)&hqStaInd->rguSnMapInfo[idx - 1], sizeof(RguSnMapInfo))) 
            != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
         SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
                __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
                (ErrVal)ERGU075, (ErrVal)0, "SGetSBuf failed");
#endif      
         SPutMsg(mBuf);
         RETVALUE(RFAILED);
      }
      CMCHKUNPK(cmUnpkRguSnMapInfo, hqStaInd->rguSnMapInfo[idx - 1], mBuf);
   }
   for(idx = hqStaInd->numTbs; idx > 0; idx--)
   {
   ...
   SPutMsg(mBuf);
   RETVALUE((*func)(pst, suId, hqStaInd));
}
#endif /* LTE_L2_MEAS */
...
#endif
{
   ...
   if (SPutSBuf(pst->region, pst->pool, (Data *)staInd, sizeof(RguDStaIndInfo)) != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
      SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
          __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
...
#endif
{
   ...
   if(param->snMapPres)
   {
     CMCHKPK(cmPkRguSnMapInfo, param->rguSnInfo, mBuf);
   }
   CMCHKPK(SPkU8, param->snMapPres, mBuf);
#endif
...
#endif
{
   ...
   /*rgu_c_001.main_5 - ADD - L2M Support */
#ifdef LTE_L2_MEAS
   CMCHKUNPK(SUnpkU8, &param->snMapPres, mBuf);
   if(param->snMapPres)
   {
      if ((SGetSBuf(pst->region, pst->pool, 
           (Data **)&param->rguSnInfo, sizeof(RguSnMapInfo))) 
            != ROK) {
#if (ERRCLASS & ERRCLS_ADD_RES)      
         SLogError(pst->srcEnt, pst->srcInst, pst->srcProcId,
                __FILE__, __LINE__, (ErrCls)ERRCLS_ADD_RES,
                (ErrVal)ERGU083, (ErrVal)0, "SGetSBuf failed");
#endif      
         SPutMsg(mBuf);
         RETVALUE(RFAILED);
      }
      CMCHKUNPK(cmUnpkRguSnMapInfo, param->rguSnInfo, mBuf);
   }
#endif
   RETVALUE(ROK);
}
...
#endif
{
   ...
   for (i=RGU_MAX_TB-1; i >= 0; i--) {
      CMCHKPK(cmPkRguDatReqTb, &param->datReqTb[i], mBuf);
   /*rgu_c_001.main_5 - ADD - L2M Support */
#ifdef LTE_L2_MEAS
      if(param->datReqTb[i].rguSnInfo != NULLP)
      {
        SPutSBuf(pst->region, pst->pool, 
           (Data *)param->datReqTb[i].rguSnInfo, sizeof(RguSnMapInfo));
      }
#endif
   }
...
#ifdef LTE_L2_MEAS
     /* After Merging from 2.1 to 2.2 */
     retVal = cmUnpkRguDatReqTb(pst, &param->datReqTb[i], mBuf);
     if(retVal != ROK)
     {
        RETVALUE(RFAILED);
     }
#else
      CMCHKUNPK(cmUnpkRguDatReqTb, &param->datReqTb[i], mBuf);
#endif
   }
   RETVALUE(ROK);
}
...
#ifdef LTE_L2_MEAS
#ifdef LTEMAC_R9
   CMCHKPK(cmPkRguL2MUlThInfo, &param->timeInfo, mBuf);
#endif
#endif
      CMCHKPK(cmPkRguPduInfo, &param->pdu, mBuf);
   CMCHKPK(cmPkLteLcId, param->lcId, mBuf);
...
#ifdef LTE_L2_MEAS
#ifdef LTEMAC_R9
   CMCHKUNPK(cmUnpkRguL2MUlThInfo, &param->timeInfo, mBuf);
#endif
#endif
   RETVALUE(ROK);
}
...
#endif
{
   ...
   for (i=param->nmbOfTbs-1; i >= 0; i--) {
      CMCHKPK(cmPkRguStaIndTb, &param->staIndTb[i], mBuf);
   }
   CMCHKPK(SPkU8, param->nmbOfTbs, mBuf);
   CMCHKPK(SPkU32, param->transId, mBuf);
   CMCHKPK(cmPkLteRnti, param->rnti, mBuf);
...
#endif
{
   ...
   CMCHKUNPK(SUnpkU32, &param->transId, mBuf);
   CMCHKUNPK(SUnpkU8, &param->nmbOfTbs, mBuf);
   for (i=0; i<param->nmbOfTbs; i++) {
      CMCHKUNPK(cmUnpkRguStaIndTb, &param->staIndTb[i], mBuf);
   }
   RETVALUE(ROK);
}
