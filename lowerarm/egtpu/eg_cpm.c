...
#else
PUBLIC S16 egUSendEchoReqOnLclPort(ipCb, echoEvnt, echoReqCb, echoSeqNo)
EgIpCb  *ipCb;
EgUEvnt *echoEvnt;
EgUEchoReqCb *echoReqCb;
U16 *echoSeqNo;
...
#endif
{
   /*ccpu00125923: klocwork fix, Removed local variable initializations */
   EgUEvnt *echoEvnt;
   S16 ret;
   Mem mem;
   ...
   EG_TRC2(egUSendEchoReq)
	...
	/* ccpu00125923: Initializing local variables */
   ret = 0;
   prevEchoReq = NULLP;
   nextEchoReq = NULLP;
   ...
   * ***********************************************/
   /* ccpu00125923: klocwork fix; removed memsetfor sta, done when alarm is sent */
   ...
   EGU_ALLOCEVNT (&echoEvnt);
	/* ccpu00125923: klocwork fix, added NULL check */
	if (!echoEvnt)
	{
         ...
         for(;ROK ==(ret = cmHashListGetNext(&(egUSap->ipCbHashList), prevSrcAddr, &nextSrcAddr));) /*for1*/
         {
            ...
            if (ret != ROK)
            {
               EG_DBG_ERR (0, 0, (egp, "\n egUSendEchoReq:Memory allocation\
                        for egMsg failed "));
               /* ccpu00125923: klocwork fix: removed return here */
			...
			/* ccpu00125923:klocwork fix , added check here */
			if (ROK != ret)
...
#endif
{
   /* ccpu00125923: klocwrok fixed: removed variable initializations */
   ...
   /* ccpu00125923: klocwork fix: unnecessary klocwork initializations */
   /***********************************************
   ...
   if((EgUUtilGmGetIeOccrCnt(eguEvnt->egMsg,&ieType,&count) == ROK))
   {
	...
	/* ccpu00125923: klocwork fix : RFAILED case  for EgUUtilGmGetIeOccrCnt */
	else
	{
		EgUUtilDeAllocGmMsg(&(eguEvnt->egMsg));
   }
   ...
   ret = egUEncMsgReq(outeguEvnt);
	...
	if (ROK == ret) 
	{
	   ...
	   if(ret != ROK)
	   {
	       ...
	       **********************************/
	       /* ccpu00125923: klocwork fix: removed here */
...
#endif
{
   /* ccpu00125923: klocwork fixes removed initializations  */
      ...
      else
      {
         ...
         ******************************************************/
         echoReqCb->destAddr.u.ipv4TptAddr.port = 0;
		   ...
		   {
		      /* ccpu00125923: klocwork fix : removed unnecessary if check */  
	         EGU_ALLOC(&egtUiMsg, sizeof(EgtUEvnt),eguEvnt->region, eguEvnt->pool);
			   if (egtUiMsg)
				{
				   egtUiMsg->u.egMsg = eguEvnt->egMsg;
...
#endif
{
   /* ccpu00125923: klocwork fix: removed initialization */
   S16     ret; 
...
#endif
{
   EgtUEvnt    *egtErrEvnt; /* For sending error indicatio to the user: klocwork fix added */
   EgTptSrvCb  *srvCb;      /* to store the server control block locally: klocwork fix added */
   S16         ret;         /* return value: klocwork fix added */
...
#endif
{
   ...
   *****************************************************/
  while(teidCb->seqInfo->pduArray[tmpCnt] == NULLP)
  {
    idx++;
    if(teidCb->reordAlgo == EG_REORDER_TYPE_1)
    {
   ...
   *******************************************************************/
  if(teidCb->seqInfo->pduArray[tmpCnt] != NULLP && idx < pduWindSize)
  {
...
/main/3      eg012.201    avenugop          1. Fix for error in egUCpmHdlDecErr for handling error cases
                                            2. Added SS_DIAG changes
*********************************************************************91*/
