...
#endif
{
   ...
   /* eg003.201 : merge eg006.102 changes */ 
   RETVOID;
...
#endif
{
   ...
   U8 grpCnt1 = 0;
   U8 ret = RFAILED;
   LegIeProperties *ieEntry = NULLP;
   LegIeProperties **ieProperties = NULLP;
   ...
   if( ret == ROK && (gmCTXRSPCb->ieProperties[ieCnt].groupedIe) != NULLP)
   {
     ...
     ret = SGetSBuf(DFLT_REGION,DFLT_POOL ,(Data **)(ieProperties) ,(sizeof(LegIeProperties) *(
             EG_CTXRSP_PDN_CONN0_GRP_NMB_IES)));
     if( ret == ROK && *ieProperties != NULLP)
     {
       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_APN_IETYPE,
           EG_IE_MANDATORY,0,FALSE, EG_CTXRSP_PDN_CONN0_GRP_NMB_IES);

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_APN_RESTRICT_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_SEL_MODE_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_IPADDR_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_IPADDR_IETYPE,
           EG_IE_CONDITIONAL,1,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_EBI_IETYPE,
           EG_IE_MANDATORY,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_FTEID_IETYPE,
           EG_IE_MANDATORY,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_FQDN_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt, EG_BEARER_CNTX_IETYPE,
           EG_IE_MANDATORY,0,TRUE, 0 );

       grpCnt1 = 0;

       /* take a copy of the Bearer context IE from PDN Connection*/
       ieEntry = &gmCTXRSPCb->ieProperties[ieCnt].groupedIe->ieProperties[grpCnt];

       ret = SGetSBuf(DFLT_REGION,DFLT_POOL,(Data **)&(ieEntry->groupedIe),sizeof(LegGroupedIe));

       if( ret == ROK && (ieEntry->groupedIe) != NULLP)
       {
         ieProperties = &ieEntry->groupedIe->ieProperties;
         ret = SGetSBuf(DFLT_REGION,DFLT_POOL ,(Data **)(ieProperties) ,(sizeof(LegIeProperties) *(
                 EG_CTXRSP_BEARER_CNTX00_GRP_NMB_IES)));
         if( ret == ROK && *ieProperties != NULLP)
         {
           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_EBI_IETYPE,EG_IE_MANDATORY,0,FALSE, 
               EG_CTXRSP_BEARER_CNTX00_GRP_NMB_IES);

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_BEARER_TFT_IETYPE,EG_IE_CONDITIONAL,0,
               FALSE, 0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_FTEID_IETYPE,EG_IE_MANDATORY,0,FALSE, 0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_FTEID_IETYPE,EG_IE_CONDITIONAL,1,
               FALSE, 0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_BEARER_QOS_IETYPE,EG_IE_MANDATORY,0,
               FALSE, 0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_F_CONTAINER_IETYPE,EG_IE_CONDITIONAL,0,
               FALSE, 0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_TRANS_ID_IETYPE,EG_IE_CONDITIONAL,0,
               FALSE, 0 );
         }
       }
       grpCnt++;

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_AMBR_IETYPE,
           EG_IE_MANDATORY,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_CHARGING_CHAR_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );

       egMakeAppChildIeEntry((gmCTXRSPCb->ieProperties[ieCnt].groupedIe), grpCnt++, EG_CNG_RPT_ACT_IETYPE,
           EG_IE_CONDITIONAL,0,FALSE, 0 );
     }
   }
   ieCnt++;
...
#endif
{
   ...
   if( ret == ROK && (gmFWDRELREQCb->ieProperties[ieCnt].groupedIe) != NULLP)
   {
     ...
     if( ret == ROK && *ieProperties != NULLP)
     {
       ...
       /* take a copy of the Bearer context IE from PDN Connection*/
       ieEntry = &gmFWDRELREQCb->ieProperties[ieCnt].groupedIe->ieProperties[grpCnt];
       ...
       if( ret == ROK && (ieEntry->groupedIe) != NULLP)
       {
         ...
         if( ret == ROK && *ieProperties != NULLP)
         {
           ...
           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_BEARER_TFT_IETYPE,EG_IE_CONDITIONAL,0,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_FTEID_IETYPE,EG_IE_MANDATORY,0,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_FTEID_IETYPE,EG_IE_CONDITIONAL,1,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_BEARER_QOS_IETYPE,EG_IE_MANDATORY,0,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_F_CONTAINER_IETYPE,EG_IE_OPTIONAL,0,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_TRANS_ID_IETYPE,EG_IE_CONDITIONAL,0,FALSE,
               0 );

           egMakeAppChildIeEntry((ieEntry->groupedIe), grpCnt1++, EG_BEARER_FLAGS_IETYPE,EG_IE_OPTIONAL,0,FALSE,
               0 );
         }
       }
